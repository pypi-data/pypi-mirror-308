...
class Validator(BaseGraVal):
    ...
    def decrypt(
        ...
        ct.ciphertext = cast(ct_buffer, POINTER(c_char))

        iv_array = (c_char * 16)()
        for i, b in enumerate(iv[:16]):
            iv_array[i] = bytes([b])[0] if isinstance(b, int) else b
        ct.initialization_vector = bytes(iv_array)
