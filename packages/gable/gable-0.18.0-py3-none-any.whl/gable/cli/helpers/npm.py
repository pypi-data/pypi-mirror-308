...
def get_cmd(gable_api_endpoint: Union[str, None], args: list[str]) -> list[str]:
    ...
    # clear npx cache to ensure latest sca version is used
...
def run_gable_code_shadow(gable_api_endpoint: Union[str, None]):
    logger.info("Running Gable Code in shadow mode")
    gable_code_path = _get_gable_code_path(gable_api_endpoint)
    try:
        result = subprocess.run(
            [gable_code_path, "--mock-data"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
        )
        if result.returncode != 0:
            logger.debug(result.stderr)
            raise Exception(f"Error running Gable Code in shadow mode: {result.stderr}")
        logger.debug(result.stdout)
        logger.trace(result.stderr)
    except Exception as e:
        logger.opt(exception=e).debug("Error running Gable Code in shadow mode")


def _get_gable_code_path(gable_api_endpoint: Union[str, None]) -> str:
    """Returns the path to Gable Code"""
    if should_use_local_sca(gable_api_endpoint):
        # Needs to be a dynamic import because this file is excluded from the bundled package
        from gable.cli.local import get_local_gable_code

        local_gable_code = get_local_gable_code()
        logger.trace(f"Using local Gable Code: {local_gable_code}")
        return local_gable_code
    return os.path.join(get_installed_package_dir(), "dist/gable-code")
