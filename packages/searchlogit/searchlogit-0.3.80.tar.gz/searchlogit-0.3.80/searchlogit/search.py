...
# {
   ...
   '''
    ...
    # {
        ...
        if kwargs.get('ps_as_vars'):
            ps_ss = ['Income'
            ]
            id_x = [1 if x in ps_ss else 0 for x in self.asvarnames]
            id_x = [1 if x in kwargs.get('ps_as_vars')  else 0 for x in self.asvarnames]
...
# {
    ...
    # {
        ...
        # Check to see where we have room to place the arrays
        available_arrays =  [i for i, arr in enumerate(member_params_spec) if new_param not in arr]
        choose_add = np.random.choice(available_arrays) #randomly allocate from available choice
    ...
    # {
        ...
        member_params_spec_new = [
            np.sort(np.delete(member_params, np.where(member_params == rem_member)[0][0])) if i == choose_rem else member_params
    ...
    # {
        ...
        # {
            ...
            if rem_asvar and rem_asvar not in np.concatenate(class_params_spec):
            # {
                solution['asvars'] = [var for var in solution['asvars'] if var != rem_asvar]
