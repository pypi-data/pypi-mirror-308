...
def calc_midx(r, x_bounds, y_bounds, z_bounds):
    ...
    # calculate the index
    xidx = min(np.searchsorted(x_bounds, r[0], side='right'), len(x_bounds)-1) - 1
    yidx = min(np.searchsorted(y_bounds, r[1], side='right'), len(y_bounds)-1) - 1
    zidx = min(np.searchsorted(z_bounds, r[2], side='right'), len(z_bounds)-1) - 1
...
def calc_midxes_in_box(box, x_bounds, y_bounds, z_bounds):
    ...
    """
    # lower conner
    xidx_min = min(np.searchsorted(x_bounds, box[0], side='right'), len(x_bounds)-1) - 1
    xidx_max = np.searchsorted(x_bounds, box[3]) - 1
    yidx_min = min(np.searchsorted(y_bounds, box[1], side='right'), len(y_bounds)-1) - 1
    yidx_max = np.searchsorted(y_bounds, box[4]) - 1
    zidx_min = min(np.searchsorted(z_bounds, box[2], side='right'), len(z_bounds)-1) - 1
...
def calc_eidx(energy, e_bounds):
    ...
    if energy < e_bounds[0]:
        raise ValueError(f"Energy {energy} is out of the lower bound {e_bounds[0]}")
    if energy == e_bounds[0]:
        return 0
    for i in range(len(e_bounds) - 1):
        if e_bounds[i] < energy <= e_bounds[i + 1]:
            return i
    return len(e_bounds) - 1  # If energy is equal to or greater than the last bound
...
def get_result_by_config(pos_config):
    ...
    try:
        read_whole_mesh = conf['general'].getboolean('read_whole_mesh',fallback=True)
    except:
        pass
    print(f"read_whole_mesh: {read_whole_mesh}")
    
    # read the positons from the pos_file
    coordinates = df['Coordinate'].apply(lambda pos_str: list(map(float, pos_str.strip().split())))
    ...
    for column in file_info[1:]:
        ...
        if read_whole_mesh:
            ...
            print(f"    reading meshtally for tally {tally_id}, particle type: {particle}")
            meshtal = MeshtalWithNumber(mc_file, tags=tags, tally_number=tally_id)
            meshtally = meshtal.tally[tally_id]
            multiplier = conf[column].getfloat('multiplier')
            print(f"    scale the meshtally results with multiplier {multiplier}")
            ...
            x_bounds, y_bounds, z_bounds, e_bounds = read_mesh_bounds(mc_file, tally_id)
            if len(e_bounds)>2:
                e_data_count = len(e_bounds)
            else:
                e_data_count = 1
            print(f"    reading meshtally for tally {tally_id}, particle type: {particle}")
            data = extract_data_from_tally(mc_file, tally_id, coordinates)
            for i, pos in enumerate(coordinates):
                print(f"        dealing with {i}-th position: {pos}")
                ...
                midx = zidx + yidx * (len(z_bounds) - 1) + xidx * (len(y_bounds) - 1) * (len(z_bounds) - 1)
                for j in range(len(e_bounds) -1):
                    df.at[i, f"{pos_name}_e_{e_bounds[j+1]}_result"] = data[data['midx'] == midx]['result'][j]
                    df.at[i, f"{pos_name}_e_{e_bounds[j+1]}_error"] = data[data['midx'] == midx]['rel_error'][j]
                df.at[i, f"{pos_name}_{ptype}_total_result"] = data[data['midx'] == midx]['result'][e_data_count-1]
                df.at[i, f"{pos_name}_{ptype}_total_rel_error"] = data[data['midx'] == midx]['rel_error'][e_data_count-1]
                ...
                if int(line.split()[3]) == tally_num:
                    while True:
                        ...
                        if "Tally bin boundaries:" in line:
                            x_bounds = np.array([float(x) for x in f.readline().split(':')[-1].split()])
                            y_bounds = np.array([float(x) for x in f.readline().split(':')[-1].split()])
                            z_bounds = np.array([float(x) for x in f.readline().split(':')[-1].split()])
                            e_bounds = np.array([float(x) for x in f.readline().split(':')[-1].split()])
...
def extract_data_from_tally(filename, tally_num, positions, multiplier=1.0):
    ...
    """
    x_bounds, y_bounds, z_bounds, e_bounds = read_mesh_bounds(filename, tally_num)
    # let data to be a numpy array with columns: energy, x, y, z, result, error, midx
    if len(e_bounds)>2:
        e_data_count = len(e_bounds)
    else:
        e_data_count = 1
        print("        WARNING: no detail energy bins in the meshtally {tally_num}, only total bin")
    data = np.zeros(len(positions)*e_data_count, dtype=[(str('midx'), np.int64),
                                          (str('eidx'), np.int64),
                                          (str('result'), np.float64),
                ...
                if int(line.split()[3]) == tally_num:
                    while True:
                        ...
                        if line == "":
                            break
                        if is_tally_data_header(line):
                            print(f"            reading tally data for tally {tally_num}")
                            ...
                            while True:
                                ...
                                if len(tokens) == items_count:
                                    ...
                                    midx = calc_midx([x, y, z], x_bounds, y_bounds, z_bounds)
                                    if midx in midxes_request:
                                        data[count] = (midx, eidx, result, error)
                                        count += 1
                            break
    return data
