...
def get_result_by_config(pos_config):
    ...
    read_whole_mesh = conf['general'].getboolean('read_whole_mesh')
    ...
    for column in file_info[1:]:
        ...
        if read_whole_mesh:
            ...
            for i, pos in enumerate(coordinates):
                ...
                if not (x_bounds[0] <= pos[0] <= x_bounds[-1]):
                    raise ValueError(f"FATAL: position {pos} out of x-dimension range")
                if not (y_bounds[0] <= pos[1] <= y_bounds[-1]):
                    raise ValueError(f"FATAL: position {pos} out of y-dimension range")
                if not (z_bounds[0] <= pos[2] <= z_bounds[-1]):
                    raise ValueError(f"FATAL: position {pos} out of z-dimension range")
                ...
                midx = zidx + yidx * nz + xidx * ny * nz

                if midx >= nx * ny * nz:
                    print(f"WARNING: point {pos} out of mesh")
                    continue
