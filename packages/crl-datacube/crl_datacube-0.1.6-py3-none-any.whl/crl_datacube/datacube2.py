...
class DataCube:
    ...
    def set_data(self, var: str, idx: tuple[int, int], ds: xr.DataArray, group: str = None, store_idxs: bool = False):
    ...
    def intake_data(self, 
        ...
        if parallelism > 1:
            Parallel(n_jobs=parallelism)(delayed(f)(idx) for idx in tqdm(idxs, total=len(idxs)))
        else:
            for idx in tqdm(idxs, total=len(idxs)):
                f(idx)
                
                
    def get_single_xarray_tile(self, var: str, idx: tuple[int, int], group: str = None) -> xr.DataArray:
        src = self.storage.get_group(group)[var]
    ...
    def get_xarray_tiles(self, var:str, filter_nan: bool = True, get_idxs: list[tuple[int, int]] = [], group: str = None, parallelism: int = 1) -> list[xr.DataArray]:
        if group:
            src = self.storage.get_group(group)[var]
        else:
        ...
        to_return = Parallel(n_jobs=parallelism)(delayed(process)(idx) for idx in tqdm(all_tiles, total=len(all_tiles)))
    ...
    def export_as_tif(self, var: str, output: str, group: str = None, idxs: list[tuple[int, int]] = [], COG=False) -> None:
        ...
        vrt = None
        if COG:
            gdal.Translate(output, tmp_vrt, format="COG", creationOptions=["COMPRESS=DEFLATE", "BIGTIFF=YES"],)
        else:
            gdal.Translate(output, tmp_vrt, format="GTiff", creationOptions=["COMPRESS=LZW", "BIGTIFF=YES"],)
        return output
        
        
        
        
    def export_as_tif_tiles(self, var: str, output: str, group: str = None, idxs: list[tuple[int, int]] = []) -> None:
