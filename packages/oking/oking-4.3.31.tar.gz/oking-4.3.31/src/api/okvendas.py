...
def post_send_distribution_center(body, job_config_dict):
    try:
        url = src.client_data.get('url_api_principal') + '/api/catalogo/unidadeDistribuicao'
        header = {
            # 'Accept': 'application/json',
            # 'access-token': src.client_data.get('token_api_integracao'),
            'Content-type': 'application/json'
        ...
        if response.ok:
            ...
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                     f'Erro ao realizar POST na api okvendas {src.client_data.get("url_api_principal")}/api/catalogo/unidadeDistribuicao '
                     + response_data, 'error', 'envia_centro_distribuicao_job', 'ENVIA_CENTRO_DISTRIBUICAO')
            return False
...
def post_send_filial(body, job_config_dict):
    try:
        url = src.client_data.get('url_api_principal') + '/api/catalogo/filial'
        header = {
            # 'Accept': 'application/json',
            # 'access-token': src.client_data.get('token_api_integracao'),
            'Content-type': 'application/json'
        ...
        if response.ok:
            ...
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                     f'Erro ao realizar POST na api okvendas {src.client_data.get("url_api_terciario")}/api/catalogo/filial '
                     + response_data, 'error', 'envia_filial_job', 'ENVIA_FILIAL')
            return False
