...
def job_send_distribution_center(job_config_dict: dict):
    """
    ...
    with lock:
        ...
        # LOG de Inicialização do Método - Para acompanhamento de execução
        send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                 f'Centro de Distribuição - Iniciado', 'exec', 'envia_centro_distribuicao_job', 'ENVIA_CENTRO_DISTRIBUICAO')
        # Executa o Comando da Semaforo Antes do Query Principal
        if job_config_dict['executar_query_semaforo'] == 'S':
            logger.info(f'Executando a query semáforo')
            executa_comando_sql(db_config, job_config_dict)
        ...
        # Executa a query Principal
        logger.info(f'Executando a query principal')
        distribution_centers = query_distribution_center(job_config_dict, db_config)

        if distribution_centers is not None and len(distribution_centers) or 0 > 0:
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                     f'Unidades de Distribuição a serem inseridas: {len(distribution_centers)}', 'info',
                     'envia_centro_distribuicao_job', 'ENVIA_CENTRO_DISTRIBUICAO')
                ...
                try:
                    ...
                    if api_distribution_center.__len__() < 50:
                        ...
                        send_log(job_config_dict.get('job_name'), False, False,
                                 ...
                                 'info', 'envia_centro_distribuicao_job', 'ENVIA_CENTRO_DISTRIBUICAO')
                        response = api_okVendas.post_send_distribution_center(api_distribution_center)
                        api_distribution_center = []
                        ...
                        validade_response_distribution_center(response, db_config, job_config_dict)
            ...
            if api_distribution_center.__len__() > 0:
                send_log(job_config_dict.get('job_name'), False, False, f'Enviando Pacote: {api_distribution_center.__len__()}',
                         'info',
                         'envia_centro_distribuicao_job', 'ENVIA_CENTRO_DISTRIBUICAO')
                response = api_okVendas.post_send_distribution_center(api_distribution_center)
                send_log(job_config_dict.get('job_name'), False, False, f'Tratando retorno', 'info',
            ...
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                     f'Nao ha unidades de distribuicao para inserir no momento', 'warning',
                     'envia_centro_distribuicao_job', 'ENVIA_CENTRO_DISTRIBUICAO')


def query_distribution_center(job_config_dict: dict, db_config: DatabaseConfig):
    """
    ...
    if db_config.sql is None or db_config.sql == '':
        ...
        send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                 f'Query unidade de distribuicao nao configurada', 'warning', 'envia_centro_distribuicao_job', 'ESTOQUE')
    else:
        try:
            # monta query com EXISTS e NOT EXISTS
            # verificar se já possui WHERE
...
def validade_response_distribution_center(response, db_config, job_config_dict):
...
def job_send_filial(job_config_dict: dict):
    """
    ...
    with lock:
        ...
        # LOG de Inicialização do Método - Para acompanhamento de execução
        send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                 f'Filial - Iniciado', 'exec', 'envia_filial_job', 'ENVIA_FILIAL')
        # Executa o Comando da Semaforo Antes do Query Principal
        if job_config_dict['executar_query_semaforo'] == 'S':
            logger.info(f'Executando a query semáforo')
            executa_comando_sql(db_config, job_config_dict)
        ...
        # Executa a query Principal
        logger.info(f'Executando a query principal')
        filiais = query_filial(job_config_dict, db_config)

        if filiais is not None and len(filiais) or 0 > 0:
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                     f'Filiais serem inseridas: {len(filiais)}', 'info',
                     'envia_filial_job', 'ENVIA_FILIAL')
            api_filial = []
                ...
                try:
                    ...
                    if api_filial.__len__() < 50:
                        ...
                        send_log(job_config_dict.get('job_name'), False, False,
                                 ...
                                 'info', 'envia_filial_job', 'ENVIA_FILIAL')
                        response = api_okVendas.post_send_filial(api_filial, job_config_dict)
                        api_filial = []
                        send_log(job_config_dict.get('job_name'), False, False, f'Tratando retorno', 'info',
                                 'envia_filial_job',
                                 'ENVIA_FILIAL')
                        validade_response_filial(response, db_config, job_config_dict)
            ...
            if api_filial.__len__() > 0:
                send_log(job_config_dict.get('job_name'), False, False, f'Enviando Pacote: {api_filial.__len__()}',
                         'info',
                         'envia_filial_job', 'ENVIA_FILIAL')
                response = api_okVendas.post_send_filial(api_filial, job_config_dict)
                send_log(job_config_dict.get('job_name'), False, False, f'Tratando retorno', 'info',
...
def query_filial(job_config_dict: dict, db_config: DatabaseConfig):
    """
    ...
    if db_config.sql is None or db_config.sql == '':
        ...
        send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                 f'Query filial nao configurada', 'warning', 'envia_filial_job', 'FILIAL')
    else:
        try:
            # monta query com EXISTS e NOT EXISTS
            # verificar se já possui WHERE
...
def filial_dict(filial):
    ...
    for row in filial:
        pdict = {
            "nome": str(row['nome']),
            "cnpj": str(row['cnpj']),
            "codigo_externo": str(row['logradouro']),
            "valo_minimo_pedido": float(row['valo_minimo_pedido']),
            "restricao_mix": bool(row['restricao_mix']),
...
def validade_response_filial(response, db_config, job_config_dict):
