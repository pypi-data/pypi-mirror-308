...
class iSQLSession(iSession):
    ...
    async def _inspect_schema(self, _uri: DURI, data=None) -> StructShema:
        """Guess table schema by inspecting returned data.
        Session is authenticated already.
        """
        N = 10
        data = data or await self.get_samples(_uri, N)
    ...
    async def update_params(self, url: URI, params: JSON, context: JSON):
        ...
        limit = params.get(LIMIT_KEY_VALUE, context.get(LIMIT_KEY_VALUE)) or 1024 * 1
        if limit:
            query += f" LIMIT {limit}"
