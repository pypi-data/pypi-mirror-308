...
class iSession(iContext, iSchema, iRegistry):  # , iAuthenticator):
    ...
    async def _schema(self, uri: URI, data=None, **kw):
        ...
        if not schema:
            schema = await self._get_schema(uri, data)
        return schema
    ...
    async def _process_response(self, response):
        ...
        try:
            ...
            elif content_type in (APPLICATION_OCTECT_STREAM,):
                raw = await response.read()  # bytes object
                try:
                    df = pd.read_excel(
    ...
    async def _get_schema(self, uri: URI, data=None):
        # uri = _uri["uri"]
        # kind = _uri[KIND_KEY]
        # schema = self.CACHE.setdefault(uri, {})[kind] = await self._inspect_schema(_uri, data)
        schema = self.CACHE.get(uri)
        if not schema:
            schema = self.CACHE[uri] = await self._inspect_schema(uri, data)

        return schema

    async def _inspect_schema(self, uri
: URI, data=None) -> StructShema:
    ...
    async def update_params(self, url, params, context):
        ...
        if self.bot.preauth:
            # result = await iAuthenticator.auth(url, params, self)
            schema = {}
            monotonic_since_key = "datetime"
        else:

            schema = await self._schema(url)
