...
def validate_test_by_values(test: list, function: Function, solver: TomatoSolver):
    def print_exit_messages(messages, success):
        messages[0] = messages[0] + f'{VALID if success else INVALID}'
    ...
    def check_output_values(test_case):
        output_values = [test_case[i] if not isinstance(parameters[i], OutputParameter) else parameter.default_value for i in range(len(test_case))]
