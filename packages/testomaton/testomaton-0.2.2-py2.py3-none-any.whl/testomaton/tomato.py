...
from testomaton.exit_codes import ExitCodes as ec
...
#     validation_group.add_argument("-v", "--validate-model", action='store_true', default=False, 
...
# and blacklists. Other arguments are silently ignored.")
    validation_group.add_argument("-V", "--validate-tests", nargs='?', type=str, const=None, 
...
def main():
    ...
    if input_file is not None:
        try:
            with open(input_file, 'r', encoding='utf-8') as f:
                parsed_yaml_model = yaml.safe_load(f.read())
        except OSError as e:
        ...
        try:
            ...
            parsed_yaml_model = yaml.safe_load(StringIO(yaml_content))
    ...
    if args.nwise:
        ...
        gen = generator.nwise(function, solver, n, coverage=coverage)
    ...
    if not args.no_headrow:
        writer.writerow(function.get_parameter_names())
    #generate test cases with the selected generator
    for test in gen:
        if not args.use_choice_names:
            parameter_names = function.get_parameter_names()
