...
from PyQt5.QtWidgets import QComboBox, QLabel, QCheckBox, QBoxLayout
    ...
    def update_curves(self, first: int, last: int, roles: list[int]) -> None:
        if first <= self.profile <= last:
            ...
            if Qt.EditRole in roles:
                ...
                self.profile_curve.opts["name"] = (f"{name} at "
                                                   f"{self.plot_model.loader.timestamps[self.date]}")
