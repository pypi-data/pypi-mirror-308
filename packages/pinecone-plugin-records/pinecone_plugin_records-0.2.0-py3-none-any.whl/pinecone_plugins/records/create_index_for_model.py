...
class CreateIndexForModel(PineconePlugin):
    ...
    def __call__(
        ...
        :type region: str
        :param model: The name of the embedding model to use for the index.
        :type model: str, required
        :param field_map: A mapping of field names to their types.
        :type field_map: Dict[str, Any], required
        :param metric: The metric to use for the index. If not provided, the default metric for the model is used.
        :type metric: Optional[str], optional
        :param read_parameters: The parameters to use when reading from the index.
        :type read_parameters: Optional[Dict[str, Any]], optional
        :param write_parameters: The parameters to use when writing to the index.
        :type write_parameters: Optional[Dict[str, Any]], optional
        ...
        :rtype: IndexModel
        """
        if any(param is None for param in (name, cloud, region, model, field_map)):
            raise ValueError(
        ...
        if metric is not None:
            embed_parameters["metric"] = metric

        request = CreateIndexForModelRequest(
            name=name,
            cloud=cloud,
            region=region,
            embed=CreateIndexForModelRequestEmbed(**embed_parameters),
        )
