...
class Utilities:
	...
	Access snippet: ``utils = RsCmwBluetoothSig.utilities``"""
	...
	def logger(self) -> ScpiLogger:
		...
		Access snippet: ``logger = RsCmwBluetoothSig.utilities.logger``"""
	...
	def get_lock(self) -> threading.RLock:
		...
		By default:
			- If you create standard new RsCmwBluetoothSig instance with new VISA session, the session gets a new thread lock. You can assign it to other RsCmwBluetoothSig sessions in order to share one physical instrument with a multi-thread access.
			- If you create new RsCmwBluetoothSig from an existing session, the thread lock is shared automatically making both instances multi-thread safe.
