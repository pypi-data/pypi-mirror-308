...
class A1NreferenceCls:
	"""A1Nreference commands group definition. 1 total commands, 0 Subgroups, 1 group commands"""
	...
	def set(self, limit: float or bool, enable: bool) -> None:
		...
		Snippet: driver.configure.rxQuality.iqCoherency.limit.lowEnergy.le2M.a1Nreference.set(limit = 1.0, enable = False) \n
		...
		self._core.io.write(f'CONFigure:BLUetooth:SIGNaling<Instance>:RXQuality:IQCoherency:LIMit:LENergy:LE2M:A1NReference {param}'.rstrip())
	...
	def get(self) -> A1NreferenceStruct:
		"""SCPI: CONFigure:BLUetooth:SIGNaling<Instance>:RXQuality:IQCoherency:LIMit:LENergy:LE2M:A1NReference \n
		Snippet: value: A1NreferenceStruct = driver.configure.rxQuality.iqCoherency.limit.lowEnergy.le2M.a1Nreference.get() \n
		Defines the IQ samples coherency limit for 95% relative phase values RP(m) for non-reference antennas A1NReference to
		A3NReference. Commands for uncoded LE 1M PHY (..:LE1M..) and LE 2M PHY (..:LE2M..) are available. \n
			:return: structure: for return value, see the help for A1NreferenceStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:BLUetooth:SIGNaling<Instance>:RXQuality:IQCoherency:LIMit:LENergy:LE2M:A1NReference?', self.__class__.A1NreferenceStruct())
