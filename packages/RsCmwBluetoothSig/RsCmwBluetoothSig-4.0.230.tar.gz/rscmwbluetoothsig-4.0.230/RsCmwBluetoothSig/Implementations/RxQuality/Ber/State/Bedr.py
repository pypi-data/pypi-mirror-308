...
class BedrCls:
	...
	def fetch(self) -> enums.ResourceState:
		...
		Snippet: value: enums.ResourceState = driver.rxQuality.ber.state.bedr.fetch() \n
		...
		response = self._core.io.query_str(f'FETCh:BLUetooth:SIGNaling<Instance>:RXQuality:BER:STATe:BEDR?')
