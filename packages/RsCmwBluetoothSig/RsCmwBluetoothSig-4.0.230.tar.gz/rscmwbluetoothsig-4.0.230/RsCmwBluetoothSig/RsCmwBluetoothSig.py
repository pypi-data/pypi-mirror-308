...
class RsCmwBluetoothSig:
	"""655 total commands, 10 Subgroups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMW500/CMW100/CMW270/CMW280/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,4.0.230.0029'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		Parameter options tokens examples:
			...
			- ``LogToGlobalTarget = True`` - Sets the logging target to the class-property previously set with RsCmwBluetoothSig.set_global_logging_target() Default: ``False``
		...
		self._core = Core(resource_name, id_query, reset, RsCmwBluetoothSig._driver_options, options, direct_session)
		self._core.driver_version = '4.0.230.0029'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.0.230.0029'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwBluetoothSig version failed. Current version: '4.0.230.0029', minimum required version: '{min_version}'")
	...
	def _sync_to_custom_properties(self, cloned: 'RsCmwBluetoothSig') -> None:
	...
	@property
	def configure(self):
		"""configure commands group. 12 Sub-classes, 4 commands."""
	...
	def connection(self):
		...
		if not hasattr(self, '_connection'):
			from .Implementations.Connection import ConnectionCls
	...
	@property
	def call(self):
		"""call commands group. 4 Sub-classes, 0 commands."""
	...
	def lowEnergy(self):
		...
		if not hasattr(self, '_lowEnergy'):
			from .Implementations.LowEnergy import LowEnergyCls
