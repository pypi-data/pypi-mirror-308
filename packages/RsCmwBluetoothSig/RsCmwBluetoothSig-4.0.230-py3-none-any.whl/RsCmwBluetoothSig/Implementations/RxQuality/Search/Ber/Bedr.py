...
class BedrCls:
	...
	def initiate(self, opc_timeout_ms: int = -1) -> None:
		...
		self._core.io.write_with_opc(f'INITiate:BLUetooth:SIGNaling<Instance>:RXQuality:SEARch:BER:BEDR', opc_timeout_ms)
	...
	def abort(self, opc_timeout_ms: int = -1) -> None:
		...
		Use FETCh...STATe? to query the current measurement state. \n
			:param opc_timeout_ms: Maximum time to wait in milliseconds, valid only for this call."""
		self._core.io.write_with_opc(f'ABORt:BLUetooth:SIGNaling<Instance>:RXQuality:SEARch:BER:BEDR', opc_timeout_ms)
	...
	def stop(self, opc_timeout_ms: int = -1) -> None:
		...
		Use FETCh...STATe? to query the current measurement state. \n
			:param opc_timeout_ms: Maximum time to wait in milliseconds, valid only for this call."""
		self._core.io.write_with_opc(f'STOP:BLUetooth:SIGNaling<Instance>:RXQuality:SEARch:BER:BEDR', opc_timeout_ms)
	...
	class ResultData(StructBase):
		"""Response structure. Fields: \n
			...
			- Search_Result: float: float TX level of the R&S CMW resulting in the configured BER search limit Range: -999 dBm to 0 dBm, Unit: dBm"""
		__meta_args_list = [
	...
	class CalculateStruct(StructBase):
		"""Response structure. Fields: \n
			...
			- Search_Result: float or bool: float TX level of the R&S CMW resulting in the configured BER search limit Range: -999 dBm to 0 dBm, Unit: dBm"""
		__meta_args_list = [
