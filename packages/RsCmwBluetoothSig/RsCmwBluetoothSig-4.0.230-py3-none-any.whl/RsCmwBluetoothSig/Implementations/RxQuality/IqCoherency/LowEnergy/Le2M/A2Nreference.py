...
class A2NreferenceCls:
	...
	def read(self) -> ResultData:
		...
		Snippet: value: ResultData = driver.rxQuality.iqCoherency.lowEnergy.le2M.a2Nreference.read() \n
		...
		return self._core.io.query_struct(f'READ:BLUetooth:SIGNaling<Instance>:RXQuality:IQCoherency:LENergy:LE2M:A2NReference?', self.__class__.ResultData())

	def fetch(self) -> ResultData:
		"""SCPI: FETCh:BLUetooth:SIGNaling<Instance>:RXQuality:IQCoherency:LENergy:LE2M:A2NReference \n
		Snippet: value: ResultData = driver.rxQuality.iqCoherency.lowEnergy.le2M.a2Nreference.fetch() \n
		Returns the results of IQ samples coherency RP(m) measurement for the specified non-reference antenna. Commands for
		uncoded LE 1M PHY (..:LE1M..) and LE 2M PHY (..:LE2M..) are available. Commands for the mandatory non-reference antenna (.
		..:A1NReference) , and optional third (...:A2NReference) and fourth (...:A3NReference) antennas are available. The values
		described below are returned by FETCh and READ commands. CALCulate commands return limit check results instead, one value
		...
		Use RsCmwBluetoothSig.reliability.last_value to read the updated reliability indicator. \n
			:return: structure: for return value, see the help for ResultData structure arguments."""
		return self._core.io.query_struct(f'FETCh:BLUetooth:SIGNaling<Instance>:RXQuality:IQCoherency:LENergy:LE2M:A2NReference?', self.__class__.ResultData())
	...
	class CalculateStruct(StructBase):
		"""Response structure. Fields: \n
			...
			- Rec_Events_Cnt: float or bool: decimal No. of received events
		...
		__meta_args_list = [
			...
			ArgStruct.scalar_float_ext('Rec_Events_Cnt'),
		...
		def __init__(self):
			...
			self.Rec_Events_Cnt: float or bool = None
	...
	def calculate(self) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.rxQuality.iqCoherency.lowEnergy.le2M.a2Nreference.calculate() \n
		Returns the results of IQ samples coherency RP(m) measurement for the specified non-reference antenna. Commands for
		uncoded LE 1M PHY (..:LE1M..) and LE 2M PHY (..:LE2M..) are available. Commands for the mandatory non-reference antenna (.
		..:A1NReference) , and optional third (...:A2NReference) and fourth (...:A3NReference) antennas are available. The values
		described below are returned by FETCh and READ commands. CALCulate commands return limit check results instead, one value
		...
		return self._core.io.query_struct(f'CALCulate:BLUetooth:SIGNaling<Instance>:RXQuality:IQCoherency:LENergy:LE2M:A2NReference?', self.__class__.CalculateStruct())
