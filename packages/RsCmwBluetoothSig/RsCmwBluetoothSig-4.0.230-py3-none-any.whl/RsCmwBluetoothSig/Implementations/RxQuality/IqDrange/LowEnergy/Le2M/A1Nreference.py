...
class A1NreferenceCls:
	"""A1Nreference commands group definition. 3 total commands, 0 Subgroups, 3 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._cmd_group = CommandsGroup("a1Nreference", core, parent)
	...
	class ResultData(StructBase):
		...
		__meta_args_list = [
			...
			ArgStruct.scalar_int('No_Valid_Iqp_Airs'),
		...
		def __init__(self):
			...
			self.No_Valid_Iqp_Airs: int = None
	...
	def read(self) -> ResultData:
		...
		Snippet: value: ResultData = driver.rxQuality.iqDrange.lowEnergy.le2M.a1Nreference.read() \n
		...
		return self._core.io.query_struct(f'READ:BLUetooth:SIGNaling<Instance>:RXQuality:IQDRange:LENergy:LE2M:A1NReference?', self.__class__.ResultData())

	def fetch(self) -> ResultData:
		"""SCPI: FETCh:BLUetooth:SIGNaling<Instance>:RXQuality:IQDRange:LENergy:LE2M:A1NReference \n
		Snippet: value: ResultData = driver.rxQuality.iqDrange.lowEnergy.le2M.a1Nreference.fetch() \n
		Returns the results of IQ dynamic range Rx measurement for the specified antenna. Commands for uncoded LE 1M PHY (..:LE1M.
		.) and LE 2M PHY (..:LE2M..) are available. Commands for reference antenna (...:A0Reference) , mandatory second
		non-reference antenna (...:A1NReference) , and optional third and fourth non-reference antennas are available. The values
		described below are returned by FETCh and READ commands. CALCulate commands return limit check results instead, one value
		...
		Use RsCmwBluetoothSig.reliability.last_value to read the updated reliability indicator. \n
			:return: structure: for return value, see the help for ResultData structure arguments."""
		return self._core.io.query_struct(f'FETCh:BLUetooth:SIGNaling<Instance>:RXQuality:IQDRange:LENergy:LE2M:A1NReference?', self.__class__.ResultData())

	# noinspection PyTypeChecker
	class CalculateStruct(StructBase):
		"""Response structure. Fields: \n
			- Trans_Packets: float or bool: decimal No. of transmitted packets Range: 0 to 200E+6
			- Rec_Events: float or bool: No parameter help available
			- Rec_Events_Val_Crc: float or bool: No parameter help available
			- No_Valid_Iqp_Airs: float or bool: decimal No. of valid measurements Range: 0 to 100E+3
			- Valid_Iq_Pairs_Rec: float or bool: float Percentage of received valid IQ sample pairs related to all received IQ pairs Range: 0 % to 100 % , Unit: %
		...
		__meta_args_list = [
			ArgStruct.scalar_float_ext('Trans_Packets'),
			ArgStruct.scalar_float_ext('Rec_Events'),
			ArgStruct.scalar_float_ext('Rec_Events_Val_Crc'),
			ArgStruct.scalar_float_ext('No_Valid_Iqp_Airs'),
			ArgStruct.scalar_float_ext('Valid_Iq_Pairs_Rec'),
			...
			ArgStruct.scalar_enum('Nof_Pos_Amp', enums.ResultStatus2)]

		def __init__(self):
			StructBase.__init__(self, self)
			self.Trans_Packets: float or bool = None
			self.Rec_Events: float or bool = None
			self.Rec_Events_Val_Crc: float or bool = None
			self.No_Valid_Iqp_Airs: float or bool = None
			self.Valid_Iq_Pairs_Rec: float or bool = None
			...
			self.Nof_Pos_Amp: enums.ResultStatus2 = None

	def calculate(self) -> CalculateStruct:
		"""SCPI: CALCulate:BLUetooth:SIGNaling<Instance>:RXQuality:IQDRange:LENergy:LE2M:A1NReference \n
		Snippet: value: CalculateStruct = driver.rxQuality.iqDrange.lowEnergy.le2M.a1Nreference.calculate() \n
		Returns the results of IQ dynamic range Rx measurement for the specified antenna. Commands for uncoded LE 1M PHY (..:LE1M.
		.) and LE 2M PHY (..:LE2M..) are available. Commands for reference antenna (...:A0Reference) , mandatory second
		non-reference antenna (...:A1NReference) , and optional third and fourth non-reference antennas are available. The values
		described below are returned by FETCh and READ commands. CALCulate commands return limit check results instead, one value
		...
		return self._core.io.query_struct(f'CALCulate:BLUetooth:SIGNaling<Instance>:RXQuality:IQDRange:LENergy:LE2M:A1NReference?', self.__class__.CalculateStruct())
