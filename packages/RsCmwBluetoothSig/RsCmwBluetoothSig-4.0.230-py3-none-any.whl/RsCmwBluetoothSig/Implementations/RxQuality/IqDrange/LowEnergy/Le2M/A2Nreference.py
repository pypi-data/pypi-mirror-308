...
class A2NreferenceCls:
	"""A2Nreference commands group definition. 3 total commands, 0 Subgroups, 3 group commands"""
	...
	def read(self) -> ReadStruct:
		...
		Snippet: value: ReadStruct = driver.rxQuality.iqDrange.lowEnergy.le2M.a2Nreference.read() \n
		...
		return self._core.io.query_struct(f'READ:BLUetooth:SIGNaling<Instance>:RXQuality:IQDRange:LENergy:LE2M:A2NReference?', self.__class__.ReadStruct())
	...
	class FetchStruct(StructBase):
		...
		__meta_args_list = [
			...
			ArgStruct.scalar_int('Pro_Events_Cnt'),
			ArgStruct.scalar_int('Pro_Events_Val_Crc'),
			ArgStruct.scalar_int('Nos_Pos_Amp')]
		...
		def __init__(self):
			...
			self.Pro_Events_Cnt: int = None
			self.Pro_Events_Val_Crc: int = None
			self.Nos_Pos_Amp: int = None

	def fetch(self) -> FetchStruct:
		"""SCPI: FETCh:BLUetooth:SIGNaling<Instance>:RXQuality:IQDRange:LENergy:LE2M:A2NReference \n
		Snippet: value: FetchStruct = driver.rxQuality.iqDrange.lowEnergy.le2M.a2Nreference.fetch() \n
		Returns the results of IQ dynamic range Rx measurement for the specified antenna. Commands for uncoded LE 1M PHY (..:LE1M.
		.) and LE 2M PHY (..:LE2M..) are available. Commands for reference antenna (...:A0Reference) , mandatory second
		non-reference antenna (...:A1NReference) , and optional third and fourth non-reference antennas are available. The values
		described below are returned by FETCh and READ commands. CALCulate commands return limit check results instead, one value
		...
		return self._core.io.query_struct(f'FETCh:BLUetooth:SIGNaling<Instance>:RXQuality:IQDRange:LENergy:LE2M:A2NReference?', self.__class__.FetchStruct())
	...
	def calculate(self) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.rxQuality.iqDrange.lowEnergy.le2M.a2Nreference.calculate() \n
		Returns the results of IQ dynamic range Rx measurement for the specified antenna. Commands for uncoded LE 1M PHY (..:LE1M.
		.) and LE 2M PHY (..:LE2M..) are available. Commands for reference antenna (...:A0Reference) , mandatory second
		non-reference antenna (...:A1NReference) , and optional third and fourth non-reference antennas are available. The values
		described below are returned by FETCh and READ commands. CALCulate commands return limit check results instead, one value
		...
		return self._core.io.query_struct(f'CALCulate:BLUetooth:SIGNaling<Instance>:RXQuality:IQDRange:LENergy:LE2M:A2NReference?', self.__class__.CalculateStruct())
