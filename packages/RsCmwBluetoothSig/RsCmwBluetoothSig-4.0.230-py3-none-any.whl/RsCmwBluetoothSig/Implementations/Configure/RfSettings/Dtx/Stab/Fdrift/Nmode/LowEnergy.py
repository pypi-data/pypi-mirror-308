...
class LowEnergyCls:
	...
	def get_le_2_m(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.configure.rfSettings.dtx.stab.fdrift.nmode.lowEnergy.get_le_2_m() \n
		...
		"""
		response = self._core.io.query_bin_or_ascii_float_list('CONFigure:BLUetooth:SIGNaling<Instance>:RFSettings:DTX:STAB:FDRift:NMODe:LENergy:LE2M?')
		return response
	...
	def get_lrange(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.configure.rfSettings.dtx.stab.fdrift.nmode.lowEnergy.get_lrange() \n
		Query the dirty transmitter frequency drift set according to the test specification for Bluetooth wireless technology.
			INTRO_CMD_HELP: Refer also to the following commands: \n
			- RF tests:
		Commands for test mode classic (..:BRATe..) , (..:EDRate..) and for LE direct test mode (..:LE1M..) , (..:LE2M..) , (..
		...
		:TMODe:LENergy:LRANge..) are available. \n
			:return: freq_drift: float Frequency drift is always enabled, according to the test specification for Bluetooth wireless technology.
		"""
		response = self._core.io.query_bin_or_ascii_float_list('CONFigure:BLUetooth:SIGNaling<Instance>:RFSettings:DTX:STAB:FDRift:NMODe:LENergy:LRANge?')
		return response

	def get_le_1_m(self) -> List[float]:
		"""SCPI: CONFigure:BLUetooth:SIGNaling<Instance>:RFSettings:DTX:STAB:FDRift:NMODe:LENergy:LE1M \n
		Snippet: value: List[float] = driver.configure.rfSettings.dtx.stab.fdrift.nmode.lowEnergy.get_le_1_m() \n
		Query the dirty transmitter frequency drift set according to the test specification for Bluetooth wireless technology.
			INTRO_CMD_HELP: Refer also to the following commands: \n
			- RF tests:
		Commands for test mode classic (..:BRATe..) , (..:EDRate..) and for LE direct test mode (..:LE1M..) , (..:LE2M..) , (..
		...
		:TMODe:LENergy:LRANge..) are available. \n
			:return: freq_drift: float Frequency drift is always enabled, according to the test specification for Bluetooth wireless technology.
		"""
		response = self._core.io.query_bin_or_ascii_float_list('CONFigure:BLUetooth:SIGNaling<Instance>:RFSettings:DTX:STAB:FDRift:NMODe:LENergy:LE1M?')
		return response
