...
class LrangeCls:
	...
	def set(self, no_bits_to_corrupt: int, byte_start_err: int) -> None:
		...
		Snippet: driver.configure.rxQuality.cbits.tmode.lowEnergy.lrange.set(no_bits_to_corrupt = 1, byte_start_err = 1) \n
		...
		self._core.io.write(f'CONFigure:BLUetooth:SIGNaling<Instance>:RXQuality:CBITs:TMODe:LENergy:LRANge {param}'.rstrip())
	...
	def get(self) -> LrangeStruct:
		"""SCPI: CONFigure:BLUetooth:SIGNaling<Instance>:RXQuality:CBITs:TMODe:LENergy:LRANge \n
		Snippet: value: LrangeStruct = driver.configure.rxQuality.cbits.tmode.lowEnergy.lrange.get() \n
		No command help available \n
			:return: structure: for return value, see the help for LrangeStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:BLUetooth:SIGNaling<Instance>:RXQuality:CBITs:TMODe:LENergy:LRANge?', self.__class__.LrangeStruct())
