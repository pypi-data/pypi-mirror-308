...
class LowEnergyCls:
	...
	def get_le_1_m(self) -> int:
		"""SCPI: CONFigure:BLUetooth:SIGNaling<Instance>:RXQuality:PACKets:AUDio:LENergy[:LE1M] \n
		Snippet: value: int = driver.configure.rxQuality.packets.audio.lowEnergy.get_le_1_m() \n
		...
		"""
		response = self._core.io.query_str('CONFigure:BLUetooth:SIGNaling<Instance>:RXQuality:PACKets:AUDio:LENergy:LE1M?')
		return Conversions.str_to_int(response)

	def set_le_1_m(self, number_packets: int) -> None:
		"""SCPI: CONFigure:BLUetooth:SIGNaling<Instance>:RXQuality:PACKets:AUDio:LENergy[:LE1M] \n
		Snippet: driver.configure.rxQuality.packets.audio.lowEnergy.set_le_1_m(number_packets = 1) \n
		Defines the number of packets to be measured per measurement cycle (statistics cycle) .
			INTRO_CMD_HELP: Refer also to the following commands: \n
			- LE direct test mode: Commands for uncoded LE 1M PHY (..:LE1M..) , LE 2M PHY (..:LE2M..) , and LE coded PHY (..:LRANge..) are available.
			- LE connection tests (normal mode) : Commands for uncoded LE 1M PHY (..:NMODe:LENergy:LE1M..) , LE 2M PHY (..:NMODe:LENergy:LE2M..) , and LE coded PHY (..:NMODe:LENergy:LRANge..) are available.
		...
		self._core.io.write(f'CONFigure:BLUetooth:SIGNaling<Instance>:RXQuality:PACKets:AUDio:LENergy:LE1M {param}')
