...
class Generate_Transcation:
    ...
    def get_model_from_schema_name(self):
        ...
        if "dbDetails" in jsondata and jsondata["dbDetails"] != None:
            ...
            raise KeyError(f"Missing schema_name value")
        
        if "object_name" in jsondata and jsondata["object_name"] != None:
            object_name = jsondata["object_name"]
            logger.log(f"\nInside object_name value:::\t{object_name} \t{type(object_name)}","0")
        else:
            raise KeyError(f"Missing object_name value")
        ...
        return dbDetails, object_name, schema_name, transaction_model
    ...
    def enhancement_in_model(self, transaction_model, object_name):
        ...
        sql_models = self.update_obj_actions(sql_models, object_name)
    ...
    def update_obj_actions(self, sql_models,object_name):
        self.line_no = 1

        for sql_model in sql_models:
            actions_details = [] 
            if "actions_links" in sql_model['sql_model']:
                ...
                sql_model['sql_model']['actions'] = {}
                actions_detail_data = {"action": actions_details} 
                sql_model['sql_model']['actions'] = actions_detail_data
            else:
                logger.log("action_links not found in sql_models")   
                actions = sql_model['sql_model'].get('actions', {}).get('action', [])
                form_no = sql_model['sql_model'].get('form_no', "")
                
                if actions:
                    logger.log(f"Actions data: {actions}")
                ...
                sql_model['sql_model']['actions']['action'] = actions_details
