...
class Transcriptor:
    ...
    def __init__(self, file, log_folder, language, modelSize, voices_folder=None, quantization=False, max_speakers=10, pyannote_model="pyannote/speaker-diarization@2.1", hf_token=ACCESS_TOKEN, skip_diarization=False):
    ...
    def transcribe(self):
        segments_list, entry, overlaps = core_analysis(self.file, self.voices_folder, self.log_folder, self.language, self.modelSize, self.quantization, self.max_speakers, self.pyannote_model, self.hf_token, self.skip_diarization)
        return segments_list, entry, overlaps
