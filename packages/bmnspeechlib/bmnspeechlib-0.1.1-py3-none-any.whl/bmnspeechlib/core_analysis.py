...
def core_analysis(file_name, voices_folder, log_folder, language, modelSize, quantization=False, max_speakers=10, pyannote_model="pyannote/speaker-diarization@2.1", hf_token=ACCESS_TOKEN, skip_diarization=False):
            ...
            if speaker == speaker_map[spk_tag]:
                for segment in spk_segments:
                    if start == segment[0] and end == segment[1]:
                        common_segments.append([start, end, segment[2], speaker, segment[3]])
        """
        TO-DO: RE-WRITE ^ in self-documenting way; list indices usage is too much
        """

    # writing log file
    entry = write_log_file(common_segments, log_folder, file_name, language)  

    return common_segments, entry, diarization.get_overlap()
