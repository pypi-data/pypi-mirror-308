...
def update_toolbox_resources():
    ...
    try: 
        logger.info(f"Now fetching resources (icons, templates) from bitbucket...")
        subprocess.run(
            [
                "git",
                "clone",
        ...
        logger.debug(f"flex-toolbox bitbucket repository has been cloned successfully")
        ...
        logger.info(f"flex-icons have been updated successfully ('~/.ftbx/flex-icons/'). ")
        ...
        logger.info(
            f"flex-templates have been updated successfully ('~/.ftbx/templates/')."
        )
        ...
        logger.debug(f"Temporary repository deleted successfully. ")
    except:
        logger.warning("Could not fetch icons nor templates as no SSH key was detected. This will prevent you from rendering workflow graphs as images and using the `ftbx create` commands. ")
