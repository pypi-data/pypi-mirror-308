    ...
    def fail_jobs(self, job_ids: List[int]) -> List[Dict]:
        ...
        for job in tqdm(jobs, desc="Failing jobs"):
            ...
            if current_state == 'FAILED':
                logger.error(f"Job `{job_id}` is already in FAILED state, skipping...")
                continue
            
            if current_state == 'COMPLETED':
                logger.error(f"Job `{job_id}` is COMPLETED, skipping...")
                continue
            
            if job_class in ['MioSystemActionJob', 'MioTimedActionJob']:
                logger.error(f"Job `{job_id}` is of type `{job_class}`, skipping...")
                continue
