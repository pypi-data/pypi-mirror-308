...
from .chat import (
    ...
    StreamingChatCompletionResultModel,
)
...
class AssistantModel:
    ...
    def chat_completions(
    ...
    ) -> Union[ChatResultModel, Iterable[StreamingChatCompletionResultModel]]:
        """
        ...
        >>> resp = assistant.chat_completions(chat_context=chat_context)
        >>> print(resp)
    ...
    def _chat_completions_single(
        ...
        messages = [
            ChatMessages(role=ctx.role, content=ctx.content) for ctx in messages
        ]
        context = (
        ...
        search_result = self._assistant_data_api.chat_completion_assistant(
            assistant_name=self.name, search_completions=context
        )
    ...
    def _chat_completions_streaming(
        ...
        model: str = "gpt-4o",
        filter: Optional[dict[str, any]] = None,
    ) -> Iterable[StreamingChatCompletionResultModel]:
        ...
        try:
            ...
            for line in response.iter_lines():
                if line:
                    ...
                    res = StreamingChatCompletionResultModel(data=json_data)
    ...
    def _chat_single(
        ...
        search_result = self._assistant_data_api.chat_assistant(
            assistant_name=self.name, chat=context
        )
        return search_result
            ...
            for line in response.iter_lines():
                if line:
                    ...
                    if json_data.get("type") == "message_start":
                        res = StreamChatResultModelMessageStart(data=json_data)
                    elif json_data.get("type") == "content_chunk":
                        res = StreamChatResultModelContentDelta(data=json_data)
                    elif json_data.get("type") == "citation":
                        res = StreamChatResultModelCitation(data=json_data)
                    elif json_data.get("type") == "message_end":
                        res = StreamChatResultModelMessageEnd(data=json_data)
