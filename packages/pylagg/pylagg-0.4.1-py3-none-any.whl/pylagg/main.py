...
def cgr_command(
    ...
    if config:
        conf.config_accession_to_cgr(config)
        ...
        with open(counts_file_path) as f:
            ...
            if(input.rfind(".txt") != -1):
                    ...
                    cgr.count_file_to_image_file(f, output_path + "/" + inputname + ".png", size=size)
...
def ena_command(
    ...
    for number in accession_list:  
        files = ena.download_fastq(number, output_path)
    
        if kmer_length is not None:
            print(files)
            jellyfish.fastq_to_kmer_counts(files, kmer_length, output_path, thread_count)
...
def jellyfish_command(
    ...
    jellyfish.fastq_to_kmer_counts([input_fastq], kmer, output_path, thread_count)
