...
class AggregatedDiscoveryServiceServicer(
    ...
    * Listeners are the ports that Envoy listens on, and the rules that it
    ...
    def __init__(self):
        self._clusters: list[cluster_pb2.Cluster] = []
        self._listeners: list[listener_pb2.Listener] = []
    ...
    async def _wait_until_all_clients_at_version(self, version_info: str):
        for client in self._clients:
            while not client.disconnected and any(
                client_version_info < version_info
                for client_version_info in client.version_info_by_type.values()
            ):
