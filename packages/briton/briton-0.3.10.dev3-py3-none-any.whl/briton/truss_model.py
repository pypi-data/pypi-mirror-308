...
class Model:
    def __init__(self, **kwargs):
        ...
        # Infer adapter provides a way to override the default infer method e.g.
        # to do extra processing for write in margin mechanism.
        # TODO(pankaj) Define signature for infer adapter
        self._infer_adapter = model_metadata.get("infer_adapter", None)
    ...
    async def predict(self, model_input: Dict[str, Any], request: fastapi.Request):
        ...
        resp_iter = await self._infer(briton_request, model_input)
    ...
    async def _infer(self, briton_request: InferenceRequest, model_input: dict):
        if self._infer_adapter is not None:
            return await self._infer_adapter(
                self._stub, 
                briton_request, 
                model_input,
                self._calc_request_id, 
                self._tokenizer,
            )
        return self._stub.Infer(briton_request)
