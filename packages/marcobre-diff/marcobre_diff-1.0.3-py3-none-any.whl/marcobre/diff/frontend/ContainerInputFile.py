...
class ContainerInputFile(ctk.CTkFrame):
    ...
    def on_submit(self):
        ...
        ref_required_columns = ["EAST", "NORTH", "ELEV", "CUT", "CUAS", "CUCN", "CUR", "C", "FE", "AG", "S", "AU", "AS", "P", "LITH"]

        actual_required_columns = ["X", "Y", "Z", "CUT", "CUAS", "CUCN", "CUR", "C", "FE", "AG", "S", "AU", "AS", "P", "LITH"]

        if not self.validate_csv_columns(ref_file, ref_required_columns):
            return

        if not self.validate_csv_columns(cmp_file, actual_required_columns):
            return
