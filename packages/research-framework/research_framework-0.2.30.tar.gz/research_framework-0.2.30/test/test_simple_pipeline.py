...
from research_framework.pipeline.model.pipeline_model import SimpleInputFilterModel, SimplePipelineModel, FilterModel, MetricModel
...
def simple_pipeline():
    ...
    pipeline = SimpleFitPredictPipeline(copy.deepcopy(test_pipeline), project="Test")
    pipeline.start()
    return pipeline
...
def test_pipeline_with_prev_stored_items(delete_pipeline_items: Any):
    ...
    new_pipeline = SimpleFitPredictPipeline(copy.deepcopy(test_pipeline), project="Test")
    new_pipeline.start()

    assert len(new_pipeline.pipeline.train_input.items) == len(new_pipeline.pipeline.filters) + 1
    assert len(new_pipeline.pipeline.test_input.items) == len(new_pipeline.pipeline.filters) + 1
