...
def positive_int(value):
    ivalue = int(value)
    if ivalue <= 0:
        raise argparse.ArgumentTypeError(f"{value} is an invalid positive int value. It must be greater than 0.")
    return ivalue
...
def parse_args():
    ...
    parser.add_argument("--conv_points", nargs='+', type=positive_int, help='Cut-off values for VDR segmentation, use multiple values for convergence mode, one value for single mode', required=True)
    parser.add_argument("--conv_points_num", type=positive_int, help='Number of cut-off data points to use between range specified in --conv_points, only required for --mode convergence')
