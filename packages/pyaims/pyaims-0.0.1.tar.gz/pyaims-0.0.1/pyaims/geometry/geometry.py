...
from pyaims.geometry.atom import FHIAimsAtom, ATOMIC_SYMBOLS_TO_NUMBERS
...
from pyaims.utils.typecast import Vector3D, Matrix3D, to_matrix3d, to_vector3d
...
class AimsGeometry:
    ...
    hessian_block: Optional[Sequence[tuple[int, int, Matrix3D]]]
        ...
        line for each 3×3 block. The option block consists of 9 numbers in column-first
    ...
    homogeneous_field: Optional[Vector3D]
        Allows to perform a calculation for a system in a homogeneous electrical 
    ...
    multipole: Optional[Sequence[tuple[float, float, float, int, float]]]
        ...
        to simulate an embedding potential.
            ...
            order : Integer number, specifies the order of the multipole 
    ...
    verbatim_writeout: Optional[bool] = True
    ...
    def __post_init__(self):
        ...
        if self.lattice_vectors is not None:
            ...
            if any([inp is not None for inp in [self.symmetry_n_params, self.symmetry_params, self.symmetry_lv, self.symmetry_frac, self.symmetry_frac_change_threshold]]):
    ...
    @classmethod
    def from_lines(cls, lines: list[str]) -> "AimsGeometry":
        """Read in a geometry.in file and construct a Geometry object
        ...
        is_pseudocore: Sequence[bool] = []
        is_fractional: Sequence[bool] = []

        hessian_block: Sequence[tuple[int, int, Matrix3D]] = []
        hessian_block_lv: Sequence[tuple[float, float, Matrix3D]] = []
        hessian_block_lv_atoms: Sequence[tuple[float, float, Matrix3D]] = []

        symmetry_n_params: tuple[int, int, int] = (0, 0, 0)
        symmetry_params: Sequence[str] = []
        symmetry_lv: tuple[str, str, str] = []
        symmetry_frac: Sequence[tuple[str, str, str]] = []
        ...
        homogeneous_field: None | Vector3D = None
        esp_constraint: None | tuple[float, float, float] | tuple[float, float] = None
        ...
        lattice_constraints: dict[int, list[bool]] = {}
        ...
        for line in lines:
            ...
            if inp[0] in ["atom", "atom_frac", "empty", "pseudocore"]:
                ...
                if last_add == "atom":
                    nuclear_constraints[len(coords) - 1] = [False, False, False]
                    if "x" in inp[1:]:
                        nuclear_constraints[len(coords) - 1][0] = True
                    if "y" in inp[1:]:
                        nuclear_constraints[len(coords) - 1][1] = True
                    if "z" in inp[1:]:
                        nuclear_constraints[len(coords) - 1][2] = True
                    if "true" in inp[1:]:
                        nuclear_constraints[len(coords) - 1][:] = True
                if last_add == "lattice":
                    lattice_constraints[len(coords) - 1] = [False, False, False]
                    if "x" in inp[1:]:
                        lattice_constraints[len(coords) - 1][0] = True
                    if "y" in inp[1:]:
                        lattice_constraints[len(coords) - 1][1] = True
                    if "z" in inp[1:]:
                        lattice_constraints[len(coords) - 1][2] = True
                    if "true" in inp[1:]:
                        lattice_constraints[len(coords) - 1][:] = True
            elif inp[0] == "velocity":
        ...
        def dct2arr(dct: dict[int, Any], n_comp:int):
    ...
    def load_species(self, species_directory:str | Path="/home/tpurcell/git/FHIaims/species_defaults/defaults_2020/tight/"):
    ...
    def set_species(self, sym: str, species: SpeciesDefaults):
        ...
        ----------
        sym: str
            The symbol to add teh species for
    ...
    def file_content(self):
        ...
        if self.lattice_vectors is not None and 3 == len(self.lattice_vectors):
            for lv, lv_const in zip(self.lattice_vectors, self.lattice_constraints):
                content_str += f"\nlattice_vector {lv[0]:.15e} {lv[1]:.15e} {lv[2]:.15e}"
                if np.any(lv_const):
        ...
        for atom in self.atoms:
            content_str += atom.geometry_in_block

        if self.homogeneous_field is not None:
            content_str += f"\nhomogeneous_field {self.homogeneous_field[0]:>20.12e} {self.homogeneous_field[1]:>20.12e} {self.homogeneous_field[2]:>20.12e}"

        if self.multipole is not None and len(self.multipole) == 3:
            content_str += f"\nmultipole {self.multipole[0]:>20.12e} {self.multipole[1]:>20.12e} {self.multipole[2]:>20.12e} {self.multipole[3]} {self.multipole[4]:>20.12e}"

        if self.esp_constraint is not None and len(self.esp_constraint) > 1:
            if len(self.esp_constraint) == 2:
                content_str += f"\nesp_constraint {self.esp_constraint[0]:>20.12e} {self.esp_constraint[1]:>20.12e}"
            else:
                content_str += f"\nesp_constraint {self.esp_constraint[0]:>20.12e} {self.esp_constraint[1]:>20.12e} {self.esp_constraint[2]:>20.12e}"

        if self.verbatim_writeout is not None:
            content_str += f"\nverbatim_writeout {'.true.' if self.verbatim_writeout else '.false.'}"

        if self.calculate_friction is not None:
            content_str += f"\ncalculate_friction {'.true.' if self.calculate_friction else '.false.'}"

        if self.symmetry_n_params:
            content_str += f"\nsymmetry_n_params {self.symmetry_n_params[0]} {self.symmetry_n_params[1]} {self.symmetry_n_params[2]}"
 
        if self.symmetry_params:
            content_str += f"\nsymmetry_params " + " ".join(self.symmetry_params)
        if self.symmetry_lv is not None:
            for line in self.symmetry_lv:
                content_str += f"\nsymmetry_lv " + " , ".join(line)
        if self.symmetry_frac is not None:
            for line in self.symmetry_frac:
                content_str += f"\nsymmetry_frac " + " , ".join(line)
        if self.symmetry_frac_change_threshold:
            content_str += f"\nsymmetry_frac_change_threshold {self.symmetry_frac_change_threshold}"

        if self.hessian_block is not None:
            for line in self.hessian_block:
                content_str += f"\nhessian_block {line[0]} {line[1]} " + " ".join([f"{ll:>20.12e}" for ll in line[2:]])
        if self.hessian_block_lv is not None:
            for line in self.hessian_block_lv:
                content_str += f"\nhessian_block_lv {line[0]} {line[1]} " + " ".join([f"{ll:>20.12e}" for ll in line[2:]])
        if self.hessian_block_lv_atoms is not None:
            for line in self.hessian_block_lv_atoms:
                content_str += f"\nhessian_block_lv_atoms {line[0]} {line[1]} " + " ".join([f"{ll:>20.12e}" for ll in line[2:]])
    ...
    def masses(self):
        ...
        return np.array([self.species_dict[sym].mass * (not self.is_empty[ss]) for ss, sym in enumerate(self.symbols)])
    
    @property
    def nuclear_charge(self):
        """Get the nuclear charges of the structure"""
        if any([sym not in self.species_dict for sym in np.unique(self.symbols)]):
            raise InvalidGeometryError("Species are not defined for all atoms in the structure")

        return np.array([self.species_dict[sym].nucleus * (not self.is_empty[ss]) for ss, sym in enumerate(self.symbols)])