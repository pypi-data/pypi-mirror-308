...
Date: April 2020
	...
	Contains some useful functions (run some commands, parse the options, do some timing, etc.)
...
def parseOptions(options):
	...
	# method
	method = [k for k, v in options.items() if v and k in ("hsd", "sd", "hs")][0]
...
__params__ =  {'--alias': None,
			 ...
			 '--v': get_version(os.path.dirname(os.path.realpath(__file__))+'/__init__.py')
}
...
def customArgParser(args, params):
	...
	args_rm = [i.replace("--", "") for i in args]
	args_clean = [i.split('=') for i in args_rm]
	print(args_rm, args_clean)
	for i in args_clean:
		if i[0] not in {'hs', 'hsd', 'sd'}:
			param = '--'+i[0] 
			if param in params_copy:
				if param in {'True', 'true'}:
					params_copy[param] = True
				elif param in {'False', 'false'}:
					params_copy[param] = False
				else:
					params_copy[param] = i[1]
		else:
