...
from .config import get_config, get_package_version
...
class AsyncBaseClient:
    def __init__(
        ...
        cfg = get_config()
        self.config = {**cfg, **config} if config else cfg
        package_version = get_package_version()
        client_id: str = self.config.client_id or ""
        api_key: str = self.config.api_key or ""
        required_headers = {"ClientId": client_id, "X-Api-Key": api_key, "x-sdk-version": f'py-{package_version}'}

        self.url = url if url else f"{self.config.url}{self.config.api_endpoint}"
        self.headers = (
            {**required_headers, **headers} if headers or required_headers else {}
        )
        ...
        self.http_client = (
            ...
            if http_client
                ...
                event_hooks={"request": [self.add_authorization]},
            )
        )
    ...
    async def add_authorization(self, request: httpx.Request) -> httpx.Request:
        access_token = await get_access_token(self.config)
