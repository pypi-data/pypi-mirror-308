...
class MusicPlayer(anywidget.AnyWidget):
    tracks = traitlets.Dict().tag(sync=True)
...
container.append(topContainer, timelineContainer, timeDisplay);
    ...
    try {
        ...
        let musicData = Object.values(model.get('tracks'));
        const trackNames = Object.keys(model.get('tracks'));
        ...
        musicData.forEach((_, index) => {
            ...
            synthLabel.textContent = trackNames[index];
...
@staticmethod
def show(
    tracks: Union[Dict[str, List[Tuple[int, float, float]]], 
    ...
    if isinstance(tracks, list):
        if isinstance(tracks[0], tuple):  # Single track
            tracks = {"Instrument 1": tracks}
        else:  # Multiple tracks as list
            tracks = {f"Instrument {i+1}": track for i, track in enumerate(tracks)}
    elif not isinstance(tracks, dict):
        raise ValueError("Tracks must be a dictionary of tracks, a single track, or a list of tracks")
