...
class Open5GS:
    ...
    def _wait_for_containers(self, timeout: int = 30) -> bool:
        start_time = time.time()
        while (time.time() - start_time) < timeout:
            try:
                ue_check = subprocess.run(
                    ["docker", "container", "inspect", "-f", "{{.State.Running}}", "ue"],
                    capture_output=True, text=True, check=True
                )
                upf_check = subprocess.run(
                    ["docker", "container", "inspect", "-f", "{{.State.Running}}", "upf"],
                    capture_output=True, text=True, check=True
                )
                
                if "true" in ue_check.stdout and "true" in upf_check.stdout:
        ...
        try:
            for attempt in range(3):  
                ...
                if result.stdout.strip() != "0":
                    ...
                    return False, "UE API endpoints not accessible (TUN interfaces exist but API not responding)"
    ...
    def run_background_nodes(self, timeout: Optional[int] = None):
        try:
            ...
            logger.info("Ensuring Docker containers are up...")
            result = subprocess.run(["docker", "ps", "-q"], capture_output=True, text=True)
            if not result.stdout.strip():
                logger.info("No running containers found. Starting Docker deployment...")
                subprocess.run(["docker", "compose", "up", "-d"], check=True)
                
                if not self._wait_for_containers(timeout=30):
                    raise ConfigurationError("Failed to start Docker containers")
            ...
            logger.info("Waiting for UE initialization...")
            time.sleep(3)
            ...
            with ThreadPoolExecutor(max_workers=1) as executor:
                self._process_monitor = executor.submit(
