...
class CommunicationInterface:
    ...
    def send_data(self, endpoint: str, data: Dict[str, Any], ue_index: int = None) -> Dict[str, Any]:
        try:
            url = self.base_url
            if 'ue' in endpoint and ue_index is not None:
                port = 8080 + ue_index
                url = f"{url}:{port}"
...
class Open5GS:
    ...
    def _initialize(self):
        self.policy = Policy()
        self.ue_base_url = "http://10.10.0.132"  # Base URL without port
        self.upf_base_url = "http://10.10.0.112:8081"
        self._process_monitor = None
        self.env_path = None
        self._env_config = None
        self._last_env_modified_time = None
        self.ue_interfaces = {}  # Store UE interfaces
        self._update_ue_interfaces()
    ...
    def _update_ue_interfaces(self):
        try:
            num_ues = self.get_num_ues()
            self.ue_interfaces = {}
            base_port = 8080
            
            for i in range(num_ues):
                port = base_port + i
                interface = UEInterface(f"{self.ue_base_url}:{port}")
                self.ue_interfaces[i] = interface
                
            logger.info(f"Updated UE interfaces for {num_ues} UEs")
        except Exception as e:
            logger.error(f"Error updating UE interfaces: {e}")
    ...
    def set_num_ues(self, num_ues: int):
        ...
        self._update_ue_interfaces()  # Update interfaces after changing NUM_UEs
        logger.info(f"Updated NUM_UES to {num_ues} and reconfigured interfaces")
    ...
    def ue(self, endpoint: str, ue_index: int = 0) -> str:
            if ue_index not in self.ue_interfaces:
                raise ValueError(f"Invalid UE index: {ue_index}")
            return self.ue_interfaces[ue_index].base_url + "/" + endpoint
    ...
    def send_data(self, endpoint: str, data: Any, ue_index: int = None) -> Dict[str, Any]:
        try:
            # If no ue_index specified, use the first UE (uesimtun0)
            if ue_index is None:
                ue_index = 0
            ...
            return self.ue_interfaces[ue_index].send_data(endpoint, data)
        except Exception as e:
            logger.error(f"Error sending data through UE {ue_index}: {e}")
            raise
