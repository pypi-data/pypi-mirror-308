...
def workspace(config_content: str, host: str):
    ...
    start_vscode_service(workspace_dir, workspace_config.ttl_seconds)
...
def monitor_vscode(child_process, ttl_seconds):
    ...
    while True:
        ...
        if delta > ttl_seconds:
            logger.info(f"code server is idle for more than {ttl_seconds} seconds. Terminating...")
...
def start_vscode_service(workspace_root: Path, ttl_seconds: int):
