...
def main():
    ...
    elif run_mode == 'copy_pk' or run_mode == 'copy_uk' or run_mode == 'copy_fk':
...
def get_table_metadata(src_db, table_map, global_scn):
    ...
    def fenye(table_map, mess, table_scn, table_metadata):
        if max_page_num == 1:
            ...
            # 分页
            count_sql = 'select /*+full(t)*/ count(1) from ' + mess + ' as of scn ' + table_scn + ' t where ' + src_where
...
def copy_index():
    ...
    def copy_one_index(src_db, dst_db, table_map):
        src_schema = table_map[0]
        src_table = table_map[1]
        dst_schema = table_map[2].lower()
        dst_table = table_map[3].lower()
    ...
    tp = ThreadPool(parallel_num, while_wait_time=0.1)
    for table_map in table_map_list:
        tp.run(copy_one_index, (src_db, dst_db, table_map))
    tp.wait()
...
def copy_table(run_mode):
    ...
    def copy_one_table(src_db, dst_db, table_map, run_mode):
        ...
        for sql in sqls:
            if sql.startswith('alter table') and ' add foreign key ' in sql:
                pass
            elif sql.startswith('alter table') and ' add primary key ' in sql:
                pass
            elif sql.startswith('alter table') and ' add unique (' in sql:
                pass
            elif sql.startswith('create index ') or sql.startswith('create UNIQUE index '):
                pass
            else:
...
def copy_constraint(run_mode):
    ...
    def copy_k(src_db, dst_db, table_map, run_mode):
        src_schema = table_map[0]
        src_table = table_map[1]
        dst_schema = table_map[2].lower()
        dst_table = table_map[3].lower()
        ...
        for sql in sqls:
            if sql.startswith('create table '):
                ...
                pass
            elif sql.startswith('alter table ') and 'add foreign key' in sql and run_mode == 'copy_fk':
                sql = sql.replace(' ' + src_schema.lower() + '.', ' ' + dst_schema + '.')
                sql = sql.replace('."' + src_table.lower() + '"', '."' + dst_table + '"')
                try:
                    dst_cur.execute(sql)
                    ...
                    log.error('PG执行SQL失败', sql, str(_))
            elif sql.startswith('alter table ') and 'add unique' in sql and run_mode == 'copy_uk':
                sql = sql.replace(' ' + src_schema.lower() + '.', ' ' + dst_schema + '.')
                sql = sql.replace('."' + src_table.lower() + '"', '."' + dst_table + '"')
                try:
                    dst_cur.execute(sql)
                    ...
                    log.error('PG执行SQL失败', sql, str(_))
            elif sql.startswith('alter table ') and 'add primary key' in sql and run_mode == 'copy_pk':
                sql = sql.replace(' ' + src_schema.lower() + '.', ' ' + dst_schema + '.')
                sql = sql.replace('."' + src_table.lower() + '"', '."' + dst_table + '"')
                try:
                    dst_cur.execute(sql)
    ...
    tp = ThreadPool(parallel_num, while_wait_time=0.1)
    for table_map in table_map_list:
        tp.run(copy_k, (src_db, dst_db, table_map, run_mode))
    tp.wait()
...
def compare_data_number():
    ...
    def count(table_map, src_db, dst_db, global_scn):
        ...
        src_sql = 'select /*+full(t)*/ count(1) from ' + src_schema + '."' + src_table + '" as of scn ' + table_scn + ' t where ' + src_where
