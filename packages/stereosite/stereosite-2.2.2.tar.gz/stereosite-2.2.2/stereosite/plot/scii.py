...
class CustomDotplot(sc.pl.DotPlot):
    ...
    def __init__(self, minn: float, delta: float, alpha: float, *args: Any, **kwargs: Any):
...
def _unique_order_preserving(iterable) -> tuple:
...
def save_fig(fig, path, make_dir= True, ext="png", **kwargs) -> None:
    """
    Save a figure.
    Parameters
    ----------
...
def _dendrogram(data, method, **kwargs):
...
def ligrec(
    ...
    **kwargs,
    ...
    intensity_range
        Only show interactions whose intensity are within this **closed** interval.
    pvalue_threshold
    ...
    def filter_values(
        pvals: pd.DataFrame, intensities: pd.DataFrame, *, mask: pd.DataFrame, kind: str
    ):
        mask_rows = mask.any(axis=1)
        pvals = pvals.loc[mask_rows]
        intensities = intensities.loc[mask_rows]
    ...
    if isinstance(adata, AnnData):
        ...
        cluster_key = adata.uns.ligrec(cluster_key)
    ...
    if len(intensities_range) != 2:
        raise ValueError(f"Expected `intensities_range` to be a sequence of size `2`, found `{len(intensities_range)}`.")
    intensities_range = tuple(sorted(intensities_range))  # type: ignore[assignment]
    ...
    intensities: pd.DataFrame = adata["intensities"].loc[:, (source_groups, target_groups)]

    if pvals.empty:
        raise ValueError("No valid clusters have been selected.")

    intensities = intensities[(intensities >= intensities_range[0]) & (intensities <= intensities_range[1])]
    pvals = pvals[pvals <= pvalue_threshold]

    if remove_empty_interactions:
        pvals, intensities = filter_values(pvals, intensities, mask=~(pd.isnull(intensities) | pd.isnull(pvals)), kind="NaN")
    if remove_nonsig_interactions and alpha is not None:
        pvals, intensities = filter_values(pvals, intensities, mask=pvals <= alpha, kind="non-significant")
    ...
    intensities = intensities[label_ranges.keys()].fillna(0)
    intensities.columns = map(_SEP.join, intensities.columns.to_flat_index())
    intensities.index = map(_SEP.join, intensities.index.to_flat_index())
    ...
    try:
        if dendrogram == DendrogramAxis.BOTH:
            ...
            intensities = intensities.iloc[row_order, :].iloc[:, col_order]
    ...
    dp = (
        ...
        .legend(
            ...
            colorbar_title=r"$log_{10}(intensity + 1)$",
