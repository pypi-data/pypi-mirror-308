...
def main():
    ...
    while tl > 0:
        ...
        # Handle continuous empty days
        if skip_days_left > 0:
            print(f"Skipping day (part of empty period): {current_date.strftime('%Y-%m-%d')}")
            pointer += 1
            tl -= 1
            skip_days_left -= 1
            continue

        # Randomly decide if we should enter an empty period
        if add_empty_days and random.random() < 0.1:  # 10% chance to start skipping days
            skip_days_left = random.randint(1, emptyDays)
            print(f"Entering empty period of {skip_days_left} days.")
            continue
