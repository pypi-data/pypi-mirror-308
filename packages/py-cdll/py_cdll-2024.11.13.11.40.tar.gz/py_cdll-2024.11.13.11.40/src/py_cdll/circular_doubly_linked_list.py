...
class CDLL(MutableSequence[_T], Generic[_T]):
    ...
    def _nodes(self) -> Iterator[tuple[int, Node]]:
        ...
        # TODO: refactor all relying on index to use node.index instead
        for index, node in enumerate(nodes(head=self._head, callback=self._head_)):
            yield index, node
    ...
    def _node_replace(self, target: Node, replacement: Node) -> None:
        ...
        if is_target_head:
            ...
            # self._length = length(node=replacement)
