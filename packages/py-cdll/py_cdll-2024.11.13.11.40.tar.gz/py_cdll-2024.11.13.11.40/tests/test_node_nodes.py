...
# TODO: create test with callback, that has head change between yields, and then ends up in infinite loop,
#       because break condition of current being same as initial head can never be reached
#       because original head was removed from sequence
#       it seems like there may be a function overwriting head that is not testing sufficiently for head replacement
#       and triggering head_update()
#       so it is a combination of definitely needing to set head correctly when making changes
#       and then ensuring that the callback provides the correct head for the iterator when getting back around to it
