...
def test_replace_after_unique_overwrite_success():
    ...
    # Execution
    list0.replace_after_unique(value=data0, replacement=data)

    # Validation
    assert list0.after_unique(value=data0) is data
...
def test_replace_after_unique_single_entry_list_failure():
    ...
    with pytest.raises(NoAdjacentValueError):
        list0.replace_after_unique(value=data0, replacement=data1)
