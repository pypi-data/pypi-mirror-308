...
The REST API documentation can be found on [docs.llama-stack.com](https://docs.llama-stack.com). The full API of this library can be found in [api.md](api.md).
...
> [!NOTE]
> Once this package is [published to PyPI](https://app.stainlessapi.com/docs/guides/publish), this will become: `pip install --pre llama_stack_client`
...
client.with_options(max_retries=5).models.register(
    model={
        "identifier": "identifier",
        "llama_model": "llama_model",
        "metadata": {"foo": True},
...
client.with_options(timeout=5.0).models.register(
    model={
        "identifier": "identifier",
        "llama_model": "llama_model",
        "metadata": {"foo": True},
...
model = response.parse()  # get the object that `models.register()` would have returned
print(model)
```
