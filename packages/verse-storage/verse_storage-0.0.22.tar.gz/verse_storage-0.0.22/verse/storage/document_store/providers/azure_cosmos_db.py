...
class AzureCosmosDB(StoreProvider):
    collection: str | None
    endpoint: str
    database: str
    container: str | None
    ...
    def init(self, context: Context | None = None) -> None:
        ...
        ) = self._init_credential_client(CosmosClient, get_credential)
    ...
    async def ainit(self, context: Context | None = None) -> None:
        ...
        ) = self._init_credential_client(CosmosClient, aget_credential)
    ...
    def _init_credential_client(
        ...
        return credential, document_client, database_client
    ...
    def _get_collections(
        ...
        id_map_field = ParameterParser.get_collection_parameter(
            self.id_map_field, container
        )
        pk_map_field = ParameterParser.get_collection_parameter(
            self.pk_map_field, container
        )
    ...
    async def arun(
        ...
        if ncall is None:
            return await super().arun(operation=operation, context=context)
        nresult = await ncall.ainvoke()
        result = self._convert_nresult(nresult, state, op_parser, collections)
        if op_parser.op_equals(StoreOperation.CREATE_COLLECTION):
            self._adatabase_client = nresult
        return Response(result=result, native=dict(result=nresult, call=ncall))
    ...
    def _get_ncall(
        ...
        if op_parser.op_equals(StoreOperation.CREATE_COLLECTION):
            ...
            call = NCall(
                ...
                args,
                None,
                {CosmosResourceExistsError: ConflictError},
            )
        # DROP COLLECTION
...
class IndexHelper:
    @staticmethod
    def create_container_metadata(indexes: list[Index]) -> dict:
        metadata: dict = {"indexingPolicy": {}}
        for index in indexes:
            metadata = IndexHelper.add_index(metadata, index)
        return metadata

    @staticmethod
    def add_index(container_metadata: dict, index: Index) -> dict:
        ...
        if isinstance(
            ...
            path = IndexHelper._convert_to_index_path(index.path)
            added = IndexHelper._add_path(
        ...
        if not added:
            ...
            warning = f"Skipped adding index: {index.to_dict()}"
            warnings.warn(warning)
        return container_metadata

    @staticmethod
    def remove_index(container_metadata: dict, index: Index) -> dict:
        indexing_policy = container_metadata["indexingPolicy"]
        removed = False
        if isinstance(
            index,
        ...
        if not removed:
            ...
            warning = f"Skipped removing index: {index.to_dict()}"
            warnings.warn(warning)
        return container_metadata
    ...
    def _convert_spatial_index(
        ...
        path = IndexHelper._convert_to_index_path(field, scalar=False)
        path = path + "/*"
        if field_type == "point" or field_type is None:
            type = "Point"
    ...
    def _list_spatial_indexes(indexing_policy: dict) -> list[GeospatialIndex]:
        ...
        for idx in indexing_policy[set]:
            for type in idx["types"]:
                indexes.append(
                    GeospatialIndex(
                        field=IndexHelper._convert_to_field(idx["path"]),
                        field_type=field_type_mapping[idx["type"]],
                    )
                )
        return indexes
    ...
    def _list_wildcard_indexes(indexing_policy: dict) -> list[WildcardIndex]:
        ...
        for idx in indexing_policy[set]:
            if idx["path"].endswith("*"):
                ...
                indexes.append(WildcardIndex(path=path, excluded=excluded))
        return indexes
...
class ResourceHelper:
    ...
    def create_collection(
        ...
        args = {"id": container, "partition_key": pk}
        if (
            config is not None
            and config.indexes is not None
            and len(config.indexes) > 0
        ):
            metadata = IndexHelper.create_container_metadata(config.indexes)
    ...
    def drop_collection(self, container: str, nargs: Any):
        NCall(
    ...
    def create_index(self, client: Any, index: Index, nargs: Any) -> Any:
    ...
    def drop_index(self, client: Any, index: Index, nargs: Any) -> Any:
        metadata = NCall(client.read, None).invoke()
        metadata = IndexHelper.remove_index(metadata, index)
        args = {
            "container": client,
            "partition_key": metadata["partitionKey"],
            "indexing_policy": metadata["indexingPolicy"],
...
class AsyncResourceHelper:
    ...
    async def drop_collection(self, container: str, nargs: Any):
        await NCall(
            self.database_client.delete_container,
            {"container": container},
            nargs,
    ...
    async def create_index(self, client: Any, index: Index, nargs: Any) -> Any:
    ...
    async def drop_index(self, client: Any, index: Index, nargs: Any) -> Any:
        metadata = await NCall(client.read, None).ainvoke()
        metadata = IndexHelper.remove_index(metadata, index)
        args = {
            "container": client,
