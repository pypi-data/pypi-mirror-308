...
def get_tool_request_status(tool_request_group_id: UUID, client: Client) -> Status | None:
    response = get_request_group_status_sync_detailed(
        request_group_id=tool_request_group_id,
        client=client
    )
    if response.status_code == 200 and response.parsed:
        return response.parsed
    else:
        print(f"Failed to retrieve tool request status for request group ID {tool_request_group_id}. Response: {response}")
        return None
