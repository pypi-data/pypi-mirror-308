...
class OpenAI:
    ...
    def _ensure_messages(
        self, messages: list[Union[dict, ChatCompletionMessageType, ChatCompletionMessage]]
    ) -> list[ChatCompletionMessageType]:
        out = []
        for message in messages:
            if self._is_chat_completion_message_type(message):
                out.append(message)
            elif isinstance(message, dict):
                out.append(
                    self._create_message_param(
                        message["role"], message["content"], message.get("tool_call_id", None)
                    )
                )
            else:
    ...
    def call(
        ...
        if messages is None:
            ...
            if is_question:
                ...
                messages.append(
                    self._create_message_param(
                        "user",
                        "Can you solve or answer my question please with using your functions like `execute_subtask_parallel`?",
                    )
                )
                messages.append(
                    self._create_message_param(
                        ...
                        (
                            ...
                            "Therefore, I will answer or solve it without any hesitation."
                            "Also, I will use my functions like `execute_subtask_parallel` to solve or answer it to show my ability."
                            "</thoughts>"
                            "<final_answer>\n"
                ...
                messages.append(
                    self._create_message_param(
                        ...
                        (
                            "<thoughts>"
                            "It depends on the question but I am in the demonstration session."
                            "So I always answer or solve whatever I am asked to do."
                            "</thoughts>"
                            "<final_answer>\n"
                            "Since I am in the demonstration session, I always respond to your questions."
            ...
            with TPE(max_workers=mp.cpu_count()) as exe:
                ...
                for future in futures:
                    ...
                    content = (
                        content
                        + "\n"
                        + "Please log the function's name and its argments that you called and summarize its output in the `<memo>` tag with your brief explanation."
                    )
        ...
        return self._ensure_messages(messages)
    ...
    def truncate(self, messages: list[ChatCompletionMessage]) -> list[ChatCompletionMessage]:
        ...
        """
        messages = self._clean_tool_calls_after_assistant_answer(messages)
    ...
    def set_browser_functions(self):
        web_search, extract_page_content_from_url = get_browser_functions(self)
    ...
    def set_bash_function(self):
        ...
        self.set_function(file_creator_with_content)
