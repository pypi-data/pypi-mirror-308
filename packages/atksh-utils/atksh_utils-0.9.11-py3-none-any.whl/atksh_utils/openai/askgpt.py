...
def setup_ai(use_4o) -> OpenAI:
    ...
    ai = OpenAI(key, "gpt-4o-mini" if not use_4o else "gpt-4o")
        ...
        while True:
            ...
            if user_prompt:
                if not_called:
                    user_prompt = (
                        user_prompt
                        + "\nThink carefully and plan to answer the input within the <thoughts> tag before answering within the <final_answer> tag."
                        + "Do not include the function/tool calls in your plain text response but call them directly."
                        + "\nNow, you can start answering the input within the <thoughts> tag. Plus, please call tools/functions directly before the <final_answer> tag."
                        "Do not output the plain json or text of your callings like `tool_uses` or `recipient_name`. Just call them directly."
                    )
                if not_called:
