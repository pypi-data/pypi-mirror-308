...
def get_volumes(self, volumeId=None, workspaceId=None, organizationId=None):
    ...
    volumes = self.ana_api.getVolumes(organizationId=organizationId, workspaceId=workspaceId, volumeId=volumeId)
    if volumes:
...
def download_volume_data(self, volumeId, files=[], localDir=None, recursive=True, sync=False):
    ...
    while condition:
        result = self.ana_api.getVolumeData(volumeId=volumeId, keys=files, dir="", limit=100, recursive=recursive, offset=offset)
    ...
    for index, file in enumerate(source_files):
        ...
        if (sync == True and (source_hashes[index] in destination_hashes)):
            ...
            try:
                ...
                fileinfo = self.ana_api.putVolumeData(volumeId=volumeId, key=file, size=filesize)
