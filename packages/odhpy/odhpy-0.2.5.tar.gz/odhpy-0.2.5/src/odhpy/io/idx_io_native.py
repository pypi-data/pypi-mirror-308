...
def read_idx(filename, skip_header_bytes=None) -> pd.DataFrame:
    """_summary_

    Args:
        filename (_type_): Name of the IDX file.
        skip_header_bytes (bool | None): Whether to skip header bytes in the IDX file (related to the compiler used for IQQM). If set to None, attempt to detect the presence of header bytes automatically.
...
def write_idx_native(df: pd.DataFrame, filepath, type="None", units="None") -> None:
    ...
    with open(filepath, 'w') as f:
        ...
        ljust_or_truncate = lambda s, l: s.ljust(l)[0:l]
        for idx, col_name in enumerate(col_names): 
            ...
            f.write(f"{source_entry} {name_entry} {type_entry} {units_entry}\n")
    # write binary 
