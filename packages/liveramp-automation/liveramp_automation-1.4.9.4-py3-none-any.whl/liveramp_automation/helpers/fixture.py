...
def page(request) -> Generator[BrowserContext, None, None]:
    ...
    recording_broken_flag = os.getenv['PAGE_SCOPE'] == "session" and len(os.getenv["BROWSER_TYPES"]) > 1
    ...
    if webm_files and recording_broken_flag is not True:
        recording_path = os.path.join(os.getcwd(), recording_directory, webm_files[0])
        recording_name = "Recording_video_{}".format(request.param)
