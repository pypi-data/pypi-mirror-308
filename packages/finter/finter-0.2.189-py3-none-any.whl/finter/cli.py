    ...
    if poetry_path:
        ...
        if os.path.exists(home_poetry_file) and os.path.exists(home_poetry_lock_file):
            try:
                ...
                click.echo(
                    f"Error copying Poetry files from home directory: {e}", err=True
                )
                return
...
def start(path):
    ...
    if os.path.exists(full_path):
        if os.listdir(full_path):
            click.echo(f"Error: Directory '{full_path}' already exists.", err=True)
...
''')
        ...
        click.echo(f"Error creating model template: {e}", err=True)
        return
