...
from .models import (
    ...
    ListInspectionAggregateResultsOutput,
...
async def _deserialize_describe_recipient(
    ...
    if (_sub_processor_id := output.get("subProcessorId")) is not None:
        kwargs["sub_processor_id"] = expect_type(str, _sub_processor_id)
...
async def _deserialize_list_inspection_aggregate_results(
    http_response: HTTPResponse, config: Config
) -> ListInspectionAggregateResultsOutput:
    if http_response.status != 200 and http_response.status >= 300:
        raise await _deserialize_error_list_inspection_aggregate_results(
            http_response, config
        )
    ...
    if (_inspection_results := output.get("inspectionResults")) is not None:
        kwargs["inspection_results"] = _deserialize_inspection_aggregate_result_list(
    ...
    return ListInspectionAggregateResultsOutput(**kwargs)


async def _deserialize_error_list_inspection_aggregate_results(
    http_response: HTTPResponse, config: Config
) -> ApiError:
    code, message, parsed_body = await parse_rest_json_error_info(http_response)
...
async def _deserialize_error_data_validation_exception(
    ...
    if (_data := output.get("data")) is not None:
        kwargs["data"] = _deserialize_data_validation_exception_data(_data, config)
...
def _deserialize_all_sample_tokens(
    output: DocumentValue, config: Config
) -> list[dict[str, list[SampleTokens]]]:
    if not isinstance(output, list):
        raise ServiceError(f"Expected list, found {type(output)}")
    return [_deserialize_tokens(e, config) for e in output]
...
def _deserialize_confidence_snapshots(
    output: DocumentValue, config: Config
) -> list[dict[str, ConfidenceSnapshot]]:
    if not isinstance(output, list):
        raise ServiceError(f"Expected list, found {type(output)}")
    return [_deserialize_inspection_confidence_snapshot(e, config) for e in output]
...
def _deserialize_data_validation_exception_data(
    output: DocumentValue, config: Config
) -> dict[str, str]:
    if not isinstance(output, dict):
        raise ServiceError(f"Expected dict, found { type(output) }")
...
def _deserialize_inspection_aggregate_result(
    output: DocumentValue, config: Config
) -> InspectionAggregateResult:
    if not isinstance(output, dict):
        raise ServiceError(f"Expected dict, found {type(output)}")
    ...
    if (_latest_updated_at := output.get("latestUpdatedAt")) is not None:
        kwargs["latest_updated_at"] = epoch_seconds_to_datetime(
            expect_type(int | float, _latest_updated_at)
        )
    ...
    if (_snapshots := output.get("snapshots")) is not None:
        kwargs["snapshots"] = _deserialize_snapshot_list(_snapshots, config)

    if (_statuses := output.get("statuses")) is not None:
        kwargs["statuses"] = _deserialize_status_list(_statuses, config)

    if (_all_sample_tokens := output.get("allSampleTokens")) is not None:
        kwargs["all_sample_tokens"] = _deserialize_all_sample_tokens(
            _all_sample_tokens, config
        )

    if (_confidence_snapshots := output.get("confidenceSnapshots")) is not None:
        kwargs["confidence_snapshots"] = _deserialize_confidence_snapshots(
            _confidence_snapshots, config
        )

    if (_scan_iteration_ids := output.get("scanIterationIds")) is not None:
        kwargs["scan_iteration_ids"] = _deserialize_scan_iteration_ids(
            _scan_iteration_ids, config
        )

    return InspectionAggregateResult(**kwargs)


def _deserialize_inspection_aggregate_result_list(
    output: DocumentValue, config: Config
) -> list[InspectionAggregateResult]:
    if not isinstance(output, list):
        raise ServiceError(f"Expected list, found {type(output)}")
    return [_deserialize_inspection_aggregate_result(e, config) for e in output]
...
def _deserialize_scan_iteration_ids(output: DocumentValue, config: Config) -> list[str]:
    if not isinstance(output, list):
        raise ServiceError(f"Expected list, found {type(output)}")
    return [expect_type(str, e) for e in output]
...
def _deserialize_snapshot_list(
    output: DocumentValue, config: Config
) -> list[dict[str, InfotypeSnapshot]]:
    if not isinstance(output, list):
        raise ServiceError(f"Expected list, found {type(output)}")
    return [_deserialize_inspection_result_snapshot(e, config) for e in output]
...
def _deserialize_status_list(output: DocumentValue, config: Config) -> list[str]:
    if not isinstance(output, list):
        raise ServiceError(f"Expected list, found {type(output)}")
    return [expect_type(str, e) for e in output]
