...
from .models import (
    ...
    InspectionAggregateResultFilter,
    InspectionPolicy,
...
async def _serialize_create_recipient(
    ...
    if input.sub_processor_id is not None:
        result["subProcessorId"] = input.sub_processor_id
...
async def _serialize_list_inspection_aggregate_results(
    input: ListInspectionAggregateResultsInput, config: Config
) -> HTTPRequest:
    path = "/insight/list-aggregate"
    query: str = f""
    ...
    if input.filter is not None:
        result["filter"] = _serialize_inspection_aggregate_result_filter(
            input.filter, config
        )
...
def _serialize_inspection_aggregate_result_filter(
    input: InspectionAggregateResultFilter, config: Config
) -> dict[str, DocumentValue]:
    result: dict[str, DocumentValue] = {}
