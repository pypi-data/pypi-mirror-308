...
class Scheduler:
    ...
    def __init__(self,
        ...
        self.scheduler: BackgroundScheduler = self._init_scheduler(
            name, scheduler_type, auto_start, logger, timezone, executor_type,
            pool_size)
        ...
        if single_job and self.redis_client:
            ...
            if self.redis_client.set(lock_key, "locked", nx=True, ex=2):
                if iscoroutinefunction(func):
                    job = self.scheduler.add_job(self._lock_wrapper_async,
                                                 args=(func, job_id, args,
                                                       kwargs),
                                                 trigger=trigger,
                                                 **job_kwargs)
                else:
                    job = self.scheduler.add_job(self._lock_wrapper_sync,
                                                 args=(func, job_id, args,
                                                       kwargs),
                                                 trigger=trigger,
                                                 **job_kwargs)
    ...
    def _init_scheduler(self, name: str, scheduler_type: str, auto_start: bool,
                        logger: object, timezone: str, executor_type: str,
                        pool_size: int):
        global SCHEDULERS
    ...
    def _heartbeat(self, lock_key):
        """心跳线程，定期刷新锁的过期时间。"""
        while lock_key and self.scheduler.running and self.redis_client.get(
                lock_key):
    ...
    def shutdown(self,
        ...
        if self.scheduler.running:
            if wait:
                # 等待调度器关闭，设置超时
                futures = [
                    self.executor.submit(self.scheduler.shutdown, wait=True)
                ]
                try:
