...
class CellApi:
    ...
    async def get_cell(
        ...
        List all cell resources.  > **Note:** The output generated by this endpoint can be too large for the site too handle, and may produce an error or incorrect output. > Use `curl` in combination with `> output.json` to capture the output, or use an API client like Postman. 
