...
void copy_blocks(
  ...
  VLLM_DISPATCH_FLOATING_AND_BYTE_TYPES(
    key_caches[0].scalar_type(), "copy_blocks_kernel", ([&] {
      attention_kernels::copy_blocks_kernel<scalar_t><<<grid, block, 0, stream>>>(
...
#define CALL_RESHAPE_AND_CACHE(KV_T, CACHE_T, IS_FP8_KV_CACHE)                                     \
  attention_kernels::reshape_and_cache_kernel<KV_T, CACHE_T, IS_FP8_KV_CACHE><<<grid, block, 0, stream>>>(      \
...
#define CALL_CONVERT_FP8(Tout, Tin)                                 \
  attention_kernels::convert_fp8_kernel<Tout, Tin><<<grid, block, 0, stream>>>(  \
