...
#define LAUNCH_PAGED_ATTENTION_V1(HEAD_SIZE)                                                  \
  VLLM_DevFuncAttribute_SET_MaxDynamicSharedMemorySize(                                       \
    ((void*)attention_kernels::paged_attention_v1_kernel<T, CACHE_T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS,   \
      IS_FP8_KV_CACHE>), shared_mem_size);                                                    \
  attention_kernels::paged_attention_v1_kernel<T, CACHE_T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS,             \
...
#define LAUNCH_PAGED_ATTENTION_V2(HEAD_SIZE)                                                  \
  attention_kernels::paged_attention_v2_kernel<T, CACHE_T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS,             \
  ...
  <<<grid, block, shared_mem_size, stream>>>(                                                 \
    ...
    kv_head_stride,                                                                           \
    kv_scale);                                                                                \
  attention_kernels::paged_attention_v2_reduce_kernel<T, HEAD_SIZE, NUM_THREADS, PARTITION_SIZE>           \
