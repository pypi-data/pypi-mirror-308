...
class AndromedaSecurity:
    ...
    async def _get_cookie_token_api(self, api_token):
        ...
        """
        logger_function.info("Started retrieving cookie token API.")
        cookie_data = {}
        ...
        try:
            ...
            logger_function.info(f"Request took: {elapsed_time:.2f} seconds.")
            ...
            logger_function.error(f"Request error occurred: {cookie_data['err']}")
        except Exception as e:
            cookie_data["err"] = f"An unexpected error occurred: {str(e)}"
            logger_function.error(f"An unexpected error occurred: {cookie_data['err']}")
    ...
    async def _create_access_request_api(
        ...
        """
        logger_function.info("Started creating access request API")
    ...
    async def _get_origins_user_api(
        ...
        try:
            ...
            logger_function.error(f"HTTP error occurred: {origin_results['err']}")
        except requests.RequestException as e:
            origin_results["err"] = f"Request error occurred: {e}"
            logger_function.error(f"Request error occurred: {origin_results['err']}")
        except Exception as e:
    ...
    async def _end_session(self, provider_id, account_id, request_id, api_key):
        logger_function.info("Started ending the session for the access request in API")
    ...
    async def _get_reviewers_details(self, review_ident_id, request_id, api_token):
        ...
        """
        logger_function.info("Started getting andromeda reviewer details")
        reviewr_err = {}
        ...
        try:
            ...
            logger_function.error(f"Unexpected error: {str(err)}")
            return {"err": str(err)}
    ...
    async def _deny_request(self, account_id, request_id, review_id, api_key):
        ...
        """
        logger_function.info("Started denying the access request in API")
        reject = {}
    ...
    async def _get_access_request_details(self, identity_id, token_key=None):
        ...
        """
        logger_function.info("Started getting access request details")
        requestor_summary = {}
    ...
    async def _get_header(self, token):
        ...
        header_property = {}
        if not token:
            logger_function.error("Andromeda API key is not found or expired")
        ...
        if "err" not in cookie_key.keys():
            logger_function.info("Returning header and cookie token")
    ...
    async def _get_provider_api(self, andro_token, andro_user_id):
        ...
        """
        provider_result = {}
        logger_function.info(f"Started getting provider ANDRO USER: {andro_user_id}")
        ...
        try:
            ...
            logger_function.error(f"Error in provider API request: {str(e)}")
    ...
    async def _get_provider_account_api(self, provider_id, andro_token, andro_user_id):
        ...
        try:
            ...
            logger_function.error(f"Error in provider account API request: {str(e)}")
            return {"err": "Failed to connect to provider account API"}
    ...
    async def _get_all_eligibility_api(
        ...
        try:
            ...
            logger_function.error(f"Error in eligibility API request: {str(e)}")
