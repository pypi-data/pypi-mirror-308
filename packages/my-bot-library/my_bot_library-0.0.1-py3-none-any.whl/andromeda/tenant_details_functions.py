...
class TenantDetails:
    ...
    async def get_db_details(self, user_id, querys):
        ...
        logger.info(f"DB query executed successfully with : {cursor}")
        if cursor:
    ...
    async def get_db_activity_details(self, params, query_data):
        ...
        if cursor:
            ...
            logger.warning(f"couldn't find the teams in DB for this User ID:")
        return None, err
    ...
    async def insert_db_activity_details(self, params, query_data):
        ...
        if err:
            logger.error(f"Error in inserting the db activity details {err}")
