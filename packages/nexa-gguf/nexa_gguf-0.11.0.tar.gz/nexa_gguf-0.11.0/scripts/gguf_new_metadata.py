...
class MetadataDetails(NamedTuple):
    type: nexa_gguf.GGUFValueType
    value: Any
    description: str = ''


def get_byteorder(reader: nexa_gguf.GGUFReader) -> nexa_gguf.GGUFEndian:
    if np.uint32(1) == np.uint32(1).newbyteorder("<"):
        # Host is little endian
        host_endian = nexa_gguf.GGUFEndian.LITTLE
        swapped_endian = nexa_gguf.GGUFEndian.BIG
    else:
        # Sorry PDP or other weird systems that don't use BE or LE.
        host_endian = nexa_gguf.GGUFEndian.BIG
        swapped_endian = nexa_gguf.GGUFEndian.LITTLE
...
def decode_field(field: nexa_gguf.ReaderField | None) -> Any:
    if field and field.types:
        main_type = field.types[0]

        if main_type == nexa_gguf.GGUFValueType.ARRAY:
            sub_type = field.types[-1]

            if sub_type == nexa_gguf.GGUFValueType.STRING:
                return [str(bytes(field.parts[idx]), encoding='utf-8') for idx in field.data]
            else:
                return [pv for idx in field.data for pv in field.parts[idx].tolist()]
        if main_type == nexa_gguf.GGUFValueType.STRING:
...
def get_field_data(reader: nexa_gguf.GGUFReader, key: str) -> Any:
...
def copy_with_new_metadata(reader: nexa_gguf.GGUFReader, writer: nexa_gguf.GGUFWriter, new_metadata: dict[str, MetadataDetails], remove_metadata: Sequence[str]) -> None:
    for field in reader.fields.values():
        # Suppress virtual fields and fields written by GGUFWriter
        if field.name == nexa_gguf.Keys.General.ARCHITECTURE or field.name.startswith('GGUF.'):
            logger.debug(f'Suppressing {field.name}')
            continue

        # Skip old chat templates if we have new ones
        if field.name.startswith(nexa_gguf.Keys.Tokenizer.CHAT_TEMPLATE) and nexa_gguf.Keys.Tokenizer.CHAT_TEMPLATE in new_metadata:
    ...
    if nexa_gguf.Keys.Tokenizer.CHAT_TEMPLATE in new_metadata:
        logger.debug('Adding chat template(s)')
        writer.add_chat_template(new_metadata[nexa_gguf.Keys.Tokenizer.CHAT_TEMPLATE].value)
        del new_metadata[nexa_gguf.Keys.Tokenizer.CHAT_TEMPLATE]
...
def main() -> None:
    tokenizer_metadata = (getattr(nexa_gguf.Keys.Tokenizer, n) for n in nexa_gguf.Keys.Tokenizer.__dict__.keys() if not n.startswith('_'))
    ...
    if args.general_name:
        new_metadata[nexa_gguf.Keys.General.NAME] = MetadataDetails(nexa_gguf.GGUFValueType.STRING, args.general_name)

    if args.general_description:
        new_metadata[nexa_gguf.Keys.General.DESCRIPTION] = MetadataDetails(nexa_gguf.GGUFValueType.STRING, args.general_description)

    if args.chat_template:
        new_metadata[nexa_gguf.Keys.Tokenizer.CHAT_TEMPLATE] = MetadataDetails(nexa_gguf.GGUFValueType.STRING, json.loads(args.chat_template) if args.chat_template.startswith('[') else args.chat_template)
    ...
    if args.chat_template_config:
        with open(args.chat_template_config, 'r') as fp:
            ...
            if template:
                new_metadata[nexa_gguf.Keys.Tokenizer.CHAT_TEMPLATE] = MetadataDetails(nexa_gguf.GGUFValueType.STRING, template)

    if args.pre_tokenizer:
        new_metadata[nexa_gguf.Keys.Tokenizer.PRE] = MetadataDetails(nexa_gguf.GGUFValueType.STRING, args.pre_tokenizer)
    ...
    reader = nexa_gguf.GGUFReader(args.input, 'r')

    arch = get_field_data(reader, nexa_gguf.Keys.General.ARCHITECTURE)
    endianess = get_byteorder(reader)

    token_list = get_field_data(reader, nexa_gguf.Keys.Tokenizer.LIST) or []
    ...
    for name, token in args.special_token or []:
        if name not in token_names:
            ...
            new_metadata[token_names[name]] = MetadataDetails(nexa_gguf.GGUFValueType.UINT32, ids[0], f'= {token}')
    ...
    for name, id_string in args.special_token_by_id or []:
        if name not in token_names:
            ...
            if id_int >= 0 and id_int < len(token_list):
                new_metadata[token_names[name]] = MetadataDetails(nexa_gguf.GGUFValueType.UINT32, id_int, f'= {token_list[id_int]}')
            else:
    ...
    writer = nexa_gguf.GGUFWriter(args.output, arch=arch, endianess=endianess)

    alignment = get_field_data(reader, nexa_gguf.Keys.General.ALIGNMENT)
