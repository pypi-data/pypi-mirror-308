...
def convert_byteorder(reader: nexa_gguf.GGUFReader, args: argparse.Namespace) -> None:
    ...
    for tensor in reader.tensors:
        if tensor.tensor_type not in (
            nexa_gguf.GGMLQuantizationType.F32,
            nexa_gguf.GGMLQuantizationType.F16,
            nexa_gguf.GGMLQuantizationType.Q8_0,
        ):
    ...
    for idx, tensor in enumerate(pbar := tqdm(reader.tensors, desc="Converting tensor")):
        ...
        if tensor.tensor_type == nexa_gguf.GGMLQuantizationType.Q8_0:
...
def main() -> None:
    ...
    reader = nexa_gguf.GGUFReader(args.model, 'r' if args.dry_run else 'r+')
