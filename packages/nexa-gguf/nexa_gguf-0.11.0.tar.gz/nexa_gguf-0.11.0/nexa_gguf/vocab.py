...
class Vocab(BaseVocab, Protocol):
    ...
    def all_tokens(self) -> Iterable[tuple[bytes, float, nexa_gguf.TokenType]]: ...
...
class BpeVocab(Vocab):
    ...
    def bpe_tokens(self) -> Iterable[tuple[bytes, float, nexa_gguf.TokenType]]:
        reverse_vocab = {id: encoded_tok for encoded_tok, id in self.vocab.items()}

        for i, _ in enumerate(self.vocab):
            yield reverse_vocab[i], 0.0, nexa_gguf.TokenType.NORMAL

    def added_tokens(self) -> Iterable[tuple[bytes, float, nexa_gguf.TokenType]]:
        for text in self.added_tokens_list:
            score = -1000.0
            yield text.encode("utf-8"), score, nexa_gguf.TokenType.CONTROL
...
class SentencePieceVocab(Vocab):
    ...
    def sentencepiece_tokens(self) -> Iterable[tuple[bytes, float, nexa_gguf.TokenType]]:
        ...
        for i in range(tokenizer.vocab_size()):
            ...
            if tokenizer.IsUnknown(i):
                toktype = nexa_gguf.TokenType.UNKNOWN
            ...
            if tokenizer.IsUnused(i):
                toktype = nexa_gguf.TokenType.UNUSED
            if tokenizer.IsByte(i):
                toktype = nexa_gguf.TokenType.BYTE
    ...
    def added_tokens(self) -> Iterable[tuple[bytes, float, nexa_gguf.TokenType]]:
        for text in self.added_tokens_list:
            score = -1000.0
            yield text.encode("utf-8"), score, nexa_gguf.TokenType.USER_DEFINED
...
class LlamaHfVocab(Vocab):
    ...
    def hf_tokens(self) -> Iterable[tuple[bytes, float, nexa_gguf.TokenType]]:
        reverse_vocab = {
    ...
    def get_token_type(self, token_id: int, token_text: bytes, special_ids: set[int]) -> nexa_gguf.TokenType:
        # Special case for byte tokens
        if re.fullmatch(br"<0x[0-9A-Fa-f]{2}>", token_text):
            return nexa_gguf.TokenType.BYTE

        # Determine token type based on whether it's a special token
        return nexa_gguf.TokenType.CONTROL if token_id in special_ids else nexa_gguf.TokenType.NORMAL
