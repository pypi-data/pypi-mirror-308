...
columns_full = ['platform', 'id', 'type', 'action', 'actor_id', 'actor_login', 'repo_id', 'repo_name', 'org_id', 'org_login', 'created_at', 'issue_id', 'issue_number', 'issue_title', 'body', 'issue_labels.name', 'issue_labels.color', 'issue_labels.default',
                ...
                'repo_has_projects', 'repo_has_downloads', 'repo_has_wiki', 'repo_has_pages', 'repo_license', 'repo_default_branch', 'repo_created_at', 'repo_updated_at', 'repo_pushed_at', 'repo_topics.name', 'pull_review_state', 'pull_review_author_association',
...
columns_simple = ['id', 'type', 'action', 'actor_id', 'actor_login', 'repo_id', 'repo_name', 'org_id', 'org_login', 'created_at', 'issue_id', 'issue_number', 'issue_title', 'body', 'issue_author_id', 'issue_author_login', 'issue_assignee_id',
                  'issue_assignee_login', 'issue_assignees.login', 'issue_assignees.id', 'issue_comment_id', 'issue_comment_author_id', 'issue_comment_author_login', 'pull_merged', 'pull_merge_commit_sha', 'pull_merged_by_id', 'pull_merged_by_login',
                  'pull_requested_reviewer_id', 'pull_requested_reviewer_login', 'pull_base_ref', 'pull_head_repo_id', 'pull_head_repo_name', 'pull_head_ref', 'repo_description', 'pull_review_id', 'pull_review_comment_id', 'pull_review_comment_path',
                  'pull_review_comment_author_id', 'pull_review_comment_author_login', 'push_id', 'push_ref', 'push_head', 'push_commits.name', 'push_commits.email', 'push_commits.message', 'fork_forkee_id', 'fork_forkee_full_name',
                  'fork_forkee_owner_id', 'fork_forkee_owner_login', 'delete_ref', 'delete_ref_type', 'create_ref', 'create_ref_type', 'create_master_branch', 'member_id', 'member_login', 'release_id', 'release_tag_name', 'release_name',
                  'release_author_id', 'release_author_login', 'release_body', 'release_assets.name', 'commit_comment_id', 'commit_comment_author_id', 'commit_comment_author_login', 'commit_comment_path', 'commit_comment_sha']
...
# 定义正则模式
re_ref_patterns = {
    'Issue_PR': [r'https?://(?:www\.|redirect\.)?github(?:-redirect\.dependabot)?\.com/[A-Za-z0-9][-0-9a-zA-Z]*/[A-Za-z0-9][-_0-9a-zA-Z\.]*/(?:issues|pull)/\d+#[-_0-9a-zA-Z\.%#/:]+\d+(?![\d/])',  # Issue与PullRequest下的事件，包含了IssueEvent（示例：issues/{issue_number}#issue-{issue_id}）或PullRequestEvent事件（示例：(?:issues|pull)/{issue_number}#issue-{issue_id}），IssueCommentEvent事件（Issue与PullRequest下的评论均为IssueCommentEvent事件，示例{_repo_full_name}/issues/{issue_number}#issuecomment-{issue_comment_id}），PullRequestReview事件（示例：{issue_number}#pullrequestreview-{pull_review_id}），PullRequestReviewCommentEvent事件的部分格式（示例：{issue_number}#discussion_r{pull_review_comment_id}）
                 r'https?://(?:www\.|redirect\.)?github(?:-redirect\.dependabot)?\.com/[A-Za-z0-9][-0-9a-zA-Z]*/[A-Za-z0-9][-_0-9a-zA-Z\.]*/issues/\d+(?![\d/#])', r'https?://(?:www\.|redirect\.)?github(?:-redirect\.dependabot)?\.com/[A-Za-z0-9][-0-9a-zA-Z]*/[A-Za-z0-9][-_0-9a-zA-Z\.]*/pull/\d+(?![\d/#])',  # IssueEvent与PullRequestEvent事件是两个独立的事件
                 r'https?://(?:www\.|redirect\.)?github(?:-redirect\.dependabot)?\.com/[A-Za-z0-9][-0-9a-zA-Z]*/[A-Za-z0-9][-_0-9a-zA-Z\.]*/(?:issues|pull)/\d+/files(?:/[0-9a-fA-F]{40})?#r\d+(?![\d#/])',  # PullRequestReveiwCommentEvent事件的部分格式（示例：{issue_number}/files#r{pull_review_comment_id} or {issue_number}/files/{push_head}#r{pull_review_comment_id}）
                 r'(?<!\\)(?:(?!\\r|\\n|\\t)[^\s\(\[\]\>\'\";]?)*#0*[1-9][0-9]*(?![\d/#a-z])'],  # IssueEvent或PullRequestEvent事件或未知的类型
    'SHA': [r'https?://(?:www\.|redirect\.)?github(?:-redirect\.dependabot)?\.com/[A-Za-z0-9][-0-9a-zA-Z]*/[A-Za-z0-9][-_0-9a-zA-Z\.]*/pull/\d+/commits/[0-9a-fA-F]{40}(?![0-9a-fA-F/#])', r'https?://(?:www\.|redirect\.)?github(?:-redirect\.dependabot)?\.com/[A-Za-z0-9][-0-9a-zA-Z]*/[A-Za-z0-9][-_0-9a-zA-Z\.]*/commit/[0-9a-fA-F]{40}(?![0-9a-fA-F/#])', r'(?:(?<=[\s\(\[:])|(?<=^))([0-9a-fA-F]{40})(?![0-9a-fA-F/#])', r'(?i)(?:(?<=[\s\(\[:`])|(?<=^))((?:(?:commit[^0-9a-fA-F]?)|(?:sha[^0-9a-fA-F]?))?[0-9a-fA-F]{7})(?![0-9a-zA-Z/#])'],  # 包含Commit的sha，以及未知类型实体的sha
    'Actor': [r'(https?://(?:www\.|redirect\.)?github(?:-redirect\.dependabot)?\.com/[A-Za-z0-9][-0-9a-zA-Z]*(?:\[bot\])?)(?![-A-Za-z0-9/])', r'(?:(?<=[\s\(\[])|(?<=^))(@[A-Za-z0-9][-0-9a-zA-Z]*(?:\[bot\])?)(?![-A-Za-z0-9/])', r'\w[-\w.+]*@(?:[A-Za-z0-9][-A-Za-z0-9]+\.)+[A-Za-z]{2,14}'],
    'Repo': [r'https?://(?:www\.|redirect\.)?github(?:-redirect\.dependabot)?\.com/(?!apps)[A-Za-z0-9][-0-9a-zA-Z]*/[A-Za-z0-9][-_0-9a-zA-Z\.]*(?![-_A-Za-z0-9\./])'],
    'Branch_Tag_GHDir': ['https?://(?:www\.|redirect\.)?github(?:-redirect\.dependabot)?\.com/[A-Za-z0-9][-0-9a-zA-Z]*/[A-Za-z0-9][-_0-9a-zA-Z\.]*/tree/[^\:\[\?\s\*\\\\,;\]`)]*(?!>[\:\[\?\s\*,#\|\+@\<\>≤≥\=≠\&$\{\}\]\";`])'],
    'CommitComment': [r'https?://(?:www\.|redirect\.)?github(?:-redirect\.dependabot)?\.com/[A-Za-z0-9][-0-9a-zA-Z]*/[A-Za-z0-9][-_0-9a-zA-Z\.]*/commit/[0-9a-fA-F]{40}#commitcomment-\d+(?![\d#/])'],
