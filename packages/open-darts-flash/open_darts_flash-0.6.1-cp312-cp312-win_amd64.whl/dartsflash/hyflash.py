...
class HyFlash(PyFlash):
    ...
    def calc_df(self, pressure, temperature, composition, phase: str = "sI"):
        ...
        """
        self.f.evaluate_PT(pressure, temperature, composition)
    ...
    def calc_properties(self, state_spec: dict, compositions: dict, state_variables: list, flash_results: xr.Dataset,
        ...
        def evaluate(state):
            ...
            Hv = self.eos[nonaq_eos].H_PT(state[0], state[1], x[1, :]) * R
            Ha = nH * self.eos[aq_eos].H_PT(state[0], state[1], x[0, :]) * R
            Hh = self.hydrate_eos[hydrate_phase].H_PT(state[0], state[1], xH) * (nH + 1) * R
