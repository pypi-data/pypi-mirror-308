...
__all__ = ['AntPersonalAccessToken', 'Column', 'Download', 'Label', 'License', 'Link', 'Member', 'Permission', 'Project', 'Record', 'Relation', 'Role', 'SbsRecord', 'Script', 'Table', 'Task', 'User']
...
class Download(ModelBaseClass):
	...
	@classmethod
	def get(cls, download_id: str) -> dict:
		"""Download file

		Args:
			download_id(str): ID of Download
		"""
...
class Label(ModelBaseClass):
	...
	def all(cls, license_id: str, project_id: Optional[str] = None) -> list[Label]:
		...
		response: requests.Response = Label._call_api(method_type=HttpMethodType.GET, endpoint=endpoint, parameters=parameters)
		if response.ok:
...
class License(ModelBaseClass):
	...
	@classmethod
	@preprocess_arguments
	def all_modules(cls, license_id: str, filters: Optional[dict] = None) -> list[Module]:
		...
		response: requests.Response = License._call_api(method_type=HttpMethodType.GET, endpoint=endpoint, parameters=parameters)
		if response.ok:
	...
	def update_password(cls, old_password: str, new_password: str) -> list[License]:
		...
		Args:
			old_password(str): For example '123456'
			new_password(str): For example '123456'
		"""
		parameters = {
...
class Project(ModelBaseClass):
	...
	@classmethod
	@preprocess_arguments
	def update(cls, project_id: str, name: Optional[str] = None, description: Optional[str] = None, number: Optional[str] = None, is_archive: Optional[str] = None) -> Project:
		...
		Args:
			...
			is_archive(str): For example 'true'
		"""
		parameters = {
			'name': name,
			'description': description,
	...
	@classmethod
	def find_by_slug(cls, slug_id: str) -> Project:
		"""Find project by slug

		Args:
			slug_id(str): slug of project
		"""
		endpoint = f"rojects/slug/{slug_id}/find"
		response: requests.Response = Project._call_api(method_type=HttpMethodType.GET, endpoint=endpoint)
		if response.ok:
			return Project._from_json(response.json())
		else:
	...
	@classmethod
	@preprocess_arguments
	def favorite(cls, project_id: str, favorite: Optional[bool] = None) -> Project:
		"""favorite project
	...
	@classmethod
	def load_image(cls, project_id: str) -> dict:
		"""Fetch project image
...
class SbsRecord(ModelBaseClass):
	...
	def search_sbs_records(cls, project_id: str, code_id: str) -> list[SbsRecord]:
		...
		response: requests.Response = SbsRecord._call_api(method_type=HttpMethodType.GET, endpoint=endpoint, parameters=parameters)
		if response.ok:
			return [SbsRecord._from_json(i) for i in response.json()]
		else:
			raise ValueError(f'Could not load data from API, response code {response.status_code} with response {response.text}')
...
class Task(ModelBaseClass):
	...
	@classmethod
	@preprocess_arguments
	def create_check(cls, task_id: str, title: str) -> dict:
		"""Create task check
	...
	@classmethod
	@preprocess_arguments
	def query_all(cls, license_id: str, query_string: Union[str, QueryBuilder], is_task_id: Optional[bool] = None) -> dict:
	...
	@classmethod
	@preprocess_arguments
	def query(cls, license_id: str, task_id: str, query_string: Union[str, QueryBuilder]) -> dict:
		...
		endpoint = f"licenses/{license_id}/tasks/{task_id}"
		response: requests.Response = Task._call_api(method_type=HttpMethodType.GET, endpoint=endpoint, parameters=parameters)
		if response.ok:
			return response.json()
		else:
