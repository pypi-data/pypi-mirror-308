...
class ScalingAnalysis:
    ...
    def __detect_scaling_mode(self, dfs: List[pd.DataFrame]) -> ScalingMode:
        ...
        if base_case["instructions"].values[0] <= 0:
            logging.warning(
                f"Unable to detect ScalingMode, as no instructions are available. Run with PAPI. Assuming {ScalingMode.STRONG}"
            )
            return ScalingMode.STRONG
    ...
    def __compute_scaling_metrics(
        ...
        # NO Papi cases
        clean_df["instructionScaling"] = clean_df["instructionScaling"].fillna(0)
        clean_df["ipcScaling"] = clean_df["ipcScaling"].fillna(0)
        clean_df["frequencyScaling"] = clean_df["frequencyScaling"].fillna(0)
