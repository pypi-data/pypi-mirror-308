...
class VerificationParameters():
    ...
    :type tile_size: int
    :param user_name: the executing user which will be used in issue features.
    :type user_name: str
    :param desired_parallel_processing: the desired number of parallel worker processes to be used if the server allows parallel processing.
    :type desired_parallel_processing: int
    :param update_issues_in_verified_model: if True, the issues will be updated in the error datasets of the verified model.
    :type update_issues_in_verified_model: bool
    :param save_verification_statistics: if True, the verification statistics will be saved into the Data Dictionary database.
    :type save_verification_statistics: bool
    :param objects_to_verify: a dictionary containing the dataset IDs and the object IDs to be verified. The dataset ID can be looked up in the Data Dictionary Editor -> Data -> Data Models -> Dataset -> Properties.  Use the method add_objects_to_verify to add a list of IDs for a specific dataset.
    :type objects_to_verify: dict
    """
...
class InvolvedTable:
    ...
    :type table_name: str
    :param object_ids: A list of object IDs from the table that are involved in the issue.
    :type object_ids: list
    """
...
class Issue:
    ...
    :type issue_msg: shared_qa.IssueMsg

    The Issue object extracts some properties from the issue message and makes them available as attributes:
    :ivar description: The description of the issue.
    :ivar involved_objects: A list of InvolvedTable objects that are involved in the issue.
    :ivar geometry: The geometry involved of the issue.
    :ivar issue_code: The issue code ID.
    :ivar allowable: If the issue is allowable.
    :ivar stop_condition: If the issue is a stop condition.
    """
...
class Service:
    ...
    def verify(self,
        ...
        if len(parameters.objects_to_verify) > 0:
