import PyWebRequests.functions as request_functions
...
def get_random_user_agent() -> str | None:
    ...
    random_user_agent = request_functions.find_web_element(
        request_functions.get_html("https://user-agents.net/random", headers={"User-Agent": "Mozilla/5.0"}), "//ol/li/a"
    )
...
def get_free_proxies(protocol: str | list[str] | None = None) -> dict[str, str]:
    ...
    Args:
        protocol (str | list[str], optional): The desired proxy protocol(s).
