...
def preprocess_data(instances_df, Holdings_df, Items_df, df_location, df_mtypes, df_statcode, df_loans,header_dict,tenant):
...
def generate_report(final_df, report_name,tenant):
    ...
    reports = {
        "Books by Created Month": lambda: final_df.groupby(
            final_df['Created Date'].dt.to_period('M')).size().reset_index(name='Count').to_excel(
            "Books_by_Created_Month.xlsx", index=False),
        "Suppress Status Summary": lambda: final_df['Suppress from Discovery'].value_counts().reset_index(
            name='Count').to_excel("Suppress_Status_Summary.xlsx", index=False),
        "Top 10 Most Frequently Updated Titles": lambda: final_df['Title'].value_counts().head(10).reset_index(
            name='Frequency').to_excel("Top_10_Most_Frequently_Updated_Titles.xlsx", index=False),
        "Books by Material Type": lambda: final_df['Material Type'].value_counts().reset_index(name='Count').to_excel(
            "Books_by_Material_Type.xlsx", index=False),
        "Average Loan Count": lambda: pd.DataFrame(
            {"Average Loan Count": [final_df['Number of Loans'].mean()]}).to_excel("Average_Loan_Count.xlsx",
                                                                                   index=False),
        "Recently Added Records": lambda: final_df[
            final_df['Created Date'] >= datetime.now() - timedelta(days=30)].to_excel("Recently_Added_Records.xlsx",
                                                                                      index=False),
        "Top 5 Most Loaned Titles": lambda: final_df.nlargest(5, 'Number of Loans')[
            ['Title', 'Number of Loans']].to_excel("Top_5_Most_Loaned_Titles.xlsx", index=False),
        "Staff Suppressed Records": lambda: final_df['Staff Suppress'].value_counts().reset_index(
            name='Count').to_excel("Staff_Suppressed_Records.xlsx", index=False),
        "Suppressed Records Summary": lambda: final_df[
            (final_df['Suppress from Discovery'] == True) | (final_df['Staff Suppress'] == True)].to_excel(
            "Suppressed_Records_Summary.xlsx", index=False),
        "Titles with Alternative Titles": lambda: final_df[final_df['Alternative Title'].notnull()].to_excel(
            "Titles_with_Alternative_Titles.xlsx", index=False),
        "Records with High Loan Counts": lambda: final_df[final_df['Number of Loans'] > 10].to_excel(
            "Records_with_High_Loan_Counts.xlsx", index=False),
        "Titles by Updated User": lambda: final_df.groupby('Updated By')['Title'].count().reset_index(
            name='Count').to_excel("Titles_by_Updated_User.xlsx", index=False),
        "Edition Distribution": lambda: final_df['Edition'].value_counts().reset_index(name='Count').to_excel(
            "Edition_Distribution.xlsx", index=False),
        "Titles with Item Discovery Suppress": lambda: final_df[final_df['Item Discovery Suppress'].notnull()].to_excel(
            "Titles_with_Item_Discovery_Suppress.xlsx", index=False),
        "Loaned Items by Location": lambda: final_df.groupby('Location Name')[
            'Number of Loans'].sum().reset_index().to_excel("Loaned_Items_by_Location.xlsx", index=False),
        "Books with Check-in Records": lambda: final_df[final_df['Last Checkin Date'].notnull()].to_excel(
            "Books_with_Checkin_Records.xlsx", index=False),
        "Titles with Copy Numbers": lambda: final_df[final_df['Copy Number'].notnull()].to_excel(
            "Titles_with_Copy_Numbers.xlsx", index=False),
        "Records Created by User": lambda: final_df['Created By'].value_counts().reset_index(name='Count').to_excel(
            "Records_Created_by_User.xlsx", index=False),
        "Books by Loan Type": lambda: final_df['Loan Type'].value_counts().reset_index(name='Count').to_excel(
            "Books_by_Loan_Type.xlsx", index=False),
        "Titles Without Loans": lambda: final_df[final_df['Number of Loans'] == 0].to_excel("Titles_Without_Loans.xlsx",
                                                                                            index=False),
        "Loan Activity by Material Type": lambda: final_df.groupby('Material Type')[
            'Number of Loans'].sum().reset_index().to_excel("Loan_Activity_by_Material_Type.xlsx", index=False),
        "Recently Updated Records": lambda: final_df[
            final_df['Updated Date'] >= datetime.now() - timedelta(days=30)].to_excel("Recently_Updated_Records.xlsx",
                                                                                      index=False),
        "Full Bibliographic Report": lambda: final_df.to_excel(f"Bibliographic_Report_{tenant}.xlsx", index=False)
    }
