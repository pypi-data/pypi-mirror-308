...
from mke_sculib.helpers import make_zulustr, match_zulutime, get_utcnow, parse_zulutime, colors, colors_dc, print_color, is_notebook, get_ntp_time_with_socket
...
def activate_logging_mattermost(whoami, url_qry = 'http://10.98.76.45:8990/logurl'):
    ...
    activate_logging_mattermost(url, whoami)
    return True
...
def load(antenna_id='', post_put_delay=0.0, debug=False, url_qry = 'http://10.98.76.45:8990/antennas', **kwargs):
    ...
    try:
        ...
        params = {}
    dish = scu(dc['address'], post_put_delay=post_put_delay, debug=debug, antenna_id=antenna_id, antenna_dc=dc, **kwargs)
...
class EventStreamHandler():
    ...
    def __init__(self, parent, dict_size = 100, verb=True, on_new_cbs=None) -> None:
...
class DataStreamHandler():
    ...
    def __init__(self, parent, channels=None, limit_too_old_s = 0.5, verb=True, on_new_cbs=None) -> None:
        assert not parent is None, 'parent can not be None'
        self.parent = parent
    ...
    def wait_for_start(self, timeout=10.):
        if self.verb: log(f'{self.parent.ip} waiting for data stream to provide first data...', color=colors.HEADER)
        t = time.time()
        while (time.time() - t) < timeout and not self.tickcount:
            time.sleep(0.1)
...
class TrackHandler(object):
    def __init__(self, scu, t, az, el, activate_logging, config_name, wait_start=True, verb=False, wait_for_end_pos=True):
        self.scu = scu
...
def event_start_before(f):
    def wrapper(self, *args, **kwargs):
        ...
        if self.wait_done_by_uuid and (self.event_streamer is None or not self.event_streamer.is_alive()):
            log(f'Starting event stream because method with name "{f.__name__}"  requested it', colors.HEADER)
...
def event_start_before_wait_done_after(f):
    def wrapper(self, *args, **kwargs):
        ...
        if self.wait_done_by_uuid and valid_uuid:
            ...
            warnings.warn(f"wait by UUID requested for {f.__name__}, but API call did not return a valid uuid! {type(uuid)=} | {uuid=}")
...
class scu():
    ...
    def __init__(self, address='', ip='', port='8080', antenna_id='', debug=False, lims_az=(-270.0, 270, 3.0), lims_el=(15, 90, 1.35), dish_type='mke', post_put_delay=0.2, use_socket=True, influx_token='', kwargs_requests = None, wait_done_by_uuid=True, start_streams_on_construct=False, timeout_wait_uuid=900, antenna_dc=None, **kwargs):
        ...
        if (wait_done_by_uuid and use_socket) and start_streams_on_construct:
            self.start_event_stream()

        if use_socket and start_streams_on_construct:
    ...
    def __str__(self):
        ...
        return f'scu object{antenna_id} @{self.address}, dish_type: "{self.dish_type}" {is_verified}'
    ...
    def _feedback(self, r, get_uuid=True):
        ...
        if get_uuid:
            try:
                ...
                uuid = ''
                log(f'ERROR in getting UUID: {r} | {r.request.url} | {err}', color=colors.FAIL)
                if self.debug:
                    print(r.text)
    ...
    def wait_uuid_done(self, uuid:str=None, funname='unknown_command', timeout = None, dt_poll=0.1):
        ...
        if self.debug:
            log(f'wait_uuid_done | waiting for {uuid=}')
    ...
    def move_to_azel(self, az_angle, el_angle, az_vel=None, el_vel=None, wait_settle=False):
        ...
        if az_vel is None and el_vel is None: 
            ...
            log('move to az: {:.4f} el: {:.4f} (vels: ({:.4f}, {:.4f})'.format(az_angle, el_angle, az_vel, el_vel))
    ...
    def abs_azel(self, az_angle, el_angle, wait_settle=False):
        ...
        log('move to az: {:.4f} el: {:.4f}'.format(az_angle, el_angle))
...
#        log('logger state ')
    ...
    def create_configs_minimal(self):
    ...
    def start(self, az_start=None, el_start=None, band_start=None, az_speed=None, el_speed=None, send_default_configs=True, interlock_acknowledge_on_start=True, **kwargs):
        ...
        if interlock_acknowledge_on_start:
            try:
                ...
                log(f'WARNING: Interlock acknowledged failed with error: {err} --> skipping', color=colors.WARNING)
        ...
        log('Getting configs from SCU')
        configs_scu_dc = self.logger_configs()
        configs_scu = [c['name'] for c in configs_scu_dc]
        
        if not 'all' in configs_scu:
            log('WARNING: logger config "all" is missing in SCO logger configurations', color=colors.WARNING)
        if not 'full_configuration' in configs_scu:
            log('WARNING: logger config "full_configuration" is missing in SCO logger configurations', color=colors.WARNING)
    ...
    def link_stellarium_bg(self, 
        ...
        log(f'setting stellarium to: {name=} | {lat=} | {lon=} | {height=}')
        ...
        log(f'linking to stellarium @ {stellarium_address} by attaching callback', color=colors.OKBLUE)
    ...
    def link_stellarium(self, 
        ...
        if antenna_id:
            try:
                ...
                log.error(err)
                log('Falling back to default values for dish position and name...',color=colors.WARNING)
        ...
        if self.data_streamer and self.data_streamer.is_alive():
            log('stopping running data stream before connecting to stellarium!', color=colors.WARNING)
    ...
    def get_earth_location(self, antenna_id=None, as_tuple = False):     
        ...
        if as_tuple:
            ...
            if not "EarthLocation" in locals() or "EarthLocation" in globals():
    ...
    def test_all_clear(self, do_print_info=False):
        txt, is_ok = dish_checker.show_dish_state_text(self, None, short=False, return_is_ok=True)
        if do_print_info:
            print(txt)
        return is_ok
    
    def print_info(self, short=False):
        print(self.get_info(short=short))
    ...
    def info(self, short=False):
        self.print_info(short=short)

    def status(self):
        print(self.get_info(short=2))
