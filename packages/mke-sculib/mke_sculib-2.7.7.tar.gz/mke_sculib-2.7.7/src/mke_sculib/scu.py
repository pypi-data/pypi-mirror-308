...
class scu():
    ...
    def __init__(self, address='', ip='', port='8080', antenna_id='', debug=False, lims_az=(-270.0, 270, 3.0), lims_el=(15, 90, 1.35), dish_type='mke', post_put_delay=0.2, use_socket=True, influx_token='', kwargs_requests = None, wait_done_by_uuid=True, start_streams_on_construct=False, timeout_wait_uuid=900, **kwargs):
...
#        log('logger state ')
    ...
    def start(self, az_start=None, el_start=None, band_start=None, az_speed=None, el_speed=None, send_default_configs=True, interlock_acknowledge_on_start=True, **kwargs):
        ...
        if not 'all' in configs_scu:
            warnings.warn('logger config "all" is missing in SCO logger configurations')
        if not 'full_configuration' in configs_scu:
            warnings.warn('logger config "full_configuration" is missing in SCO logger configurations')
        ...
        if interlock_acknowledge_on_start:
            try:
                ...
                warnings.warn(f'Interlock acknowledged failed with error: {err} --> skipping')
    ...
    def link_stellarium(self, 
        ...
        if antenna_id:
            try:
                dc = requests.get(f'http://10.98.76.45:8990/antennas/{antenna_id}').json()    
                ...
                log.error(err)
                warnings.warn('Falling back to default values for dish position and name...')
