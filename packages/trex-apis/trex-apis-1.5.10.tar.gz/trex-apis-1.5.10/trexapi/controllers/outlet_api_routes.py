...
@auth_token_required
@elapsed_time_trace(trace_key='list_outlet_transaction')
#@test_session_expired
...
def read_outlet_reward_transaction_customer_brief(transaction_id):
    ...
    with db_client.context():
        ...
        if custome_transaction:
            transact_user_acct  = custome_transaction.transact_user_acct
            if transact_user_acct:
                dict_properties  = ['name', 'reference_code',]
...
def read_outlet_redemption_customer_brief(transaction_id):
    ...
    with db_client.context():
        ...
        if custome_redemption:
            redeemed_user_acct  = custome_redemption.redeemed_user_acct
