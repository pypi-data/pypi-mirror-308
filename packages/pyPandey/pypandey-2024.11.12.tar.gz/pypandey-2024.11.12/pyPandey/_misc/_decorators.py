            ...
            try:
                ...
                await asst.send_message(
                    ...
                    buttons=[
                        ...
                        Button.url(
                            "Repl",
                            "https://replit.com/@TeamPandey/PragyanStringSession",
                        ),
                    ],
                )
                sys.exit()
                ...
                ftext = (
                    "**Pragyan Client Error:** `Forward this to` @PragyanSupport\n\n"
                )
                ftext += "**pyPandey Version:** `" + str(pyver)
                ftext += "`\n**Pragyan Version:** `" + str(ult_ver)
                ftext += "`\n**Telethon Version:** `" + str(telever)
                ftext += f"`\n**Hosted At:** `{HOSTED_ON}`\n\n"
                ftext += "--------START Pragyan CRASH LOG--------"
                ftext += "\n**Date:** `" + date
                ...
                if len(ftext) > 4096:
                    with BytesIO(ftext.encode()) as file:
                        ...
                        error_log = await asst.send_file(
                            pdB.get_key("LOG_CHANNEL"),
                            file,
                            caption="**Pragyan Client Error:** `Forward this to` @PragyanSupport\n\n",
                        )
                else:
