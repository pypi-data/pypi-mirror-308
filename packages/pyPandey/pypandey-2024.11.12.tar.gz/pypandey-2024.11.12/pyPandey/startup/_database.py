...
class MongoDB:
    def __init__(self, key, dbname="PragyanDB"):
        self.dB = MongoClient(key, serverSelectionTimeoutMS=5000)
        self.db = self.dB[dbname]
        self.re_cache()

    def __repr__(self):
        return f"<Pragyan.MonGoDB\n -total_keys: {len(self.keys())}\n>"
    ...
    def flushall(self):
        self.dB.drop_database("PragyanDB")
        self._cache = {}
        return True
...
# for SQL Implementation in Pragyan.
#
...
class SqlDB:
    def __init__(self, url):
        ...
        try:
            ...
            self._cursor.execute(
                "CREATE TABLE IF NOT EXISTS Pragyan (PragyanCli varchar(70))"
            )
    ...
    def usage(self):
        self._cursor.execute(
            "SELECT pg_size_pretty(pg_relation_size('Pragyan')) AS size"
        )
    ...
    def get(self, variable):
        try:
            self._cursor.execute(f"SELECT {variable} FROM Pragyan")
    ...
    def set_key(self, key, value):
        try:
            self._cursor.execute(f"ALTER TABLE Pragyan DROP COLUMN IF EXISTS {key}")
        except (psycopg2.errors.UndefinedColumn, psycopg2.errors.SyntaxError):
            pass
        except BaseException as er:
            LOGS.exception(er)
        self._cache.update({key: value})
        self._cursor.execute(f"ALTER TABLE Pragyan ADD {key} TEXT")
        self._cursor.execute(f"INSERT INTO Pragyan ({key}) values (%s)", (str(value),))
        return True
    ...
    def del_key(self, key):
        if key in self._cache:
            del self._cache[key]
        try:
            self._cursor.execute(f"ALTER TABLE Pragyan DROP COLUMN {key}")
        except psycopg2.errors.UndefinedColumn:
            return False
        return True
    ...
    def flushall(self):
        self._cache.clear()
        self._cursor.execute("DROP TABLE Pragyan")
        self._cursor.execute(
            "CREATE TABLE IF NOT EXISTS Pragyan (PragyanCli varchar(70))"
        )
        return True
