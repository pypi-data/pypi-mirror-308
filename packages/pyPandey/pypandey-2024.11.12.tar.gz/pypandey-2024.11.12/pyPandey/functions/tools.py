...
async def get_chatbot_reply(message):
    chatbot_base = "https://kukiapi.xyz/api/apikey=PragyanUSERBOT/Pragyan/{}/message={}"
    req_link = chatbot_base.format(
        Pragyan_bot.me.first_name or "pragyan user",
        message,
    )
    try:
...
def telegraph_client():
    ...
    profile_url = (
        f"https://t.me/{Pragyan_bot.me.username}" if Pragyan_bot.me.username else None
    )
    try:
...
async def Carbon(
    ...
    **kwargs,
    ...
    if rayso:
        ...
        kwargs["title"] = kwargs.get("title", "Pragyan")
    else:
        kwargs["code"] = code
