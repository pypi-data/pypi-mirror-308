...
if run_as_module:
    ...
    async def safeinstall(event):
        ...
        if plug in HELP:
            ...
            for i in HELP[plug]:
                output += i
            output += "\n© @ThePragyan"
            await eod(ok, f"✓ `Pragyan - Installed`: `{plug}` ✓\n\n{output}")
    ...
    async def heroku_logs(event):
        ...
        ok = app.get_log()
        with open("pragyan-heroku.log", "w") as log:
            log.write(ok)
        await event.client.send_file(
            event.chat_id,
            file="pragyan-heroku.log",
            thumb="resources/extras/pragyan.jpg",
            caption="**Pragyan Heroku Logs.**",
        )

        os.remove("pragyan-heroku.log")
        await xx.delete()
    ...
    def gen_chlog(repo, diff):
        ...
        ch_log = tldr_log = ""
        ch = f"<b>Pragyan {Pragyan_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
        ch_tl = f"Pragyan {Pragyan_version} updates for {ac_br}:"
        d_form = "%d/%m/%y || %H:%M"
...
def make_html_telegraph(title, author, text):
    ...
    page = client.post(
        title=title,
        author=author,
        author_url="https://t.me/TeamPandey",
        text=text,
    )
    return page["url"]
