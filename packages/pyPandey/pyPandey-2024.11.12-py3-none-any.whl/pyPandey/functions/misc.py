...
from pyPandey.exceptions import DependencyMissingError
...
async def randomchannel(
    tochat, channel, range1, range2, caption=None, client=Pragyan_bot
):
...
async def allcmds(event, telegraph):
    ...
    for z in LIST.keys():
        ...
        for zz in LIST[z]:
            txt += HNDLR + zz + "\n"
        txt += "\n\n"
    t = telegraph.create_page(title="Pragyan All Cmds", content=[txt])
    await eor(event, f"All Pragyan Cmds : [Click Here]({t['url']})", link_preview=False)
...
async def _insta_login():
    if "insta_creds" in Pragyan_bot._cache:
        return Pragyan_bot._cache["insta_creds"]
    username = pdB.get_key("INSTA_USERNAME")
    password = pdB.get_key("INSTA_PASSWORD")
    if username and password:
        settings = eval(pdB["INSTA_SET"]) if pdB.get_key("INSTA_SET") else {}
        cl = Client(settings)
        try:
            cl.login(username, password)
            Pragyan_bot._cache.update({"insta_creds": cl})
        except ManualInputRequired:
            LOGS.exception(format_exc())
            # await get_insta_code(cl, username, password)
            return False
        except LoginRequired:
            pdB.del_key("INSTA_SET")
            return await _insta_login()
        except Exception:
            pdB.del_key(iter(["INSTA_USERNAME", "INSTA_PASSWORD"]))
            LOGS.exception(format_exc())
            return False
        pdB.set_key("INSTA_SET", str(cl.get_settings()))
...
async def get_insta_code(username, choice):
    ...
    async with asst.conversation(Pragyan_bot.uid, timeout=60 * 2) as conv:
...
async def create_instagram_client(event):
    ...
    if not (username and password):
        ...
        return
    settings = pdB.get_key("INSTA_SET") or {}
    cl = Client(settings)
