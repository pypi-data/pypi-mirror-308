...
def load_other_plugins(addons=None, pmbot=None, manager=None, vcbot=None):
    ...
    if addons:
        ...
        if not os.path.exists("addons"):
            os.system(
                f"git clone -q -b {Repo().active_branch} https://github.com/TeamPandey/PragyanAddons.git addons"
            )
        else:
        ...
        if not os.path.exists("addons"):
            os.system(
                "git clone -q https://github.com/TeamPandey/PragyanAddons.git addons"
            )
        ...
        _exclude = pdB.get_key("EXCLUDE_ADDONS")
        _exclude = _exclude.split() if _exclude else []
        _in_only = pdB.get_key("INCLUDE_ADDONS")
        _in_only = _in_only.split() if _in_only else []
