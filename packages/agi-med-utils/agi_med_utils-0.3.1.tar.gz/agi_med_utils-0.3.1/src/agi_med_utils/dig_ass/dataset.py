...
class DigitalAssistantDataset:
    ...
    def load_raw_data(self, chats_path: str, labels_path: str) -> t.Tuple[t.List[dict], t.List[dict]]:
        ...
        oper_files = [file_name for file_name in labels_files if file_name[file_name.find("chatid") :] in chats_files]
        ...
        for file_name in oper_files:
            chat_file_path = os.path.join(chats_path, file_name[file_name.find("chatid") :])
    ...
    def create_messages_label_from_chat_label(self, label: dict, messages: t.List[dict]) -> list:
        """
