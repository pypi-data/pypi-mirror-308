...
def process_single_table(db_type, server, user, password, database, schema, table_name, path_instaclient=None, output=None):
    """
    ...
    try:
        df = load_data_from_sql(engine, f"SELECT * FROM {schema}.[{table_name}]")
    except:
        try:
            df = load_data_from_sql(engine, f"SELECT * FROM {schema}.{table_name}")
        except:
            df = load_data_from_sql(engine, f"SELECT * FROM {table_name}")
    engine.dispose()
