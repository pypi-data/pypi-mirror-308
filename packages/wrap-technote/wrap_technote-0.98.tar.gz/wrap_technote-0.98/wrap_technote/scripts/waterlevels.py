...
def step2_defining_recovered_wls(resource, db, plot_figures=True):
    ...
    logger.info("wlrec")
    logger.info(str(wlrec.info()))

    wlrec_ranked = (
        wlrec.groupby("well_id", sort=False)
        .rswl.apply(lambda df: tn.rank_and_classify(df, "rswl"))
        .reset_index()
        .drop(["well_id"], axis=1)
    )

    logger.info("wlrec_ranked")
    logger.info(str(wlrec_ranked.info()))
