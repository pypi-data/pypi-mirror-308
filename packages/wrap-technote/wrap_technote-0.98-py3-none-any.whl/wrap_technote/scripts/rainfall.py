    ...
    if figures:
        for station in stations:
            ...
            if not internal_only:
                ...
                mean_monthly = mp_df.groupby([mp_df.Date.dt.year, mp_df.Date.dt.month]).Rain.sum()
                mean_monthly = mean_monthly.reset_index(level=0, drop=True).reset_index().groupby("Date").mean().Rain
                # mean_monthly = (
