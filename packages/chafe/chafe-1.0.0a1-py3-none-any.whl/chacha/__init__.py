...
class ChaChaCodec:
    ...
    counter is set to 0 for both operations.
    """
    
    def __init__(self, passphrase:bytes=b''):
        if not passphrase:
            raise ValueError('You must provide a pass phrase for the codec.')
...
def encrypt_file():
    ...
    codec = ChaChaCodec(passphrase)
    codec.encrypt_file(filename, check)
...
def decrypt_file():
    ...
    codec = ChaChaCodec(passphrase)
    try:
        codec.decrypt_file(filename, check)
    except ValueError:
