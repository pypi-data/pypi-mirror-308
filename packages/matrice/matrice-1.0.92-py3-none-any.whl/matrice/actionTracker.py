...
class ActionTracker:
    ...
    def save_benchmark_results(self, latency_ms, batch_size=1):  # TODO: get the values from the action details

        payload = {
            "_idActionRecord": self.action_id_str,
            "modelFamily": self.action_details.get("modelFamily", {}),
            "modelKey": self.action_details.get("modelKey", {}),
            "paramsInMillion": self.action_details.get("paramsInMillion", {}),
            "actionType": self.action_type,
            "GPUName": self.action_details.get("GPUName", {}),
            "CPUInfo": self.action_details.get("CPUInfo", {}),
            "exportFormat": self.action_details.get("runtimeFramework", ""),
            "batch_size": batch_size,
            "latency_ms": latency_ms,
        }
        url = "/v1/project/model_benchmarks"
        try:
            self.rpc.post(path=url, payload=payload)
        except Exception as e:
            log_error(__file__, "save_benchmark_results", str(e))
            print(f"Exception in save_benchmark_results: {str(e)}")
            self.update_status("error", "error", f"Failed to save benchmark results: {str(e)}")

    def calculate_metrics(
        self, split_type, outputs, targets, metrics_type
    ):  
        # TODO update if not pytorch and trained
