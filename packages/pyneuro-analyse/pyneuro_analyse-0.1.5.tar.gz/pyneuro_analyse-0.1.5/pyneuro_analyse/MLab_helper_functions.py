...
def locate_onsets_offsets(annotated_trace:np.ndarray, time_trace:np.ndarray = None, thresh=0.5, on_dur_thresh=0, off_dur_thresh=0, return_dur = False)->pd.DataFrame:
    ...
    if len(onsets)< len(offsets) or offsets[0]<onsets[0]:
        onsets=np.insert(onsets,0,float("nan"))
...
def create_timestamp_aligned_video(video:np.ndarray, 
    ...
    if len(excerpts):
        ...
        #z-score the resulting video if needed, based on the z_scoring_interval
        if z_scored:
            z_from = int((baseline+z_scoring_interval[0])*FPS)
            z_to = int((baseline+z_scoring_interval[1])*FPS)
            aligned_video = (aligned_video-aligned_video[z_from:z_to].mean(axis=0))\
                /aligned_video[z_from:z_to].std(axis=0)
