...
class ParalexValidator(object):
    ...
    def has_doc(self):
        ...
        return (Status.fails,
                f"(deprecation) readme file ({str(doc_file[0].relative_to(Path().resolve()))}) is not listed in the metadata",
    ...
    def has_data_sheet(self):
        if "data_sheet" in self.resource_types["text"]:
            ...
            return (Status.fails, f"(deprecation) Data sheet ({str(doc_file[0].relative_to(Path().resolve()))}) is not listed in the metadata", "Add it to the metadata.")
...
def paralex_validation(args):
    ...
    console.rule(f"[bold]Paralex v.{VERSION} validation")
