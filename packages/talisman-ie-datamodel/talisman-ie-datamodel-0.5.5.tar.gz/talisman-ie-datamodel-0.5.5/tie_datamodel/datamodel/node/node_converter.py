...
from typing import Callable, Iterable, Iterator, Mapping, Type, Union
...
def _convert_nodes(
    ...
    for node in nodes:
        if type(node) in mapping:
            yield mapping[type(node)].wrap(node)
        else:
            yield _find_appropriate_type(node, mapping)
...
def _convert_documents(
    ...
    return map(lambda doc: doc.with_nodes(_convert_nodes(doc.get_nodes(types), mapping)), documents)
