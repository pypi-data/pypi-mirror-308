...
class CoNLLReader(AbstractReader):
    def __init__(self, filepath: PathLike, file_parser: AbstractCoNLLFileParser):
        self._filepath = Path(filepath)
        self._file_parser = file_parser

    def read(self) -> Iterator[TalismanDocument]:
        with self._filepath.open(encoding="utf-8", mode='r') as f:
...
def _create_doc(doc_raw_tokens: List[List[str]], node_id: str) -> TalismanDocument:
    ...
    return DefaultDocumentFactory.create_document(id_=node_id) \
        ...
        .with_facts(map(lambda f: f(node), fact_factories), update=True)
