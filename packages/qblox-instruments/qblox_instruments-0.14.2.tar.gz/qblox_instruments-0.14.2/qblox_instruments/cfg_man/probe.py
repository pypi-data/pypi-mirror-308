...
# ----------------------------------------------------------------------------
def probe_port(
    host: str, port: int, version: Tuple[int, int, int], timeout: float = 10.0
) -> PortInfo:
    """
...
def connection_info_to_json(ci: ConnectionInfo) -> dict:
    ...
    d["all_models"] = list(d["all_models"])
    return d
    ...
    if protocol == "pnp":
        ...
        if fall_back_to_cfg_man:
            ...
            try:
                ...
                fall_back_to_cfg_man = True
                if not quiet:
                    log.info("Failed to connect to configuration manager (%s)", e)
                raise RuntimeError(
