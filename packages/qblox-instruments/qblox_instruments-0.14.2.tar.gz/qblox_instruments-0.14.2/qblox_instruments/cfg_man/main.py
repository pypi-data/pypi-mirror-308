...
class ConfigurationManager:
    ...
    # ------------------------------------------------------------------------
    def describe(self, as_json: bool = False):
        """
    ...
    def download_log(
        ...
        if isinstance(file, str):
            fname = file.replace(
                "%s",
                (
                    self._ci.device.serial
                    if self._ci.device.serial is not None
                    else "unknown"
                ),
            )
            fname = fname.replace(
                "%n", self._ci.name if self._ci.name is not None else "unknown"
            )
    ...
    def set_name(self, name: str) -> None:
        ...
        if not hasattr(self._conn, "set_name"):
            raise NotImplementedError(
                f"set_name() is not implemented for {self._ci.protocol} connections"
            )
...
def _main(args: Union[None, Iterable[str]] = None) -> None:
    ...
    * set-ip <ipv6>/<pre>   Configure the device to use the given static IPv6
    ...
    parser.add_argument(
        ...
        nargs="*",
        help=(
            "List of commands to execute. " "Various patterns are allowed; see above."
        ),
    )
    parser.add_argument(
        "-V",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Don't catch Python exceptions, " "so the full traceback is shown on error."
        ),
    )
    parser.add_argument(
        "-n",
        ...
        try:
            ...
            while True:
                ...
                if cmd == "describe":
                    ...
                    source = {"app": "app", "sys": "system", "cfg": "cfg_man"}.get(
                        log_type, None
                    )
                    if source is None:
                    ...
                    for i in range(len(configs)):
                        ...
                        if "/" not in configs[i]:
                            if configs[i].startswith("192.168."):
                                ...
                                raise ValueError(
                                    f"Missing prefix length for {configs[i]}"
                                )
                ...
                if reuse_key is not None:
                    if reuse_key in commands_used:
                        raise ValueError(
                            f"{cmd} command conflicts with earlier {commands_used[reuse_key]} command"
                        )
