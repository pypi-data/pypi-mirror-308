...
from carate.utils.model_files import save_model_parameters, print_summary
...
class RegressionEvaluation(Evaluation):
    ...
    def cv(
        ...
        logger.logger.info("The model parameters are\n"+str(print_summary(model=model_net)))
        for i in range(num_cv):
            ...
            for epoch in range(1, num_epoch + 1):
                train_mse_loss, train_loss, total_mse, len_train_loader = self.train(
                    model_net=model_net,
                    epoch=epoch,
                    optimizer=optimizer,
                ...
                logger.log({
                    ...
                    "Train_MSE_Loss": "{:16f}".format(train_mse_loss),
                    "Total MSE Train": "{:16f}".format(total_mse),
                    "Len Train Loader": "{:16f}".format(len_train_loader),
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        for data in train_loader:
            ...
            y_prev = data.y.clone().detach()
            
            if type(norm_factor) == float: 
                y_prev_div= torch.div(y_prev, norm_factor)
            else:
                y_prev_div= torch.div(y_prev, torch.from_numpy(norm_factor))
            ...
            optimizer.step()
            torch.cuda.empty_cache()
        return mse / len(train_loader), loss, mse, len(train_loader)
    ...
    def reshape_output_probs(self, output_probs): 

        result = []
        output_probs = output_probs.detach().numpy()
        for i in range(self.num_classes): 
            tmp = np.zeros((self.batch_size, 1))
            tmp = output_probs[:, i]

            result.append(output_probs)
