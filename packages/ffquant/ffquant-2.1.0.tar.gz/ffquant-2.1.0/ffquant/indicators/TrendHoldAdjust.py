    ...
    def signal_calcuate(self):
        ...
        if self.trendhold_last == TrendHoldAdjust.BEARISH and signal_dict['trendhold'] == TrendHoldAdjust.BULLISH:
            if self.crossover[0] != 1:
                if self.p.debug:
                    stdout_log(f"{self.__class__.__name__}, not golden RSI crossover({self.crossover[0]}), keep BEARISH")
                signal_dict['trendhold'] = self.trendhold_last
            ...
            if self.crossover[0] != -1:
                if self.p.debug:
                    stdout_log(f"{self.__class__.__name__}, not dead RSI crossover({self.crossover[0]}), keep BULLISH")
                signal_dict['trendhold'] = self.trendhold_last
                self.tha_inherited = True
        else:
            if self.tha_inherited and signal_dict['trendhold'] == TrendHoldAdjust.BULLISH and self.crossover[0] == -1:
                if self.p.debug:
                    stdout_log(f"{self.__class__.__name__}, inherited BULLISH until dead RSI crossover({self.crossover[0]}), reset to original TrendHold")
                ...
                if self.p.debug:
                    stdout_log(f"{self.__class__.__name__}, inherited BEARISH until gold RSI crossover({self.crossover[0]}), reset to original TrendHold")
                signal_dict['trendhold'] = self.trendhold[0]
                self.tha_inherited = False
