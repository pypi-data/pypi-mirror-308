...
class PriceMoving(bt.Indicator):
    ...
    def calculate_tnr(self):
        ...
        denominator_pre = sum(abs(pre_price[i] - pre_price[i - 1]) for i in range(0, len(pre_price)))
        denominator_cur = sum(abs(cur_price[i] - cur_price[i - 1]) for i in range(0, len(cur_price)))
        ...
        if pre_tnr * 2 > CRASH_TNR_THRESHOLD and cur_tnr * 2 > CRASH_TNR_THRESHOLD and pre_state == cur_state * -1 \
        ...
        elif pre_tnr * 2 > CRASH_TNR_THRESHOLD and cur_tnr * 2 < RANGE_BOUND_TNR_THRESHOLD \
            and abs(pre_price[-1] - pre_price[0]) / pre_price[0] >= CRASH_PRICE_RATE_THRESHOLD:
                if pre_state == 1:
        ...
        elif cur_tnr * 2 > CRASH_TNR_THRESHOLD \
            and abs(cur_price[-1] - cur_price[0]) / cur_price[0] >= CRASH_PRICE_RATE_THRESHOLD:
                if cur_state == 1:
