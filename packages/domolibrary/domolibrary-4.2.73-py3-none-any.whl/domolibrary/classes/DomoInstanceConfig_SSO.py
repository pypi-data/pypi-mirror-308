...
class SSO_Config:
    ...
    @classmethod
    def _parent_from_json(cls, obj : dict ,raw : dict, auth : dmda.DomoAuth,debug_prn : bool = False,  **kwargs):
        new_obj = {
...
class SSO_SAML_Config(SSO_Config):
    ...
    def _from_json(cls, auth: dmda.DomoAuth, obj: dict, debug_prn: bool = False):
        ...
        idp_certificate = obj.pop("idpCertificate") if hasattr(obj, 'idpCertificate') else None
...
class SSO:
    ...
    async def get_oidc(self, debug_prn: bool = False):
        self.OIDC = await SSO_OIDC_Config.get(auth=self.auth, debug_prn=debug_prn)

    async def get_saml(self, debug_prn: bool = False):
        self.SAML = await SSO_SAML_Config.get(auth=self.auth, debug_prn=debug_prn)

    async def get(self, debug_prn: bool = True):
        await self.get_oidc(debug_prn=debug_prn)
        await self.get_saml(debug_prn=debug_prn)
