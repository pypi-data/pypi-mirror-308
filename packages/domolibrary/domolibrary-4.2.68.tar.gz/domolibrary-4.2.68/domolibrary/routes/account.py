...
class Account_CRUD_Error(dmde.RouteError):
    def __init__(
        ...
        super().__init__(messaage=message, entity_id=account_id, res=res)
...
class Account_CreateParams_Error(dmde.DomoError):
    def __init__(
        ...
        super().__init__(messaage=message)
...
async def create_account(
    ...
    if not create_body and not (account_name and data_provider_type and config):
        raise Account_CreateParams_Error(
            messaage="if not passing complete create_body must pass account_name,data_provider_type, and config"
        )
    ...
    if not res.is_success:
        raise Account_CRUD_Error(
            ...
            messaage=f"error creating account - {res.response}",
        )
...
async def create_oauth_account(
    ...
    if not create_body and not (
        ...
        raise Account_CreateParams_Error(
            messaage="if not passing complete create_body must pass account_name,data_provider_type, origin, and config"
        )
...
async def update_account_config(
    ...
    if res.status == 400 and res.response == "Bad Request":
        raise Account_Config_Error(
            res=res,
            messaage=f"error updating config | use debug_api = True {res.response}",
            account_id=account_id,
        )
...
class ShareAccount_AccessLevel(ShareAccount, Enum):
    ...
    def generate_payload(
        ...
        if user_id:
            return {"type": "USER", "id": int(user_id), "accessLevel": self.value}

        if group_id:
            return {"type": "GROUP", "id": int(group_id), "accessLevel": self.value}
