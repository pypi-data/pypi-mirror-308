...
# %% auto 0
__all__ = ['CreateUser_MissingRole', 'DownloadAvatar_NoAvatarKey', 'DomoUser', 'DomoUsers']
...
async def by_email(
    ...
    if only_allow_one:
        matches = cls.util_match_domo_users_to_emails(domo_users, email_ls)

        if not matches:
            raise SearchUser_NoResults(
                domo_instance = auth.domo_instance,
                search_criteria = email_ls,
                function_name = 'by_email',
                parent_class = cls.__name__,
                status = res.status,
            )
...
# %% ../../nbs/classes/50_DomoUser.ipynb 55
@patch_to(DomoUser)
...
# %% ../../nbs/classes/50_DomoUser.ipynb 57
@patch_to(DomoUser)
...
# %% ../../nbs/classes/50_DomoUser.ipynb 58
@patch_to(DomoUser)
async def get_api_clients(
    ...
    domo_clients = await dmce.gather_with_concurrency(
        *[dicli.ApiClient._from_json(auth=self.auth, obj=obj) for obj in res.response],
        n=10
    )
...
# %% ../../nbs/classes/50_DomoUser.ipynb 60
@patch_to(DomoUser)
