...
system_start.insert_Boot_up(cmd='nohup kcwebplus /intapp/index/mysocket/servers --cli > app/runtime/log/mysocket.log 2>&1 &',name='通知服务自启',types='kcwebplus',icon='https://img.kwebapp.cn/icon/kcweb.png')
class sockets(kcwebsocket):
    user={}
        ...
        try:
            ...
            if self.getuser(clientid)['types']=='send':
                if is_index(data,'types') and data['types']=='ungroup':#解散分组
                    await self.ungroup('group'+data['group_id'])
                elif is_index(data,'types') and data['types']=='joinGroup':#加入组
                    await self.joinGroup(clientid,'group'+data['group_id']) #加入组
                else:
                    # print(data)
                ...
                if data['types']=='getGroupname':#获取组名称
                    ...
                    await self.joinGroup(clientid,data['group']) #加入组
                    await self.send_client(clientid,self.__successjson({
                        'types':'res',
                        'data':'success'
                    }))
                elif data['types']=='leaveGroup':#退出组
                    await self.leaveGroup(clientid,data['group']) #退出组
                    await self.send_client(clientid,self.__successjson({
                        'types':'res',
                        'data':'success'
                    }))
...
class mysocket:
    def mysend():
        ...
        return successjson()
    def servers():
        #kcwebplus /intapp/index/mysocket/servers
    ...
    def restart():
        ...
        if 'Linux' in get_sysinfo()['platform']:
            kill_route_cli("/intapp/index/mysocket/servers")
            cmd="nohup kcwebplus /intapp/index/mysocket/servers --cli > app/runtime/log/mysocket.log 2>&1 &"
            os.system(cmd)
        else:
    ...
    def __init(self):
        if not self.socket_server_ip:
            ...
            if not self.socket_server_ip:
                ...
                return False
        if times()-self.__connectiontime>3600:
            if self.ws:
                try:
                    self.ws.close()
                except:pass
