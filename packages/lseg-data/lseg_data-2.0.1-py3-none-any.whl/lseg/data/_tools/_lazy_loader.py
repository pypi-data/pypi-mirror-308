...
def attach(package_name, submodules=None, submod_attrs=None):
    ...
    def __getattr__(name):
        if name in submodules:
            ...
            mod = attr_to_modules[name]
            if mod.startswith("."):
                norm_mod = mod.lstrip(".")
                dot_count = len(mod) - len(norm_mod)
                submod_path = package_name.rsplit(".", dot_count)[0] + "." + norm_mod
            else:
                submod_path = f"{package_name}.{mod}"
...
class _StubVisitor(ast.NodeVisitor):
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom):
        if node.module:
            module_name = "." * (node.level - 1) + node.module
            attrs: list = self.submod_attrs.setdefault(module_name, [])
            attrs.extend(alias.name for alias in node.names)
        else:
            self.submodules.update(alias.name for alias in node.names)
...
def attach_stub(package_name: str, filename: str):
    ...
    return attach(package_name, visitor.submodules, visitor.submod_attrs)
