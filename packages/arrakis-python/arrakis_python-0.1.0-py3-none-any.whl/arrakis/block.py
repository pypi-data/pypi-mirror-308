...
def concatenate_blocks(*blocks: SeriesBlock) -> SeriesBlock:
    ...
    *blocks : SeriesBlock
    ...
    for block in blocks:
        assert (
            set(block.data.keys()) == channel_set
        ), "all blocks must contain the same channel sets"
        assert (
            block.time_ns == end_time_ns
        ), f"block start time ({block.time_ns}) does not match concatenated block end time ({end_time_ns})"  # noqa: E501
