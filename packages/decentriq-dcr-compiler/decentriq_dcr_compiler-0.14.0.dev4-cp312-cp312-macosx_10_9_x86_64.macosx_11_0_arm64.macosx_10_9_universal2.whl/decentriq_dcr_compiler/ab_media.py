...
def compile_ab_media_dcr(dcr: AbMediaDcr):
    return _ddc_py.compile_ab_media_dcr_serialized(
...
def get_ab_media_dcr_requirements(dcr: AbMediaDcr) -> ConsumerRequirements:
    serialised_requirements = (
        _ddc_py.get_ab_media_data_room_requirements_serialized(
            dcr.model_dump_json(by_alias=True)
        )
    )
...
def decompile_ab_media_response(
    request: AbMediaRequest, response_serialized: bytes
):
