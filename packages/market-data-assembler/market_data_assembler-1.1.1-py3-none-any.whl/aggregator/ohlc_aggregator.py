...
class OHLCAggregator:
    ...
    def aggregate(self, new_candle, instrument):
        ...
        window_end_time_x2_check = self.start_time + timedelta(seconds=self.window_sec * 2)

        if candle_time >= window_end_time:
            if candle_time >= window_end_time_x2_check or not self.current_window_candles:
                raise ValueError(
                    f"Out of aggregation window range or nothing to aggregate. {instrument}, {new_candle['t']}")
        ...
        if self.history and self._is_duplicate_candle(aggregated_candle, self.history[-1]):
            self.duplication_flags.append(True)
        else:
            self.duplication_flags.append(False)
        ...
        if len(self.duplication_flags) > self.history_size:
            self.duplication_flags.pop(0)

    @staticmethod
    def _is_duplicate_candle(candle1, candle2):
        keys_to_compare = ["o", "h", "l", "c", "v", "n", "trades", "book_depth"]
        return all(candle1[key] == candle2[key] for key in keys_to_compare)
    ...
    def is_fully_duplicated(self):
        return all(self.duplication_flags)
