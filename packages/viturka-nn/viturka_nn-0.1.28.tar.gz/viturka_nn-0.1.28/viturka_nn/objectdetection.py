...
class ModelHandler:
    ...
    def evaluate_model(self, device='cuda'):
        """
        Evaluates the model using IoU, Recall, Precision, and F1 Score on the loaded test dataset.

        :param device: Device to run the evaluation on ('cpu' or 'cuda').
        :return: Dictionary of evaluation metrics.
        """
        if self.model is None:
            raise ValueError("No model provided or loaded for evaluation.")
        if self.dataloader is None:
            raise ValueError("Test dataset not loaded. Call `load_test_data` first.")
        ...
        for images, targets in self.dataloader:
            ...
            for i, output in enumerate(outputs):
                ...
                gt_boxes = torch.tensor(
                    [
                        [bbox[0], bbox[1], bbox[0] + bbox[2], bbox[1] + bbox[3]]  # Convert format
                        for bbox in (obj["bbox"] for obj in targets[i])
                    ],
                    dtype=torch.float32,
                ).to(device)

                gt_labels = torch.tensor(
                    [obj["category_id"] for obj in targets[i]], dtype=torch.int64
                ).to(device)
                ...
                ious = torchmetrics.functional.iou(pred_boxes, gt_boxes, iou_threshold=0.5)
                iou_scores.extend(ious.cpu().tolist())

                # Precision, Recall, and F1 Score Calculation
                tp = sum((ious > 0.5) & (pred_labels == gt_labels))  # True Positives
