...
class ModelHandler:
    ...
    def evaluate_model(self, device='cuda'):
        """
        Evaluates the model using Mean Average Precision (mAP) on the loaded test dataset.
        ...
        metrics = torchmetrics.DetectionMAP()
        ...
        for images, targets in self.dataloader:
            ...
            formatted_targets = [{k: torch.tensor(v).to(device) for k, v in t.items()} for t in targets]
            formatted_outputs = [{k: v.to(device) for k, v in o.items()} for o in outputs]

            metrics.update(formatted_outputs, formatted_targets)
