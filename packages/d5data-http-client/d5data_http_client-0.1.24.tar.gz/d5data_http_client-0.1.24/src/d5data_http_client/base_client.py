...
import requests
import json
from .utils import parse_json_response, check_status_code, start_sse_listener, stop_event
import math
import requests
import hashlib
import os
...
class APIClient:
    ...
    def get(self, model, params=None):
        ...
        response = requests.get(f'{self.base_url + model}', headers=headers, params=json.dumps(payload))
        check_status_code(response)
    ...
    def post(self, model, data=None):
        ...
        response = requests.post(f'{self.base_url + model}', headers=headers, data=payload)
        check_status_code(response)
        return parse_json_response(response)

    def preference_data_cleaning(self, file_type: str, user_email: str, file_path: str, instruction_keys: List[str],
                             response_a: str, response_a_score: str = None, response_b: str = None,
                             response_b_score: str = None, return_score: bool = False, return_category: bool = False, auto_scale_y:bool = False) -> None:
        '''
        ...
        auto_scale_y: if you want the y-axis to be scaled automatically
        '''
        ...
        s3_file_name = f"{file_name}{self.user_id}{time.time()}"
        ...
        sse_thread = start_sse_listener(self, s3_file_name)
