...
def print_report(report: Dict, testcase_run: Dict, testfile_name: str):
    if testcase_run['conclusion'] == 'success':
        ...
        for idx, url in enumerate(report):
            ...
            for step in steps:
                if step['success']:
                    click.secho(f"\t\tPASSED: {step.step}: {step['msg']}", fg='green')
                else:
                    click.secho(f"\t\tFAILED: {step.step}: {step['msg']}", fg='red')
    click.echo()
...
def run(test_file, test_dir):
    ...
    if test_file:
        ...
        print_report(report, testcase_run, test_file.name)
    ...
    if test_dir:
        ...
        with ThreadPoolExecutor(max_workers=MAX_CONCURRENT_TESTS) as executor:
            ...
            for future in as_completed(futures):
                ...
                report = get_report(testcase_run['id'])
                print_report(report, testcase_run, test_file)
                if testcase_run['conclusion'] == 'failure':
                    exit_code = 1
