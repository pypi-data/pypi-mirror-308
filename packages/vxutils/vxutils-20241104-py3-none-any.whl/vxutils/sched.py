...
class VXPublisher:
    def __init__(
        self,
        context: VXContext = __context__,
        apply_method: Optional[
            Callable[[VXTaskItem, Optional[VXTrigger]], Future[Any]]
        ] = None,
    ) -> None:
        self.context = context
        self.apply_method = (
            apply_method if apply_method is not None else async_task.__executor__.apply
        )
    ...
    def __call__(
        ...
        *,
        ...
        return self.apply_method(task=task, trigger=trigger)  # type: ignore[no-any-return, operator]
...
def start(config_file: str = "etc/config.json", mod: str = "mod") -> None:
    ...
    try:
        ...
        logging.warning("Loading Config File Failed: %s", err)
        config = {}
    __context__.config.update(**config)
    logging.debug("Load Config: %s", to_json(__context__.config))

    try:
        with open("data/params.json", "r") as f:
            params = json.load(f)
            __context__.params.update(**params)
            logging.debug("Load Params: %s", to_json(__context__.params))
    except BaseException as err:
        logging.warning("Loading Params File Failed: %s", err)
...
def run(args: Optional[Namespace] = None) -> None:
    ...
    try:
        start(config_file=args.config, mod=args.mod)
        while __ACTIVE__:
            time.sleep(1)
