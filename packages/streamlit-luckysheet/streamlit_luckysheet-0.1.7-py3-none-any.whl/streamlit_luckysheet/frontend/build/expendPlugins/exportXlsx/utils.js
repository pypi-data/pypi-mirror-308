...
async function exportSheetExcel(luckysheet, name, judge) {
  ...
  tableArr.forEach(function (table) {
    ...
    const options = judge === 1 ? { views: [{ showGridLines: false }] } : {};
    const worksheet = workbook.addWorksheet(table.name, options);
...
var saveFile = function (buf, name) {
  ...
  // var name = document.getElementById("luckysheet_info_detail_input").value;
  var name = name || "default_filename.xlsx";
...
//   sheetData: luckysheet.getAllSheets(),
//   fileName: fileName,
// });
// const userDirectory = path.join(directory, userID);
// if (!fs.existsSync(userDirectory)) {
//   fs.mkdirSync(userDirectory, { recursive: true });
// }
// const filePath = path.join(userDirectory, `${fileName}.json`);
...
var setImages = function (table, worksheet, workbook) {
  let { images, visibledatacolumn, visibledatarow } = { ...table };
