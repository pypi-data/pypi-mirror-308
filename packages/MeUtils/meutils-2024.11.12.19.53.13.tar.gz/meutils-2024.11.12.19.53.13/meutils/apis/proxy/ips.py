...
@alru_cache(ttl=60)
async def get_one_proxy(token: Optional[str] = None, exclude_ips: str = "", from_redis: Optional[bool] = False):
    """

    :param token: token不为空，就按token分配固定的ip，否则随机获取ip或者从redis循环队列中获取
    :param exclude_ips:
    :param from_redis:
    :return:
    """
    if token:  # 按token分配固定的ip
        ...
        async def check_token(token, **kwargs):
            return token not in exclude_ips

        # 随机获取ip: 如果从redis就是循环队列
        ip = await get_next_token_for_polling(FEISHU_URL, check_token=check_token, from_redis=from_redis)
