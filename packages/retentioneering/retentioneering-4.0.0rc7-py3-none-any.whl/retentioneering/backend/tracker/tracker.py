...
class Tracker(Singleton):
    ...
    @staticmethod
    def get_top_call_id() -> str:
        top_call = [t for t in inspect.stack() if "tracker.py" in t.filename and t.function == "wrapper"][-1]

        top_frame_id = str(id(top_call.frame))
        top_frame_ts = str(top_call.frame.f_locals.get("start_time"))
        hasher = hashlib.sha1((top_frame_id + top_frame_ts).encode("utf-8"))
    ...
    def time_performance(
        ...
        def time_performance_decorator(func: Callable) -> Callable:
            ...
            def wrapper(*args: list[Any], **kwargs: dict[Any, Any]) -> Callable:
                ...
                start_time = datetime.now()
                print(f"event_name:{event_name}, session_id:{self.session_id}")
                top_call_id = self.get_top_call_id()
                self._track_action(
                    called_function_params={},
                    scope=scope,
                    event_name=event_name,
