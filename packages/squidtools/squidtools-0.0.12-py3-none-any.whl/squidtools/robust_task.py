...
from .util import gen_batches, print_err, read_delim
    ...
    if os.path.exists(pname):
        with open(pname, "rt") as f:
            for line in f:
                ...
                if pk and pk in objs:
                    progress[pk] = p
                else:
                    extra[pk] = p
    print_err(f"Warning: {len(extra)} entries in cache not found in input objects...")
    return progress
...
def robust_task(
    ...
    task: Callable[[str, any], any] | Callable[[str], any],
    ...
    if isinstance(objs, list):
        ...
        task_func = lambda key, _: task(key)
    else:
        task_func = task

    progress = load_progress(progress_name, objs)
    ...
    for i, name in enumerate(names):
        if skip_existing and name in progress:
            continue
        try:
            results = task_func(name, objs[name])
...
def async_robust_task(
    objs: dict[str, str] | list[str],
    task: Callable[[str, any], Awaitable[any]] | Callable[[str], Awaitable[any]],
    progress_name=DEFAULT_FILENAME,
    skip_existing=True,
    show_progress=True,
    batch_size=100,
    ...
    for batch in gen_batches(names, batch_size):
        ...
        async def process(name):
            try:
                ...
                results = await asyncio.wait_for(
                    task_func(name, value), timeout=timeout
                )
                return name, results
        ...
        results = asyncio.run(execute_all(tasks))
...
def naive_dict_to_tsv(data, header=True):
    if len(data) > 0:
        outs = io.StringIO()
