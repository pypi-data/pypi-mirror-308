...
class InorganicFinishReasonError(Exception):
    pass
...
async def basic_prompt(prompt, input, model=DEFAULT_MODEL, temp=1):
    messages = [{"role": "user", "content": f"{prompt}\n{input}"}]
    response = await generate_chat_response(messages, model, temp)
    return get_message_text(response), get_total_cost([response])


async def system_prompt(prompt, input, model=DEFAULT_MODEL, temp=1):
    messages = [
...
async def json_prompt(
    ...
    try:
        ...
        if attempt < MAX_ATTEMPTS:
            ...
            return await json_prompt(
                prompt_text, input, method, model, schema, attempt + 1, best
            )
        else:
            raise e
...
async def json_prompt_system(
    prompt_text, input, model=DEFAULT_MODEL, schema=None, temp=1
):
    return await json_prompt(
        prompt_text, input, method=system_prompt, model=model, schema=schema, temp=temp
    )


async def generate_chat_response(messages, model=DEFAULT_MODEL, attempt=0, temp=0.5):
    try:
        ...
        if fr != 'stop':
            raise InorganicFinishReasonError(fr)
        return response
    except (
        ...
        return await generate_chat_response(messages, model, attempt, temp)
...
def get_total_cost(responses):
    ...
    output_costs = {
        "gpt-4o-2024-08-06": 0.015 / 1000,
        "gpt-4-0125-preview": 0.06 / 1000,
        "gpt-4-0613": 0.06 / 1000,
        "gpt-4-1106-preview": 0.03 / 1000,
        "gpt-4o-mini-2024-07-18": 0.6 / 1000000,
    ...
    for r in responses:
        if r.model not in input_costs:
            print_err(f"[warning] {r.model} not found in input costs! update please")
            return -1
