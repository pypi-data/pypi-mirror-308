...
from moviepy.editor import VideoFileClip, ImageClip, concatenate_videoclips, CompositeVideoClip, VideoClip, ImageSequenceClip, VideoClip
...
# TODO: This below should be contained in a class to simplify
# the code, but maybe I need to refactor the whole file
def alpha_video_to_mask(alpha_video):
    """
    Turn the provided 'alpha_video' to a VideoClip that
    can be set as another clip mask. This method checks
    the provided 'alpha_video' and turns its white pixels
    to transparents and the other ones to opaque if no
    transparent pixels found in the mask. If that happens,
    only full transparent pixels found are preserved in
    the new mask and the other ones are opaque.

    This method is expected to be used with transition
    black and white clips (or alpha if possible) to be 
    able to build incredible transitions using those 
    clips as masks.
    ...
    TODO: This strategy can change in the future because
    of an intern TODO.
    """
    alpha_transition_clip = VideoParser.to_moviepy(alpha_video, True, True)

    # Need for the 'get_frame_as_mask' method
    fps = alpha_transition_clip.fps
    mask_frames = []
    
    # Look for transparent pixels in the alpha clip
    transparent_pixel_in_mask = False
    for alpha_frame in alpha_transition_clip.mask.iter_frames():
        if (alpha_frame < 1.0).any():
            transparent_pixel_in_mask = True

    # TODO: This below is setting as True even when there are
    # no values fitting the condition
    # if ((alpha_frame < 1.0).any() for alpha_frame in alpha_transition_clip.mask.iter_frames()):
    #     transparent_pixel_in_mask = True
    
    if transparent_pixel_in_mask:
        # 1. If video contains a mask (and has transparent
        # pixels), preserve the mask and turn the other pixels
        # to black pixels (opaque ones)

        # TODO: What about non-opaque but not full transparent
        # pixels, such as 0.5 (?) Think about an strategy, by
        # now I'm using the full transparents only as the ones
        # I must keep as transparent

        # TODO: Iterate over all the frames
        for alpha_frame in alpha_transition_clip.iter_frames():
            transparent_pixels = alpha_frame == 1

            new_frame = np.array(alpha_frame)
            
            # Transparent pixels to pure white
            new_frame[transparent_pixels] = [255, 255, 255]
            new_frame[~transparent_pixels] = [0, 0, 0]

            mask_frames.append(new_frame)
    else:
        # 2. If video does not contain a mask (or does not
        # have transparent pixels), convert to a pure black and
        # white image and turn white into transparent pixels 
        # and black into opaque
        for alpha_frame in alpha_transition_clip.iter_frames():
            # This is to detect white colors
            threshold = 220
            white_pixels = np.all(alpha_frame >= threshold, axis = -1)

            # Frame to completely and pure black
            new_frame = np.array(alpha_frame)
    ...
    def get_frame_as_mask(t):
        """
        Method to obtain each frame (created as RGB) as a
        mask numpy array (that is a normalized single value)
        """
        return ImageMask.to_moviepy_mask_numpy_array(mask_frames[int(t * fps)])

    return VideoClip(get_frame_as_mask, ismask = True)