...
class VideoMaskHandler:
    ...
    def invert_clip_mask(cls, clip):
        ...
        shown.
        """
        clip = VideoParser.to_moviepy(clip, has_mask = True)
...
def image_to_clip_mask(image_google_drive_url_or_filename: str):
    """
    ...
    The 'image_google_drive_url_or_filename' parameter can
    be a Google Drive url or a filename containing the
    image.

    You can see some examples in the next folder: https://drive.google.com/drive/folders/1Lc8SVml7xJLJTrrYj7J_ewAXfvlO9zC7?usp=sharing
    """  
    #image_google_drive_url_or_filename = 'https://drive.google.com/file/d/1-K6edepJkK9LTGMEaGBX_-5WPlJsrIrA/view?usp=sharing'
    #mask_720x1080_filepath = 'resources/moviepy_mask/mask_rounded_borders_720x1080.png'
    # TODO: Should we have 'output_filename' to handle
    # the resource or not (?)
    mask_filename = Resource.get(image_google_drive_url_or_filename, None)

    return ImageClip(ImageMask.to_moviepy_mask_numpy_array(mask_filename), ismask = True)

    # You can directly set this mask by doing this below
    # video = VideoFileClip(video, has_mask = True).set_mask(image_to_clip_mask(image))
    ...
    reel_clip = VideoFileClip(reel, has_mask = True).set_mask(mask_clip)
...
# TODO: Move this to another place maybe
# TODO: Maybe this is quite unnecessary
def insert_image_to_clip_mask(image: str, video):
    return VideoFileClip(video, has_mask = True).set_mask(image_to_clip_mask(image))
