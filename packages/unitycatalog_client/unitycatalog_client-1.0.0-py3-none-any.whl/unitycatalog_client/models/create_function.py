...
class CreateFunction(BaseModel):
    ...
    CreateFunction
    """
    name: StrictStr = Field(default=..., description="Name of function, relative to parent schema.")
    ...
    input_params: FunctionParameterInfos = Field(...)
    data_type: ColumnTypeName = Field(...)
    full_data_type: StrictStr = Field(default=..., description="Pretty printed function data type.")
    return_params: Optional[FunctionParameterInfos] = None
    routine_body: StrictStr = Field(default=..., description="Function language. When **EXTERNAL** is used, the language of the routine function should be specified in the __external_language__ field,  and the __return_params__ of the function cannot be used (as **TABLE** return type is not supported), and the __sql_data_access__ field must be **NO_SQL**. ")
    routine_definition: StrictStr = Field(default=..., description="Function body.")
    routine_dependencies: Optional[DependencyList] = None
    parameter_style: StrictStr = Field(default=..., description="Function parameter style. **S** is the value for SQL.")
    is_deterministic: StrictBool = Field(default=..., description="Whether the function is deterministic.")
    sql_data_access: StrictStr = Field(default=..., description="Function SQL data access.")
    is_null_call: StrictBool = Field(default=..., description="Function null call.")
    security_type: StrictStr = Field(default=..., description="Function security type.")
    specific_name: StrictStr = Field(default=..., description="Specific name of the function; Reserved for future use.")
    comment: Optional[StrictStr] = Field(default=None, description="User-provided free-form text description.")
    properties: StrictStr = Field(default=..., description="JSON-serialized key-value pair map, encoded (escaped) as a string.")
    external_language: Optional[StrictStr] = Field(default=None, description="External language of the function.")
    __properties = ["name", "catalog_name", "schema_name", "input_params", "data_type", "full_data_type", "return_params", "routine_body", "routine_definition", "routine_dependencies", "parameter_style", "is_deterministic", "sql_data_access", "is_null_call", "security_type", "specific_name", "comment", "properties", "external_language"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CreateFunction:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CreateFunction:
        """Create an instance of CreateFunction from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CreateFunction.parse_obj(obj)

        _obj = CreateFunction.parse_obj({
            "name": obj.get("name"),
