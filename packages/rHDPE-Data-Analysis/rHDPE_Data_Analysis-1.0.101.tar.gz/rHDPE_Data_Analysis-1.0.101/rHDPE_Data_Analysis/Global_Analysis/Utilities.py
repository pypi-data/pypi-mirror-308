...
def scatterplots( ip ):
    ...
    feature_1_labels = ["Colour_L*"]
    feature_2_labels = ["DSC_Crystallinity"]

    if ip.shiny:

        feature_1_labels = [ip.scatterplot_x]
        feature_2_labels = [ip.scatterplot_y]

    features_df, std_of_features_df = Preprocessing.read_unnormalised_data( ip.output_directory, feature_1_labels, feature_2_labels, ip.sample_mask )
    ...
    # feature_1_labels = ["SHM"]
    # feature_2_labels = ["ESCR_50%Failure"]
    ...
    # gu.plot_scatterplot_of_two_features( ip.directory, feature_1, feature_2, features_df.index, [resin_data.loc[i]["Label"] for i in features_df.index], errorbars = True, std = std, line_of_best_fit = False, exponential_fit = True, annotate_style = 1, xlabel = "Normalised PP Content", ylabel = "Normalised Strain at Break", savefig = True, filename = ip.output_directory + "Global/Scatterplots/PP_vs_SAB.pdf" )
...
def pca( directory, output_directory, shiny, features_df, std_of_features_df ):
    ...
    if pca_of_pca_of_individual_datasets:
        ...
        if shiny:
            ...
            pca_ft_df, pca_std = gu.perform_pca( directory, overall_pca_df, [int( i ) for i in overall_pca_df.index], std_error = True, std_of_features_df = overall_stds_df, num_components = 2, filename = output_directory + "Global/PCA/Overall.png" )

        else:

            pca_ft_df, pca_std = gu.perform_pca( directory, overall_pca_df, [int( i ) for i in overall_pca_df.index], std_error = True, std_of_features_df = overall_stds_df, num_components = 2, filename = output_directory + "Global/PCA/Overall.pdf" )
        ...
        print( type(pca_ft_df[pca_ft_df.columns[0]].tolist()) )
        print( type(pca_ft_df[pca_ft_df.columns[0]].tolist()[0]) )
        print( type(pca_std[0]) )
        print( type(pca_std[0][0]) )

        return pca_ft_df[pca_ft_df.columns[0]].tolist(), pca_ft_df[pca_ft_df.columns[1]].tolist(), np.array( pca_std[0] ).tolist(), np.array( pca_std[1] ).tolist()
...
def pca_ml( directory, ip, features_df ):
    ...
    X, _ = gu.perform_pca( directory, features_df, ip.sample_mask, num_components = num_components, analysis_of_pca = False )
...
def escr_predictor( directory, features_df, std_of_features_df ):

    # [1.12733926] -0.1283258558485133

    resin_data = gu.get_list_of_resins_data( directory )

    # target_escr = features_df["ESCR_50%Failure"]
    #
    # X_escr = pd.DataFrame( features_df["SHM"] )
    #
    # reg = LinearRegression().fit( X_escr, target_escr )
    #
    # score = reg.score( X_escr, target_escr )
    #
    # print( score )
    # print( reg.coef_, reg.intercept_)
    #
    # prediction = reg.predict( X_escr )
    #
    # gu.plot_scatterplot_of_two_features( directory, prediction, target_escr.to_numpy(), features_df.index, [resin_data.loc[i]["Label"] for i in features_df.index], errorbars = False, std = "", line_of_best_fit = False, exponential_fit = False, xlabel = "Prediction", ylabel = "Target", savefig = False, filename = "" )
    #
    # plt.show()
    #
    # plt.close()
    ...
    X = features_df.drop( ["SHM", "TT_SHM"], axis = 1 )
    # X = features_df.drop( ["SHM"], axis = 1 )

    print( X.shape )

    combinations = itertools.combinations( [i for i in range( 23 )], 3 )
    ...
    print( score, best_c, X.iloc[:, list( best_c )].columns )

    X1 = X.iloc[:, list( best_c )]

    reg = LinearRegression().fit( X1, target )

    prediction = reg.predict( X1 )

    prediction_escr = prediction * 1.12 - 0.13

    unnormalised_shm = prediction * 9 + 5
    unnormalised_escr = prediction_escr * 80 + 16

    array = np.hstack( (unnormalised_shm[:, np.newaxis], unnormalised_escr[:, np.newaxis]) )

    np.savetxt( "/Users/philsmith/Documents/Postdoc/rHDPE_Data_Analysis/Output/Global/Sandbox/ESCR_Prediction/ESCR_Prediction.csv", array, delimiter = ",", fmt = "%.5f" )
...
def sandbox( directory, features_df, std_of_features_df ):
    ...
    if perform_escr_predictor:
        ...
        escr_predictor( directory, features_df, std_of_features_df )