...
from torchgdm.tools.misc import tensor_is_diagonal_and_identical, test_positional_input
...
def G0_2d_Ep(
    ...
    G0_2d = torch.zeros(
        (*delta_r.size()[:-1], 3, 3), dtype=DTYPE_COMPLEX, device=r_probe.device
    )
...
class EnvHomogeneous2D(EnvironmentBase):
    ...
    def get_environment_permittivity_scalar(
        ...
        if not tensor_is_diagonal_and_identical(eps_env_tensor):
            raise ValueError(
