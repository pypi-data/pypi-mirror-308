        ...
        for device in self.devices:
            for conf1, conf2 in config_pairs:
                ...
                r_probe = torch.as_tensor(
                    [[120, -50, -100]], dtype=tg.constants.DTYPE_FLOAT
                ).to(device=device)
    ...
    def test_vector_directions(self):
        ...
        for device in self.devices[:1]:
            for conf in configs:
                ...
                r_probe = torch.as_tensor(
                    [[0, 0, 0]], dtype=tg.constants.DTYPE_FLOAT
                ).to(device=device)
                ...
                if e_cross_h_dot_k[0].real != 0:
                    torch.testing.assert_close(
                        torch.sign(e_cross_h_dot_k[0].real), torch.as_tensor(1.0)
                    )
                if e_cross_h_dot_k[0].imag != 0:
                    torch.testing.assert_close(
                        torch.sign(e_cross_h_dot_k[0].imag), torch.as_tensor(1.0)
                    )
