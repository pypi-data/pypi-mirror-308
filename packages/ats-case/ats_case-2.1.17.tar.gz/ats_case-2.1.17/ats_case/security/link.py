    ...
    try:
        ...
        result = pro.encode(
            func.to_dict(protocol=protocol, comm_addr=context.meter.addr,
                         operation='aarq',
                         element='',
                         parameter={'应用上下文名': 'LN_Referencing_With_Ciphering', '加密机制': 'HIGH_GMAC', '用户信息': {'安全控制': 'SC_AE', '计数器': 0, '初始化': {'一致性提议': {'引用类型': 'LN_REFERENCING_PROPOSED'}}}},
                         session_key=context.test_sn))
        frame = base.write(context, result.get('frame'))
        pro.decode(func.to_dict(protocol=protocol, frame=frame, session_key=context.test_sn))
        base.sleep(1)
        # {'应用上下文名': 'LN_Referencing_No_Ciphering', '加密机制': 'LOW', '用户信息': {'初始化': {'一致性提议': {'引用类型': 'LN_REFERENCING_PROPOSED'}}}}
    except Exception as e:
        retry_times -= 1
        if retry_times <= 0:
            raise MeterSecurityAuthenticationError('[表位:{}]{}'.format(context.meter.pos, str(e)))
