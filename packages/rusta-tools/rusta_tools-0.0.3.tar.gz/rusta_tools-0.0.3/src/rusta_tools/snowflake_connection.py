...
class DB:
    ...
    def load_db_config(self) -> dict:
        ...
        config_dir = os.path.join(parent_dir, 'files')
...
class Snowflake(DB):
    ...
    def fetch_article_data(self, articles_to_load):
        self.connect_to_snowflake('RUSTA_DWH')
        query = ""
        try:
            query = self.load_query(const.QUERY_SQL.get(articles_to_load))
        except Exception as e:
            raise Exception(e)
        try:
            cursor = self.conn.cursor()
    ...
    def delete_same_day_data(self, table_name, dwh, company=None, scrape_type=None):
        ...
        # rd78518.west-europe.azure.snowflakecomputing.com