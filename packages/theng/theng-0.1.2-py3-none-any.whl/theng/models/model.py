...
class Model(ABC):
    ...
    def __init__(self, results_file_name: str, extension: str):
        ...
        basename: str = os.path.splitext(os.path.basename(results_file_name))[0]
        self.model_name = basename
    ...
    def _take_screenshot(
        ...
        if not ssname:
            for view in ssview:
                name = f'{view}.png'
                command: List[str] = [results_path, '-screenshot', '-ssname', name, '-sswidth', str(sswidth), '-ssheight', str(ssheight), '-ssview', view, '-sstime', str(sstime), self.model_name+".pfrv"]
            ...
            for name, view in zip(ssname, ssview):
                filename = f'{name}.png'
                command: List[str] = [results_path, '-screenshot', '-ssname', filename, '-sswidth', str(sswidth), '-ssheight', str(ssheight), '-ssview', view, '-sstime', str(sstime), self.model_name+".pfrv"]
                p = subprocess.Popen(command)
                p.wait()
