...
def limpa_diretorio(dir: str, timeout_for_clear: int = 5, max_tentativas: int = 3, suportar_nomes_grandes: bool = False):
    """Limpa diretório(s) e tenta remover arquivos com nomes grandes se necessário.
    
    Args:
        dir (str): Caminho do diretório para limpar.
        timeout_for_clear (int): Tempo em segundos para esperar antes de tentar novamente em caso de PermissionError. Padrão é 5.
        max_tentativas (int): Número máximo de tentativas para limpar o diretório. Padrão é 3.
        suportar_nomes_grandes (bool): Se True, tenta lidar com arquivos que têm nomes longos. Padrão é False.
    """
    ...
    if os.path.exists(DIR):
        ...
        while not sucesso and tentativas < max_tentativas:
            try:
                if suportar_nomes_grandes:
                    shutil.rmtree(suporte_para_paths_grandes(DIR))
                else:
                    shutil.rmtree(DIR)
                sucesso = True
            except PermissionError:
                tentativas += 1
                print(f"Tentativa {tentativas} falhou. Tentando novamente em {timeout_for_clear} segundos...")
