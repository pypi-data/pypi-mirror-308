            ...
            if d.value is not None:
                try:
                    ...
                    if f.is_has_many:
                        ...
                        elif not isinstance(d.value, list):
...
"""
    ...
    def offline_query(
        ...
        elif upload_input_as_table or (len(multi_input) > 0 and len(multi_input[0]) > 100) or num_shards:
    ...
    def check(
        ...
        if not _do_query_errors_match(actual_errors, query_errors):
            ...
            errors_table.add_column("Kind", max_width=7, overflow="fold")
            errors_table.add_column("Code", max_width=7, overflow="fold")
            errors_table.add_column("Feat", max_width=20, justify="left", overflow="fold")
            errors_table.add_column("Res", max_width=20, justify="left", overflow="fold")
            errors_table.add_column("Message", overflow="fold", justify="left")
            errors_table.add_column("Exc", max_width=40, overflow="fold")

            table_errors = [
                [
                    Color.render("Match", Color.G) if q in safe else Color.render("Expect", Color.R),
                    q.code,
                    q.feature and render_fqn(q.feature),
                    q.resolver,
                    q.message,
                    str(q.exception),
                ]
                for q in query_errors
            ]

            for q in resp_errors or []:
                if q in safe:
                    continue

            table_errors.append(
                [
                    Color.render("Actual", Color.R),
                    q.code,
                    q.feature and render_fqn(q.feature),
                    q.resolver,
                    q.message,
            ...
            for row in sorted(table_errors, key=lambda k: str(k[5])):
        ...
        if resp_data is not None:
            ...
            if feature_mismatch or show_table:
                ...
                result_table.add_column("Name", overflow="fold")
                ...
                for k in expected:
                    ...
                    row = [
                        Color.render("Match", Color.G) if k.fqn in safe else Color.render("Expect", Color.R),
