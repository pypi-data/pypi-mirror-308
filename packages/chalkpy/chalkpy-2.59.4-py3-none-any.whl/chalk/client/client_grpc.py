...
from chalk.features._encoding.inputs import recursive_encode_bulk_inputs
...
class ChalkGRPCClient:
    ...
    def _make_query_bulk_request(
        ...
        if not isinstance(input, DataFrame):
            encoded_inputs, _ = recursive_encode_bulk_inputs(input)
        else:
            encoded_inputs = input
        inputs_bytes = self.get_features_feather_bytes(encoded_inputs)
