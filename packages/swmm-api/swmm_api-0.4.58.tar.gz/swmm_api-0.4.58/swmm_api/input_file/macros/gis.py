...
def gpkg_to_swmm(fn, label_sep='.', infiltration_class=None, custom_section_handler=None, simplify=False):
    ...
    for sec in NODE_SECTIONS:
        if sec not in layers_available:
            continue
        gdf = gpd.read_file(fn, layer=sec).set_index(IDENTIFIERS.name)#.fillna(np.nan)

        cols = gdf.columns[gdf.columns.str.startswith(sec)]
        inp[sec] = section_dict[sec].create_section(gdf[cols].reset_index().fillna(np.nan).values)
    ...
    if SUBCATCHMENTS in layers_available:
        gdf = gpd.read_file(fn, layer=SUBCATCHMENTS).set_index(IDENTIFIERS.name).fillna(np.nan)
