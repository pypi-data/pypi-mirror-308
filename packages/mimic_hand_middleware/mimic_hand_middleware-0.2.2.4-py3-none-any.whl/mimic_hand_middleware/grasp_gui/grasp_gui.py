...
class GraspGui(tk.Frame):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        self.gc = GripperController(
            '/dev/mimic_hand_driver',
            max_motor_current=60,
            use_sim_motors=False,
            motor_config_yml_path=cal_path,
        )
    ...
    def _get_calibration_file_path(self: 'GraspGui') -> None:
        """Return the path to the calibration file.

        This function gets the path to the calibration .yaml that stores the zero
        positions of the motors. The calibration path can be given by setting the
        environmental variable HAND_CALIBRATION_PATH. If this environmental variable
        is not set, a default will be used. The default value can be hardcoded here.

        :return: Path to the calibration .yaml.
        :rtype: string
        """
        cal_path_env_var = 'HAND_CALIBRATION_PATH'
        # set default here
        file_path = os.path.dirname(os.path.realpath(__file__))
        cal_path = os.path.join(file_path, '../calibration/motor_config.yaml')
        if cal_path_env_var in os.environ:
            cal_path = os.environ[cal_path_env_var]
        return cal_path
