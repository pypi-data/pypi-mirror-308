...
def get_related_nodes_data(bucket_name: str, node: dict, related_key: str, related_type: str, cache: dict):
    """
    Given a node, return all related nodes data.
    ...
    -------
    List[dict]
        The data for each related node, containing the `node`, the `indexed_at`,
        the `recalculated_at` and `recalculated_stage`.
    """
    ...
    return [
        {
            ...
            'indexed_at': _last_modified(
                bucket=bucket_name,
                key=join(from_type, f"{from_id}.jsonld")
            ),
            'recalculated_at': _last_modified(
                bucket=bucket_name,
                key=join('recalculated', from_type, f"{from_id}.jsonld")
            ),
