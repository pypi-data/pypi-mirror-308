...
def main() -> None:
    ...
    parser.add_argument("--port", type=int, default=443, help="Service port number")
    parser.add_argument(
        "--api-key",
    ...
    try:
        ...
        results = asyncio.run(
            hack_target(args.address, args.port, args.api_key, args.source, args.output)
        )

        if not results:
            print("HackBot did not find any issues during the analysis")
        elif args.issues_repo:
            print(f"Generating issues report on repo {args.issues_repo}")
