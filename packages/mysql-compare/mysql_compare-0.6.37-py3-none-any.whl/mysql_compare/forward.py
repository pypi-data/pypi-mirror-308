...
def running_compare_0(logger: logging.Logger, crd: CompareRunningData):
    with connect(**crd.dst_dsn) as source_con, connect(**crd.dst_dsn) as target_con:
...
def running_compare_2(logger: logging.Logger, crd: CompareRunningData):
    ...
    with connect(**crd.src_dsn) as source_con, connect(**crd.dst_dsn) as target_con:
        for diff_row in crd.temp_diff_rows:
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                src_row = executor.submit(mysql_query_row, source_con, crd.src_db, crd.src_tab, crd.table_keys, diff_row).result()
                dst_row = executor.submit(mysql_query_row, target_con, crd.dst_db, crd.dst_tab, crd.table_keys, diff_row).result()
...
def running_compare(crd: CompareRunningData):
    ...
    if crd.error is not None:
        state = "error"
    elif crd.diff_rows_total >= 1:
        state = "diff"

    with open(f"{crd.name}.state", "w") as f:
        f.write(state)

    logger.info(f"running_compare done, diff count: {crd.diff_rows_total}")
...
def main():
    ...
    running_state = json.dumps(running_state_file) if os.path.exists(running_state_file) else {}

    success_count = 1

    compare_tables = get_tables(cargs)

    crds: dict[str, CompareRunningData] = {}
    processes_running: dict[str, multiprocessing.Process] = {}

    # with concurrent.futures.ProcessPoolExecutor(max_workers=cargs.parallel) as executor:
    for src_db, src_tab, dst_db, dst_tab in compare_tables:
        ...
        if compare_name in running_state and (running_state[compare_name] == "done" or running_state[compare_name] == "diff"):
            continue
        crd = CompareRunningData(
            compare_name,
            cargs.batch_size,
            cargs.delay_check,
            cargs.repair,
        ...
        print(running_state, compare_name)
    ...
    while True:
        ...
        for compare_name, proc_running in list(processes_running.items()):
            if not proc_running.is_alive():
                ...
                write_json(running_state, running_state_file)
                print(f"{get_current_time()} -- {compare_name} -- progress, complate: {success_count}/{len(compare_tables)}")

                del processes_running[compare_name]
                success_count += 1
            else:
                print(f"{get_current_time()} -- {compare_name} -- running, complate: {success_count}/{len(compare_tables)}")

        while len(crds) >= 1 and len(processes_running) < cargs.parallel:
            compare_name, crd = crds.popitem()
            print(f"{get_current_time()} -- {compare_name} -- start, complate: {success_count}/{len(compare_tables)}")
            proc = multiprocessing.Process(target=running_compare, args=(crd,))
