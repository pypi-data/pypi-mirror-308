...
def running_compare(crd: CompareRunningData):
    ...
    logger.info("running_compare done")
...
def main():
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=cargs.parallel) as executor:
        for src_db, src_tab, dst_db, dst_tab in compare_tables:
            ...
            future_to_task[executor.submit(running_compare, crd)] = compare_name
            running_state[crd.name] = "progress"

            write_json(running_state, running_state_file)

        for future in concurrent.futures.as_completed(future_to_task):
            crd: CompareRunningData = future.result()

            if crd.error is not None:
                running_state[crd.name] = "error"
            elif crd.diff_rows_total >= 1:
                running_state[crd.name] = "diff"
            else:
                running_state[crd.name] = "done"

            write_json(running_state, running_state_file)

            success_count += 1
            print(f"{crd.name} -- progress {success_count}/{len(compare_tables)}")
