...
def mark_nested_result(data, value):
    if isinstance(data, dict):
        ...
        for key, val in data.items():
            if key != 'result':
                if key == 'face_detection' or key == 'security_features':
                    data[key] = 'clear'
                else:
...
def data_comparison_check(data, country):
    ...
    if user_data and country not in ['LBN', 'SDN', 'SYR', 'JOR', 'PSE']:
        ...
        if country in ['IRQ', 'LBN', 'QAT', 'SDN', 'SYR', 'JOR', 'PSE']:
            ...
            if country == 'LBN':
                ...
                if data.get('doc_type') == 'passport':
                    ...
                    keys = ['name', 'first_name', 'father_name', 'last_name', 'gender_ar', 'place_of_birth', 'issue_date', 'issue_date_ar', 'card_number_ar', 'id_number_ar', 'issuing_country', 'nationality']
            ...
            if country == 'SDN':
                ...
                if data.get('doc_type') == 'passport':
                    keys = ['full_name_generic', 'name_ar', 'gender', 'place_of_birth', 'place_of_issue', 'passport_number_mrz', 'national_number', 'issue_date', 'mrz', 'nationality']
                else:
                    # keys = ['father_name', 'last_name', 'name_en', 'first_name_en', 'fathers_name_en', 'last_name_en', 'gender_ar', 'place_of_birth', 'issue_date', 'issue_date_ar', 'card_number_ar', 'id_number_ar']
                    keys = ['name', 'name_ar', 'nationality', 'gender', 'place_of_birth', 'place_of_birth_en', 'id_number', 'occupation', 'occupation_en', 'issue_date', 'first_name', 'middle_name', 'last_name']
...
def form_final_data_document_report(data, front_id_text, back_id_text, country, coloured, selfie, facial_similarity, blurred, glare, missing_fields, face_match_threshold, back_img):
    try:
        ...
        keys_to_mark = ['compromised_document', 'data_consistency', 'data_comparison', 'image_integrity', 'data_validation', 'visual_authenticity']
...
def form_final_facial_similarity_report(data, selfie, facial_similarity, liveness_result, face_match_threshold, country):
    try:
        ...
        # Mark all fields as clear for facial_report wherever it sees 'consider' value in any of the key value
        def mark_all_clear(data):
            if isinstance(data, dict):
                for key, value in data.items():
                    if isinstance(value, dict):
                        mark_all_clear(value)
                    elif value == 'consider':
                        data[key] = 'clear'
            elif isinstance(data, list):
                for item in data:
