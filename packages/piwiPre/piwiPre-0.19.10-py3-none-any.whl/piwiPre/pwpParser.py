...
- modifier à la main le .ini dans THUMBNAILS
...
- '*' est une valeur spéciale, qui signifie : tous les sous-dossiers de l'album racine,
  (a condition que --recursive-verify-album true) 

So verify-album est true, alors la valeur de TRIAGE est annulée automatiquement

ATTENTION: les sous-répertoires ne sont pas vérifiés,
sauf si --recursive-verify-album true, ce qui n'est pas le défaut.
...
Autres options avec leurs valeur lorsque on utilise --verify-album::
     ...
     --recursive-verify-album true              # utile seulement si folder1 a des sous-dossiers
...
""")
        ...
        self.add_item('enable-auto-configuration',
                      help='Enables configuration of ALBUM from TRIAGE,' 
                           ' by creating a configuration file in THUMBNAILS.',
                      fr_help='Autorise la configuration automatique de ALBUM '
                              'en créant des fichiers de configuration dans THUMBNAILS ',
                      action='store',
                      choices=['true', 'false'],
                      pwp_type=PwpArgType.BOOL,
                      ...
                      default='true',
                      config="""
Enables the copy of piwiPre.ini files found in TRIAGE directory to the corresponding folder of THUMBNAILS,
...
correspondant dans THUMBNAILS, de façon à ce que les futures vérifications de ALBUM donnent le même résultat.  
""")
...
""")
        ...
        self.add_item('base',
                      ...
                      action='store',
                      location='args')
        # CAVEAT: there is NO default value for base,
        # because, base is os.getcwd()                                                                  # noqa
        # so, base in NEVER in the config file
        # and if we have --base as an argument, piwipre will do an os.chdir() to change cwd

        self.add_item("enable-create-base",
                      help="allows piwiPre to create BASE if not existing",
                      fr_help="Autorise piwiPre à créer BASE s'il n'existe pas",
                      pwp_type=PwpArgType.BOOL,
                      action='store',
                      location='args',
                      setups=None,  # We do NOT want this to appear in the UI, too dangerous
        ...
        self.add_item('base-last',
                      ...
                      fr_help="Comme --base, mais change l'historique du DERNIER dossier d'où piwiPré a été lancé,",
                      pwp_type=PwpArgType.BOOL,
                      choices=['true', 'false'],
                      default='false',
                      setups=[ServerSetup(pwp_type=PwpArgType.BOOL, )],
...
- {base} est le nom du dossier de TRIAGE dans lequel le fichier a été trouvé.
...
""")  # noqa
        ...
        self.add_item('month-name',
                      ...
                      # we DO NOT want to see month-name in the UI, until with have a multiline UI
                      location='config')
        ...
        self.add_item('authors',
                      ...
                      # we DO NOT want to see authors in the UI, until with have a multiline UI
                      default={},
                      location='config',
                      config="""
...
.. Important:: unless enable-auto-configuration == false,  
   ...
   then it  will be copied in the corresponding THUMBNAILS subdirectories,  
   so that further processing of ALBUM give the same results. 
