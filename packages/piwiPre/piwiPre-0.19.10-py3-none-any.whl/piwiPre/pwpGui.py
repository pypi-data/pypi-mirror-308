...
class PwpGui:
    ...
    def exit(self):
        print("Exiting from GUI thread")
        # self.root.quit()  # Do NOT do this, it kills TK !
        self.root.destroy()
        if self.root == PwpGui.tk:
            print("Exiting from Tk main program")
            PwpGui.delete()
...
class _DirChooserField:
    ...
    def prompt_new_dir(self):
        ...
        GuiStringEditor(father=self,
                        name="nouveau dossier" if self.root_gui.language == "fr" else "new directory",
...
class _DirChooserString:
    ...
    def __init__(self, root_gui: 'GuiDirChooser', frm, row,
        ...
        self.entry = tkinter.Entry(self.frm, background=PwpGui.GREEN, width=65,
...
# ------------------------------------------------------------------------------------------------
# TODO: add inherit() and default() ? or keep them managed by the radio buttons ?
#       but once we have changed HOME and BASE, we cannot go back to default values !
# add mkdir()
# add a separator between [..] and sub directories
# remove "Choose" for HOME, BASE, ..
# [..] is BEFORE [.]

# TODO: change the result from relative to BASE to ABSOLUTE.
#       because we may use GuiDirChooser to change BASE

# TODO: add a flag to restrict the choices to subdirectories of BASE
...
class GuiDirChooser(PwpGui):
    ...
    def __init__(self, root_gui, initial_dir, name,
        ...
        :param self_test:
        """
        super().__init__("Choisir un dossier" if language == "fr" else "Directory chooser",
                         language=language)
        ...
        self.build_list()
        if self_test:
            self.root.after(2 * 1000, lambda _foo: self.enter('..'), [None])
            self.root.after(3 * 1000, lambda _foo: self.select('..'), [None])
            self.root.after(4 * 1000, lambda _foo: self.exit(), [None])
        self.frm.focus_set()
    ...
    def build_list(self):
        ...
        all_lines = [
            ("[HOME]", self.home),
            ('[BASE]', self.base),
            ("[..] = père" if self.language == "fr" else "[..] = father", os.path.abspath(father)),
            ("-------------",
             " '.' et sous-dossiers" if self.language == "fr" else " '.' and sub directories "),
            ("'.'  = ce dossier" if self.language == "fr" else "'.'  = this directory", os.path.abspath(self.dir_name)),
            ("[NEW]", "Special value"),
        ]
        ...
        self.folders.append(_DirChooserString(self, cur_frame, row=row,
                                              ...
                                              b_text="Enter",
                                              b_fr_text="Entrer",
                                              initial="Entrer un chemin EXISTANT" if self.language == "fr"
        ...
        for line in all_lines:
            if line[0] == "[NEW]":
                self.folders.append(_DirChooserString(self, cur_frame, row=row,
                                                      ...
                                                      b_text="Create",
                                                      b_fr_text="Créer",
                                                      initial="Entrer un NOUVEAU nom" if self.language == "fr"
                                                      else "Enter a NEW name",
                                                      max_width1=max_width1,
                                                      action=self.__create))
            else:
    ...
    def __create(self, path):
        if path.startswith("./") or path.startswith("../") or path.startswith(".\\") or path.startswith("..\\"):
            self.gui_msg("Illegal path")
            # TODO: ADD a toplevel message, visible to user
            return
        ...
        self.gui_msg(f"Created dir {full_path}")
        self.build_list()

    def check_and_enter(self, path):
        if not os.path.isdir(path):
            print(f"Chosen directory'{path}' does not exist")
            return
        self.enter(path)

    @staticmethod
    def relative_path_to_cwd(path):
        full_path = os.path.abspath(path)
        base = os.path.abspath(os.getcwd())
        if full_path.startswith(base):
            return os.path.relpath(full_path, base)
        path_drive, _ = os.path.splitdrive(full_path)
        base_drive, _ = os.path.splitdrive(base)
    ...
    def select(self, path: str):
        ...
        full_path = self.relative_path_to_cwd(path)
        ...
        if GuiScrollable.canvases is not None:
            for canvas in reversed(GuiScrollable.canvases):
                try:
                    ...
                    print("Wrong canvas in Scrollable")
                    # This was BUG 3132
                    # Steps to reproduce:
                    # PwpConfigurator / Parameters / Choose subdirectory / New subdirectory / Cancel  # noqa
                    # Scroll
                    # Assumption: the window for String editor is still in the scrollable list
                    # But the String Editor has been removed to simplify the UI,
                    # so, as a side effect, bug disappeared.
