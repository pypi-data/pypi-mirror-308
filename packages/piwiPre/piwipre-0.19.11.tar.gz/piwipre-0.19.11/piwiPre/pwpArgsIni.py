...
class PwpArgsIni(argparse.ArgumentParser):
    ...
    def parse_args_and_ini(self, program: str, ini_to_parse, arguments, with_config=True,):
        ...
        def get_val_lower(flag):
            val = get_val(flag)
            return None if val is None else val.lower()

        if get_val_lower("--language") == 'fr':
            language = 'fr'
        elif get_val_lower("--language") == 'en':
            language = "en"
        else:
        ...
        old_dir = os.getcwd()
        new_dir = get_val('--base')
        base_last = get_val('--base-last')
        if new_dir:
            if not os.path.isdir(new_dir):
                if get_val_lower('--enable-create-base') == 'true':
                    ACTOR.mkdirs(new_dir, forced=True)
                else:
                    LOGGER.config_error(f"--base '{new_dir}' : non existing dir and --enable-create-base false")
            LOGGER.msg(f"base '{new_dir}'")
            os.chdir(new_dir)
        elif base_last == "true":
            new_dir = self.get_latest_cwd()
            if os.path.isdir(new_dir):
                LOGGER.msg(f"base-last '{new_dir}'")
                os.chdir(new_dir)
            else:
                LOGGER.config_error(f"--base-last : last dir '{new_dir}' does not exist ")

        new_dir = get_val('--home')
        if new_dir:
            if not os.path.isdir(new_dir):
                LOGGER.config_error(f"--home '{new_dir}' : non existing directory")
                ACTOR.mkdirs(new_dir)      # we get there ONLY if GUI, otherwise exit with error
            self.home = new_dir
        ...
        if not unknowns:
            pass   # unknown args is empty
        else:
            super().print_help()
            en_msg = f"\n\n{ini_to_parse} : Unknown arguments {unknowns} \n *** Please change configuration ***\n"
            fr_msg = f"\n\n{ini_to_parse} : Arguments inconnus {unknowns}\n *** Changer la configuration ***  \n"

            if hasattr(args, 'gui') and args.gui == 'true':
                # When we have a GUI, we want to show the error
                # BUT the main UI is not yet created.
                # see program_907 and program_1006

                LOGGER.msg(en_msg)
                gui = GuiError(en_msg, fr_msg)
                if hasattr(args, 'test_gui') and args.test_gui == 'true':
    ...
    def parse_for_dir(self, path, home: str, cwd: str, language):
        ...
        :param cwd: maybe changed with --base
