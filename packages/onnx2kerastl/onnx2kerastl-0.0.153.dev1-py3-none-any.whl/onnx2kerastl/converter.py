...
def onnx_to_keras(onnx_model, input_names, name_policy=None, verbose=True, change_ordering=False, input_types=None,
    ...
    logger.info("[MODEL_INFO] Model inputs and outputs:")
    ...
    for i, input in enumerate(onnx_inputs):
        ...
        logger.info(f"[INPUT] name={input.name}, shape={shape}, dtype={dtype}")

    # Log output details
    for i, output in enumerate(onnx_outputs):
        logger.info(f"[OUTPUT] name={output}")

    logger.info("[MODEL_INFO] Gathering weights without actual values")
    weights = {}
    for onnx_w in onnx_weights:
        weight_name = onnx_w.name
        weight_shape = onnx_w.dims
        weights[weight_name] = numpy_helper.to_array(onnx_w).shape
        logger.info(f"[WEIGHT] name={weight_name}, shape={weight_shape}")
    ...
    onnx_nodes = flatten_onnx_nodes(onnx_model.graph.node) # Pulls graph out of If nodes
        ...
        # Conversion of each ONNX node
    for node_index, node in enumerate(onnx_nodes):
        node_type = node.op_type
        node_name = node.name
        logger.debug(f"[NODE] type={node_type}, name={node_name}, index={node_index}")

        # Extract and log attributes of the node
        node_params = onnx_node_attributes_to_dict(node.attribute)
        logger.debug(f"[NODE_ATTR] name={node_name}, attributes={node_params}")
