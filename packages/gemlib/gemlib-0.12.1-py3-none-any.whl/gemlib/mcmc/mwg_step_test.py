...
from gemlib.mcmc.mwg_step import _project_position, MwgStep
...
def structure_assertion(expected_structure):
    def init_fn(target_log_prob_fn, position):
        position = tf.nest.map_structure(
            lambda x, y: x, position, expected_structure, check_types=False
        )
    ...
    def step_fn(target_log_prob_fn, chain_and_kernel_state, seed):
        ...
        position = tf.nest.map_structure(
            lambda x, y: x, cs.position, expected_structure, check_types=False
        )
