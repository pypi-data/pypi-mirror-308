...
from gemlib.tensor_util import broadcast_fn, broadcast_together


def test_broadcast_together(evaltest):
    foo = [[1.0], [0.1]]
...
def test_broadcast_fn(evaltest):
    def func():
        return tf.constant([[1], [2], [3]]), tf.constant(4)

    bcast_func = broadcast_fn(func)
