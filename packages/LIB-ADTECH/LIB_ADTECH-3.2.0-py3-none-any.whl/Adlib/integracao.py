...
from Adlib.api import EnumBanco, EnumProcesso, putRequestFunction, IntegracaoStatus
...
def importarArquivos(virtaus, idBanco: int, codigoBanco: int, nomeBanco: str, substring: str, formatoArquivo: str, usuarioWindows: str):
    """
    ...
    try:
        putRequestFunction(IntegracaoStatus.IMPORTANDO, EnumProcesso(ENUM_PROCESSO), EnumBanco(idBanco))
        ...
        if qntArquivos == 0:
            putRequestFunction(IntegracaoStatus.SEM_ARQUIVOS, EnumProcesso(ENUM_PROCESSO), EnumBanco(idBanco))
        ...
        for i, arquivo in enumerate(arquivosFiltrados, start=1):
            ...
            try:
                ...
                putRequestFunction(IntegracaoStatus.LIGADO, EnumProcesso(ENUM_PROCESSO), EnumBanco(idBanco))
        ...
        print(erro)
        print('Não deu certo')
        putRequestFunction(IntegracaoStatus.ERRO, EnumProcesso(ENUM_PROCESSO), EnumBanco(idBanco))


def integracaoVirtaus(driver, usuario: str, senha: str, idBanco: int, codigoBanco: int, nomeBanco: str,
    ...
    importarArquivos(driver, idBanco, codigoBanco, nomeBanco, substring, formatoArquivo, usuarioWindows)
...
if __name__=="__main__":
    ...
    integracaoVirtaus(driver, userVirtaus, senhaVirtaus, idBanco, codigoBanco, nomeBanco,
