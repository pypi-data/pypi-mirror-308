...
class PushAnyEnv(gym.Env):
    ...
    def _add_object(self, position, angle, object_name):
        module_name = "gym_pushany.envs.objects"
        function_name = f"add_{object_name.upper()}" if not object_name.isdigit() else "add_digit"
        try:
        ...
        return add_function(self, position, angle, 30, 'LightSlateGray', pymunk.ShapeFilter.ALL_MASKS(), object_name, body_type=pymunk.Body.DYNAMIC)
    ...
    def _add_obstacles(self):
        ...
        for _ in range(num_obstacles):
            ...
            while np.linalg.norm(np.array(pos) - self.goal_pose[:2]) < 150:
                pos = (rs.randint(80, 420), rs.randint(80, 420))
            obstacle = self._add_object(pos, rot, rs.choice(OBJECT_NAME_LIST))
