...
class PushAnyEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self._seed = None
        self.seed()
        self.window_size = ws = window_size  # The size of the PyGame window
        ...
        self.render_mode = render_mode if render_mode in ["human", "rgb_array"] else "rgb_array"
    ...
    def get_obs(self):
        return {
            "pixels": np.array(self._render_frame(self.render_mode)),
    ...
    def _render_frame(self, mode, visualize=False):
        width, height = (
            ...
            if visualize
            else (self.render_size, self.render_size)
        )
        ...
        if self.render_action and (self.latest_action is not None):
            ...
            coord = (action / 512 * [width, height]).astype(np.int32)
            marker_size = int(8/96*width)
            thickness = int(1/96*width)
