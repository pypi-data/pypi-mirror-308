...
class JWT(Module):
    ...
    Configuration:
        - JWT_SECRET_KEY: Secret key for JWT. Defaults to an environment variable or a random token.
        - JWT_TOKEN_EXPIRE_MINUTES: Token expiration time in minutes. Defaults to 30.
        - JWT_ALGORITHM: JWT encoding algorithm. Defaults to "HS256".
        - JWT_ALLOW_SELF_SIGNUP: Whether to allow self-registration. Defaults to False.
        - JWT_SQL_URL: The SQL connection URL. Defaults to "sqlite:///.db".
    """
    ...
    def __init__(
        ...
        """
        super().__init__("JWT", "JWT Authenticator", os.getenv("JWT_SQL_URL", "sqlite:///.db"))
        self._secret_key = os.getenv("JWT_SECRET_KEY", secrets.token_hex(32))
        self._token_expire_minutes = int(os.getenv("JWT_TOKEN_EXPIRE_MINUTES", 30))
        self._algorithm = os.getenv("JWT_ALGORITHM", "HS256")
        self._allow_self_signup = os.getenv("JWT_ALLOW_SELF_SIGNUP", False)
...
jwt_authenticator = JWT.get_instance()
@jwt_authenticator.router.post("/login")
async def login(form_data: Annotated[OAuth2PasswordRequestForm, Depends()]):
    ...
    Raises:
        HTTPException: If authentication fails or token creation fails.
    """
    user = jwt_authenticator._authenticate_user(form_data.username, form_data.password)
    if not user:
        raise HTTPException(
            status_code=400, detail="Incorrect username or password"
        )
    try:
        access_token = jwt_authenticator._create_access_token(user.username)
...
@jwt_authenticator.router.post("/logout")
async def logout(token: Annotated[str, Depends(jwt_authenticator.oauth2_scheme)]):
    """
    ...
    try:
        payload: dict = jwt.decode(
            token,
            jwt_authenticator._secret_key,
            algorithms=[jwt_authenticator._algorithm],
        )
        username = payload.get("username")
        with jwt_authenticator.db_session() as db:
            user = db.query(User).filter(User.username == username).first()
            if not user:
                raise HTTPException(status_code=400, detail="User not found")
...
@jwt_authenticator.router.post("/refresh")
async def refresh(user: Annotated[User, Depends(jwt_authenticator)]):
    """
...
if jwt_authenticator._allow_self_signup:

    @jwt_authenticator.router.post("/register")
...
@jwt_authenticator.router.get("/users/me", response_model=UserSchema)
async def get_user(user: Annotated[User, Depends(jwt_authenticator)]):
    """
...
@jwt_authenticator.router.get("/users", response_model=list[UserSchema])
async def get_all_users(
    user: Annotated[User, Depends(jwt_authenticator)], db: Session = Depends(jwt_authenticator.db)
):
    """
...
@jwt_authenticator.router.put("/users/update")
async def change_password(
    user_update: UserUpdateSchema,
    user: Annotated[User, Depends(jwt_authenticator)],
    db: Session = Depends(jwt_authenticator.db),
):
    """
...
@jwt_authenticator.router.put("/users/deactivate")
...
@jwt_authenticator.router.put("/users/change-group")
async def change_user_group(
    username: str,
    new_group: str,
    admin_user: Annotated[User, Depends(jwt_authenticator)],
    db: Session = Depends(jwt_authenticator.db),
):
    """
...
@jwt_authenticator.router.get("/groups", response_model=list[GroupSchema])
async def get_all_groups(
    admin_user: Annotated[User, Depends(jwt_authenticator)], db: Session = Depends(jwt_authenticator.db)
):
    """
...
@jwt_authenticator.router.post("/groups/create")
...
@jwt_authenticator.router.delete("/groups/delete")
...
@jwt_authenticator.router.get("/groups/privileges", response_model=list[UserPrivilegeSchema])
...
@jwt_authenticator.router.post("/groups/add-privilege")
...
@jwt_authenticator.router.delete("/groups/remove-privilege")
