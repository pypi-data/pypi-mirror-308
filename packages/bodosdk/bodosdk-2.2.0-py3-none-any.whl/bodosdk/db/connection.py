...
class Cursor(ICursor):
    ...
    def __init__(self, catalog: str, cluster: ICluster, timeout: int = 3600, query_id=None):
        """
    ...
    def execute_async(self, query: str, args: Union[Sequence[Any], Dict] = None) -> "Cursor":
        """
        ...
        self._results = []
        self._rows_stripped = 0
        self._job = self.cluster.run_sql_query(catalog=self._catalog, sql_query=query, args=args, store_result=True)
        return self
    ...
    def fetchmany(self, size):
        ...
        if self._job.is_ddl:
            results = self._job.sql_query_result["result"]
            data_to_return = list(tuple(v for k, v in item.items()) for item in results)[
        ...
        while size > len(results):
            if self._load_next_file():
                ...
                break
        data_to_return = results[max(self._current_row - self._rows_stripped, 0) : size]  # noqa: E203
        self._results = list(results[max(self._current_row - self._rows_stripped, 0) + size :])  # noqa: E203
        self._current_row += min(size, len(results))
    ...
    def fetchall(self):
        ...
        self._load_metadata()
        if self._job.is_ddl:
            return list(tuple(v for k, v in item.items()) for item in self._job.sql_query_result["result"])
        results = []
        results.extend(self._results)
    ...
    def _load_metadata(self):
        ...
        if not self._results_urls:
            ...
            self._file_index = 0
            self._current_row = 0
            self.download_manager = DownloadManager(self._job.uuid, self._results_urls[1:])
            self._files.extend(self.download_manager.download_files(self.uuid, self._timeout))
    ...
    def _wait_for_finished_job(self):
        ...
        try:
            self._job.wait_for_status(["SUCCEEDED", "FAILED", "CANCELLED"], timeout=self._timeout)
        except TimeoutException:
            raise DatabaseError("Query timed out")

        if self._job.status in ["FAILED", "CANCELLED"]:
            raise DatabaseError(f"Query failed due to {self._job.reason}. {self._job.get_stderr()}")
