...
class ProjectCreateInput(TJSON):
    def __init__(self, project: ProjectInput, full_copy: bool):
        self["name"] = project["name"]
        if "template" in project:
            self["templateName"] = project["template"]
        if "copy_project_id" in project and not full_copy:
            self["duplicateProjectId"] = project["copy_project_id"]
        if "copy_project_id" in project and full_copy:
