...
def parse_go_sum_file(
    resolver: Resolver | None, reader: LocationReadCloser
) -> Dict[str, str] | None:
    out = {}
...
def add_require(
    ...
    metadata = (
        GolangModuleEntry(h1_digest=sums[sum_key])
        if sums and sum_key in sums
        else None
    )
...
def replace_req(
    ...
    for matched, line_number in replace_list:
        ...
        if not version or (
            old_version and req_dict[old_pkg].version != old_version
        ):
            continue
...
def resolve_go_deps(
    content: str, location: Location, sums: dict[str, str] | None
) -> Iterator[Package]:
