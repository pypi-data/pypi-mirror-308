...
def create_package_cache(packages: list[Package]) -> dict:
    with ThreadPoolExecutor(max_workers=cpu_count()) as executor:
        return dict(zip(packages, executor.map(package_to_spdx_pkg, packages)))


def create_document_relationships(
    document: Document, spdx_packages: list[SPDX_Package]
        ...
        if isinstance(to_pkg, Package) and isinstance(from_pkg, Package):
            ...
            if to_pkg_id and from_pkg_id:
                return SPDXRelationship(
                    to_pkg_id, SPDXRelationshipType.DEPENDENCY_OF, from_pkg_id
                )
        return None

    with ThreadPoolExecutor(max_workers=cpu_count()) as executor:
        relationship_results = filter(
            None, executor.map(process_relationship, _relationships)
        )
...
def add_packages_and_relationships(
    ...
    spdx_id_cache = {
        pkg: spdx_pkg.spdx_id for pkg, spdx_pkg in package_cache.items()
    }

    document.packages = list(package_cache.values())

    document_relationships = create_document_relationships(
        document, document.packages
    )

    process_relationships(
        document, _relationships, spdx_id_cache, document_relationships
    )
