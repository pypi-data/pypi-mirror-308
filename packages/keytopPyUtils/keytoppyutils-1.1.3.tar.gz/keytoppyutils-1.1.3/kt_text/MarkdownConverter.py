            ...
            if tag.name == 'h1':
                ...
                if img_src:
                    ...
                    if response.status_code == 200:
                        ...
                        run.add_picture(img_data, width=Inches(2.5), height=Inches(2.5))

                        # 获取图片的 InlineShape 对象
                        inline_shape = run._r.find('.//w:drawing')[0]

                        # 设置图片与左边框的距离
                        left_offset = Pt(50)  # 50 磅（约 0.69 英寸）
                        inline_shape.set('distL', str(left_offset))
            elif tag.name == 'table':
                # 添加表格
                table_data = []
