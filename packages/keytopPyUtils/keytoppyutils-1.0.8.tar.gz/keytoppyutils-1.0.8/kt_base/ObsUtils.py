...
class ObsUtils:
    ...
    def upload_file(self,local_path,file_name):
        ...
        # 文件上传
        resp = self.obsClient.putFile(self.bucket_name, file_name, local_path)
    ...
    def load_stream(self,file_name):
        ...
        # 流式下载
        resp = self.obsClient.getObject(bucketName=self.bucket_name, objectKey=file_name, getObjectRequest=getObjectRequest,
    ...
    def download_file(self,file_name,local_file_path):
        # 文件下载
        resp = self.obsClient.getObject(self.bucketName, file_name, local_file_path)
        # 返回码为2xx时，接口调用成功，否则接口调用失败
        if resp.status < 300:
            return True
        else:
