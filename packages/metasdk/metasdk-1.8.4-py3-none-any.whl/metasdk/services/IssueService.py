...
from metasdk.schemas import IssueCreateItem, IssueGetItem, IssueUpdateItem
...
class IssueService:
    ...
    @common_auth
    def add_issue_msg_secure(self, issue_id, msg):
        issue_msg = self.__metadb.query("""
            INSERT INTO meta.issue_msg (msg, issue_id, user_id, last_user_id)
            VALUES (:msg, :issue_id, :user_id, :user_id)
            RETURNING id, msg
        """, {"issue_id": issue_id, "msg": msg, "user_id": self.auth_user_info["auth_user_id"]})
    ...
    @common_auth
    def change_issue_status_secure(self, issue_id, status_id: str):
        ...
        """, {
            "issue_id": issue_id, 
            "status_id": status_id,
            "user_id": self.auth_user_info["auth_user_id"]
        })
    ...
    def create_issue(self, issue: IssueCreateItem) -> int:
        ...
        query = """
            ...
            RETURNING id
        """
        ticket_id = self.__metadb.query(query, {
            ...
            "last_user_id": issue.last_user_id,
    ...
    def get_issues(
        ...
        query = """
            ...
            WHERE 2>1
                ...
                AND CASE WHEN :watcher_users::text IS NOT NULL THEN ARRAY[:watcher_users]::bigint[] <@ ARRAY(
            ...
            LIMIT 1000
        """
        issues = self.__metadb.all(query, {
            ...
            "watcher_users": ",".join(map(str, watcher_users)) if watcher_users else None,
            "user_id": self.auth_user_info["auth_user_id"]
        })
