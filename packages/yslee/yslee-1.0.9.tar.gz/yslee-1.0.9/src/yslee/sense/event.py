...
import soundfile
from yslee_sense_api import ApiClient, Configuration
from yslee_sense_api.api.audio_session_api import AudioSessionApi, SessionResult
...
from .exceptions import YsleeSenseException, TimeutException
from .result import EventResult, SoundTag, WindowResult
...
class EventClient:
    """EventClient is used to predict an audio file with Yslee.Sense."""
    ...
    def predict(self, file_path: str, timeout: Optional[float] = None) -> EventResult:
        """
    ...
    def _get_all_results(self, session: CreatedSession, timeout: Optional[float] = None) -> EventResult:
        retry_count: int = 0
        data_count: int = 0
        ...
        while True:
            ...
            if timeout is not None:
                if time.time() - start_time > timeout:
                    raise TimeutException(session.session_id, timeout)
            ...
            if (state is None) or (has_more is None) or (type(has_more) is not bool):
                raise YsleeSenseException(f'invalid Get Result API response: {response}')

            if state == 'error':
                error_msg = response.get('error', None)
                if not error_msg:
                    error_msg = 'Unexpected error occurred.'
                raise YsleeSenseException(error_msg)
                ...
                raise YsleeSenseException(f'invalid Get Result API response state: {state}')
