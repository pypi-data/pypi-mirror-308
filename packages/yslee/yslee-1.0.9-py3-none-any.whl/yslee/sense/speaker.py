...
from .exceptions import YsleeSenseException
...
class SpeakerClient:
    ...
    def _result(resp: HttpResponse) -> Optional[Union[dict, list]]:
        ...
        if 'error' in resp_json and resp_json['error'] != '':
            raise YsleeSenseException(resp_json['error'])
    ...
    def get_all(self) -> Optional[Union[dict, list]]:
        return self._result(HttpRequest.get(self.api_config.get_speaker_host() + '/speakers', headers={
            'X-Api-Key': self.project_key,
        }))

    def add(self, speaker: str, file_paths: List[str]) -> Optional[Union[dict, list]]:
        if len(file_paths) > self.api_config.speaker_upload_max_file_count:
            raise YsleeSenseException(
                'max upload file count is {}'.format(self.api_config.speaker_upload_max_file_count))

        for file_path in file_paths:
            if os.stat(file_path).st_size > self.api_config.speaker_upload_max_file_size:
                raise YsleeSenseException(
                    'one of files is bigger than {} MB'.format(
                        self.api_config.speaker_upload_max_file_size / 1024 / 1024))

        return self._result(HttpRequest.post(self.api_config.get_speaker_host() + '/speaker', data={
            'speaker': speaker,
        }, headers={
            'X-Api-Key': self.project_key,
        }, files={
    ...
    def remove(self, speaker: str) -> Optional[Union[dict, list]]:
        return self._result(
            HttpRequest.delete(self.api_config.get_speaker_host() + '/speaker?speaker={}'.format(quote_plus(speaker)),
                               headers={'X-Api-Key': self.project_key}))

    def recognize(self, file_path: str) -> Optional[Union[dict, list]]:
        if os.stat(file_path).st_size > self.api_config.speaker_upload_max_file_size:
            raise YsleeSenseException(
                'file is bigger than {} MB'.format(self.api_config.speaker_upload_max_file_size / 1024))

        return self._result(HttpRequest.post(self.api_config.get_speaker_host() + '/speaker/recognition', headers={
            'X-Api-Key': self.project_key,
        }, files={
