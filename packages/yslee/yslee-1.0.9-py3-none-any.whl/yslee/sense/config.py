...
class SensitivityScale(Enum):
    ...
    It means that Yslee.Sense API determines a sound tag is detected even though its confidence is low.
    """
...
class ChunkSize(Enum):
    ...
    A file is split into multiple chunks before it is uploaded to Yslee.Sense cloud.
...
class APIConfig:
    def __init__(
        ...
        """
        Args:
            sensitivity: SensitivityConfig to apply in Yslee.Sense API Client.
        """
        ...
        self.speaker_host: Optional[str] = speaker_host
        self.speaker_upload_max_file_count: int = 20
        self.speaker_upload_max_file_size: int = 10 * 1024 * 1024  # 10 MB

    def get_host(self) -> str:
        return 'https://api.yslee.ai/sense/api/v1' if self.host is None else self.host

    def get_speaker_host(self) -> str:
        return 'https://sr-api.yslee.ai/api/v1' if self.speaker_host is None else self.speaker_host
