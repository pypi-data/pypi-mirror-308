...
class UniverseScreener:
    ...
    def compute_top_stocks(self, span: int = 52,
                           freq: str = 'W-WED',
                           vol_span: int = 13,
                           cluster_threshold: float = 5.0,
            ...
            if available_indices[idx]:  # select the pair to the basket if both assets didn't enter existing baskets
                ...
                for idx1 in np.arange(0, n_pairs):  # selet the tird asset if it didn't entered existing assets
                    if available_indices[idx1]:
                        ...
                        if asset1 not in selected_assets and cluster1 not in selected_assets_clusters:
                            selected_assets_basket.append(asset1)
                            selected_assets.append(asset1)
                            selected_assets_clusters.append(cluster1)
                            ...
                            if len(selected_assets_basket) == basket_size:
                                ...
                                next_basket_idx += 1
                                break
                        if asset2 not in selected_assets and cluster2 not in selected_assets_clusters:
                            selected_assets_basket.append(asset2)
                            selected_assets.append(asset2)
                            selected_assets_clusters.append(cluster2)
                            available_indices[idx1] = False
...
def run_unit_test(unit_test: UnitTests):
    ...
    screener = load_universe_screener(local_path=local_path, universe=Universe.SAMPLE)
    ...
    if unit_test == UnitTests.RUN_UNIVERSE:
        ...
        baskets = screener.create_baskets_outputs(selected_baskets=selected_baskets, top_scores=top_scores)
        ...
        qis.save_df_to_excel(data=data, file_name='selected_baskets_3', local_path=lp.get_output_path(), add_current_date=True)
        ...
        qis.save_df_to_excel(sample_baskets, file_name='sample_baskets')
