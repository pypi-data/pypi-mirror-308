...
class Expansion:
    ...
    def as_integer_ratio(self) -> tuple[int, int]:
        result_numerator, result_denominator = self._components[
            0
        ].as_integer_ratio()
        ...
        *args: float,
        ...
        if args:
            try:
                ...
                raise TypeError(
                    f'Components should be of type {float!r}, '
        ...
        try:
            ...
            raise ValueError(
                ...
                f'but found: {invalid_component!r}.'
            )
    ...
    def __ceil__(self) -> int:
        return _components_to_integer(self._components) + _ceil(
            ...
            if isinstance(other, Expansion)
                ...
                if isinstance(other, float)
                    ...
                    if isinstance(other, int)
                        ...
                        _are_components_equal_to_rational(
    ...
    def __floor__(self) -> int:
        return _components_to_integer(self._components) + _floor(
            _components_to_accumulated_fraction(self._components)
        )
        ...
        return (
            ...
            if isinstance(other, Expansion)
                ...
                if isinstance(other, float)
                    ...
                    not _are_components_lesser_than_int(
                        self._components, other
                    )
                    if isinstance(other, int)
                    else (
            ...
            if isinstance(other, Expansion)
                ...
                if isinstance(other, float)
                    ...
                    if isinstance(other, int)
                        ...
                        _is_rational_lesser_than_components(
    ...
    def __le__(self, other: _Self | Number) -> Any | bool:
        return (
            ...
            if isinstance(other, Expansion)
                ...
                not _is_float_lesser_than_components(other, self._components)
                if isinstance(other, float)
                else (
                    not _is_int_lesser_than_components(other, self._components)
                    if isinstance(other, int)
                    else (
        ...
        return (
            ...
            if isinstance(other, Expansion)
                ...
                if isinstance(other, float)
                    ...
                    _are_components_lesser_than_int(self._components, other)
                    if isinstance(other, int)
                    else (
                        _are_components_lesser_than_rational(
                            self._components, other
                        )
                        if isinstance(other, _Rational)
                        else NotImplemented
    ...
    def __repr__(self) -> str:
        return type(self).__qualname__ + '({})'.format(
    ...
    def __round__(self, precision: int | None = None) -> _Self | int:
        if precision is None:
            ...
            fraction_sign = (
                -1 if _are_components_lesser_than_float(fractions, 0) else 1
            )
            if _are_components_equal_to_float(fractions, 0.5 * fraction_sign):
                sign = _to_sign(result)
                if sign != fraction_sign:
                    result -= sign
                if result & 1:
                    result += sign
            elif _is_float_lesser_than_components(
                0.5, fractions
...
def _rational_to_components(value: _Rational) -> list[float]:
    return (
        _int_to_components(int(value.numerator))
        if value.denominator == 1
        else _divide_components(
            _int_to_components(int(value.numerator)),
            _int_to_components(int(value.denominator)),
        )
    )
...
def _are_components_lesser_than_int(
    ...
    return components_integer < value or (
        components_integer == value
        and _components_to_accumulated_fraction(components) < 0.0
    )
...
def _int_to_components(value: int) -> list[float]:
    if not value:
        return [0.0]
    result = []
    while value:
...
def _invert_components(components: Sequence[float]) -> Sequence[float]:
    ...
    # for f(x) = 1 / x
    first_approximation = 1.0 / components[-1]
    ...
    for _ in _repeat(None, 6 + _ceil_log2(len(components))):
        result = _multiply_components(
            _add_float(_multiply_components(result, negated_components), 2.0),
            result,
        )
    return result
...
def _are_components_lesser_than(
    ...
    return left_size != right_size and (
        ...
        else left[left_size - right_size - 1] < 0.0
    )
...
def _add_components(
    ...
    if (right_component > left_component) is (
    ...
    if (left_index < left_length) and (right_index < right_length):
        ...
        while (left_index < left_length) and (right_index < right_length):
            ...
            if tail:
                result.append(tail)
    for rest_left_index in range(left_index, left_length):
        left_component = left[rest_left_index]
        tail, accumulator = _two_add(accumulator, left_component)
        if tail:
            result.append(tail)
    for rest_right_index in range(right_index, right_length):
        right_component = right[rest_right_index]
        tail, accumulator = _two_add(accumulator, right_component)
        if tail:
            result.append(tail)
...
def _compress_components(components: list[float]) -> list[float]:
...
def _fast_two_add(left: float, right: float) -> tuple[float, float]:
    head = left + right
...
def _scale_components(
    ...
    tail, accumulator = _two_multiply_presplit(
        next(components_iterator), scalar, scalar_low, scalar_high
    )
    result = []
    if tail:
        result.append(tail)
    for component in components_iterator:
        product_tail, product = _two_multiply_presplit(
            component, scalar, scalar_low, scalar_high
        )
...
def _square(value: float) -> tuple[float, float]:
    head = value * value
...
def _subtract_components(
    ...
    if (subtrahend_component > minuend_component) is (
        ...
        subtrahend_index += 1
    result = []
    if (minuend_index < minuend_length) and (
        subtrahend_index < subtrahend_length
    ):
        minuend_component, subtrahend_component = (
            minuend[minuend_index],
            -subtrahend[subtrahend_index],
        )
        if (subtrahend_component > minuend_component) is (
            subtrahend_component > -minuend_component
        ):
            tail, accumulator = _fast_two_add(minuend_component, accumulator)
            minuend_index += 1
        else:
            tail, accumulator = _fast_two_add(
                subtrahend_component, accumulator
            )
            subtrahend_index += 1
        if tail:
            result.append(tail)
        while (minuend_index < minuend_length) and (
            subtrahend_index < subtrahend_length
        ):
            minuend_component, subtrahend_component = (
                minuend[minuend_index],
            ...
            if tail:
                result.append(tail)
    for rest_minuend_index in range(minuend_index, minuend_length):
        minuend_component = minuend[rest_minuend_index]
        tail, accumulator = _two_add(accumulator, minuend_component)
        if tail:
            result.append(tail)
    for rest_subtrahend_index in range(subtrahend_index, subtrahend_length):
        subtrahend_component = -subtrahend[rest_subtrahend_index]
        tail, accumulator = _two_add(accumulator, subtrahend_component)
        if tail:
            result.append(tail)
    if accumulator or not result:
...
def _two_multiply(left: float, right: float) -> tuple[float, float]:
    head = left * right
...
def _two_multiply_presplit(
    left: float, right: float, right_low: float, right_high: float
) -> tuple[float, float]:
    head = left * right
    left_low, left_high = _split(left)
    first_error = head - left_high * right_high
...
def _two_one_add(
    left_tail: float, left_head: float, right: float
...
def _two_subtract(left: float, right: float) -> tuple[float, float]:
    head = left - right
...
def _two_two_add(
    left_tail: float, left_head: float, right_tail: float, right_head: float
) -> tuple[float, float, float, float]:
    third_tail, mid_tail, mid_head = _two_one_add(
        left_tail, left_head, right_tail
    )
    second_tail, first_tail, head = _two_one_add(
        mid_tail, mid_head, right_head
    )
...
def _two_two_subtract(
    ...
    second_tail, first_tail, head = _two_one_subtract(
        mid_tail, mid_head, right_head
    )
    return third_tail, second_tail, first_tail, head
...
def _cross_product(
    first_dx: float, first_dy: float, second_dx: float, second_dy: float
) -> tuple[float, float, float, float]:
...
def _squared_length(dx: float, dy: float) -> tuple[float, float, float, float]:
    dx_squared_tail, dx_squared_head = _square(dx)
    dy_squared_tail, dy_squared_head = _square(dy)
    return _two_two_add(
        dx_squared_tail, dx_squared_head, dy_squared_tail, dy_squared_head
    )
...
def _add_extras(
    ...
    if first_dx_tail or first_dy_tail:
        ...
        if first_dx_tail:
            ...
            if second_dy_tail:
                buffer_8 = _scale_components(
                    third_squared_length, first_dx_tail
                )
                first_buffer_16 = _scale_components(buffer_8, second_dy_tail)
                final_components = _add_components(
            ...
            if third_dy_tail:
                ...
                first_buffer_16 = _scale_components(buffer_8, third_dy_tail)
                final_components = _add_components(
                    final_components, first_buffer_16
                )
            first_buffer_32 = _scale_components(
            ...
            first_dx_tail_second_third_cross_product_tails = _scale_components(
            ...
            second_buffer_32 = _add_components(
        ...
        if first_dy_tail:
            ...
            first_dy_tail_second_third_cross_product_tails = _scale_components(
...
def _adaptive_incircle_determinant_estimation(
    ...
    third_y: float,
    upper_bound: float,
    second_upper_bound_coefficient: float = (44.0 + 576.0 * _EPSILON)
    * _EPSILON
    * _EPSILON,
    ...
    first_second_cross_product = _cross_product(
        first_dx, first_dy, second_dx, second_dy
    )
    second_third_cross_product = _cross_product(
        second_dx, second_dy, third_dx, third_dy
    )
    third_first_cross_product = _cross_product(
        third_dx, third_dy, first_dx, first_dy
    )
    first_components = _scale_by_squared_length(
        second_third_cross_product, first_dx, first_dy
    )
    second_components = _scale_by_squared_length(
        third_first_cross_product, second_dx, second_dy
    )
    third_components = _scale_by_squared_length(
        first_second_cross_product, third_dx, third_dy
    )
    first_second_sum_components = _add_components(
        first_components, second_components
    )
    first_buffer = _add_components(
    ...
    if (
        not first_dx_tail
        and not second_dx_tail
        and not third_dx_tail
        and not first_dy_tail
        and not second_dy_tail
        and not third_dy_tail
    ):
        return result
    threshold = (
        second_upper_bound_coefficient * upper_bound
    ...
    first_squared_length = (
        _squared_length(first_dx, first_dy)
        if (second_dx_tail or second_dy_tail or third_dx_tail or third_dy_tail)
        else (0, 0, 0, 0)
    )
    second_squared_length = (
        _squared_length(second_dx, second_dy)
        if (third_dx_tail or third_dy_tail or first_dx_tail or first_dy_tail)
        else (0, 0, 0, 0)
    )
    third_squared_length = (
        _squared_length(third_dx, third_dy)
...
def _incircle_determinant_estimation(
    ...
    third_x: float,
    third_y: float,
    upper_bound_coefficient: float = (10.0 + 96.0 * _EPSILON) * _EPSILON,
) -> float:
    first_dx = first_x - point_x
    second_dx = second_x - point_x
    third_dx = third_x - point_x
...
def _vectors_cross_product_estimation(
    ...
    return (
        result
        if (result >= threshold) or (-result >= threshold)
        else _adaptive_vectors_cross_product_estimation(
            first_start_x,
            first_start_y,
            first_end_x,
            first_end_y,
...
def _adaptive_vectors_cross_product_estimation(
    ...
    second_end_y: float,
    upper_bound: float,
    first_upper_bound_coefficient: float = (2.0 + 12.0 * _EPSILON) * _EPSILON,
    second_upper_bound_coefficient: float = (9.0 + 64.0 * _EPSILON)
    * _EPSILON
    * _EPSILON,
    ...
    first_components = _two_two_subtract(
        minuend_tail, minuend, subtrahend_tail, subtrahend
    )
    result = sum(first_components)
    threshold = first_upper_bound_coefficient * upper_bound
    if (result >= threshold) or (-result >= threshold):
        return result
    minuend_x_tail = _two_subtract_tail(first_end_x, first_start_x, minuend_x)
    subtrahend_x_tail = _two_subtract_tail(
        second_end_x, second_start_x, subtrahend_x
    )
    minuend_y_tail = _two_subtract_tail(first_end_y, first_start_y, minuend_y)
    subtrahend_y_tail = _two_subtract_tail(
        second_end_y, second_start_y, subtrahend_y
    )
    if (
        not minuend_x_tail
        and not minuend_y_tail
        and not subtrahend_x_tail
...
def _vectors_cross_product(
    ...
    return (
        ...
        else _adaptive_vectors_cross_product(
            first_start_x,
            first_start_y,
            first_end_x,
            first_end_y,
...
def _adaptive_vectors_cross_product(
    first_start_x: float,
    first_start_y: float,
    first_end_x: float,
    first_end_y: float,
    ...
    extra = (minuend_x * subtrahend_y_tail + subtrahend_y * minuend_x_tail) - (
        minuend_y * subtrahend_x_tail + subtrahend_x * minuend_y_tail
    )
    estimation += extra
    if (estimation >= threshold) or (-estimation >= threshold):
