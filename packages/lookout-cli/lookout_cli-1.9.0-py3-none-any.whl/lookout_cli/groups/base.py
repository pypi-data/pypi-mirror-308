...
def up(
    ...
    os.environ["ROS_STATIC_PEERS"] = ";".join(config.static_peers)
    ...
    os.environ["LOOKOUT_CORE_COMMAND"] = "platform ros launch lookout_bringup configure.launch.py"
    os.environ[
        "LOOKOUT_GREENSTREAM_COMMAND"
    ] = "platform ros launch lookout_greenstream_bringup configure.launch.py"
    os.environ["LOOKOUT_LOG_DIR"] = str(log_directory)
    os.environ["LOOKOUT_RECORDING_DIR"] = str(recording_directory)

    if not config.prod:
        os.environ["LOOKOUT_CORE_COMMAND"] += " --build --watch"
        os.environ["LOOKOUT_GREENSTREAM_COMMAND"] += " --build --watch"
...
def generate():  # type: ignore
    ...
    docker.compose.run("lookout_core", ["npx", "-y", "ros-typescript-generator"])
...
def configure(gpu: bool, default: bool):  # type: ignore
    ...
    if default:
        ...
        cameras: Optional[List[CameraExtended]] = None
            ...
            if gen_cameras == "y":
                cameras = [
                    CameraExtended(
                        ...
                        k_intrinsic=[1100.0, 0.0, 960.0, 0.0, 1100.0, 540.0, 0.0, 0.0, 1.0],
                        order=0,
                        pixel_height=1080,
                        pixel_width=1920,
