  ...
  <div ref="header" class="order-header flex flex-col">
    ...
    <div
      ...
      :style="{
        ...
        background: `repeating-linear-gradient(45deg,${changeColorAlpha(getColor(), 0.1)},${changeColorAlpha(getColor(), 0.1)} 10px,rgb(255, 255, 255) 0,rgb(255, 255, 255) 20px)`, 
        color: getTextColor(),
    ...
    >
      <div v-if="isStatus" class="status flex py-[0.25rem]" :style="{ color: getTextColor() }">
        <span>{{ orderStatus?.status?.toUpperCase() }}</span>
      </div>
      <div v-if="currentStep === 1" class="status-buttons flex gap-3">
        <Button v-if="true || quoteButton?.state === 'active'" class="button">
          <div class="el flex gap-2" @click="isEmailModalOpened = true">Send Client Quote</div>
        </Button>
        <Button class="button button-green flex items-center gap-2" @click="onProceedToCompliance">
      ...
      <div v-if="currentStep === 3" class="status-buttons flex gap-3">
        ...
        <Button class="button"> Submit Delivery Ticket Details </Button>
        <Button class="button cancel-button items-center gap-2">
          <img width="12" height="12" src="../../assets/icons/cross.svg" alt="delete" />
          Mark as No-Uplift
        </Button>
...
import type { IOrder, IOrderStatus } from 'shared/types';
...
const goToCompliance = ref(false);

const { data: orderStatuses } = useQueryOrderStatuses();
...
const {
  ...
  refetch: refetchProceedCompliance,
  isSuccess: isSuccessProceedCompliance
} = useQueryProceedCompliance(orderId, {
  refetchOnMount: false,
  retry: false,
  enabled: goToCompliance
});

function getColor() {
  const status = orderStatuses?.value?.find(
    (el: IOrderStatus) => el.name === orderStatus.value?.status
  );
...
function getTextColor() {
  ...
  let hex = status?.fill_colour_hex ?? '#000000';
  const amount = 80;
  hex = hex.replace(/^#/, '');
...
const onProceedToCompliance = () => {
  goToCompliance.value = true;
  refetchProceedCompliance();
};
...
watch(
  () => isSuccessProceedCompliance.value,
  (value) => {
    proceedComplianceData.value && value && orderStore.changeStep(2);
  }
);
