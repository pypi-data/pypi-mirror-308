<template>
  <div class="order-wrapper">
    ...
    <OrderForm :is-loading="isLoading" add-default-classes>
      ...
      <template v-if="isFirstStep" #content>
        <Stepper
          class="form-stepper"
      ...
      <template v-if="!isFirstStep && formModel?.type?.is_fuel" #content>
        <Stepper
          class="form-stepper"
          :steps="['General Info', 'Order Details']"
          :current-step="2"
        ...
        <div v-if="releaseType === 'Standard'" class="w-full flex flex-col">
          <div class="flex items-start w-full mb-4">
            ...
            <div class="flex items-center justify-center mt-[2.5rem] w-1/12">
              <CheckboxField v-model="formModel.fuel_order!.arrival_time_tbc" class="mb-0 mr-1" />
          ...
          <div class="flex items-start w-full mb-4">
            ...
            <div class="flex items-center justify-center mt-[2.5rem] w-1/12">
              <CheckboxField v-model="formModel.fuel_order!.departure_time_tbc" class="mb-0 mr-1" />
              <p class="text-base whitespace-nowrap">TBC</p>
            </div>
          </div>
          <div class="w-11/12">
        ...
        <div v-if="releaseType === 'Open'" class="w-full flex flex-col">
          <div class="flex items-start w-full mb-4">
            ...
            <div class="flex items-center justify-center mt-[2.5rem] w-1/12">
              <CheckboxField v-model="ufn" class="mb-0 mr-1" />
              <p class="text-base whitespace-nowrap">UFN</p>
            </div>
          </div>
        </div>
      </template>
      <template v-if="!isFirstStep && formModel?.type?.is_gh" #content>
        ...
        <div class="flex items-start w-full mb-4">
          ...
          <div class="flex items-center justify-center mt-[2.5rem] w-1/12">
            <CheckboxField v-model="formModel.gh_order!.arrival_time_tbc" class="mb-0 mr-1" />
            <p class="text-base whitespace-nowrap">TBC</p>
          </div>
        </div>
        <div class="flex items-start w-full mb-4">
          ...
          <div class="flex items-center justify-center mt-[2.5rem] w-1/12">
            <CheckboxField v-model="formModel.gh_order!.departure_time_tbc" class="mb-0 mr-1" />
            <p class="text-base whitespace-nowrap">TBC</p>
          </div>
        </div>
        <div class="flex items-start w-11/12">
          <div class="w-full flex gap-x-3">
            <SelectField
              v-model="formModel.gh_order!.ground_handler"
              class="w-full"
              label-text="Ground Handler"
              placeholder="Please select ground handler"
              :disabled="!meta && !isAdmin && !getIsAdmin()"
              :errors="validationInfo?.gh_order?.ground_handler?.$errors"
              :is-validation-dirty="validationInfo?.$dirty"
              label="full_repr"
              :options="handlers"
              :loading="false"
        ...
        <AirportLocationApiSelectField
          ...
          class="w-11/12"
          :is-location="false"
          :is-tbc="formModel.destination === null"
          :errors="validationInfo?.destination?.$errors"
          :is-validation-dirty="validationInfo?.$dirty"
          label-text="Destination Airport:"
          placeholder="Please select Destination Airport"
...
const { data: handlers, callFetch: fetchHandlers } = useFetch(
  OrderReferences.fetchGroundHandlers.bind(OrderReferences)
);
...
watch(
  ...
  async (airportId: number | undefined | null) => {
    if (airportId) {
      return await Promise.allSettled([fetchHandlers(airportId as any)]);
    }
  }
);
