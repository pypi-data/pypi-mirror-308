<template>
  ...
  <div v-else class="credit-wrap flex w-full py-[2.25rem] px-[0.75rem]">
    <div
...
const maximum = computed(() => creditExposure.value?.uplift_exposure ?? 0);
...
watch(
  () => [orderStore.order?.id, orderStore.currentStep],
  ([id, step]) => {
    id && step === 2 && fetchCreditExposure(id);
  }
);
</script>
