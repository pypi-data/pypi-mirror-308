<template>
  <div id="order-scroll" class="w-full flex justify-center h-screen overflow-auto">
    <div v-show="order" class="w-full bg-white flex flex-col">
      <OrderHeader ref="headerRef" :is-loading="loading" :order="order!"></OrderHeader>
      <div class="content-wrap flex gap-2 flex-grow">
        <div class="flex-1 flex flex-col items-center gap-2">
          ...
          <OrderGhTabServicing
            ...
            :order="order!"
          />
          <OrderTabOrder v-show="currentStep === 3" :is-loading="loading" :order="order!" />
...
watch(
  ...
  async (id) => {
    if (id) {
      if (route?.query.tab) orderStore.changeStep(parseInt(route.query.tab as string));
