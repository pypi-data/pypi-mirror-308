...
default_features = ["date_block_num", "item_price", "item_price_lag_1", "item_cnt_day_lag_1", "item_price_lag_2",
                     "item_cnt_day_lag_2", "item_price_lag_3", "item_cnt_day_lag_3", "item_price_lag_4", "item_cnt_day_lag_4", "month", "is_NewYear", "group", "item_cnt_day"]

default_unique_features = ["date", "shop_id", "item_id"]

default_feature_map = {"date" : "%d.%m.%Y",
            "date_block_num" : "int",
            "shop_id" : "O",
            "item_id" : "O",
            "item_price" : "float",
...
class UniquenessTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, features=default_unique_features):
...
class MergeTransformer(BaseEstimator, TransformerMixin):
    ...
    def __init__ (self, merge_list): #1!!!!!!!!1!!!!!!!!!!
...
class NegativeValueTransformer(BaseEstimator, TransformerMixin):
    ...
    def __init__ (self, feature="item_price"):
...
class DtypesTransformer(BaseEstimator, TransformerMixin):
    ...
    def __init__ (self, feature_map=default_feature_map):
...
class PriceClusterTransform(BaseEstimator, TransformerMixin):
    ...
    def __init__ (self, price_column="item_price", n_clusters=4):
...
class NewCategoriesTransformer(BaseEstimator, TransformerMixin): 
    ...
    @staticmethod
    def get_city_name_(x:str):
        if x[0] == "!":
            x = x[1:]
        return x.split()[0]
    
    @staticmethod
    def get_shop_type_(x:str):
        words = x.split()
    ...
    def transform(self, X, y=None):
        X["shop_type"] = X["shop_name"].apply(self.get_shop_type_)
        X["city_name"] = X["shop_name"].apply(self.get_city_name_)
        X["group"] = X["item_category_name"].apply(self.get_group_)
...
class CategoryOneHotEncoder(BaseEstimator, TransformerMixin):
    ...
    def __init__ (self, columns=["city_name", "group", "shop_type"]):
...
class OutliersCleaningTransformer(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, outliers_map={"item_cnt_day": 400, "item_price": 60000}):
...
class TestPreprocessTransformer(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, raw_train, start_date="01.01.2013", period=34):
