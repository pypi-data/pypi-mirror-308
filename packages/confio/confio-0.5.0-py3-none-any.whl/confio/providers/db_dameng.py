...
from confio.core.base import ConfItem, IConf, logger


class ConfDameng(IConf):
    """
    基于达梦数据库的配置存储
    """
    SQL = {
        # 查询表是否存在
        'table_exists': """SELECT COUNT(*) as "count" FROM ALL_TABLES WHERE OWNER = '{db_name}' AND TABLE_NAME = '{table_name}'""",
        # 创建表结构
        ...
        # 加载匹配指定前缀的数据
        'match_items': 'SELECT * FROM "{table_name}" WHERE "id" LIKE ?',
        # 更新数据值
        'update_sys_value_by_id': 'UPDATE "{table_name}" SET "value"=?, "value_type"=? WHERE "id"=?',
        'update_user_value_by_id': 'UPDATE "{table_name}" SET "user_value"=?, "value_type"=? WHERE "id"=?',
        # 更新所有字段
        ...
        'add_item': 'INSERT INTO "{table_name}"'
                    ...
                    'VALUES(?,?,?,?,?,?,?,?)',
        # 移除项
        'remove_item': 'DELETE FROM "{table_name}" WHERE "id"=?',
        # 清空数据
    ...
    def __init__(self,
                 ...
                 **pool_args):
        """
        :pool_args: 指定连接池的一些参数，如 mincached maxcached maxshared maxconnections blocking setsession ping 等
        """
        ...
        self._tested = False
        super(ConfDameng, self).__init__()
    ...
    def _table_exists(self) -> bool:
        _, data = self._execute(self.SQL['table_exists'].format(
            db_name=self.con_option['schema'],
            table_name=self.table_name
        ))
        return data[0]['count'] > 0

    def _create_table(self):
        create_table_sql = self.SQL['create_table'].format(
            table_name=self.table_name
        )
        create_table_sql = create_table_sql.split('ENGINE=InnoDB DEFAULT CHARSET=utf8')[0].rstrip()
        self._execute(create_table_sql)

    def load(self) -> List[ConfItem]:
        rows, data = self._execute(self.SQL['load_all'].format(
            table_name=self.table_name
        ))
        return [
    ...
    def get(self, id_: str, default=None, value_only=True) -> Union[ConfItem, Any]:
        rows, data = self._execute(self.SQL['get_by_id'].format(
            table_name=self.table_name
        ), id_)
    ...
    def match(self, prefix: str, value_only=True, fullkey=False) -> Dict[str, Union[ConfItem, Any]]:
        ...
        rows, data = self._execute(self.SQL['match_items'].format(
            table_name=self.table_name
        ), prefix + '%')
    ...
    def set(self, id_: str, value, update_sys_value=False, allow_add=False):
        rows, _ = self._execute(self.SQL['get_by_id'].format(
            table_name=self.table_name
        ), id_)
        if not rows:
            if not allow_add:
    ...
    def remove(self, id_: Union[str, List[str], Tuple[str]]) -> Optional[Union[ConfItem, List[ConfItem]]]:
        ...
        for id_ in id_list:
            if id_ not in data_map:
                logger.warning('Conf item "%s" not found' % id_, stack_info=self.STACK_INFO)
                continue
            self._execute(self.SQL['remove_item'].format(
                table_name=self.table_name
            ), id_)
    ...
    def clear(self):
        ...
        :return:
        """
        self._execute(self.SQL['clear_table'].format(
            table_name=self.table_name
        ))
    ...
    def _set_item(self, id_, value, update_sys_value: bool):
        if isinstance(value, ConfItem):
            ...
            self._execute(self.SQL['update_by_id'].format(
                table_name=self.table_name
        ...
        if update_sys_value:
            ...
            sql = self.SQL['update_sys_value_by_id'].format(
                table_name=self.table_name
            )
            ...
            sql = self.SQL['update_user_value_by_id'].format(
                table_name=self.table_name
            )
            self._execute(sql, dumped_value, value_type, id_)
    ...
    def _add_item(self, id_, value, update_sys_value):
        ...
        self._execute(self.SQL['add_item'].format(
            table_name=self.table_name
