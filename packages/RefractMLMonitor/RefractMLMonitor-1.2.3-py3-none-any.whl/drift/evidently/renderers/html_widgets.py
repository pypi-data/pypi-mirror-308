...
def get_roc_auc_tab_data(
    ...
    for label in curr_roc_curve.keys():
        ...
        fig.update_layout(yaxis_title="True Positive Rate", showlegend=True,plot_bgcolor="#FFFFFF")
...
def get_pr_rec_plot_data(
    ...
    for label in current_pr_curve.keys():
        ...
        fig.update_layout(yaxis_title="Precision", showlegend=True,plot_bgcolor="#FFFFFF")

        additional_plots.append((str(label), plotly_figure(title="", figure=fig)))
    return additional_plots


def class_separation_traces_raw(df, label, target_name, color_options,reference_plot=None):
    traces = []
    sep_color = color_options.primary_color if not reference_plot else color_options.secondary_color
    traces.append(
        go.Scatter(
            ...
            marker=dict(size=6, color=sep_color),
        )
    )
    traces.append(
        go.Scatter(
...
def class_separation_traces_agg(df, label, color_options,reference_plot=None):
    traces = []
    sep_color = color_options.primary_color if not reference_plot else color_options.secondary_color
...
def get_class_separation_plot_data(
    ...
    for label in current_plot.columns.drop(target_name):
        ...
        if reference_plot is not None:
            traces = class_separation_traces_raw(reference_plot, label, target_name, color_options,reference_plot=reference_plot)
            for trace in traces:
                fig.add_trace(trace, 1, 2)
            fig.update_xaxes(dict(range=(-2, 3), showticklabels=False), row=1, col=2)

        fig.update_layout(yaxis_title="Probability", showlegend=True,plot_bgcolor="#FFFFFF")
...
def get_class_separation_plot_data_agg(
    ...
    for label in current_plot.keys():
        ...
        if reference_plot is not None:
            traces = class_separation_traces_agg(reference_plot[label], label, color_options,reference_plot=reference_plot)
            for trace in traces:
                fig.add_trace(trace, 1, 2)

        fig.update_layout(yaxis_title="Probability", showlegend=False,plot_bgcolor="#FFFFFF")
