...
if __name__ == "__main__":
    ...
    SAR_q_BS_design = data['sat_type_C']['SAR_q_BS_design']

    for sat_index, SAR_position in enumerate(SAR_list):

        SAR_theta_matrix_BL_deg = np.array(SAR_position['SAR_theta_matrix_BL_deg'])
        SAR_theta_matrix_LB_deg = SAR_theta_matrix_BL_deg.transpose()

        q_BS, light_axis_err, rotvec_norm_deg = theta_measure_to_quat(
            SAR_theta_matrix_LB_deg,
            data['SAR_A_LS'],
            SAR_q_BS_design)
        ...
        # print(f"    星敏安装偏差(欧拉转角): {rotvec_norm_deg:.3f} deg", file=output_file_SAR)
