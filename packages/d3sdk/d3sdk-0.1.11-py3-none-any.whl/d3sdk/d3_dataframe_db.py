...
class D3DataFrameDB:
    ...
    def getDeviceFailureRecords(self, 
        ...
        # 添加机组列表条件
        if devices:
            sql += f" and ai.name in ({','.join(f"'{device}'" for device in devices)})"

        # 添加诊断模块类型名称列表条件
        if fmtNames:
            sql += f" and fmt.display_name in ({','.join(f"'{fmtName}'" for fmtName in fmtNames)})"

        # 添加报警类型列表条件
        if alarmTypes:
            sql += f" and ag.dfem_bjlx in ({','.join(f"'alarm_type_{alarmType}'" for alarmType in alarmTypes)})"
