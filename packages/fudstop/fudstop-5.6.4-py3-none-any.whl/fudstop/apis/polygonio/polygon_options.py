...
# Example usage
    ...
    async def batch_upsert_dataframe(
        ...
        unique_columns: list,
        batch_size: int = 1000,
        keep_history: bool = False  # Optional parameter to maintain history
    ):
        try:
            async with lock:
                ...
                # Add a timestamp column if keeping history
                if keep_history:
                    df['insertion_timestamp'] = pd.to_datetime(datetime.utcnow())
                ...
                # Prepare the SQL insert query with conflict handling
                columns = df.columns.tolist()
                quoted_columns = [f'"{col}"' for col in columns]
                value_placeholders = ', '.join(f'${i}' for i in range(1, len(columns) + 1))
                update_set = ', '.join(
                ...
                # Prepare batch data with proper type conversions
                batch_data = []
                for record in records:
                    new_record = []
                    for col in columns:
                        ...
                        if pd.isna(val):
                            ...
                            if isinstance(val, (np.datetime64, datetime)):
                                new_record.append(pd.to_datetime(val).to_pydatetime())
                            else:
                                new_record.append(None)
                        elif pg_type in ('double precision', 'real', 'numeric', 'decimal'):
                            try:
                                new_record.append(float(val))
