...
from instigator_py import __version__
from instigator_py.config import get_user_config
from instigator_py.exceptions import (
...
from instigator_py.log import configure_logger
from instigator_py.main import instigator_py


def version_msg() -> str:
    """Return the instigator_py version, location and Python powering it."""
    python_version = sys.version
    location = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    return f"instigator_py {__version__} from {location} (Python {python_version})"
...
def list_installed_templates(
    ...
    """List installed (locally cloned) templates. Use instigator_py --list-installed."""
    config = get_user_config(passed_config_file, default_config)
    instigator_py_folder: str = config['instigator_pys_dir']
    if not os.path.exists(instigator_py_folder):
        click.echo(
            f"Error: Cannot list installed templates. "
            f"Folder does not exist: {instigator_py_folder}"
        )
        sys.exit(-1)

    template_names = [
        folder
        for folder in os.listdir(instigator_py_folder)
        if os.path.exists(
            os.path.join(instigator_py_folder, folder, 'instigator_py.json')
        )
    ]
...
@click.option(
    '--no-input',
    is_flag=True,
    help='Do not prompt for parameters and only use instigator_py.json file content. '
...
@click.option(
    '--directory',
    help='Directory within repo that holds instigator_py.json file '
...
def main(
    ...
    """Create a project from a instigator_py project template (TEMPLATE).

    instigator_py is free and open source software, developed and managed by
    volunteers. If you would like to help out or fund the project, please get
    in touch at https://github.com/devxhub/instigator_py.
    """
    ...
    try:
        instigator_py(
            ...
            password=os.environ.get('instigator_py_REPO_PASSWORD'),
            directory=directory,
