...
from instigator_py.find import find_template
from instigator_py.hooks import run_hook_from_repo_dir
from instigator_py.prompt import YesNoPrompt
from instigator_py.utils import (
...
def is_copy_only_path(path: str, context: dict[str, Any]) -> bool:
    ...
    :param context: instigator_py context.
    """
    try:
        for dont_render in context['instigator_py']['_copy_without_render']:
        ...
        if isinstance(context_value, list):
            ...
            if isinstance(overwrite, list):
                ...
                if overwrite in context_value:
                    ...
                    # see ``instigator_py.prompt.prompt_choice_for_config``
...
def generate_context(
    context_file: str = 'instigator_py.json',
    default_context: dict[str, Any] | None = None,
    extra_context: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Generate the context for a instigator_py project template.

    Loads the JSON file as a Python object, with key being the JSON filename.

    :param context_file: JSON file containing key/value pairs for populating
        the instigator_py's variables.
...
def generate_file(
    ...
    :param infile: Input file to generate the file from. Relative to the root
        template dir.
    :param context: Dict for populating the instigator_py's variables.
    ...
    if context['instigator_py'].get('_new_lines', False):
        # Use `_new_lines` from context, if configured.
        newline = context['instigator_py']['_new_lines']
...
def _run_hook_from_repo_dir(
    ...
    :param context: instigator_py project context.
    :param delete_project_on_failure: Delete the project directory on hook
        failure?
    """
    warnings.warn(
        "The '_run_hook_from_repo_dir' function is deprecated, "
        "use 'instigator_py.hooks.run_hook_from_repo_dir' instead",
        DeprecationWarning,
        2,
    )
