...
from instigator_py.exceptions import UndefinedVariableInTemplate
from instigator_py.utils import create_env_with_context, rmtree
...
def render_variable(
    env: Environment,
    raw: _Raw,
    instigator_py_dict: dict[str, Any],
) -> str:
    """Render the next variable to be displayed in the user prompt.

    Inside the prompting taken from the instigator_py.json file, this renders
    the next variable. For example, if a project_name is "Peanut Butter
    Cookie", the repo_name could be be rendered with:

        `{{ instigator_py.project_name.replace(" ", "_") }}`.

    This is then presented to the user as the default.

    :param Environment env: A Jinja2 Environment object.
    :param raw: The next value to be prompted for by the user.
    :param dict instigator_py_dict: The current context as it's gradually
    ...
    if raw is None or isinstance(raw, bool):
        ...
        return {
            render_variable(env, k, instigator_py_dict): render_variable(
                env, v, instigator_py_dict
            )
            for k, v in raw.items()
        }
    elif isinstance(raw, list):
        return [render_variable(env, v, instigator_py_dict) for v in raw]
    elif not isinstance(raw, str):
        raw = str(raw)

    template = env.from_string(raw)

    return template.render(instigator_py=instigator_py_dict)
...
def prompt_choice_for_config(
    ...
    :param no_input: Do not prompt for user input and return the first available option.
    """
    rendered_options = [render_variable(env, raw, instigator_py_dict) for raw in options]
    if no_input:
...
def prompt_for_config(
    ...
    """
    instigator_py_dict = OrderedDict([])
    env = create_env_with_context(context)
    prompts = context['instigator_py'].pop('__prompts__', {})

    # First pass: Handle simple and raw variables, plus choices.
    # These must be done first because the dictionaries keys and
    # values might refer to them.
    count = 0
    all_prompts = context['instigator_py'].items()
    ...
    for key, raw in all_prompts:
        if key.startswith('_') and not key.startswith('__'):
            instigator_py_dict[key] = raw
            continue
        elif key.startswith('__'):
            instigator_py_dict[key] = render_variable(env, raw, instigator_py_dict)
            continue
        ...
        try:
            if isinstance(raw, list):
                ...
                val = prompt_choice_for_config(
                    instigator_py_dict, env, key, raw, no_input, prompts, prefix
                )
                instigator_py_dict[key] = val
            elif isinstance(raw, bool):
                # We are dealing with a boolean variable
                if no_input:
                    instigator_py_dict[key] = render_variable(
                        env, raw, instigator_py_dict
                    )
                else:
                    instigator_py_dict[key] = read_user_yes_no(key, raw, prompts, prefix)
            elif not isinstance(raw, dict):
                # We are dealing with a regular variable
                val = render_variable(env, raw, instigator_py_dict)
    ...
    for key, raw in context['instigator_py'].items():
...
def choose_nested_template(
    ...
    """
    instigator_py_dict: OrderedDict[str, Any] = OrderedDict([])
    env = create_env_with_context(context)
    prefix = ""
    prompts = context['instigator_py'].pop('__prompts__', {})
    key = "templates"
    config = context['instigator_py'].get(key, {})
    if config:
        # Pass
        ...
        config = context['instigator_py'].get(key, [])
        val = prompt_choice_for_config(
            instigator_py_dict, env, key, config, no_input, prompts, prefix
        )
...
def prompt_and_delete(path: Path | str, no_input: bool = False) -> bool:
    ...
    reused. If yes, it's reused; otherwise, instigator_py exits.
