...
from instigator_py.exceptions import ConfigDoesNotExistException, InvalidConfiguration
...
USER_CONFIG_PATH = os.path.expanduser('~/.instigator_pyrc')
...
DEFAULT_CONFIG = {
    'instigator_pys_dir': os.path.expanduser('~/.instigator_pys/'),
    'replay_dir': os.path.expanduser('~/.instigator_py_replay/'),
...
def get_config(config_path: Path | str) -> dict[str, Any]:
    ...
    raw_cookies_dir = config_dict['instigator_pys_dir']
    config_dict['instigator_pys_dir'] = _expand_path(raw_cookies_dir)
...
def get_user_config(
    ...
    Otherwise look up the config file path in the ``instigator_py_CONFIG``
    ...
    try:
        ...
        env_config_file = os.environ['instigator_py_CONFIG']
    except KeyError:
