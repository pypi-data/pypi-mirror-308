...
:::::::::::::::::::::::::::::::::::::
The standard Runestone files in this template was generated using ``runestone init`` with default settings; the project name was ``runestone_template``. The CodeChat project configuration file, ``codechat_config.yaml``, has the ``source_path`` set to ``_source`` (matching the default Runestone setting) and the ``output_path`` set to ``build/runestone_template``. **Important**: The ``output_path`` for your project *must* match the project name.
