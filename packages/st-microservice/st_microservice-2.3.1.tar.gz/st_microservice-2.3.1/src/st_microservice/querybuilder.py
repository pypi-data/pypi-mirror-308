...
class BuildContext:
    ...
    def get_dict_params(self, **kwvalues) -> Sequence:
        if keys_diff := set(kwvalues.keys()).symmetric_difference(
            set(self.parameter_dict_keys)
        ):
...
def _init_db_type(from_table: TableParam, db_type: DBTypes | None) -> DBTypes:
    if db_type is None:
        if not inspect.isclass(from_table) or not issubclass(
            from_table, ModelInterface
        ):
            raise QueryBuilderException(
                "db_type must be specified if from_table is not a Model"
            )
...
class Value(Term, Aliasable):
    def __init__(
        self, value: str | int | float | Decimal | None, alias: str | None = None
    ):
        super().__init__(alias)
        self.value = value
...
class Column(Term, Aliasable):
    def __init__(
        self, name: str, table: TableBase | str | None = None, alias: str | None = None
    ):
        super().__init__(alias)
        self.name = name
        self.table = table
...
class PositionalParameter(Term):
    ...
    def render(self, build_context: BuildContext) -> str:
        if build_context.parameter_dict_keys:
            raise QueryBuilderException(
                "Cannot use PositionalParameter if KeyParameter is used"
            )
...
class KeyParameter(Term):
    ...
    def render(self, build_context: BuildContext) -> str:
        if build_context.parameter_list_values:
            raise QueryBuilderException(
                "Cannot use KeyParameter if PositionalParameter is used"
            )
        ...
        if (
            self.key not in build_context.parameter_dict_keys
        ):  # Re-use same postgres parameter
            build_context.parameter_dict_keys.append(self.key)
...
class Select(QueryBuilderBase):
    ...
    def is_joined(self, table: TableParam):
        return _get_table(table).identifier in [
            t.table.identifier for t in self._joinlist
        ]
    ...
    def join(
        ...
        *on_conditions,
        ...
        if self.is_joined(table):
            raise QueryBuilderException(
                f"Table {table.identifier} already in JOIN clause"
            )

        self._joinlist.append(
            _JoinClause(table, _check_terms(on_conditions), using, how)
        )
        return self

    def left_join(
        self, table: TableParam, *on_conditions, using: list[str] | None = None
    ):
    ...
    def join_relation(
        ...
        try:
            ...
            raise Exception(
                f"Could not find Relation {relation_name} in model {model.__name__}"
            )
    ...
    def invert_order_by(self):
        ...
        self._order_by = [
            col.term if isinstance(col, Descending) else Descending(col)
    ...
    def render(
    ...
    ) -> str:  # Make sure to build the SQL in top-down order so SQL Server paramters get injected in order
        # Validating
        if not self._select:
        ...
        # Building
        select_str = ", ".join(
            s.render_with_alias(build_context)
            if isinstance(s, Column)
            else s.render(build_context)
...
def _try_pure_value(v):
    ...
    if isinstance(v, (Value, PositionalParameter, KeyParameter)):
        ...
        if isinstance(v, Term):
            raise QueryBuilderException(
                "Term can only be of instance Value or Parameter"
            )
        v = Value(v)
    return v
...
class Insert(QueryBuilderBase):
    ...
    def set_columns_from_table(self):
        ...
        if not self._column_names:
            raise QueryBuilderException(
                f"Table {self._table.identifier} does not have columns"
            )
        return self
    ...
    def values(self, *pos_values, **kw_values):
        ...
        if pos_values and kw_values:
            raise QueryBuilderException(
                "Cannot use positional and keyword values together"
            )
    ...
    def render(self, build_context: BuildContext) -> str:
        ...
        if self._on_conflict is not None:
            ...
            if self._on_conflict == "update":
                ...
                raise QueryBuilderException(
                    f"Cannot handle ON CONFLICT action {self._on_conflict}"
                )
        return res
...
class Update(QueryBuilderBase):
    ...
    def render(self, build_context: BuildContext) -> str:
        ...
        res += ",\n".join(
            f'"{cn}" = {value.render(build_context)}'
            for cn, value in self._values.items()
        )
        res += "\nWHERE" + "\nAND ".join(
            f"({w.render(build_context)})" for w in self._where
        )
        return res
...
class Cast(Term, Aliasable):
    ...
    def render(self, build_context: BuildContext) -> str:
        return self._handle_alias(
            f"CAST({self.term.render(build_context)} AS {self.newtype})"
        )
...
class TableSubQuery(TableBase):
    ...
    def identifier(self) -> str:
        if self.alias is None:
            raise QueryBuilderException(
                "SubQuery does not have identifier. Please supply an alias"
            )
        return self.alias
    ...
    def __init__(
        ...
        *array_column_list: Term,
        ...
        if column_names is not None:
            if self.alias is None:
                raise QueryBuilderException(
                    "Cannot add columns if not alias is specified"
                )
