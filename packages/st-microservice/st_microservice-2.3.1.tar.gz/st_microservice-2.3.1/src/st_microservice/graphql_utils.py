...
def process_controls(model: type[BaseModel], controls: dict) -> TableControls:
    ...
    if order_by_raw is not None:
        ...
        for col_order in order_by_raw:
            ...
            new_order_by.append(
                Descending(new_col) if col_order["descending"] else new_col
            )
...
async def get_or_gql_error[T: BaseModel](
    db: DBInterface, model: type[T], **primary_keys
) -> T:
...
def setup_query(query_type: QueryType, model_list: list[type[BaseModel]]):
    ...
    def resolve_type_inspector(_, info: GraphQLResolveInfo, type_name: str):
        ...
        if model is not None:
            ...
            if hasattr(gqltype, "__modelname__"):
                raise GraphQLError(
                    f"Could not find model with name {gqltype.__modelname__}"
                )
        ...
        for field_name, field in gqltype.fields.items():
            caption = getattr(
                field, "__caption__", field_name.title().replace("_", " ")
            )
            dataclass_field = (
                model.__metadata__.dataclass_fields.get(field_name)
                if model is not None
                else None
            )
    ...
    async def resolve_load_objects(_, info, type_name: str, controls: dict):
        ...
        if model is None:
            raise GraphQLError(
                f"Could not find database model for GraphQL Object {type_name}"
            )

        q = load_from_model_query(model, **process_controls(model, controls))

        recs = await (model.__metadata__.registry.db_connection_factory or get_db)(
            info
        ).fetch(*q.to_sql())
...
def setup_mutation(mutation_type: MutationType, model_list: list[type[BaseModel]]):
    ...
    def get_model_and_check_scope(
        info: GraphQLResolveInfo, type_name: str
    ) -> type[BaseModel]:
        gqltype = _get_gql_type(info, type_name)
    ...
    async def resolve_insert_object(
        _, info: GraphQLResolveInfo, type_name: str, payload: dict
    ):
    ...
    async def resolve_delete_object(
        _, info: GraphQLResolveInfo, type_name: str, payload: dict
    ):
    ...
    async def resolve_edit_object(
        ...
        for i, field in enumerate(fields):
            try:
                ...
                raise GraphQLError(
                    f"Error decoding payload at fields position {i}: {e}"
                )
...
def apply_filter_to_query(q: Select, column: Column, field_type, value) -> Select:
    try:
        if field_type is str:
            return q.where(
                Like(column, PositionalParameter(value) if q.use_parameters else value)
            )
...
def load_from_model_query(
    ...
    *,
    ...
    order_by: Sequence[Term]
    | None = None,  # None for primary key ordering, [] for no ordering
    ...
    for filt in filters:
        ...
        *relation_names, col_name = full_name.split(".")
        ...
        if f.is_field_list_type(current_model, col_name):  # Handle lists
            unnested_table = Unnest(
                column, alias=col_name + "_unnested", column_names=["col1"]
            )
            q.where(
                Exists(
    ...
    if order_by:
        ...
        q.order_by(
            *(
                f.table(model).columns_map[pkname]
                for pkname in model.__metadata__.primary_keys
            )
        )
