...
def primary_key_filter(model: type[BaseModel], **primary_keys) -> Term:
    ...
    if diff := set(primary_keys.keys()).symmetric_difference(
        set(metadata.primary_keys)
    ):
        raise QueryBuilderException(
            f"Please specify all primary keys. Difference is : {diff}"
        )
    return And(
        *(metadata.table.columns_map[pk] == value for pk, value in primary_keys.items())
    )
...
def get_query(model: type[BaseModel], **primary_keys) -> Select:
    ...
    q = (
        Select(model)
        .select_main_columns()
        .where(primary_key_filter(model, **primary_keys))
    )
    return q
...
async def get_or_error[T: BaseModel](
    db: DBInterface, model: type[T], **primary_keys
) -> T:
...
async def update[T: BaseModel](db: DBInterface, model: type[T], obj: T, **primary_keys):
    ...
    await db.execute(
        *q.to_sql(**asdict(obj), **{"pk_" + k: v for k, v in primary_keys.items()})
    )
...
async def update_many[T: BaseModel](db: DBInterface, model: type[T], objs: Iterable[T]):
    ...
    q_str = q.render(build_context)
    await db.executemany(
        q_str, (build_context.get_dict_params(**obj) for obj in obj_dicts)
    )
...
async def batch_load[T: BaseModel](
    ...
    if len(columns) == 1:
        ...
        if q.db_type == DBTypes.SQLSERVER:
            ...
            column_and_types = {
                col: type_to_col[model.__metadata__.dataclass_fields[col].type]
                for col in columns
            }
            # Build table
            ...
            q.where(
                In(
                    ...
                    [
                        Tuple(*(PositionalParameter(key) for key in keys))
...
async def dataloader_get[T: BaseModel](
    ...
    try:
        ...
        max_batch_size = (
            floor(2000 / len(pk_names))
            if registry.db_type == DBTypes.SQLSERVER
            else 10_000
        )
...
async def dataloader_load[T: BaseModel](
    ...
    try:
        ...
        async def batch_get_wrapper(keys_list: Sequence[tuple]):
            ...
            for obj in objs:
                d.setdefault(tuple(getattr(obj, col) for col in columns), []).append(
                    obj
                )
            return [d.get(keys, []) for keys in keys_list]
...
def set_dataclass_attribute(obj: BaseModel, field_name: str, field_value):
    ...
    if field_type is Decimal and isinstance(
        field_value, float
