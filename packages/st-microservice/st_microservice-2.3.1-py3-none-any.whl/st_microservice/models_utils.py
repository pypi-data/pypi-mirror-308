...
class Registry:
    def __init__(
        ...
        db_connection_factory: Callable[[GraphQLResolveInfo], DBInterface]
        | None = None,
    ):
    ...
    def database_model_sub[T: type[BaseModel]](cls: T) -> T:
        ...
        for pk in primary_keys:
            if pk not in [f.name for f in dc_fields]:
                raise TypeError(
                    f"primary key column '{pk}' not found in {cls.__name__}"
                )
...
class Relation:
    def __init__(
        ...
        self.join_on = (
            join_on  # key is model's column name and value is rel_model's Column object
        )
