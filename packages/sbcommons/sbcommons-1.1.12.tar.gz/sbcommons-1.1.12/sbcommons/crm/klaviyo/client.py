...
class KlaviyoClient(CrmClient):
    ...
    def _start_bulk_import_profile_job(self, format_method: Callable[[Any], dict],
        ...
        # max retries - setting this to 180,
        #  so we have 180*5 = 900 seconds (15 minutes) before
        #  method is considered "timed out"
        max_retries = 180
        ...
        all_jobs = copy.deepcopy(jobs)

        retries = 0
        num_jobs_processing = len(jobs)
        while num_jobs_processing > 0 and retries < max_retries:
            retries = retries + 1
            time.sleep(5)
            jobs = [job['id'] for job in self._get_incomplete_jobs(url=url, job_id_filter=all_jobs)]
            num_jobs_processing = len(jobs)
            self.logger.info(f'Waiting on [{num_jobs_processing}] jobs...: {jobs}')

        if num_jobs_processing > 0:
            self.logger.warning(f'Timed out waiting for [{jobs}]!')

        successful = self._post_process_bulk_import_profile_jobs(all_jobs, url) and len(jobs) == 0
    ...
    def _get_incomplete_jobs(self, url: str, job_id_filter: List[str] = None) -> List[
        Dict[Any, Any]]:
        f = '?filter=any%28status%2C%5B%22queued%22%2C%22processing%22%5D%29&page[size]=100'
        return self._get_filtered_jobs(url=url, query_filter=f, job_id_filter=job_id_filter)

    def _get_complete_jobs(self, url: str, job_id_filter: List[str] = None) -> List[Dict[Any, Any]]:
        f = '?filter=equals%28status%2C%22complete%22%29&page[size]=100'
        return self._get_filtered_jobs(url=url, query_filter=f, job_id_filter=job_id_filter)
    ...
    def _get_filtered_jobs(
            ...
            url: str,
            query_filter: str,
            job_id_filter: List[str] = None
    ) -> List[Dict[Any, Any]]:
        """
        Get information on all jobs based on a query filter
        Args:
            url: endpoint url
            query_filter: the filter to add to the url
            job_id_filter: List of job_ids to include in the output.
                If this is empty then no job_ids are filtered away.
        Returns:
            A list of jobs that satisfy the query and job filters.
        """
        endpoint = url + query_filter
        jobs = []
        has_next = endpoint
        while has_next:
            time.sleep(0.1)
            self.logger.info(f'Querying: {has_next}')
            response = self._request(url=has_next)

            response_json = response.json()
            has_next = response_json.get('next')

            for job in response_json['data']:
                jobs.append(job)

        if job_id_filter:
            jobs = [job for job in jobs if job['id'] in job_id_filter]

        return jobs

    def _post_process_bulk_import_profile_jobs(self, jobs: List[str], url: str) -> bool:
        """
        Post-processing for bulk import profile jobs.
        Args:
            jobs: list of jobs to post process
            url: klaviyo endpoint for jobs

        Returns:
            True if all jobs ran without any failure, false otherwise.
        """
        self.logger.info(f'Post processing started for [{jobs}]...')
        all_successful = True

        complete_jobs = self._get_complete_jobs(url=url, job_id_filter=jobs)
        incomplete_jobs = self._get_incomplete_jobs(url=url, job_id_filter=jobs)
        ...
        for job in all_jobs:
            ...
            completed_count = attributes['completed_count']
            failed_count = attributes['failed_count']

            if failed_count > 0 or (total_count != completed_count):
                all_successful = False
                self.logger.warning(
                    f'Job {job_id} did not complete successfully, expected [{total_count}] '
                    f'profiles updated but got [{completed_count}]. '
                    f'[{failed_count}] profile updates failed.'
                    f'\nFor more info, see: '
                    f'{url}/{job_id}/import-errors/')
                self.logger.warning(f'Response: {job}')
