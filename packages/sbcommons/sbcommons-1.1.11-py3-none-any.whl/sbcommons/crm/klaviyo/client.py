...
class KlaviyoClient(CrmClient):
    ...
    def _start_bulk_import_profile_job(self, format_method: Callable[[Any], dict],
        ...
        # max retries - setting this to 60,
        #  so we have 60*10 = 600 seconds (10 minutes) before
        ...
        retries = 0
        while jobs and retries < max_retries:
            retries = retries + 1
            time.sleep(10)
            jobs = [job for job in jobs if not self._check_if_finished(url=url, job_id=job)]
            self.logger.info(f'Waiting on [{len(jobs)}] jobs...: {jobs}')
    ...
    def _check_if_finished(self, url: str, job_id: str) -> bool:
        """
        Check is bulk import job has finished.
        All allowed statuses for a bulk import jobs are:
        ["cancelled", "complete", "processing", "queued"]
        This method checks if a job is in a finished state, i.e.
        ...
        Returns:
            True if job_id is in a finished state, false otherwise
        """
        # query endpoint and just check for job status
        endpoint = url + job_id
        ...
        return status == 'complete' or status == 'cancelled'
