...
class grove():
    ...
    def calculateGrove(self):
        ...
        for nt in self.ntrees:
            # Generate predictions for the current grove with `nt` trees
            # potentiell predictions schliefe ersetzen weil brauche ja nur die erste
            # print(f"prdeictions_gen für {nt}: {predictions_gen}")
            for i, prediction in enumerate(predictions_gen, start=1):
                if i == nt:
                    predictions = prediction  # Speichere die Vorhersage für den Zustand bis `nt` Bäume
                    break
            ...
            for tid in range(nt):
                ...
                for node_id in range(tree.node_count):
                    if tree.children_left[node_id] != tree.children_right[node_id]:  # Split node
                        ...
                        rules.append(rule)
                # nur den besten/letzten Baum speichern vong Regeln her brudi
