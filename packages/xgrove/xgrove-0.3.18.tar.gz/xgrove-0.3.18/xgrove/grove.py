...
class grove():
    ...
    def getGBM(self):
        grove = GradientBoostingRegressor(max_depth=1, n_estimators=max(self.ntrees), learning_rate=self.shrink, subsample=self.b_frac, random_state=self.seed)
        grove.fit(self.data, self.surrTar)
        if grove.fit(self.data, self.surrTar) == grove.fit(self.data, self.model.fit(self.tar)):print("HALLELUJA ITS THE SAME")
        return grove
