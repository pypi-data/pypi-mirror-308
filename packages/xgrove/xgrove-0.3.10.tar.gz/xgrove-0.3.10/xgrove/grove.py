...
class grove():
    ...
    def getGBM(self):
        grove = GradientBoostingRegressor(n_estimators=max(self.ntrees), learning_rate=self.shrink, subsample=self.b_frac, random_state=self.seed)
        grove.fit(self.data, self.surrTar)
        return grove
    ...
    def calculateGrove(self):
        ...
        for nt in self.ntrees:
            ...
            # Debugging output before grouping
            print(f"DataFrame before grouping: {df.head()}")
            ...
            # Combine intercept and current interpretation, then aggregate
            df_small = df.groupby(["variable", "upper_bound_left", "levels_left"], as_index=False).agg({
                "pleft": "sum",
                "pright": "sum"
            })

            # Debugging output after grouping
            print(f"df_small grouped: {df_small.head()}")

            # Check for NaN values in the grouped DataFrame
            print(f"NaN counts in df_small: {df_small.isnull().sum()}")

            # Concatenate intercept and grouped df_small
            df = pd.concat([intercept_df, df], ignore_index=True)
            df_small = pd.concat([intercept_df, df_small], ignore_index=True)

            # Debugging output after concatenation
            print(f"df_small after concatenation: {df_small.head()}")
