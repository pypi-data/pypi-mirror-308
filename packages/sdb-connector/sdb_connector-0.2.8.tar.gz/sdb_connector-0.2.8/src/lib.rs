...
async fn select_measurement_data_async(
    ...
    let db = connect_to_db(ip, port, user, pw, namespace, db_name).await?;
    // let mut general_info_result = query_general_information(&db, "amv_tag_40", run_id).await?;
    // let general_info : Vec<UdpTag40> = match general_info_result.take(0) {
    //     Ok(data) => data,
    //     Err(e) => {
    //         println!("Error selecting measurement data: {:?}", e);
    //         return Err(Box::new(e));
...
pub async fn process_raw_data(result: Response) -> Result<Vec<(u64, u8, i32, String)>, Box<dyn Error>> {
    ...
    for tag in data {
            ...
            for data_value in tag.data {
                i += 1; 
                let new_timestamp = timestamp + Duration::milliseconds(i as i64 * 250);
                exploded_data.push((run_counter, channel_value, data_value, new_timestamp.clone().to_string()));
            }
        }
