...
class MRSClient:
    ...
    def request(self, method: str, endpoint: str, data=None, json=None, headers=None, enable_validation=True):
        return self._run_async(
            self.async_client.request(method, endpoint, data, json, headers, enable_validation)
        )
    ...
    def es_request(self, es_host: str, es_port: int, memoza_namespace: str, memoza_class: str, es_index: str, es_query: Callable[[str, bool, list[str]], Dict[str, Any]], enable_validation: bool = True):
        return self._run_async(
            self.async_client.es_request(es_host, es_port, memoza_namespace, memoza_class, es_index, es_query, enable_validation)
        )
    ...
    def upload_file(self, domain: dict, destinationPath: str, data: bytes):
        return self._run_async(self.async_client.upload_file(domain, destinationPath, data))
