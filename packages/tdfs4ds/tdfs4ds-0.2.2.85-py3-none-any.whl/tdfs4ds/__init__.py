...
FEATURE_PARTITION_N     = 2000
FEATURE_PARTITION_EACH  = 10
...
def augment_source_with_features(source_schema, source_name, entity_id, feature_selection, output_view_schema,
    ...
    for feature_store_view, list_features in locations.items():
        ...
        for feature in list_features:
            ...
            query_join.append(
                f"""
                {join_type} (CURRENT VALIDTIME SELECT A.*, A.FEATURE_VALUE AS {feature[2]} FROM {feature_store_view} A WHERE FEATURE_ID = {feature[0]} AND FEATURE_VERSION = '{feature[1]}') {table_name}
                ON {on_clause}
                """
            )
    ...
    return tdml.DataFrame(tdml.in_schema(output_view_schema, output_view_name))
