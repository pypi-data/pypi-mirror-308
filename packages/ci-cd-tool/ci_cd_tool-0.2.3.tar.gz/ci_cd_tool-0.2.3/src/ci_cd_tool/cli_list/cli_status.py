...
from ci_cd_tool.config.config_manager import load_config
import os
import yaml
...
# 설정 파일 로드 함수
# def load_config():
#     print(os.path.join(os.path.dirname(__file__), '..', 'config', 'config.yml'))
#     config_path = "/Users/jeondonghwan/Desktop/CCProject/src/ci_cd_tool/config/config.yml"
...
def cli_status(env, details, limit):
    ...
    try:
        ...
        console.print(Panel(f"[red]설정 파일에서 누락된 키가 있습니다: {e}[/red]", title="설정 오류", border_style="red"))
    except requests.exceptions.RequestException as e:
        console.print(Panel(f"[red]파이프라인 상태 확인 중 오류가 발생했습니다: {str(e)}[/red]", title="오류", border_style="red"))
    except Exception as e:
        console.print(Panel(f"[red]예상치 못한 오류가 발생했습니다: {str(e)}[/red]", title="오류", border_style="red"))
