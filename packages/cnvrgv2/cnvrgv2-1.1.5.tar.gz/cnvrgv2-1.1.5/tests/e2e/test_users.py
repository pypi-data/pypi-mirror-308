...
class TestUsers:
    ...
    def test_user_login(self, class_context, domain, e2e_env):
        ...
        token, org, _ = uc.login(user=user_info["email"], password=user_info["password"])
    ...
    def test_user_login_using_env_vars(self, class_context, domain, e2e_env):
        uc = UsersClient(domain=domain)
        rand_name = class_context.generate_name(5)
        user_info = {
            "username": rand_name,
    ...
    def test_user_login_using_bad_env_vars(self, class_context, domain, e2e_env):
        uc = UsersClient(domain=domain)
        rand_name = class_context.generate_name(5)
        user_info = {
            "username": rand_name,
        ...
        with pytest.raises(ConnectionError) as exception_info:
            c = Cnvrg()
        assert "Max retries" in str(exception_info.value)
