...
class ProX(cns.sqlX, cns.SQLstrX):
    ...
    tmpl['sum'] = {'模板': "{连接2}{名字}(`{值}`) as '{值}'", '连接':',','前缀':'IFNULL(', '后缀':',0)'} # ok
    tmpl['isum'] = {'模板': "{连接2}{前缀}SUM(`{值}`){后缀} as '{值}'", '连接':',','前缀':'IFNULL(', '后缀':',0)'}
    ...
    tmpl['内连接'] = {'头部':'INNER JOIN `{表名}` ON', '模板':"{连接2} t0.`{值}`= t{计数}.`{值}`",  '连接':' AND '}
    tmpl['左连接'] = {'头部':'LEFT JOIN  `{表名}` ON', '模板':"{连接2} t0.`{值}`= t{计数}.`{值}`",  '连接':' AND '}
    tmpl['右连接'] = {'头部':'RIGHT JOIN `{表名}` ON', '模板':"{连接2} t0.`{值}`= t{计数}.`{值}`",  '连接':' AND '}
    tmpl['全连接'] = {'头部':'RFULL OUTER JOIN `{表名}` ON', '模板':"{连接2} t0.`{值}`= t{计数}.`{值}`",  '连接':' AND '} 
