...
class CompletionApiModel(ApiModel):
    ...
    def report_streaming_metrics(self, reporting_values, model_name, token_usage):
        try:
            # adds cost metric
            reporting_values.update(openai_utils.get_reporting_values_streaming_type(
...
class AsyncCreateInstrumentor(OpenAiMethodInstrumentor):
    ...
    async def _moderation_acreate(self, acreate, *args, **kwargs):
        ...
        try:
            ...
            input_query = openai_utils.get_moderation_input_query(args, kwargs)
            moderation_response_parsed = openai_utils.parse_moderation_response(moderation_acreate_response,
                                                                                self.openai_major_version, self.agent)
            if exit_call_obj:
                if input_query:
                    exit_call_obj.optional_properties[OpenaiConstants.MODERATION_QUERY_KEY] = input_query
    ...
    async def _acreate(self, acreate, *args, **kwargs):
        ...
        exit_call_obj = None
        exit_call_operation = openai_utils.get_completion_opr_string(acreate.__module__.lower())
        try:
            ...
            if not is_streaming_response or OpenaiConstants.ERROR_METRIC_NAME in reporting_values:
                self.end_exit_call(exit_call=exit_call_obj)
                try:
                    self.completionApiModelInstance.report_metrics(
        ...
        try:
            async for chunk in response_generator:
                response_content, complete_response, usage_data = \
                    openai_utils.set_parsed_chunk_values(chunk, response_content,
            ...
            if exit_call_obj:
                exit_call_obj.optional_properties[OpenaiConstants.STREAMING_CONTENT_STRING] = \
                    str(response_content)
                self.end_exit_call(exit_call=exit_call_obj)
            token_usage = openai_utils.get_token_usage_data(usage_data, complete_response["model"], input_messages,
    ...
    async def _arequest(self, arequest, *args, **kwargs):
        ...
        openai_utils.add_multiple_exit_suppression_header(url_string, headers)
...
class SyncCreateInstrumentor(OpenAiMethodInstrumentor):
    ...
    def _moderation_create(self, create, *args, **kwargs):
        ...
        try:
            ...
            input_query = openai_utils.get_moderation_input_query(args, kwargs)
            moderation_response_parsed = openai_utils.parse_moderation_response(moderation_create_response,
                                                                                self.openai_major_version, self.agent)
            if exit_call_obj:
                if input_query:
                    exit_call_obj.optional_properties[OpenaiConstants.MODERATION_QUERY_KEY] = input_query
    ...
    def _create(self, create, *args, **kwargs):
        ...
        exit_call_obj = None
        is_streaming_response = kwargs.get("stream", False)
        exit_call_operation = openai_utils.get_completion_opr_string(create.__module__.lower())
        try:
            exit_call_obj = self.make_exit_call(path=exit_call_operation)
