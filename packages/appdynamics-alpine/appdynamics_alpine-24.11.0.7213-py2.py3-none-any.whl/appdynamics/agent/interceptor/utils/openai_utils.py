...
class OpenaiConstants():
    ...
    STREAMING_CONTENT_STRING = "Streaming content"
...
MODEL_COST_MAP = {
    ...
    # embeddings is the cheapest one now
    "text-embedding-ada-002": {'prompt_cost': 1, 'completion_cost': 0},
    "text-embedding-3-large": {'prompt_cost': 1, 'completion_cost': 0},
    "text-embedding-3-small": {'prompt_cost': 1, 'completion_cost': 0},
...
def get_reporting_values_per_request(model_name=None, agent=None, endpoint_response=None):
    ...
    try:
        ...
        # prompt and total_tokens found but no completions, defaulting to 0
        completion_tokens = 0

    # Calculating current request cost per model
    try:
        if not(prompt_tokens is None or completion_tokens is None):
...
def get_reporting_values_streaming_type(model_name=None, token_usage=None, agent=None):
    ...
    try:
        if token_usage[OpenaiConstants.PROMPT_TOKENS_STRING] and token_usage[OpenaiConstants.COMPLETION_TOKENS_STRING]:
            reporting_values[OpenaiConstants.TOKENS_METRIC_NAME] = token_usage[OpenaiConstants.PROMPT_TOKENS_STRING] \
                + token_usage[OpenaiConstants.COMPLETION_TOKENS_STRING]
            reporting_values[OpenaiConstants.COST_METRIC_NAME] = get_cost_per_request(
                model_name=model_name,
                prompt_tokens=token_usage[OpenaiConstants.PROMPT_TOKENS_STRING],
                completion_tokens=token_usage[OpenaiConstants.COMPLETION_TOKENS_STRING],
            )
    except UnsupportedModelException as exec:
        agent.logger.error(str(exec))
    finally:
