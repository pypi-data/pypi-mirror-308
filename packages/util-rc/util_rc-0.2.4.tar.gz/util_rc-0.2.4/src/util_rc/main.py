...
class util_rc:
    ...
    def __init__(self, modeltype, choice, amt1, prob1, amt2, prob2):
        ...
        if all(self.choice) or not any(self.choice):
            ...
            self.params = self.flagged()
        else:
            self.params = self.min_func()
            self.predict()

        self.params = self.min_func()
        self.predict()
            # print(f""
            # fitted parameter: {self.params[0]}
            # inverse temperature: {self.params[1]}
            # fit metrics: {self.params[2]}
            # model type: {self.params[3]}
            # num of observations: {self.params[4]}
            # """)
    ...
    def predict(self):
        """
        uses the fitted parameter to predict choices to check if fitted model will predict one-sided chocies
        """
        if self.modeltype == "E":
            util = self.eut(self.params[0])
        elif self.modeltype == "R":
            util = self.risk_return(self.params[0])
        elif self.modeltype == "W":
            util = self.co_of_var(self.params[0])
        else:
            util = self.prob_discount(self.params[0])
    ...
    def eut(self, a):
        ...
        :return: the expected utility theory utility calculated using a and the amounts and probabilities arrays for option1 and optoin2
        """
    ...
    def risk_return(self, b):
        ...
        :return: the risk return utility calculated with b and the amounts and probabilities arrays for option1 and optoin2
        """
    ...
    def co_of_var(self, b):
        ...
        :return: the Weber utility calculated with b and the amounts and probabilities arrays for option1 and optoin2
        """
        option1 = self.ev_var(self.amt1, self.prob1)
        option2 = self.ev_var(self.amt2, self.prob2)
    ...
    def prob_discount(self, h):
        ...
        :return: the Hyperbolic utility calculated with h and the amounts and probabilities arrays for option1 and optoin2
        """
