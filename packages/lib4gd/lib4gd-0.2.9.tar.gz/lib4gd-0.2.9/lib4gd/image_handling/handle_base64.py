...
class ImageHandler:
    ...
    def base64_to_pil(self, base64_string: str) -> Image.Image:
        ...
        :raises ValueError: If the base64 string or image decoding fails.
        """
        try:
            decoded_data = base64.b64decode(base64_string)
            return Image.open(BytesIO(decoded_data))
        except (base64.binascii.Error, UnidentifiedImageError) as e:
            raise ValueError(f"Invalid base64 string or image error: {str(e)}")
    ...
    def pil_to_base64(self, pil_image: Image.Image) -> str:
        ...
        try:
            ...
            raise ValueError(f"Failed to convert PIL image to base64: {str(e)}")
    ...
    def base64_to_cv2(self, base64_string: str) -> np.ndarray:
        ...
        :raises ValueError: If the base64 string or OpenCV decoding fails.
        """
        try:
            decoded_data = base64.b64decode(base64_string)
            ...
            raise ValueError(f"Invalid base64 string or OpenCV image error: {str(e)}")
        except Exception as e:
            raise RuntimeError(f"An unexpected error occurred: {str(e)}")
    ...
    def cv2_to_base64(self, cv2_image: np.ndarray) -> str:
        ...
        try:
            ...
            raise ValueError(f"Failed to convert OpenCV image to base64: {str(e)}")
    ...
    async def _process_batch_async(self, inputs: list, conversion_type: str) -> list:
        ...
        tasks = [asyncio.to_thread(conversion_func, input_item) for input_item in inputs]
    ...
    def _select_conversion_function(self, conversion_type: str):
        ...
        """
        conversion_map = {
            "base64_to_cv2": self.base64_to_cv2,
            "cv2_to_base64": self.cv2_to_base64,
            "base64_to_pil": self.base64_to_pil,
            "pil_to_base64": self.pil_to_base64,
        }
        if conversion_type not in conversion_map:
            raise ValueError(f"Unsupported conversion type: {conversion_type}")
        return conversion_map[conversion_type]
    ...
    def process_batch(self, inputs: list, conversion_type: str) -> list:
        ...
        :return: A list of processed results.
        """
        return asyncio.run(self._process_batch_async(inputs, conversion_type))
