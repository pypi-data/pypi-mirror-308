...
class GoogleDriveHandler:
    ...
    async def _download_file_async(self, file_id: str, file_name: str, destination_folder: str):
        async with ResourceManager.get_semaphore():
            ...
            await loop.run_in_executor(ResourceManager.get_thread_pool(), self.download_file, file_id, file_name, destination_folder)
    ...
    async def process_file_in_memory(self, file_id: str):
        ...
        while not done:
            _, done = await asyncio.get_event_loop().run_in_executor(ResourceManager.get_thread_pool(), downloader.next_chunk)
        fh.seek(0)
        image = Image.open(fh)
        return image
    
    async def process_files_in_memory_in_batches(self, file_list, batch_size=100):
        """Process files in memory in smaller batches, gathering results for each batch."""
        all_results = []
    ...
    async def _upload_file_async(self, local_file_path: str, folder_id: str):
        async with ResourceManager.get_semaphore():
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(ResourceManager.get_thread_pool(), self.upload_file, local_file_path, folder_id)
