...
class StigMappingEngine:
    ...
    def map_associated_stigs_to_asset(self, asset: Asset, ssp_id: int) -> List[AssetMapping]:
        ...
        if not hasattr(self, "_mapping_cache"):
            existing_mappings = AssetMapping.get_all_by_parent(
                parent_module=Asset.get_module_slug(), parent_id=asset.id
            )
            self._mapping_cache = {comp.title: comp for comp in components}
            existing_component_ids = {m.componentId for m in existing_mappings}
        else:
            existing_mappings = self._mapping_cache.values()
            existing_component_ids = {m.componentId for m in existing_mappings}
