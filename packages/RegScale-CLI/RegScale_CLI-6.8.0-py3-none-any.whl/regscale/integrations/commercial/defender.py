...
def fetch_resources_from_azure(api: Api, headers: dict, skip_token: Optional[str] = None) -> list[dict]:
    """
    ...
    if skip_token:
        url += f"$skipToken={skip_token}" if skip_token else ""
    ...
    try:
        ...
        api.logger.info(f"Received {count}/{total_records} items from Microsoft Defender.")
        # try to get the values from the api response
    ...
    if response.status_code == 200 and skip_token:
        # get the rest of the data
        defender_data.extend(fetch_resources_from_azure(api=api, headers=headers, skip_token=skip_token))
    # return the defender recommendations
    return defender_data
...
def map_asset(data: dict, existing_assets: dict[str, Asset]) -> Asset:
    ...
    return Asset(
        ...
        fqdn=fqdn_mapping.get(resource_type, properties.get("dnsSettings", {}).get("fqdn")),
        ipAddress=ip_mapping.get(resource_type, properties.get("ipAddress")),
