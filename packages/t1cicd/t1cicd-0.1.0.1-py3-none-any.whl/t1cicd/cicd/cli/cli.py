...
def report(ctx, repo, local, pipeline, run, stage, job):
    ...
    print(f"Showing reports in repo: {repo}")
...
@click.pass_context
def run(ctx, repo, local, branch, commit, pipeline, file, override):
    ...
    cicd_cmds = CicdCommands(ctx.obj["client"])
    cicd_cmds.run_pipeline(repo, local, branch, commit, pipeline, file)
    print(f"Running all pipelines in repo: {repo}")
