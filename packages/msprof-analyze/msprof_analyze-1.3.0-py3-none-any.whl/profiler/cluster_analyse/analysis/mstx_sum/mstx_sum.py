...
from cluster_statistics_export.mstx_event_export import MstxMarkExport, MstxRangeExport
...
def handle_mark_data(mark_df: pd.DataFrame, data_map: list) -> list:
    res = []
...
class MstxSum(BaseRecipeAnalysis):
    ...
    def _mapper_func(data_map, analysis_class):
        ...
        range_df = MstxRangeExport(data_map[1], analysis_class).read_export_db()

        mstx_res = []
        if not mark_df.empty:
            mstx_res += handle_mark_data(mark_df, data_map)
        if not range_df.empty:
            mstx_res += handle_range_data(range_df)
        if not mstx_res:
            print(f"[WARNING] There is no mstx data in {data_map[1]}.")
            return None

        mstx_stats_df = pd.DataFrame(mstx_res).assign(Rank=data_map[0])
