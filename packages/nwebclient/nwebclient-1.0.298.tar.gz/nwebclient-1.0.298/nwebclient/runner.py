...
class BaseJobExecutor(base.Base):
    ...
    def page_events(self, params={}):
        ...
        for event in self.event_defs:
            p.div(event['type'])
        p.prop("Event-Listener-Count", len(self.event_listener))
        p.hr()
        ...
        p.input('event_type', value='')
        p.input('new_type', value='')
    ...
    def execute(self, data):
        if 'setvar' in data and data['setvar'] in self.var_names:
            ...
            return {'success': False, 'message': 'Unknown Operation (BaseJobExecutor)', 'request_keys': list(data.keys())}
    ...
    def action_btn_parametric(self, title, data):
        return web.button_js(title, 'exec_job_p('+json.dumps(data)+');')
...
class JobRunner(base.Base):
    ...
    def page_registry_show(self, params={}):
        ...
        try:
            ...
            if 'pip' in params:
                p(self.registry_pip_install(params, getattr(runner, 'MODULES', [])))
...
class SerialExecutor(ThreadedQueueExecutor):
    ...
    def _sendData(self):
        if self.send is not None:
            ...
            time.sleep(0.1)  # besser serial flush
            if self.rs485:
...
class NamedJobs(BaseJobExecutor):
    ...
    def __init__(self, args:util.Args={}, name_key='named_jobs'):
        self.args = args
