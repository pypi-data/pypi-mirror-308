...
{GMAIL_WARNING}"""
    ...
    def _forward(self, _):
        ...
        emails = sorted(emails, key=lambda x: int(x["internalDate"]))
...
class SheetsWrite(Google):
    ...
    def _write_values(self, data: List[Dict[str, str]]):
        ...
        if current_values and set(current_values[0]) == set(headers):
            existing_header = current_values[0]
            rows = self._reorder_rows_to_write(existing_header, data)
            logger.debug("Headers already present, skipping header row")
            body = {"values": rows}
        else:
            existing_headers = list(data[0].keys())
            rows = [existing_headers] + self._reorder_rows_to_write(
                existing_headers, data
            )
