...
class _events_utils:
    ...
    def increment_active_dataset(self, viewer=None, key=None):
        try:
            if self.dataset_dict != {}:
                ...
                self.gui.dataset_selector.setCurrentIndex(new_dataset_index)
    ...
    def update_active_image(self, channel=None, dataset=None, event=None, frame_index=None):
        try:
            if self.verbose:
            ...
            if dataset_name in self.dataset_dict.keys():
                ...
                if "data" in self.dataset_dict[dataset_name][channel].keys():
                    ...
                    pixel_size = float(self.dataset_dict[dataset_name][channel]["pixel_size"])
                    ...
                    if hasattr(self, "image_layer") == False:
                        self.image_layer = self.viewer.add_image(image, name=layer_name, colormap="gray", blending="additive", visible=True, scale=scale, )
                    ...
                    if type(frame_index) == int:
                        ...
                        active_frame_index = current_step[0]

                        if frame_index != active_frame_index:

                            current_step[0] = frame_index
                            self.viewer.dims.current_step = tuple(current_step)
                            self.image_layer.time_index = frame_index
    ...
    def update_channel_select_buttons(self):
        try:
            ...
            if datast_name in self.dataset_dict.keys():
                ...
                self.gui.show_dd.clicked.connect(lambda: None)
                self.gui.show_da.clicked.connect(lambda: None)
                self.gui.show_aa.clicked.connect(lambda: None)
                self.gui.show_ad.clicked.connect(lambda: None)

                if fret_mode == True:
                    self.gui.show_dd.setVisible(True)
                    self.gui.show_da.setVisible(True)
                    self.gui.show_aa.setVisible(False)
                    self.gui.show_ad.setVisible(False)

                    if "dd" in channel_refs:
                        self.gui.show_dd.setEnabled(True)
                        self.gui.show_dd.setText("Donor [F1]")
                        self.viewer.bind_key("F1", func=self.select_channel_donor, overwrite=True, )
                        self.gui.show_dd.clicked.connect(partial(self.update_active_image, channel="donor"))
                    else:
                        self.gui.show_dd.setEnabled(False)
                        self.gui.show_dd.setText("")

                    if "da" in channel_refs:
                        self.gui.show_da.setEnabled(True)
                        self.gui.show_da.setText("Acceptor [F2]")
                        self.viewer.bind_key("F2", func=self.select_channel_acceptor, overwrite=True, )
                        self.gui.show_da.clicked.connect(partial(self.update_active_image, channel="acceptor"))
                    else:
                        self.gui.show_da.setEnabled(False)
                        self.gui.show_da.setText("")

                else:
                    self.gui.show_dd.setVisible(True)
                    self.gui.show_da.setVisible(True)
                    self.gui.show_aa.setVisible(True)
                    self.gui.show_ad.setVisible(True)

                    if "dd" in channel_refs:
                        self.gui.show_dd.setText("DD [F1]")
                        self.gui.show_dd.setEnabled(True)
                        self.viewer.bind_key("F1", func=self.select_channel_dd, overwrite=True, )
                        self.gui.show_dd.clicked.connect(partial(self.update_active_image, channel="dd"))
                    ...
                    if "da" in channel_refs:
                        self.gui.show_da.setText("DA [F2]")
                        self.gui.show_da.setEnabled(True)
                        self.viewer.bind_key("F2", func=self.select_channel_da, overwrite=True, )
                        self.gui.show_da.clicked.connect(partial(self.update_active_image, channel="da"))
                    else:
                        self.gui.show_da.setText("")
                        self.gui.show_da.setEnabled(False)

                    if "ad" in channel_refs:
                        self.gui.show_ad.setText("AD [F3]")
                        self.gui.show_ad.setEnabled(True)
                        self.viewer.bind_key("F3", func=self.select_channel_ad, overwrite=True, )
                        self.gui.show_ad.clicked.connect(partial(self.update_active_image, channel="ad"))
                    else:
                        self.gui.show_ad.setText("")
                        self.gui.show_ad.setEnabled(False)

                    if "aa" in channel_refs:
                        self.gui.show_aa.setText("AA [F4]")
                        self.gui.show_aa.setEnabled(True)
                        self.viewer.bind_key("F4", func=self.select_channel_aa, overwrite=True, )
                        self.gui.show_aa.clicked.connect(partial(self.update_active_image, channel="aa"))
                    else:
                        self.gui.show_aa.setText("")
                        self.gui.show_aa.setEnabled(False)
    ...
    def delete_dataset(self):
        try:
            ...
            if dataset_name in self.dataset_dict.keys():
                ...
                self.localisation_dict["molecules"].pop(dataset_name)
        ...
        try:
            ...
            if old_name != "":
                if new_name == "":
                    ...
                    localisation_data = self.localisation_dict["molecules"].pop(old_name)
                    self.localisation_dict["molecules"][new_name] = localisation_data
