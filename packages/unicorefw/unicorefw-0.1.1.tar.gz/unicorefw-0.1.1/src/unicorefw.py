...
class unicore:
    ...
    def __init__(self, collection):
        self.wrapper = UnicoreWrapper(collection)

    def __getattr__(self, item):
        # Delegate attribute access to the wrapped instance
        if hasattr(self.wrapper, item):
            return getattr(self.wrapper, item)
        elif hasattr(unicore, item):
            return getattr(unicore, item)
        raise AttributeError(f"'Unicore' object has no attribute '{item}'")

    def __call__(self, collection):
        # Return a new UnicoreWrapper instance when called
        return UnicoreWrapper(collection)
    ...
    def isSymbol(obj):
        ...
        """
        return isinstance(obj, (types.ModuleType, types.BuiltinFunctionType, types.FunctionType, type))
    ...
    def functions(obj):
        ...
        return [key for key, value in obj.items() if callable(value) and not key.startswith('__')]
...
## UnicoreWrapper
################################################################################################################
class UnicoreWrapper:
    def __init__(self, collection):
        """
        Initializes a new instance of the UnicoreWrapper class with the provided collection.

        Parameters:
            collection (iterable): The collection of items to be wrapped and manipulated 
            using the unicore functions.
        """
        self.collection = collection

    def _apply_unicore_function(self, function_name, *args, **kwargs):
        func = getattr(unicore, function_name, None)
        if callable(func):
            result = func(self.collection, *args, **kwargs)
        elif hasattr(self.collection, function_name) and callable(getattr(self.collection, function_name)):
            method = getattr(self.collection, function_name)
            result = method(*args, **kwargs)
        else:
            raise AttributeError(f"'unicore' and Python do not have a callable function named '{function_name}'")

        if isinstance(result, (list, tuple, set)):
            return UnicoreWrapper(result)
        return result
    ...
    def filter(self, predicate):
        ...
        Returns:
            UnicoreWrapper: A new UnicoreWrapper instance with the filtered elements.
        """
    ...
    def reduce(self, func, initial=None):
        ...
        """
        return self._apply_unicore_function('reduce', func, initial) if initial is not None else self._apply_unicore_function('reduce', func)

    def sortBy(self, key_func):
        return self._apply_unicore_function('sortBy', key_func)

    def groupBy(self, key_func):
        return self._apply_unicore_function('groupBy', key_func)

    def indexBy(self, key_func):
        return self._apply_unicore_function('indexBy', key_func)

    def countBy(self, key_func):
        return self._apply_unicore_function('countBy', key_func)
    ...
    def invoke(self, method_name, *args, **kwargs):
        ...
        for item in self.collection:
            ...
            if callable(method):
                ...
                raise AttributeError(f"Item '{item}' does not have a callable method '{method_name}'")
        return UnicoreWrapper(new_collection)
    ...
    def isString(self):
        return self._apply_unicore_function('isString')

    def isArray(self):
        return self._apply_unicore_function('isArray')

    def isObject(self):
        return self._apply_unicore_function('isObject')

    def isEqual(self, other):
        return self._apply_unicore_function('isEqual', other)
    ...
    def allKeys(self):
        return self._apply_unicore_function('allKeys')
    ...
    def uniqueId(self, prefix=''):
        return self._apply_unicore_function('uniqueId', prefix)
    ...
    def propertyOf(self, obj):
        return self._apply_unicore_function('propertyOf', obj)

    def isMatch(self, obj, attrs):
        return self._apply_unicore_function('isMatch', obj, attrs)

    def isUndefined(self, obj):
        return self._apply_unicore_function('isUndefined', obj)

    def isNull(self, obj):
        return self._apply_unicore_function('isNull', obj)

    def isBoolean(self, obj):
        return self._apply_unicore_function('isBoolean', obj)
