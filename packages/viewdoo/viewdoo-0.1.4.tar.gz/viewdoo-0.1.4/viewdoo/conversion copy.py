    ...
    if not dicom_series_uids:
        ...
        for series_uid in dicom_series_uids:
            ...
            if len(dicom_names) > 0:
                ...
                print('here', len(dicom_names), dicom_names)
...
def process_dicom(file_path):
    try:
        ...
        if hasattr(dicom_data, 'SOPInstanceUID'):
            return dicom_data.SOPInstanceUID, file_path
    except:
        return None

def get_sop_instance_uids(path, max_workers=None):
    sop_instance_uids = {}
    file_paths = []
    ...
    # If max_workers is not provided, set it to the number of available CPU cores minus 1
    if max_workers is None:
        max_workers = max(1, multiprocessing.cpu_count() - 1)
    
    # Use ProcessPoolExecutor to process files in parallel
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        future_to_file = {executor.submit(process_dicom, file_path): file_path for file_path in file_paths}
        
        for future in as_completed(future_to_file):
            result = future.result()
            if result:
                sop_instance_uids[result[0]] = result[1]
...
class ImageConverter:
    def __init__(self, path_in, path_out, format_out=None, perform_resample=False, resample_shape=None, resample_voxels=None, resample_order=1, perform_crop=False, crop_entry=None, apply_affine=False):
        ...
        # If contains both an image and mask in list
