...
def db_create(db):
    ...
    Q2 = '''
        CREATE TABLE IF NOT EXISTS api_tasks (
            ...
            kind CHAR(32),
            status CHAR(32),
            inputs TEXT, -- json stringyfied list of dicts with url and file name
            result_url TEXT,
            eta DATETIME,
...
class APIServer:
    ...
    async def req(self, request):
        """Request for async proc. Expects param inputs with list dicts with url and file name."""
        key = self.check_access(request)
        data = await request.json()
        inputs = get_param(request, 'inputs', list, data=data, LOG=self.LOG)
        req_uid = get_uid()
        task = {'req_uid':req_uid, 'created_at':now(), 'key':key, 'kind':'ASYNC', 'status':'REQUESTED',
                'inputs': json_dumps(inputs)}
    ...
    async def check(self, request):
        """Check async proc. response has task status and if DONE the results url."""
