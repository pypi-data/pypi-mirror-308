...
class GS:
    ...
    def upload_mem(self, bucket_name, blob_name, data, content_type="application/octet-stream", use_pickle=False):
        """
        ...
        """
        if not use_pickle and not isinstance(data, str) and isinstance(data, bytes):
            raise TypeError('Data must be bytes or string if use_pickle is False.')
        try:
            if use_pickle:
    ...
    def download_mem(self, bucket_name, blob_name, as_string=False, encoding="utf-8", use_pickle=False):
        """
    ...
    def upload_file(self, bucket_name, blob_name, local_file_path, content_type="application/octet-stream"):
        """
        ...
        try:
            ...
            print(f"Uploaded {local_file_path} to {bucket_name}/{blob_name}")
            return 0
        except Exception as exc:
    ...
    def download_file(self, bucket_name, blob_name, local_file_path):
        ...
        try:
            ...
            print(f"Downloaded {bucket_name}/{blob_name} to {local_file_path}")
            return 0
        except Exception as exc:
        ...
        try:
            for root,dirs,files in os.walk(local_folder_path):
                for file in files:
                    ...
                    relative_path = relative_path.replace(os.sep, "/")  # Ensure UNIX-style path separators
                    # Construct remote file path
                    remote_file_path = os.path.join(blob_name, relative_path).replace(os.sep, "/")
                    # Upload the file
                    content_type, _ = mimetypes.guess_type(local_file_path)
                    if content_type is None: content_type = "application/octet-stream"
                    not_success = self.upload_file(bucket_name, remote_file_path, local_file_path, content_type=content_type)
                    if not_success: print(f"Failed to upload {local_file_path}")
            print(f"Uploaded folder '{local_folder_path}' to '{bucket_name}/{blob_name}'")
        except Exception as exc:
    ...
    def download_folder(self, bucket_name, blob_name, local_folder_path):
        ...
        try:
            ...
            print(f"Downloaded folder '{bucket_name}/{blob_name}' to '{local_folder_path}'")
        except Exception as exc:
