...
class BaseFetchHandler(CategoryHandler, abc.ABC):
    ...
    def _load_entry(self):
        ...
        context_point = self.fs.context_point or self.fs.condition_point
        self._context_entry = load_class(context_point) if context_point else None
    ...
    def _do_loop_fetch(self, fetch_type, context):
        index = 0
        while True:
            status = self.fetch(fetch_type, index, context)
    ...
    def _build_fetch_direct(self, fetch_type, page_index, context):
        response = self._call(
            self._fetch_entry, fetch_type=fetch_type, page_index=page_index, page_size=self._size, context=context
        )
    ...
    def fetch(self, fetch_type, index, context):
        return self._build_fetch_direct(fetch_type, index, context)

    def _do_count_fetch(self, fetch_type, context):
        count_response = self._call(self._count_entry, error_back=0, fetch_type=fetch_type, context=context)
        if not count_response.flag:
            raise ValueError(count_response.message)
        times = int(math.ceil(count_response.data / self._size))

        for index in range(times):
            self.fetch(fetch_type, index, context)
    ...
    def fetch_by_fetch_type(self, fetch_type):
        if self._context_entry:
            context = self._call(self._context_entry, error_back={}, fetch_type=fetch_type).data
        else:
            context = {}
        if self._count_entry:
            return self._do_count_fetch(fetch_type, context)
        return self._do_loop_fetch(fetch_type, context)
