...
from cracknuts_panel.ui_sync import ConfigProxy, observe_interceptor
...
class CrackerPanelWidget(MsgHandlerPanelWidget):
    ...
    osc_trigger_source = traitlets.Int(1).tag(sync=True)
    ...
    def __init__(self, *args: Any, **kwargs: Any):
        ...
        self.load_current_path_config_to_cracker()
    ...
    def load_current_path_config_to_cracker(self):
        """
        Load the workspace configuration to cracker if it exists.
        """
        current_config_path = os.path.join(os.getcwd(), ".config")
    ...
    def msg_connection_button_on_click(self, args: dict[str, typing.Any]):
        if args.get("action") == "connect":
            ...
            self.connect_status = False
        self.send({"connectFinished": True})

    def dump_config_button_click(self, args: dict[str, typing.Any]):
        self.send(
            {
                "dumpConfigCompleted": json.dumps(
                    {
                        "connection_info": self.cracker.get_uri(),
                        "config_info": self.cracker.dump_config(),
                    }
                )
            }
        )
    ...
    def load_config_button_click(self, args: dict[str, typing.Any]):
        connection_info = args.get("connection_info")
        config_info = args.get("config_info")
        self.cracker.load_config_from_str(config_info)
        self.cracker.set_uri(connection_info)
        self.sync_config()
