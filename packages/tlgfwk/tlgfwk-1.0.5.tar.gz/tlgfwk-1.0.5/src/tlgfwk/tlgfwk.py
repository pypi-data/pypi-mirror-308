...
__version__ = """1.0.5 Hotfix for false command not implemented"""
...
{self.links_string.replace(',', os.linesep)}"""
    ...
    def __init__(self, 
        ...
        validate_token = True,
        quit_if_error = True,
        env_file: str = os.path.join(os.path.dirname(os.path.abspath(__file__)), '.env'), 
        bot_owner: str = None, 
        ...
        try: 
            ...
            self.env_file = os.path.join(script_path, env_file) if env_file else os.path.join(script_path, '.env')
            self.logger = logger 
