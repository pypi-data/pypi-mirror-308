...
class AffineWarp(object):
    ...
    def warp_image(self, input_image, output_pixels_per_lenslet, direction="R", cropToInside = False):
        ...
        # Create the output image
        output_image = cv2.CreateMat(outputsize[0], outputsize[1], im.type)
        ...
        if (direction == 'f' or direction == 'F'):
            ...
            # TODO: Check if coeff elements are integers if they should be.
            coeff = cv2.CreateMat(2,3,cv2.CV_32FC1)
...
class CubicWarp(object):
    ...
    def fit(self, data):
        ...
        # Possibly should be rcond=None instead of rcond=-1
        self.forwardCoefficients[0,:] = linalg.lstsq(A, bx, rcond=-1)[0]
        self.forwardCoefficients[1,:] = linalg.lstsq(A, by, rcond=-1)[0]
        self.reverseCoefficients[0,:] = linalg.lstsq(Ar, bxr, rcond=-1)[0]
        self.reverseCoefficients[1,:] = linalg.lstsq(Ar, byr, rcond=-1)[0]
    ...
    # Warp an image.
    # This function expects imput_image to be a numpy float32 array.  Returns numpy array.
    def warp_image(self, input_image, output_pixels_per_lenslet, direction="R",
                   cropToInside = False, lenslet_offset = None, output_size = None):

        ul = self.eval_point([0, 0], 'f')
        ur = self.eval_point([0, input_image.shape[1]], 'f')
        ll = self.eval_point([input_image.shape[0], 0], 'f')
        lr = self.eval_point([input_image.shape[0], input_image.shape[1]], 'f')
        ...
        # Create the output image
        output_image = np.zeros(putative_output_size, dtype=np.float32)
        ...
        output_image = cv2.remap(input_image, ix, iy, cv2.INTER_LINEAR)
        result = output_image
        # python3 opencv contains function INTER_LINEAR, but not WARP_FILL_OUTLIERS and WARP_INVERSE_MAP
        # python2 output_image is generated as shown below 
        # cv2.Remap(im, output_image, ix, iy, flags=cv2.cv2_INTER_LINEAR+cv2.CV_WARP_FILL_OUTLIERS+cv2.CV_WARP_INVERSE_MAP)
        # result = np.asarray(output_image)
