...
class LightFieldCalibration(object):
    ...
    def __init__(self, options, ulens_focal_length, ulens_focal_distance, ulens_pitch, pixel_size,
    ...
    def print_summary(self):
        ...
        if self.rayspread_db_generated:
            ...
            if self.rayspread_db:
                print('\trayspreads in the database: %d' % (len(list(self.rayspread_db.rayspreads.keys()))))
            if self.psf_db:
                print('\tlight field psfs in the database: %d' % (len(list(self.psf_db.psf_coordinates.keys()))))
        print('-------------------------------------------------------------')
    ...
    def load(cls, filename):
        ...
        # Optics
        options = calibration_file['/optics']
        ...
        instance = cls(options, ulens_focal_length, ulens_focal_distance, ulens_pitch, pixel_size,
                       objective_magnification, objective_na, medium_index,
                       tubelens_focal_length,
    ...
    def calibrate_geometry(self, calibration_image_file, skip_alignment = False,
                           skip_subpixel_alignment = False, chief_ray_image = False,
        ...
        if self.skip_alignment:
            ...
            try: 
                ...
                print("Please check your calibration parameters and try again.")
                raise
            
            if radiometry_file is not None and align_radiometry == True:
    ...
    def correct_radiometry(self, im):
        if self.radiometric_correction is not None:
    ...
    def calibrate_radiometry(self, calibration_image_file, radiometry_frame_file = None, dark_frame_file = None):
        ...
        self.ideal_lf = lfproj.project(vol_ones,use_sing_prec=self.options.use_sing_prec).asimage(representation = LightField.TILED_LENSLET)
        ...
        if aperture_avg.max() == 0:
            raise Exception("ERROR: radiometry calibration failed.  No non-zero pixels found in the calibrated radiometry image!")
    ...
    def generate_raydb(self, num_slices, um_per_slice, supersample_factor = 1, z_center = 0.0,
        ...
        # Note: use_geometric_optics if/else statement was removed, eliminating the option 
        # to use ray optics instead of wave optics. - gschlafly
