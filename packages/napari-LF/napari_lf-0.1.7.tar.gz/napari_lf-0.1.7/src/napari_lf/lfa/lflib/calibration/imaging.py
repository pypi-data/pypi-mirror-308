...
def refine_centers_subpixel(putative_centers, lf_tophat):
    ...
    pinvA = np.array([[ 1.0/6,  1.0/6,  1.0/6, -1.0/3, -1.0/3, -1.0/3,  1.0/6,  1.0/6,  1.0/6],
    ...
    for i in range(putative_centers.shape[0]):
        ...
        points = np.array([a0, a1, a2, a3, a4, a5, a6, a7, a8]).transpose()
        c = pinvA @ points
...
def geometric_calibration(calibration_image, pixel_size, lenslet_pitch, alignment_method,
    ...
    if chief_rays == True:
        ...
        lf_tophat = scipy.ndimage.convolve(calibration_image, kernel)
