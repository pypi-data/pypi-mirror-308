    ...
    return [{
        'wstl': {
            ...
            'hea': {'href': str(URL(href_) / str(obj['id'])),
                    ...
                    'attribute_permissions': [{k: v for k, v in (await desktop_object.get_all_attribute_permissions(context)).items()} for desktop_object in desktop_objects]
                    },
            'actions': actions,
...
async def expected_one(fixtures: Mapping[CollectionKey | str, List[DesktopObjectDict]],
    ...
    perms = await desktop_object_from_dict(obj).get_permissions(context)
    ...
    for action, action_name, rel, itemif in ((wstl_builder.find_action(a.name), a.name, a.rel, a.itemif) for a in get_actions_):
        ...
        if itemif is None or get_eval_for(obj).eval(itemif):
            _set_collection_template(action, collection, obj, 1, rel, context.sub)
    return [collection]
...
async def expected_all(fixtures: Mapping[CollectionKey | str, List[DesktopObjectDict]],
    ...
    for action, action_name, rel, itemif in ((wstl_builder.find_action(a.name), a.name, a.rel, a.itemif) for a in get_all_actions_):
        ...
        if len(set(i['type'] for i in collection)) < 2 and (itemif is None or get_eval_for(f).eval(itemif)):
            _set_collection_template(action, collection_doc, f, len(collection), rel, context.sub)
    top_level_links_ = top_level_links()
    if top_level_links_:
...
def _template_data_generator(sub: str, action: dict[str, Any], desktop_object_dict: DesktopObjectDict, len_fixtures=1):
    ...
    for input in action.get('inputs', []):
        ...
        if 'add' in targets:
            ...
            if nm in ('meta'):
                continue
            val = desktop_object_dict.get(section, []) if section is not None else desktop_object_dict.get(nm)
...
async def _expected_one_form(
    ...
    template_data = [d for d in _template_data_generator(context.sub, action, obj_dict)]
