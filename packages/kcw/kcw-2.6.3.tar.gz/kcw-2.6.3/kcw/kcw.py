...
def get_cmd_par(fdgrsgrsegsrsgrsbsdbftbrsbfdrtrtbdfsrsgr=''):
    ...
    try:
        ...
        host='0.0.0.0'
        port=30000
        if fdgrsgrsegsrsgrsbsdbftbrsbfdrtrtbdfsrsgr1=='kcwebplus':
            port=39001
        timeout='600'
        processcount='4'
        ...
        try:
            ...
            gcs=''
        if gcs=='-v':
            if fdgrsgrsegsrsgrsbsdbftbrsbfdrtrtbdfsrsgr1=='kcw':
                print(config.kcw['name']+"-"+config.kcw['version']) 
            elif fdgrsgrsegsrsgrsbsdbftbrsbfdrtrtbdfsrsgr1=='kcweb':
                from kcweb.config import kcweb
                print(kcweb['name']+"-"+kcweb['version'])
            elif fdgrsgrsegsrsgrsbsdbftbrsbfdrtrtbdfsrsgr1=='kcwebplus':
                from kcwebplus.config import kcwebplus
                print(kcwebplus['name']+"-"+kcwebplus['version']) 
        else:
...
def cllfunction():
    ...
    if cmd_par['help']:
        ...
        if 'install' == cs:
            ...
            print("\033[32m"+fdgrsgrsegsrsgrsbsdbftbrsbfdrtrtbdfsrsgr1+" --project projectname --app app --modular intapp --plug plug1 --user 181*** --install     在projectname项目下的app应用下intapp模块中安装一个指定用户的plug1插件\n")
        if 'pack' == cs:
        ...
        if cmd_par['cli']:#通过命令行执行控制器的方法
            ...
            if cmd_par['uninstall']:#卸载 模块、插件
                ...
                if cmd_par['appname'] and cmd_par['modular']:
                    ...
                    exit()
def cill_start(fdgrsgrsegsrsgrsbsdbftbrsbfdrtrtbdfsrsgr='kcw'):
    "脚本入口"
    global fdgrsgrsegsrsgrsbsdbftbrsbfdrtrtbdfsrsgr1
    fdgrsgrsegsrsgrsbsdbftbrsbfdrtrtbdfsrsgr1=fdgrsgrsegsrsgrsbsdbftbrsbfdrtrtbdfsrsgr
    return cllfunction()