...
from ...types.completions.arguments import CompletionsArguments
...
def collect_completion_args(args : dict[str, Any]) -> CompletionsArguments:
    """
    ...
    try:
        ...
        valid_args = {k: v for k, v in args.items() if k in CompletionsArguments.model_fields}
        
        # build the completion arguments
        completion_args = CompletionsArguments(**valid_args)
    except ValidationError as e:
...
# build post request
def build_post_request(args: CompletionsArguments, instructor : bool = False) -> dict[str, Any]:
    """
