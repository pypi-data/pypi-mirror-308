...
static void write_then_read(uint8_t CS, uint8_t *write_buffer, size_t write_len, uint8_t *read_buffer, size_t read_len)
{
  gpio_put(CS, LOW);

  // Write
  SPI_write(SPI_BUS_SENSORS, write_buffer, write_len);

  // Read
  SPI_read(SPI_BUS_SENSORS, read_buffer, read_len);

  gpio_put(CS, HIGH);
}

static uint8_t transceive(uint8_t CS, uint8_t *txbuf, uint8_t txlen, uint8_t *rxbuf, uint8_t rxlen, uint8_t interdelay)
{
  uint8_t status = 0;
  uint8_t i;
...
static bool writeRegister(uint8_t CS, uint8_t reg, uint16_t data)
{
...
static bool readRegister(uint8_t CS, uint8_t reg, uint16_t *data)
{
...
static bool setGain(uint8_t CS, mlx90393_gain_t gain)
{
  _gain = gain;
...
static mlx90393_gain_t getGain(void)
{
  return _gain;
}

static bool setResolution(uint8_t CS, mlx90393_axis_t axis, mlx90393_resolution_t resolution)
{
  uint16_t data = 0;
...
static mlx90393_resolution_t getResolution(mlx90393_axis_t axis)
{
  switch (axis)
  {
    case MLX90393_X:
...
static bool setFilter(uint8_t CS, mlx90393_filter_t filter)
{
  _dig_filt = filter;
...
static mlx90393_filter_t getFilter(void)
{
  return _dig_filt;
}

static bool setOversampling(uint8_t CS, mlx90393_oversampling_t oversampling)
{
  _osr = oversampling;
...
static mlx90393_oversampling_t getOversampling(void)
{
  return _osr;
}

static bool startSingleMeasurement(uint8_t CS)
{
...
static bool readMeasurement(uint8_t CS, float* outputPointer)
{
...
static void crossProduct(const float vec1[3], const float vec2[3], float resVec[3])
{
...
static float VecLength(const float vec[3])
{
  return sqrt(vec[0]*vec[0] + vec[1]*vec[1] + vec[2]*vec[2]);
}

static float angleVecToVecDIR(const float vec1[3], const float vec2[3], const float planeNorm[3])
{
...
bool readHES(uint8_t CS, float* outputPointer)
{
...
void testHES(uint8_t CS, uint16_t* outputPointer)
{
