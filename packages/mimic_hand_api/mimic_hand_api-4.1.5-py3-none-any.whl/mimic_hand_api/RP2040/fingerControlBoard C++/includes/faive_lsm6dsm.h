...
class faiveLSM6DSM
{
    public:
        ...
        void read(uint8_t CS, float* outputPointer);

        void test(uint8_t CS, uint8_t* outputPointer);
    ...
    private:
        ...
        void readMultipleRegisters(uint8_t CS, uint8_t* outputPointer, uint8_t reg, uint8_t length);

        // Write multiple 8-bit registers
        void writeMultipleRegisters(uint8_t CS, uint8_t reg, uint8_t* data, uint8_t length);

        // Convert Raw ADC value to dps
        float calculate_gyro(uint16_t raw_Gyro);

        // Convert Raw ADC value to g
        float calculate_acceleration(uint16_t rawAcceleration);

        // Calculate the twos complement of an unisgned 16-bit integer
        int16_t twosComplement(uint16_t input);
};
