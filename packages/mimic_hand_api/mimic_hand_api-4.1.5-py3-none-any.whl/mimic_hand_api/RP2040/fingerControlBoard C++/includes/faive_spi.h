...
class faiveSPI
{
    public:
        ...
        faiveSPI(uint8_t bus, uint32_t speed, uint8_t mode, spi_order_t order);

        void init(uint8_t bus, uint32_t speed, uint8_t mode, spi_order_t order);

        void write(uint8_t* data, uint8_t len);

        void read(uint8_t* data, uint8_t len);

        void write_read(uint8_t* writeData, uint8_t writeLen, uint8_t* readData, uint8_t readLen);
