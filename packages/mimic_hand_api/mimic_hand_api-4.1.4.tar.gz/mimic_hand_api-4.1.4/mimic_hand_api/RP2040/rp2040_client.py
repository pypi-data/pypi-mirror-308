...
class RP2040Client:
        ...
        if self.__write(message):
            ...
            if raw_reply:
                ...
                for i in range(len(reply.motors)):
                    positions[reply.motors[i].motorid] = reply.motors[
                        i
                    ].position
                return positions
    ...
    def __calibrationProcess(
        self, stopProcess, uartIDs, flag_getData, motorDataQueue
    ):
        ...
        while not stopProcess.is_set():
            for uartID in uartIDs:
                ...
                if uartID in desiredMotorPos and desiredMotorPos[uartID]:
                    status = self.__setMotorPos(
                        uartID, desiredMotorPos[uartID]
                    )
