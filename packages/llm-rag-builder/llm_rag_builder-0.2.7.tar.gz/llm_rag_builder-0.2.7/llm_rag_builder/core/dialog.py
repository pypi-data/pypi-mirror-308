...
class BaseMessage(ABC, metaclass=ABCMeta):
    content: str
    type: str
    hidden: bool = False
    role: str
...
class BaseDialog(ABC, metaclass=ABCMeta):
    default_llm: BaseLLM
    title: str | None
    ...
    def generate_agent_message(self, message_type, llm=None) -> BaseAgentMessage:
        ...
        agent_message = llm.generate_response(self._messages)
        self.add_agent_message(agent_message, message_type)
    ...
    def from_json(self, json) -> None:
        ...
        for message in json["messages"]:
            if message["role"] == "user":
                ...
                self.add_agent_message(message["content"], message["type"])
