...
class IRobotState(ABC):
    ...
    def cartesian_position(self) -> CartesianPose:
        """The end effector's pose, in mm and degrees, where the angles are extrinsic Euler angles in XYZ order."""

    @property
    @abstractmethod
    def joint_angles(self) -> JointAnglesDegrees:
        """The robot current joint angles."""
...
class IRobot(ABC):
    ...
    def movel(
        ...
        Args:
            ...
            reference_frame (CartesianPose): The reference frame to move relative to. If None,
    ...
    def wait_for_motion_completion(self, timeout: float | None = None) -> bool:
        ...
        Raises:
            ActuatorException: If the request fails or the move did not complete in
