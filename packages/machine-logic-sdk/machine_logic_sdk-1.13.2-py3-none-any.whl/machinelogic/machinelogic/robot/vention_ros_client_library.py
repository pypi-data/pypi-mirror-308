...
class PublicAPI:
    class Service:
        ...
        GetCartesianAccelerationLimit = (
            "vention_robots/get_cartesian_acceleration_limit"
        )
...
class Utils:
    @staticmethod
    def threshold(value: float, minimum: float, maximum: float):
    ...
    @staticmethod
    def to_millimeters(
        distances: List[float], from_units: DistanceUnit = DistanceUnit.Meters
    ) -> List[float]:
    ...
    def to_quaternions(
        ...
        quaternions = list(
            scipy_rotation.from_euler(
                euler_angle_order, euler_angles, degrees=is_degrees
            ).as_quat()
        )
    ...
    def to_euler_angles(
        ...
        is_degrees = AngleUnit.Degrees == angle_units
        euler_angles = scipy_rotation.from_quat(quaternions).as_euler(
            order, degrees=is_degrees
        )
    ...
    def euler_to_rotation_matrix(
        cartesian_pose: CartesianPose, euler_angle_order: str = "XYZ"
    ) -> np.ndarray:
        rotation_matrix_rad = scipy_rotation.from_euler(
            euler_angle_order, cartesian_pose[3:], degrees=True
        ).as_matrix()
    ...
    def euler_to_homogeneous_transform_matrix(
        ...
        rotation_matrix_rad = Utils.euler_to_rotation_matrix(
            cartesian_pose, euler_angle_order=euler_angle_order
        )
        homogeneous_transform_matrix = np.zeros((4, 4))
        for row_index in range(3):
            ...
            for col_index in range(3):
                homogeneous_transform_matrix[
                    row_index, col_index
                ] = rotation_matrix_rad[row_index, col_index]
        # one inside of 4,4 position.
        homogeneous_transform_matrix[3, 3] = 1.0
        return homogeneous_transform_matrix

    @staticmethod
    def homogeneous_tranform_matrix_to_euler(
        homogeneous_matrix: np.ndarray, euler_angle_order: str = "XYZ"
    ) -> CartesianPose:
        pose_result = [0.0] * 6
        ...
        with warnings.catch_warnings():
            ...
            euler_angles_degrees = rotation_object.as_euler(
                euler_angle_order, degrees=True
            )
        for i in range(3):
    ...
    def transform(
        ...
        # transform a vector from "from_reference_frame" to "to_reference_frame".
        position_matrix = Utils.euler_to_homogeneous_transform_matrix(
            position, euler_angle_order=euler_angle_order
        )
        from_reference_matrix = Utils.euler_to_homogeneous_transform_matrix(
            from_reference_frame, euler_angle_order=euler_angle_order
        )
        to_reference_matrix = Utils.euler_to_homogeneous_transform_matrix(
            to_reference_frame, euler_angle_order=euler_angle_order
        )
        transform_matrix = (
            np.linalg.inv(to_reference_matrix) @ from_reference_matrix @ position_matrix
        )
        return Utils.homogeneous_tranform_matrix_to_euler(transform_matrix)

    @staticmethod
    def doosan_trans(
        pos: CartesianPose,
        delta: CartesianPose = [0.0] * 6,
        ref: CartesianPose = [0.0] * 6,
        ref_out: CartesianPose = [0.0] * 6,
    ) -> CartesianPose:
        # replicates the doosan drl "trans" function.
        def trans_rot_matrix(
            coord: CartesianPose,
        ) -> Tuple[np.ndarray, np.ndarray]:
            translation = Utils.euler_to_homogeneous_transform_matrix(
                coord[:3] + [0.0] * 3
            )
            rotation = Utils.euler_to_homogeneous_transform_matrix(
                [0.0] * 3 + coord[-3:]
            )
            return (translation, rotation)

        pos_translation, pos_rotation = trans_rot_matrix(pos)
        delta_translation, delta_rotation = trans_rot_matrix(delta)
        ref_matrix = Utils.euler_to_homogeneous_transform_matrix(ref)
        ref_out_matrix = Utils.euler_to_homogeneous_transform_matrix(ref_out)
        transform_matrix = (
            np.linalg.inv(ref_out_matrix)
            @ ref_matrix
            @ delta_translation
            @ pos_translation
            @ delta_rotation
    ...
    @staticmethod
    def angular_velocity_to_scale_factor(
        velocity: DegreesPerSecond, limits: DegreesPerSecond
    ) -> ScaleFactor:
    ...
    @staticmethod
    def cartesian_velocity_to_scale_factor(
        velocity: MillimetersPerSecond, limit: MillimetersPerSecond
    ) -> ScaleFactor:
...
def _get_movej_message(
    ...
    blend_radius_m = Utils.to_meters(
        [blend_radius_mm], from_units=DistanceUnit.Millimeters
    )[0]
...
def _to_moveit_cartesian_pose(
    ...
    position_meters = Utils.to_meters(
        position_and_euler_angles[:3], from_units=DistanceUnit.Millimeters
    )
...
def _from_moveit_cartesian_pose(
    ...
    euler_angles = Utils.to_degrees(
        Utils.to_euler_angles(
            position_from_moveit[3:], order="XYZ", angle_units=AngleUnit.Radians
        )
    )
...
class _Move:
    ...
    def message(
        ...
        if _MoveType.J == self.move_type:
            velocity_scale = Utils.angular_velocity_to_scale_factor(
                self.velocity, angular_velocity_limit
            )
            acceleration_scale = Utils.angular_acceleration_to_scale_factor(
                self.acceleration, angular_acceleration_limit
            )
        ...
        if _MoveType.L == self.move_type:
            velocity_scale = Utils.cartesian_velocity_to_scale_factor(
                cast(MillimetersPerSecond, self.velocity), cartesian_velocity_limit
            )
            acceleration_scale = Utils.angular_acceleration_to_scale_factor(
...
class RosRobotClient:
    ...
    class SequenceBuilder(ISequenceBuilder):
        ...
        def append_movel(
            ...
            target_wrt_ref = Utils.transform(
                target, from_reference_frame=reference_frame
            )
            self.sequence.append(
                _Move(
                    _MoveType.L,
    ...
    def __init__(
        ...
        on_state_change: Union[
            None, Callable[[RobotOperationalState, RobotSafetyState], None]
        ] = None,
        ...
        if not skip_container_management:
            if not self.container_mananger.is_running():
                ...
                self.container_mananger.run(
                    model, robot_controller_ip, container_port=port
                )
    ...
    def _init_ros_client(self):
        ...
        # register callbacks
        log_topic_listener.subscribe(
            lambda data: self.log_listener.handle(RobotAlarm.into(data))
        )
    ...
    def restart_container(self):
        if self._skip_container_management:
            warnings.warn(
                "skip_container_management = True, skipping attempt to restart container."
            )
            return
    ...
    def jog(
        ...
        target_position = self.get_joint_positions()
    ...
    def linear_jog(
        ...
        jog_target = self.get_cartesian_position()
    ...
    def get_joint_positions(self) -> JointAnglesDegrees:
        lRadPosition: List[float] = self._call_service(
            PublicAPI.Service.GetJointPosition,
            PublicAPI.Type.GetCurrentPosJ,
        )["position"]
        return Utils.to_degrees(lRadPosition)
    ...
    def get_cartesian_position(self) -> CartesianPose:
        lPosition = self._call_service(
            PublicAPI.Service.GetCartesianPosition,
            PublicAPI.Type.GetCurrentPosX,
        )["position"]
        return _from_moveit_cartesian_pose(lPosition)
    ...
    def _move_sequence(
        ...
        if self.move_in_progress:
            self.move_stop()
            raise Exception(
                "Cannot perform new move sequence while async move sequence is in progress"
            )
    ...
    def inverse_kinematics(
        ...
        ik_result = self._call_service(
            ...
            dict(pos=quat_position, sol_space=solution_space, ref=ref),
            timeout=180,
        )["conv_posj"]
    ...
    def execute_sequence(
        self, sequence: "RosRobotClient.SequenceBuilder", is_async: bool = False
    ) -> bool:
    ...
    def _calculate_blend_radii(self, move_sequence: List[_Move]) -> List[Millimeters]:
        ...
        move_poses: List[CartesianPose] = [
            self._pose_for_move(move) for move in move_sequence
        ]
        ...
        for index, move in enumerate(move_sequence):
            ...
            distance_to_prev_point = Utils.get_distance(
                target_pose, current_position if index == 0 else move_poses[index - 1]
            )
            distance_to_next_point = Utils.get_distance(
                target_pose, move_poses[index + 1]
            )
            minimum_blend_distance = min(distance_to_prev_point, distance_to_next_point)
            blend_radii[index] = min(
                minimum_blend_distance * _MAX_BLEND_FRACTION, move.blend_radius
            )
        return blend_radii
