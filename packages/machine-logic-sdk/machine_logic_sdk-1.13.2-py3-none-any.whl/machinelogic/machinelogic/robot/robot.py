...
class RobotState(IRobotState):
    ...
    def cartesian_position(self) -> CartesianPose:
        return round_array_elements(self._ros_robot.get_cartesian_position())

    @property
    def joint_angles(self) -> JointAnglesDegrees:
        return round_array_elements(self._ros_robot.get_joint_positions())
...
class Robot(IRobot):
    ...
    def __init__(self, configuration: RobotConfiguration, api: Api):
        ...
        ros_robot = RosRobotSingleton.get_instance(
            ros_address=configuration._ros_address
        )
        configuration._set_cartesian_velocity_limit(
            ros_robot.get_cartesian_velocity_limit()
        )
        configuration._set_joint_velocity_limit(ros_robot.get_joint_velocity_limit())
    ...
    def compute_inverse_kinematics(
        ...
        joint_angles = self._robot.inverse_kinematics(cartesian_position)
