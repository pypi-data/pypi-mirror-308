...
def weighted_downside_beta(returns: pd.Series, underlying: pd.Series) -> float:
    intersection = underlying[underlying < 0].index.intersection(
        returns[returns < 0].index
    )
    downside_underlying = underlying.loc[intersection]
    downside_returns = returns.loc[intersection]
