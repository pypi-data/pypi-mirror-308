...
@_call_stack_check("__request")
def header_handler(all_params: dict, headers_by_rest: dict, method: str = HttpMethod.GET.value,
                   headers_by_config: dict = None, headers_by_req: dict = None, headers_by_opts: dict = None):
...
def cookies_handler(req, rest_cookies, opts_cookies):
    if not isinstance(opts_cookies, dict):
        if not isinstance(rest_cookies, dict):
            return {}
        return rest_cookies
    req[_Constant.COOKIES] = opts_cookies
...
def server_name_handler(rest_server_name: str, server_name: str, func: callable) -> str:
    ...
    raise RuntimeError("server name is a must.")
...
def files_handler(req):
    ...
    if isinstance(files, BaseStreamFile):
        m = files.build_encoder()
        req[_Constant.DATA] = m.data
        headers = req.get(_Constant.HEADERS, {})
        headers.update({_Constant.CONTENT_TYPE: m.content_type})
