...
def to_acc_address(data: ValAddress) -> AccAddress:
    ...
    return AccAddress(bech32_encode("paloma", vals[1]))
...
def to_val_address(data: AccAddress) -> ValAddress:
    ...
    if vals[1] is None:
        raise ValueError(f"invalid bech32: {data}")
    return ValAddress(bech32_encode("palomavaloper", vals[1]))
...
def to_acc_pubkey(data: ValPubKey) -> AccPubKey:
    ...
    if vals[1] is None:
        raise ValueError(f"invalid bech32: {data}")
    return AccPubKey(bech32_encode("palomapub", vals[1]))
...
def to_val_pubkey(data: AccPubKey) -> ValPubKey:
    ...
    if vals[1] is None:
        raise ValueError(f"invalid bech32: {data}")
    return ValPubKey(bech32_encode("palomavaloperpub", vals[1]))
