...
class Coin(JSONSerializable):
    ...
    denom: str = attr.ib()
    """Coin's denomination, only ``ugrain``."""
    ...
    def from_str(cls, string: str) -> Coin:
        ...
        ``283923ugrain`` (``int``-Coin) or ``23920.23020ugrain`` (:class:`Dec`-Coin).

        >>> int_coin = Coin.from_str("230920ugrain")
        >>> int_coin.denom
        'ugrain'
        >>> int_coin.amount
        230920
        >>> dec_coin = Coin.from_str("203922.223ugrain")
        >>> dec_coin.denom
        'ugrain'
        >>> dec_coin.amount
        Dec('203922.223')
