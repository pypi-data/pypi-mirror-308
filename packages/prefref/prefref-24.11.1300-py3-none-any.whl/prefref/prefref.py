...
class Config:
  ...
  # default constructor
  def __init__(self, options: Config_Options, app_name: str, app_desc: str, config_filepath: str = None, logger = None, providers: list[Config_Provider] = [Config_Provider.ARG, Config_Provider.ENV, Config_Provider.JSON], unknown_args_fatal: bool = False, reduce_options_to_key_value: bool = True, no_create_file: bool = False) -> None:
    ...
    self.no_create_file = no_create_file
    ...
    # json
    if Config_Provider.JSON in self.providers and not no_create_file:
  ...
  def create_json_config(self, options: Config_Options) -> dict:
    ...
    os.makedirs(os.path.dirname(options.config_filepath.value if isinstance(options.config_filepath, Config_Option) else options.config_filepath), exist_ok=True)
