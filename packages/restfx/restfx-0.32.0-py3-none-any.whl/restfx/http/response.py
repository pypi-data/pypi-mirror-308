...
class FileResponse(HttpResponse):
    def __init__(self, fp: Union[str, bytes, IOBase], attachment: [str, bool] = None,
        ...
        # 需要分块返回文件
        if ranges:
            # fix https://gitee.com/wangankeji-fe/restfx/issues/I3SBXR
            ...
            # fix https://gitee.com/wangankeji-fe/restfx/issues/I3UN41
...
class MovedPermanent(HttpResponse):
    def __init__(self, location, status=301, **kwargs):
...
class Forbidden(HttpResponse):
    def __init__(self, content=None, **kwargs):
        super().__init__(content, status=403, **kwargs)
...
class MethodNotAllowed(HttpResponse):
    def __init__(self, content=None, **kwargs):
        super().__init__(content, status=405, **kwargs)
