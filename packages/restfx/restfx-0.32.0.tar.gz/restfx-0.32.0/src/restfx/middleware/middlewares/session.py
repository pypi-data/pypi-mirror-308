...
class SessionMiddleware(MiddlewareBase):
    ...
    def check_and_make_session(self, config, request):
        ...
        def create_session():
            ...
            session = self.provider.create(sid)
        ...
        session = self.provider.get(session_id)
        ...
        # session 过期
        if self.provider.is_expired(session):
            self.logger.debug('Session %r is expired.' % session_id)
            return create_session()

        # 检查客户端信息与 session 存储的是否一致
        if self.check_flags:
            if (self.check_flags & self.FLAG_REMOTE_ADDR) and (session.remote_addr != remote_addr):
            ...
            if (self.check_flags & self.FLAG_USER_AGENT) and (session.user_agent != user_agent):
    ...
    def on_coming(self, request):
        ...
        ensure_session = functools.partial(self.check_and_make_session, config, request)
        ...
        class SessionProxy:
            def __getattr__(self, item):
                if not store[0]:
                    store[0] = ensure_session()
                    request.session = store[0]
                return getattr(store[0], item)

            def __setattr__(self, key, value):
                if not store[0]:
                    store[0] = ensure_session()
                    request.session = store[0]
                setattr(store[0], key, value)

        request.session = SessionProxy()
    ...
    def on_leaving(self, request, response):
        ...
        if not isinstance(request.session, HttpSession):
            return
