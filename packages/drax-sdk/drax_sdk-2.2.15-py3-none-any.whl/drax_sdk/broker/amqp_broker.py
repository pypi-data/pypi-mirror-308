...
from drax_sdk.utils.bytes_codec import base64_to_bytes
...
class DraxAmqpBroker:
    ...
    def send_state(self, state, routing_key):
        if self._channel is None:
            logger.error("Channel is None. Cannot send state.")
            return
        logger.info("Sending state: %s with routing key: %s", state, routing_key)
        self._channel.basic_publish(
            exchange="topic_logs", routing_key=routing_key, body=state
        )
