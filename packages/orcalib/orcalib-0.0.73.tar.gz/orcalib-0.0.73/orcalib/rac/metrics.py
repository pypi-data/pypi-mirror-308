...
from orcalib.memoryset import InputType
...
def compute_prediction_metrics(preds: list[tuple[int, torch.Tensor]], correct_labels) -> EvalResult:
    logger = logging.getLogger(__name__)
    predicted_labels = [pred[0] for pred in preds]
    labels_w_confidence = [pred[1].tolist() for pred in preds]
...
) -> dict[int, LabeledMemoryLookupResult]:
    logger = logging.getLogger(__name__)
    memory_stats: dict[int, LabeledMemoryLookupResult] = {}
            ...
            if memory_id is not None:
                if memory_id in memory_stats:
                    ...
                    if memory.label == correct_label:
                        memory_stats[memory_id].correct = memory_stats[memory_id].correct + 1
                    else:
                        memory_stats[memory_id].incorrect = memory_stats[memory_id].incorrect + 1
                else:
                    memory_stats[memory_id] = LabeledMemoryLookupResult(
...
def analyze_result(result: PredictionResult) -> AnalyzeResult:
    ...
    for memory in result.memories:
        memory_stat = {
            ...
            "attention_weight": memory.attention_weight,
    ...
    return AnalyzeResult(
        ...
        mean_memory_attention_weight=float(
            np.mean([memory.attention_weight for memory in result.memories if memory.attention_weight is not None])
        ),
