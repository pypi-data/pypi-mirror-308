...
class SentenceEmbeddingGenerator(nn.Module):
    ...
    def __init__(
        ...
        self.encoder: PreTrainedModel = AutoModel.from_pretrained(base_model, trust_remote_code=True)
        ...
        *,
        ...
        dimensions = (
            ...
            if isinstance(text, str)
                ...
                if isinstance(text, list) and isinstance(text[0], list) and isinstance(text[0][0], str)
                else None
            )
        )
    ...
    def forward(self, input_ids: Tensor, attention_mask: Tensor) -> Tensor:
        ...
        Args:
            input_ids: input token ids, long tensor of shape batch_size (x num_memories) x max_token_length
            attention_mask: input mask, float tensor of shape batch_size (x num_memories) x max_token_length
