...
class EmbeddingSimilarity(nn.Module, ABC):
    ...
    @abstractmethod
    def forward(self, input_embedding: Tensor, memories_embedding: Tensor) -> Tensor:
        """
        ...
        Args:
            ...
            memories_embedding: memory embeddings, float tensor of shape batch_size (x num_memories) x embedding_dim
...
class FeedForwardSimilarity(EmbeddingSimilarity):
    ...
    def forward(self, input_embedding: Tensor, memories_embedding: Tensor) -> Tensor:
        if len(memories_embedding.shape) == 3:
            # Expand input_embedding to match the shape of memories_embedding
            input_embedding = input_embedding.unsqueeze(1).expand(-1, memories_embedding.size(1), -1)
        # Concatenate along the last dimension
        combined = torch.cat([input_embedding, memories_embedding], dim=-1)
...
class CosineSimilarity(EmbeddingSimilarity):
    ...
    def forward(self, input_embedding: Tensor, memories_embedding: Tensor) -> Tensor:
        ...
        return self.head(
            input_embedding.unsqueeze(1) if len(memories_embedding.shape) == 3 else input_embedding,
            memories_embedding,
        )
...
class InnerProductSimilarity(EmbeddingSimilarity):
    ...
    def forward(self, input_embedding: Tensor, memories_embedding: Tensor) -> Tensor:
        if len(memories_embedding.shape) == 3:
            return torch.bmm(
                input_embedding.unsqueeze(1) if len(memories_embedding.shape) == 3 else input_embedding,
                memories_embedding.transpose(1, 2),
            ).squeeze(1)
        else:
            return torch.sum(input_embedding * memories_embedding, dim=-1)
