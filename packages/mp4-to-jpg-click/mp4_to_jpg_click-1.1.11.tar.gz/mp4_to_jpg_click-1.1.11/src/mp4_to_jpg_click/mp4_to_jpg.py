...
def select_input_files():
    """Select multiple video files using GUI dialog or CLI fallback."""
    ...
    try:
        ...
        logger.info("Using CLI file selection due to: {e}")
        return cli_select_input_files()


def select_output_directory():
    """Select output directory using GUI dialog or CLI fallback."""
    # Check if display is available
    try:
        if "DISPLAY" not in os.environ:
            raise Exception("No display available")
        ...
        logger.info("Using CLI directory selection due to: {e}")
        return cli_select_output_directory()


def cli_select_input_files():
    """Select multiple video files via CLI using prompt_toolkit."""
    input_files = []
    ...
    print(
        "\nEnter video file paths (press Tab for completion, type 'done' when finished):"
    )
    while True:
        try:
            path = prompt(
            ...
            if os.path.isfile(path):
                if path.lower().endswith((".mp4", ".mov")):
                    ...
                    print("Error: File must be an MP4 or MOV video file")
            else:
...
def cli_select_output_directory():
    """Select output directory via CLI using prompt_toolkit."""
    dir_completer = PathCompleter(only_directories=True)

    print("\nSelect output directory (press Tab for completion):")
    while True:
        try:
            output_dir = prompt(
                "Output directory: ",
            ...
            if os.path.isdir(output_dir):
                ...
                create = (
                    prompt(
                        f"Directory '{output_dir}' does not exist. Create it? [y/N]: "
                    )
                    .lower()
                    .strip()
                )
