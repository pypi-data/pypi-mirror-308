...
def flags_plotv2(pathwork,
    ...
    if family in ('amsua', 'amsub', 'atms'): 
        predefined_bit_pairs = [(0,'or',7),(8,),(9,16,),(9,18,),(9,23,),(9,),(12,),(23,)]  # Ejemplo con exclusión de bit 23
        predefined_bit_pairs2 = [(0,'or',7,'n8'),(8,),(9,16,'n8'),(9,18,'n8'),(9,23,'n8'),(9,'n8'),(12,),(23, 12)]  # Ejemplo con exclusión de bit 23
      
    if family in ('radar'):
       predefined_bit_pairs = [(12,)]  # Ejemplo con exclusión de bit 23
       #a predefined_bit_pairs2 = [(0,'or',2,'or',9,'n16'),(8,),(9,16,'n8'),(11,7,'n8'),(11,19,'n8'),(11,21,'n8'),(11,23,'n8'),(12,)]  # Ejemplo con exclusión de bit 23
       predefined_bit_pairs2 = [(12,)]  # Ejemplo con exclusión de bit 23
       

    # Total de observaciones en todas las banderas (flags)
    total_observations_all_files = 0
    bit_pair_observations = defaultdict(int) 
    bit_pair_observations2 = defaultdict(int)
    ...
    for date in dates:
        ...
        if not vcoord_data:
            ...
            makedo=False
            cursor.execute(f"""
            SELECT distinct(vcoord) FROM flag_observations  where  (flag & 256) = 256;
             """) 
            vcoord_data = cursor.fetchall()
        ...
        for flag, num_observations, vcoord_ in flag_data:
            ...
            for bit_pair in valid_bit_pairs:
                bit_pair_observations[bit_pair] += num_observations  # Solo suma los pares de bits válidos
            if family in ('cris','iasi'):
              if  8 not in  bits:
           #a      total_observations_n8 += num_observations 
                 if vcoord_ in vcoord_data and makedo==True:
            #         print ("MAMA")
                     total_observations_n8  += num_observations
                 if makedo==False:
                     total_observations_n8  +=num_observations


              if 0 in bits or 2  in bits or (16 in bits and 9 not in bits):
                 total_observations_n8v2 += num_observations 
            #     total_observations_n8v2  += num_observations

           #      if makedo==False:
               #      print ("PAPA")
            #         total_observations_n8v2  += num_observations
            #     print ( vcoord_, len(vcoord_data), vcoord_ in vcoord_data  , makedo)
              if 0 in bits or 2  in bits or (16 not  in bits and 9  in bits):
                  if vcoord_ in vcoord_data and makedo==True:
            #         print ("MAMA")
                     total_observations_n8v3  += num_observations
                  if  makedo== False:
                     total_observations_n8v3  += num_observations
            ...
            if family in ('amsua','amsub', 'atms' ):
              ...
              if 8 not in  bits: 
                ...
                if vcoord_ in vcoord_data and makedo==True:
                    if  23 in  bits:
                      print ("SSSS  dddda",vcoord_)
                      if vcoord_ ==31:
                          total_31_n23 +=num_observations
                      if vcoord_ ==32:
                          total_32_n23 +=num_observations

                    if vcoord_ ==31:
                      total_31+=num_observations
                    if vcoord_ ==32:
                      total_32+=num_observations



                total_observations_n8 += num_observations
              #if vcoord in vcoord_data:
                total_observations_n8v2  += num_observations
             

            total_observations_all_files += num_observations  # Suma todas las observaciones
   # print ('paco',total_31_n23 *100/total_31,total_32_n23 *100/total_32 )

    print ("JANINA", total_observations_n8,   total_observations_n8v2,   total_observations_n8v3 , total_observations_n8v2*100/total_observations_all_files , total_observations_n8v3*100/total_observations_all_files   )   
    #exit()
    # Calcula los porcentajes para los pares de bits en base al total de todas las observaciones
    bit_pair_percentages = {pair: (bit_pair_observations[pair] / total_observations_all_files) * 100 for pair in bit_pair_observations}
   # bit_pair_percentages2 = {pair: (bit_pair_observations2[pair] / total_observations_all_files) * 100 for pair in bit_pair_observations2}
   # print (bit_pair_percentages2 )

    bit_pair_percentages2 = {pair: (bit_pair_observations2[pair], (bit_pair_observations2[pair] / total_observations_all_files) * 100) for pair in bit_pair_observations2}
    print (bit_pair_percentages2 )
  #  exit()
    print ("total_observations_all_files",  bit_pair_percentages )
    print ("total_observations_n8", total_observations_n8)
    print ("total_observations_all_files",total_observations_all_files)
    print ("bit_pair_observations", bit_pair_observations)
...
#                        }    
  ...
  #  print (bit_pair_percentages)
    ...
    bit_pair_percentages_sorted2 = dict(sorted(bit_pair_percentages2.items(), key=lambda x: x[1][1], reverse=True))
    ...
    print (total_observations_n8v2, total_observations_n8)
    for bit_pair, (observations, percentage)  in bit_pair_percentages_sorted2.items():
    ...
    #     percentages_text += f"*{additional_title}: {percentage/((total_observations_n8-total_observations_n8v3) *100/ total_observations_all_files)   }\n"
         try:
           percentages_text += f"*{additional_title}:  {(observations*100/total_observations_n8):.4g}%\n"
         except:
           print ("")
       ...
       if bit_pair==(0,'or',2,'or',9,'n16'):
         additional_title = bit_pair_titles.get(tuple(bit_pair), "Bits " + str(bit_pair))
         try:
           percentages_text += f"*{additional_title}: {total_observations_n8v3*100/total_observations_n8:.4g}%\n"
           print (f'{(total_observations_n8v3*100)/total_observations_n8:.4g}', 100- total_observations_n8v2*100/total_observations_n8)
           print ( total_observations_n8v2, total_observations_n8,total_observations_n8v2*100/total_observations_n8) 
         except:
           print ("")
    ...
    print (f'{pathwork}/combined_{pathwork[0:7]}_channel_{vcoord}_id_stn_{id_stn}_{varno}_plot.png')
    fig.text(0.55, 0.52,percentages_text, ha='left', va='bottom', fontsize=12)
    plt.savefig(f'{pathwork}/combined_{pathwork[0:7]}_channel_{vcoord}_id_stn_{id_stn}_{varno}_plot.png', bbox_inches='tight')
