...
from lastmile.types.dataset_create_response import Dataset, DatasetID
from lastmile.types.fine_tune_job_create_params import FineTuneJobConfig
from lastmile.types.fine_tune_job_create_response import JobID
from lastmile.types.evaluation_get_metric_response import Metric
from lastmile.types.label_dataset_job_create_params import (
    PseudoLabelJobConfig,
    PseudoLabelJobConfigPromptTemplate,
)
from lastmile.types.fine_tune_job_list_base_models_response import ModelID
...
""",
    ...
    def upload_dataset(
        ...
        try:
            ...
            create_response = self.client.datasets.create(name=name, **create_kwargs)  # type: ignore
            dataset_id = create_response.dataset.id
            self.logger.info(f"Dataset created with ID: {dataset_id.value}")
        ...
        try:
            upload_file_response = self.client.datasets.upload_file(dataset_id=DatasetID(value=dataset_id.value))  # type: ignore
        ...
        try:
            self.client.datasets.finalize_file_upload(
                dataset_id=DatasetID(value=dataset_id.value),  # type: ignore
                s3_presigned_post=s3_presigned_post,# type: ignore
            )
        except Exception as e:
        ...
        self._wait_for_dataset_ready(dataset_id=DatasetID(value=dataset_id.value))
    ...
    def download_dataset(
        ...
        Args:
            dataset_id (DatasetID): The ID of the dataset to download.
        ...
        Example:
            ...
            ```
        """
        self.logger.info(f"Fetching download URL for dataset {dataset_id.value}...")
        try:
            get_download_url_response = self.client.datasets.get_download_url(dataset_id=DatasetID(value=dataset_id.value))  # type: ignore
    ...
    def list_datasets(self) -> List[Dataset]:
        ...
        try:
            return self.client.datasets.list().datasets  # type: ignore
        except Exception as e:
    ...
    def label_dataset(
        ...
        Args:
            dataset_id (DatasetID): The ID of the dataset to use.
        ...
        # Handle prompt_template
        if isinstance(prompt_template, Metric):
            # Use predefined template
            prompt_template_str = self.PROMPT_TEMPLATES[prompt_template.name]["templateContent"]
        elif isinstance(prompt_template, str):  # type: ignore
            # User provided a custom template
            prompt_template_str = prompt_template
        else:
            raise ValueError("prompt_template must be a string or a Metric.")

        # Extract variables from the prompt template
        variables = self._extract_variables_from_template(prompt_template_str)  # type: ignore

        # Ensure dataset contains the required columns
        dataset_columns = self._get_dataset_columns(dataset_id=DatasetID(value=dataset_id.value))  
        ...
        few_shot_dataset_id = None
        if few_shot_examples is not None:
            few_shot_dataset_id = self._upload_few_shot_examples(few_shot_examples, variables)

        job_config = PseudoLabelJobConfig(
            dataset_id=DatasetID(value=dataset_id.value),  # type: ignore
            prompt_template=PseudoLabelJobConfigPromptTemplate(template=prompt_template_str),  # type: ignore
            few_shot_dataset_id=few_shot_dataset_id,  # type: ignore
        ...
        try:
            ...
            job_id = create_response.job_id.value
        ...
        try:
            self.client.label_dataset_jobs.submit(
                job_id=JobID(value=create_response.job_id.value),  # type: ignore
                pseudo_label_job_config=job_config,
            )
        except Exception as e:
    ...
    def _get_dataset_columns(self, dataset_id: DatasetID) -> List[str]:
        """
        ...
        self.logger.info(f"Retrieving columns for dataset {dataset_id.value}...")
        try:
            dataset_info = self.client.datasets.get(id=DatasetID(value=dataset_id.value))  # type: ignore
    ...
    def _upload_few_shot_examples(self, few_shot_examples: pd.DataFrame, required_columns: List[str]) -> DatasetID:
        """
        ...
        Returns:
            DatasetID: The ID of the uploaded few-shot examples dataset.
        ...
        with tempfile.NamedTemporaryFile(suffix=".csv", delete=False) as tmp_file:
            few_shot_examples.to_csv(tmp_file.name, index=False)  # type: ignore
    ...
    def fine_tune_model(
        ...
        Args:
            train_dataset_id (DatasetID): The ID of the training dataset.
            test_dataset_id (DatasetID): The ID of the test/validation dataset, if any.
        ...
        fine_tune_job_config = FineTuneJobConfig(
            ...
            baseline_model_id=ModelID(value=baseline_model_id),  # type: ignore
            train_dataset_id=DatasetID(value=train_dataset_id.value),  # type: ignore
            test_dataset_id=DatasetID(value=test_dataset_id.value),  # type: ignore
        ...
        try:
            self.client.fine_tune_jobs.submit(
                job_id=create_response.job_id,# type: ignore
                fine_tune_job_config=fine_tune_job_config,
            )
        except Exception as e:
        ...
        for metric in metrics_list:
            ...
            try:
                evaluate_response: EvaluationEvaluateResponse = self.client.evaluation.evaluate(
                    ...
                    input=data["input"].tolist() if "input" in data.columns else None,  # type: ignore
                    output=data["output"].tolist() if "output" in data.columns else None,  # type: ignore
                    ground_truth=data["ground_truth"].tolist() if "ground_truth" in data.columns else None,  # type: ignore
                )
    ...
    def evaluate_dataset(
        ...
        Args:
            dataset_id (DatasetID): The ID of the dataset to evaluate.
            metrics (Union[Metric, List[Metric]]): The metric(s) to evaluate.
        ...
        for metric in metrics_list:
            ...
            self.logger.info(f"Evaluating metric '{metric.name}' on dataset {dataset_id.value}...")
    ...
    def list_metrics(self) -> List[Metric]:
        ...
        return self.client.evaluation.list_metrics().metrics # type: ignore
    ...
    def get_metric(self, metric: Metric) -> Metric:
        ...
        return self.client.evaluation.get_metric(metric=metric).metric  # type: ignore

    def wait_for_metric_online(self, metric: Metric, timeout: int = 120, retry_interval: int = 10) -> Metric:
        """
        ...
        while waited < timeout:
            ...
            output_metric = self.client.evaluation.get_metric(metric=metric).metric  # type: ignore
            if output_metric:
                metric = output_metric
                self.logger.info(f"Metric {metric.name} has deployment status {metric.deployment_status}")
                if metric.deployment_status == "MODEL_DEPLOYMENT_STATUS_ONLINE":
                    return metric

            self.logger.info(
                f"Metric name={metric.name}, id={metric.id} not online yet. Waiting {retry_interval}s and looking again..."
            )
            time.sleep(retry_interval)
            waited += retry_interval

        raise TimeoutError(f"Metric name={metric.id}, id={metric.id} not available after {timeout}s.")
    ...
    def get_prompt_templates(self) -> Dict[Metric, str]:
        ...
        Example:
            ...
            ```
        """
        return {info["metric"]: info["templateContent"] for info in self.PROMPT_TEMPLATES.values()}  # type: ignore
            ...
            try:
                if job_type == "label_dataset":
                    get_status_response_label: LabelDatasetJobGetStatusResponse = (
                        self.client.label_dataset_jobs.get_status(job_id=JobID(value=job_id))  # type: ignore
                    )
                    ...
                    get_status_response_fine_tune: FineTuneJobGetStatusResponse = self.client.fine_tune_jobs.get_status(
                        job_id=JobID(value=job_id)  # type: ignore
                    )
    ...
    def _wait_for_dataset_ready(
        ...
        while elapsed_time < timeout:
            try:
                get_response: DatasetGetResponse = self.client.datasets.get(id=dataset_id)  # type: ignore
            ...
            self.logger.info(f"Dataset {dataset_id.value} status: {status}")
            if status == "JOB_STATUS_COMPLETED":
                return
            elif status == "JOB_STATUS_FAILED":
                raise RuntimeError(f"Dataset {dataset_id.value} initialization failed.")
            elif status == "JOB_STATUS_CANCELLED":
                raise RuntimeError(f"Dataset {dataset_id.value} initialization cancelled.")
            else:
                # Implement exponential backoff with a maximum sleep time
                sleep_time = min(
                    interval * (2 ** (elapsed_time // interval)),
                ...
                time.sleep(sleep_time)
                elapsed_time += sleep_time
        raise TimeoutError(f"Dataset {dataset_id.value} did not become ready in time after {elapsed_time} seconds")
