...
from lastmile.types.dataset_finalize_file_upload_params import (
    S3PresignedPost as FinalizeS3PresignedPost,
    S3PresignedPostField as FinalizeS3PresignedPostField,
)
...
""",
    ...
    def upload_dataset(
        ...
        try:
            with open(file_path, "rb") as f:
                ...
                data = {field.key: field.value for field in s3_presigned_post.fields}
        ...
        try:
            # Create new fields with the correct type
            finalize_fields = [
                FinalizeS3PresignedPostField(key=field.key, value=field.value) for field in s3_presigned_post.fields
            ]

            # Create a new S3PresignedPost for finalize_file_upload
            finalize_s3_presigned_post = FinalizeS3PresignedPost(
                url=s3_presigned_post.url,
                fields=finalize_fields,
            )

            # Finalize the upload using the correct type
            self.client.datasets.finalize_file_upload(
                dataset_id=dataset_id,
                s3_presigned_post=finalize_s3_presigned_post,
            )
        except Exception as e:
    ...
    def label_dataset(
        ...
        job_config = PseudoLabelJobConfig(  # type: ignore
            dataset_id=dataset_id,
            prompt_template=PseudoLabelJobConfigPromptTemplate(  # type: ignore
                template=prompt_template_str
            ),
            few_shot_dataset_id=(few_shot_dataset_id if few_shot_dataset_id else ""),
