...
from lastmile import NOT_GIVEN, Lastmile
...
from lastmile.types.dataset_create_response import Dataset
...
""",
    ...
    def upload_dataset(
        ...
        try:
            create_response = self.client.datasets.create(
                name=name,
                description=(
                    description if description is not None else NOT_GIVEN
                ),
            )
        ...
        try:
            ...
            raise LastmileAPIError(
                f"Failed to get presigned URL: {str(e)}"
            ) from e
        ...
        try:
            ...
            raise LastmileAPIError(
                f"Failed to finalize upload: {str(e)}"
            ) from e
    ...
    def download_dataset(
        ...
        try:
            get_download_url_response = self.client.datasets.get_download_url(
                dataset_id=dataset_id
            )
    ...
    def label_dataset(
        ...
        if isinstance(prompt_template, Metric):
            ...
            prompt_template_str = self.PROMPT_TEMPLATES[prompt_template.name][
                "templateContent"
            ]
        ...
        missing_columns = [
            var for var in variables if var not in dataset_columns
        ]
        if missing_columns:
            raise ValueError(
                f"Dataset does not contain required columns {missing_columns} for prompt template."
            )

        # Handle few-shot examples
        few_shot_dataset_id = None
        if few_shot_examples is not None:
            few_shot_dataset_id = self._upload_few_shot_examples(
                few_shot_examples, variables
            )
        ...
        try:
            ...
            raise LastmileAPIError(
                f"Failed to create label dataset job: {str(e)}"
            ) from e
        ...
        try:
            ...
            raise LastmileAPIError(
                f"Failed to submit label dataset job: {str(e)}"
            ) from e
    ...
    def _get_dataset_columns(self, dataset_id: str) -> List[str]:
        ...
        try:
            ...
            raise LastmileAPIError(
                f"Failed to get dataset columns: {str(e)}"
            ) from e

    def _upload_few_shot_examples(
        self, few_shot_examples: pd.DataFrame, required_columns: List[str]
    ) -> str:
        """
        ...
        missing_columns = [
            ...
            if col not in few_shot_examples.columns
        ]
        if missing_columns:
            raise ValueError(
                f"Few-shot examples DataFrame is missing required columns {missing_columns}."
            )
    ...
    def fine_tune_model(
        ...
        try:
            base_models_response: FineTuneJobListBaseModelsResponse = (
                self.client.fine_tune_jobs.list_base_models()
            )
            ...
            if baseline_model_id:
                ...
                if baseline_model_id not in model_ids:
                    raise ValueError(
                        f"Baseline model ID '{baseline_model_id}' is not valid."
                    )
            else:
                baseline_model_id = base_models[0].id
                self.logger.info(
                    f"No baseline model ID provided. Using default model ID: {baseline_model_id}"
                )
        except Exception as e:
            self.logger.error(f"Failed to fetch fine-tunable models: {str(e)}")
            raise LastmileAPIError(
                f"Failed to fetch fine-tunable models: {str(e)}"
            ) from e
        ...
        try:
            create_response: FineTuneJobCreateResponse = (
                self.client.fine_tune_jobs.create(
            ...
            raise LastmileAPIError(
                f"Failed to create fine-tune job: {str(e)}"
            ) from e
        ...
        try:
            ...
            raise LastmileAPIError(
                f"Failed to submit fine-tune job: {str(e)}"
            ) from e
    ...
    def wait_for_fine_tune_job(
        ...
        """
        self._wait_for_job_completion(
            job_id, job_type="fine_tune", timeout=timeout, interval=interval
        )
    ...
    def evaluate_data(
        ...
        for metric in metrics_list:
            ...
            try:
                evaluate_response: EvaluationEvaluateResponse = (
                    self.client.evaluation.evaluate(
                ...
                data_with_scores[f"{evaluate_response.metric.name}_score"] = (
                    evaluate_response.scores
                )
            except Exception as e:
                self.logger.error(f"Failed to evaluate data: {str(e)}")
                raise LastmileAPIError(
                    f"Failed to evaluate data: {str(e)}"
                ) from e
    ...
    def evaluate_dataset(
        ...
        for metric in metrics_list:
            ...
            self.logger.info(
                f"Evaluating metric '{metric.name}' on dataset {dataset_id}..."
            )
            ...
            try:
                evaluate_dataset_response: (
                    EvaluationEvaluateDatasetResponse
                ) = self.client.evaluation.evaluate_dataset(
                    metric=metric,  # type: ignore
                    dataset_id=dataset_id,
                )
                # Add the scores to the DataFrame
                data_with_scores[
                    f"{evaluate_dataset_response.metric.name}_score"
                ] = evaluate_dataset_response.scores
            except Exception as e:
                self.logger.error(f"Failed to evaluate dataset: {str(e)}")
                raise LastmileAPIError(
                    f"Failed to evaluate dataset: {str(e)}"
                ) from e
    ...
    def list_metrics(self) -> List[Metric]:
        ...
        client_response_metrics: List[ListMetricsMetric] = (
            self.client.evaluation.list_metrics().metrics
        )
        return [
            Metric(
                id=metric.id,
    ...
    def get_metric(self, metric: Metric) -> Metric:
        ...
        if not (metric.id or metric.name):
            raise ValueError(
                "One of metric.id or metric.name must be provided to get_metric."
            )
        return self.client.evaluation.get_metric(metric=metric).metric  # type: ignore

    def wait_for_metric_online(
        self, metric: Metric, timeout: int = 120, retry_interval: int = 10
    ) -> Metric:
        """
        ...
        while waited < timeout:
            self.logger.info(
                f"Polling deployment status for metric {metric.name}..."
            )
            output_metric = self.client.evaluation.get_metric(metric=metric).metric  # type: ignore
            if output_metric:
                metric = output_metric
                self.logger.info(
                    f"Metric {metric.name} has deployment status {metric.deployment_status}"
                )
                if (
        ...
        raise TimeoutError(
            f"Metric name={metric.id}, id={metric.id} not available after {timeout}s."
        )
        ...
        while elapsed_time < timeout:
            try:
                if job_type == "label_dataset":
                    get_status_response_label: (
                        LabelDatasetJobGetStatusResponse
                    ) = self.client.label_dataset_jobs.get_status(
                        job_id=job_id
                    )
                    status = get_status_response_label.status
                elif job_type == "fine_tune":
                    get_status_response_fine_tune: (
                        FineTuneJobGetStatusResponse
                    ) = self.client.fine_tune_jobs.get_status(job_id=job_id)
            ...
            if status == "JOB_STATUS_COMPLETED":
                ...
                self.logger.debug(
                    f"Sleeping for {sleep_time} seconds before next status check."
                )
                time.sleep(sleep_time)
                elapsed_time += sleep_time
        raise TimeoutError(
            f"{job_type} job {job_id} did not complete in time after {elapsed_time} seconds"
        )
    ...
    def _wait_for_dataset_ready(
        ...
        while elapsed_time < timeout:
            try:
                ...
                raise LastmileAPIError(
                    f"Failed to get dataset status: {str(e)}"
                ) from e
            ...
            if status == "JOB_STATUS_COMPLETED":
                ...
                raise RuntimeError(
                    f"Dataset {dataset_id} initialization failed."
                )
            elif status == "JOB_STATUS_CANCELLED":
                raise RuntimeError(
                    f"Dataset {dataset_id} initialization cancelled."
                )
            else:
                # Implement exponential backoff with a maximum sleep time
                sleep_time = min(
                ...
                elapsed_time += sleep_time
        raise TimeoutError(
            f"Dataset {dataset_id} did not become ready in time after {elapsed_time} seconds"
        )
