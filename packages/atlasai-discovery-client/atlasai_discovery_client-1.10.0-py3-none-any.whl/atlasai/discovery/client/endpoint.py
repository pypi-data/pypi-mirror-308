...
def load_credentials(access_key=None, secret_key=None):
    access_key = access_key or os.getenv(constants.DISCOVERY_ACCESS_KEY)
    secret_key = secret_key or os.getenv(constants.DISCOVERY_SECRET_KEY)
    api_secret = os.getenv(constants.DISCOVERY_API_SECRET)

    if not secret_key and api_secret:
        secret_key = get_secret_data(api_secret)
        if secret_key is not None:
            os.environ[constants.DISCOVERY_SECRET_KEY] = secret_key
...
def include_authorization(
    ...
    bearer_token = bearer_token or os.getenv(constants.DISCOVERY_BEARER_TOKEN)
    if bearer_token:
