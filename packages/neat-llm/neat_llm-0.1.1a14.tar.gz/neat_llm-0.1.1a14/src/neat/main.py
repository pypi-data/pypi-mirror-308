...
class Neat:
    ...
    async def process_messages_sync(
        ...
        """Handle synchronous message processing."""
        processed_messages: List[Message] = list(messages)
        tool_definitions = (
            self.tool_manager.get_tool_definitions()
            if tools and not response_model
            else []
    ...
    def lm(
        ...
        *,
        ...
        [Callable[..., Awaitable[Sequence[Message]]]],
        ...
        def decorator(
            func: Callable[..., Awaitable[Union[Sequence[Message], Sequence[Dict[str, str]]]]]
        ) -> Callable[..., Awaitable[Union[T, str, AsyncGenerator[StreamContent, None]]]]:
            if not asyncio.iscoroutinefunction(func):
                raise TypeError("The decorated function must be async. Use 'async def'.")

            @wraps(func)
            async def wrapper(*args: Any, **kwargs: Any) -> Union[T, str, AsyncGenerator[StreamContent, None]]:
                messages = await func(*args, **kwargs)

                # Process messages to ensure they are Message objects
                ...
                for message in messages:
                    if isinstance(message, dict):
                        ...
                        raise ValueError(f"Invalid message type: {type(message)}. Expected Message or dict.")
                ...
                if stream:
                    return self.process_messages_stream(
                        processed_messages,
                        model,
                        temperature,
                        tools,
