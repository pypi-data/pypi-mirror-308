...
class Table:
    ...
    def __init__(self, schema, name):
        ...
        result = self.database.execute(command = f"SELECT table_schema AS table_schema, table_name AS table_name, table_type AS table_type, table_collation AS table_collation FROM information_schema.TABLES WHERE table_schema = '{self.schema.name}' AND table_name = '{self.name}'")
        self.ddl = ""
        self.columns = []
        self.keys = {}
