...
def format_data_frame(
    ...
    float_format: str | typing.Callable[[float], str] = "%.2f",
    integer_format: str | typing.Callable[[int], str] = "%d",
    ...
    Args:
        ...
        float_format (str | typing.Callable[[float], str]): The format string or a callable function for float values. Defaults to "%.2f".
        integer_format (str | typing.Callable[[int], str]): The format string or a callable function for integer values. Defaults to "%d".
        columns_split (str | None): The string used to separate columns. Defaults to " || ".
        header_border (str | None): The character used for the header border. Defaults to "=".
        top_border (str | None): The character used for the top border. Defaults to "=".
        left_border (str | None): The string used for the left border. Defaults to "|| ".
        bottom_border (str | None): The character used for the bottom border. Defaults to "=".
        right_border (str | None): The string used for the right border. Defaults to " ||".
    ...
    """
    float_format_is_string = isinstance(float_format, str)
    integer_format_is_string = isinstance(integer_format, str)
    ...
    size_of_table = (
        sum(size_of_columns) + len(columns_split) * (len(size_of_columns) - 1) + len(left_border) + len(right_border)
    )
    ...
    output_lines = [
        "%s%s%s" % (left_border, columns_split.join([column for column in row.values.tolist()]), right_border)
