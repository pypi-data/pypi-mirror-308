...
class GraphSection:
    ...
    def __init__(self, points: list[GraphPoint], angle_sensitivity: float = 0.0) -> None:
    ...
    def add(self, point: GraphPoint) -> None:
    ...
    def get_angle_degree(self) -> float | None:
        """Calculates the angle of the section.

        Returns:
            float | None: The angle in degrees, or None if the section has less than two points.
        """
    ...
    def get_direction(self) -> str | None:
        ...
        Returns:
            str | None: The direction ("increasing", "decreasing", "straight"), or None if the section has less than two points.
        """
        if len(self.points) >= 2:
    ...
    def remove_point_after_max(self) -> None:
    ...
    def remove_point_after_min(self) -> None:
...
class ThresholdSensitivity:
    ...
    def __init__(
        self, threshold_sensitivity: float = 0.0, type_: typing.Literal["absolute", "relative"] = "absolute"
    ) -> None:
    ...
    def get_decrease_sensitive_point(self, point: GraphPoint | int | float) -> float:
        """Calculates the decreased sensitive point based on the sensitivity type.

        Args:
            point (GraphPoint | int | float): The point or value to adjust.
    ...
    def get_increase_sensitive_point(self, point: GraphPoint | int | float) -> float:
...
class Graph:
    ...
    Attributes:
        ...
        min (GraphPoint | None): The point with the minimum y-value.
        max (GraphPoint | None): The point with the maximum y-value.
        average (float | None): The average y-value of all points.
    ...
    def __init__(self, points: list[GraphPoint] | None = None) -> None:
        """Initializes a new Graph object.

        Args:
            points (list[GraphPoint], optional): Initial list of points. Defaults to None.
        """
    ...
    def __str__(self) -> str:
        ...
        average_string = (
            f"{self.average:.4f}" if self.average and isinstance(self.average, float) else str(self.average)
        )
    ...
    def get_sections(
    ...
    ) -> typing.Generator[GraphSection | None, typing.Any, None]:
        ...
        Yields:
           typing.Generator[GraphSection | None, Any, None]: A generator of GraphSections.
        """
        graph_section = None
        ...
        for i in range(len(self.points)):
            if graph_section is None:
                ...
                if direction is None:
                    ...
                    if self.points[i].y < threshold_sensitivity.get_increase_sensitive_point(
                        graph_section.average
