...
def filter_classifications(correspondence) -> dict[str, str]:
    ...
    filtered_table = correspondence[~correspondence.index.isin(relations_to_ignore.index)]

    return filtered_table


def generate_classification_mapping(filtered_table, target_column, resource_info):
    mapping = {}
    many_to_one = defaultdict(list)
    for _, row in filtered_table.iterrows():
        if target_column == "location":
            bonsai_code = row.iloc[1]
            target_code = row.iloc[0]
        else:
            bonsai_code = row.iloc[0]
            target_code = row.iloc[1]
        comment = row["comment"]
        # Handle one-to-one and one-to-many correspondences
        if comment in ["one-to-one correspondence", "one-to-many correspondence"]:
            mapping[target_code] = bonsai_code
        # Handle many-to-one correspondence (left blank for now)
        elif comment == "many-to-one correspondence":
            many_to_one[target_code].append(bonsai_code)

    # Handle many-to-one correspondences
    if many_to_one:
        tree_bonsai_df = load_tree_bonsai(target_column, resource_info)
        for target_code, bonsai_codes in many_to_one.items():
            common_parent, max_steps = find_nearest_parent(bonsai_codes, tree_bonsai_df)
            mapping[target_code] = common_parent
