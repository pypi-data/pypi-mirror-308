...
class FactBaseModel(BonsaiBaseModel):
...
class DimensionModel(BonsaiBaseModel):
    ...
    def check_code_names(cls, v, field):
        if " " in v:
            raise ValueError(f"{field.field_name} not allowed to contain whitespaces.")
        return v
...
class MatrixModel:
    column_schema: FactBaseModel
    row_schema: FactBaseModel
