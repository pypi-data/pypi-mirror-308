...
class MinimumCls:
	...
	def read(self) -> ResultData:
		"""SCPI: READ:BLUetooth:MEASurement<Instance>:CSOunding:PVAPath:MINimum \n
		Snippet: value: ResultData = driver.csounding.pvaPath.minimum.read() \n
		...
		return self._core.io.query_struct(f'READ:BLUetooth:MEASurement<Instance>:CSOunding:PVAPath:MINimum?', self.__class__.ResultData())

	def fetch(self) -> ResultData:
		"""SCPI: FETCh:BLUetooth:MEASurement<Instance>:CSOunding:PVAPath:MINimum \n
		Snippet: value: ResultData = driver.csounding.pvaPath.minimum.fetch() \n
		Returns the power vs antenna path scalar results. The values described below are returned by FETCh and READ commands.
		CALCulate commands return limit check results instead, one value for each result listed below. \n
			:return: structure: for return value, see the help for ResultData structure arguments."""
		return self._core.io.query_struct(f'FETCh:BLUetooth:MEASurement<Instance>:CSOunding:PVAPath:MINimum?', self.__class__.ResultData())
	...
	def calculate(self) -> CalculateStruct:
		"""SCPI: CALCulate:BLUetooth:MEASurement<Instance>:CSOunding:PVAPath:MINimum \n
		Snippet: value: CalculateStruct = driver.csounding.pvaPath.minimum.calculate() \n
		Returns the power vs antenna path scalar results. The values described below are returned by FETCh and READ commands.
		CALCulate commands return limit check results instead, one value for each result listed below. \n
			:return: structure: for return value, see the help for CalculateStruct structure arguments."""
		return self._core.io.query_struct(f'CALCulate:BLUetooth:MEASurement<Instance>:CSOunding:PVAPath:MINimum?', self.__class__.CalculateStruct())
