...
class AllCls:
	...
	def fetch(self, timeout: float = None, target_main_state: enums.ResourceState = None, target_sync_state: enums.SyncState = None) -> enums.ResourceState:
		...
		Snippet: value: enums.ResourceState = driver.hdr.state.all.fetch(timeout = 1.0, target_main_state = enums.ResourceState.ACTive, target_sync_state = enums.SyncState.ADJusted) \n
		...
		response = self._core.io.query_str(f'FETCh:BLUetooth:MEASurement<Instance>:HDR:STATe:ALL? {param}'.rstrip())
