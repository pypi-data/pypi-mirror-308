...
class Df2SCls:
	...
	def set(self, freq_dev_f_2_lower: float, freq_dev_f_2_upper: float, freq_dev_f_2_enable: List[bool]) -> None:
		...
		Snippet: driver.configure.multiEval.limit.lowEnergy.le2M.daverage.df2S.set(freq_dev_f_2_lower = 1.0, freq_dev_f_2_upper = 1.0, freq_dev_f_2_enable = [True, False, True]) \n
	...
	def get(self) -> Df2SStruct:
		"""SCPI: CONFigure:BLUetooth:MEASurement<Instance>:MEValuation:LIMit:LENergy:LE2M:DAVerage:DF2S \n
		Snippet: value: Df2SStruct = driver.configure.multiEval.limit.lowEnergy.le2M.daverage.df2S.get() \n
		Defines the lower and upper deltaf2 frequency deviation limits for LE 2M PHY. The mnemonics DAVerage, DMINimum, DMAXimum
		distinguish average, minimum and maximum frequency deviations. \n
