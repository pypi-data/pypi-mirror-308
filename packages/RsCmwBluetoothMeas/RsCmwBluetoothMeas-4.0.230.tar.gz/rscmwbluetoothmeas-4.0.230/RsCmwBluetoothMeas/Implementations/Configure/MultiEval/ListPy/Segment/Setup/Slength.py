...
class SlengthCls:
	...
	def set(self, segment_length: int, segment=repcap.Segment.Default) -> None:
		...
		Defines the number of measured bursts in the segment \n
			:param segment_length: numeric The sum of the length of all active segments must not exceed 6700 timeslots (1 timeslot = 625 us duration) . Range: 1 to 1000
	...
	def get(self, segment=repcap.Segment.Default) -> int:
		...
		Defines the number of measured bursts in the segment \n
			:param segment: optional repeated capability selector. Default value: S1 (settable in the interface 'Segment')
			:return: segment_length: numeric The sum of the length of all active segments must not exceed 6700 timeslots (1 timeslot = 625 us duration) . Range: 1 to 1000"""
		segment_cmd_val = self._cmd_group.get_repcap_cmd_value(segment, repcap.Segment)
