...
class FoffsetCls:
	...
	def set(self, freq_offset: float, freq_offset_enable: List[bool]) -> None:
		...
		Snippet: driver.configure.multiEval.limit.cte.lowEnergy.le2M.foffset.set(freq_offset = 1.0, freq_offset_enable = [True, False, True]) \n
		Sets/gets the frequency offset limit for the CTE portion. Commands for uncoded LE 1M PHY (..:LE1M..) and LE 2M PHY (..
		:LE2M..) are available. \n
			:param freq_offset: numeric Range: 0 Hz to 500 kHz, Unit: Hz
	...
	class FoffsetStruct(StructBase):
		"""Response structure. Fields: \n
			- Freq_Offset: float: numeric Range: 0 Hz to 500 kHz, Unit: Hz
	...
	def get(self) -> FoffsetStruct:
		"""SCPI: CONFigure:BLUetooth:MEASurement<Instance>:MEValuation:LIMit:CTE:LENergy:LE2M:FOFFset \n
		Snippet: value: FoffsetStruct = driver.configure.multiEval.limit.cte.lowEnergy.le2M.foffset.get() \n
		Sets/gets the frequency offset limit for the CTE portion. Commands for uncoded LE 1M PHY (..:LE1M..) and LE 2M PHY (..
		:LE2M..) are available. \n
