...
class RsCmwBluetoothMeas:
	"""1341 total commands, 15 Subgroups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMW500/CMW100/CMW270/CMW280/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,4.0.230.0032'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '4.0.230.0032'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.0.230.0032'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwBluetoothMeas version failed. Current version: '4.0.230.0032', minimum required version: '{min_version}'")
	...
	@property
	def configure(self):
		"""configure commands group. 11 Sub-classes, 5 commands."""
	...
	@property
	def csounding(self):
		"""csounding commands group. 7 Sub-classes, 3 commands."""
		if not hasattr(self, '_csounding'):
			from .Implementations.Csounding import CsoundingCls
