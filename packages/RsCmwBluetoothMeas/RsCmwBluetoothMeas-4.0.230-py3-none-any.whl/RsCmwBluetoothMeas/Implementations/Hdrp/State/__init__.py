...
class StateCls:
	...
	def fetch(self, timeout: float = None, target_main_state: enums.TargetMainState = None, target_sync_state: enums.SyncState = None) -> enums.ResourceState:
		...
		Snippet: value: enums.ResourceState = driver.hdrp.state.fetch(timeout = 1.0, target_main_state = enums.TargetMainState.OFF, target_sync_state = enums.SyncState.ADJusted) \n
		...
		response = self._core.io.query_str(f'FETCh:BLUetooth:MEASurement<Instance>:HDRP:STATe? {param}'.rstrip())
