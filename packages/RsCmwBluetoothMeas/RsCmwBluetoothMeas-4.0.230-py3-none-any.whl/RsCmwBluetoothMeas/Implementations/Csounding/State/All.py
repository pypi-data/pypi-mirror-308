...
class AllCls:
	...
	class FetchStruct(StructBase):
		"""Response structure. Fields: \n
			...
			- Sync_State: enums.SyncState: PEND | ADJ PEND: transition to MainState ongoing ADJ: MainState reached
	...
	def fetch(self, timeout: float = None, target_main_state: enums.TargetMainState = None, target_sync_state: enums.SyncState = None) -> FetchStruct:
		"""SCPI: FETCh:BLUetooth:MEASurement<Instance>:CSOunding:STATe:ALL \n
		Snippet: value: FetchStruct = driver.csounding.state.all.fetch(timeout = 1.0, target_main_state = enums.TargetMainState.OFF, target_sync_state = enums.SyncState.ADJusted) \n
		...
		return self._core.io.query_struct(f'FETCh:BLUetooth:MEASurement<Instance>:CSOunding:STATe:ALL? {param}'.rstrip(), self.__class__.FetchStruct())
