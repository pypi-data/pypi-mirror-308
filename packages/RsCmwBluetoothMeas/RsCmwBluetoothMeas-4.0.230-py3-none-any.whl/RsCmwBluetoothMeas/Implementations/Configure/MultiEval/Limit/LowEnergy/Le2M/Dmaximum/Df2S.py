...
class Df2SCls:
	...
	def set(self, freq_dev_f_2_lower: float, freq_dev_f_2_upper: float, freq_dev_f_2_enable: List[bool]) -> None:
		"""SCPI: CONFigure:BLUetooth:MEASurement<Instance>:MEValuation:LIMit:LENergy:LE2M:DMAXimum:DF2S \n
		Snippet: driver.configure.multiEval.limit.lowEnergy.le2M.dmaximum.df2S.set(freq_dev_f_2_lower = 1.0, freq_dev_f_2_upper = 1.0, freq_dev_f_2_enable = [True, False, True]) \n
		...
		self._core.io.write(f'CONFigure:BLUetooth:MEASurement<Instance>:MEValuation:LIMit:LENergy:LE2M:DMAXimum:DF2S {param}'.rstrip())
	...
	def get(self) -> Df2SStruct:
		"""SCPI: CONFigure:BLUetooth:MEASurement<Instance>:MEValuation:LIMit:LENergy:LE2M:DMAXimum:DF2S \n
		Snippet: value: Df2SStruct = driver.configure.multiEval.limit.lowEnergy.le2M.dmaximum.df2S.get() \n
		Defines the lower and upper deltaf2 frequency deviation limits for LE 2M PHY. The mnemonics DAVerage, DMINimum, DMAXimum
		distinguish average, minimum and maximum frequency deviations. \n
			:return: structure: for return value, see the help for Df2SStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:BLUetooth:MEASurement<Instance>:MEValuation:LIMit:LENergy:LE2M:DMAXimum:DF2S?', self.__class__.Df2SStruct())
