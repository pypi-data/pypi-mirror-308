...
def valid_value(r_min, r_max, nvPrettyPrint):
    ...
    class ValidValue(argparse.Action):
        ...
        def __call__(self, parser, namespace, value, *args, **kwargs):
            if not r_min <= value <= r_max:
                if value < r_min:
                    value = self.default
                    nvPrettyPrint.print_ok("Number of '{}' set to default: '{}'".format(self.dest, self.default))
                elif value > r_max:
                    value = r_max
                    nvPrettyPrint.print_ok("Number of '{}' set to max: '{}'".format(self.dest, r_max))
            setattr(namespace, self.dest, value)
