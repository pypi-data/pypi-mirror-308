...
class ConfigCommand(CLICommand):  # noqa: D101
    ...
    def clear(self, _args):  # pylint: disable=no-self-use
        ...
        ConfigCommand.config.clear_configs()
    ...
    def clear_cache(self, _args):  # pylint: disable=no-self-use
        ...
        ConfigCommand.config.clear_tokens()
    ...
    def set(self, args):
        ...
        if org_global and (org_global != self.config._db.org):  # pylint: disable=protected-access
            ...
            if not team_global or not team_global_rm:
                ConfigCommand.config.team_name = "no-team"
            if not ace_global or not ace_global_rm:
                ConfigCommand.config.ace_name = "no-ace"
    ...
    def _set_config_file_as_current_config(
        ...
        if team_global:
            ...
            self.config.team_name = (
                ConfigCommand.config.team_name if ConfigCommand.config.team_name == "no-team" else self.config._db.team
            )
        if ace_global:
            ...
            self.config.ace_name = ConfigCommand.config.ace_name = (
                "no-ace" if ConfigCommand.config.ace_name == "no-ace" else self.config._db.ace
            )
    ...
    def current(self, args):  # pylint: disable=no-self-use,unused-argument
        ...
        if args.show_auth_token:
            ...
            if confirmation == "yes":
                ...
                if config_instance.starfleet_kas_session_key:
                    try:
    ...
    def _set_api_key(self, interactive, base_url, key_name=None):
        current_key = ConfigCommand.config.app_key
        if current_key:
        ...
        if MULTIPLE_CONFIGURATIONS:
            ...
            if interactive:
                if ConfigCommand.config.configurations:
                    self.printer.print_ok(app_key_prompt)
                    self.printer.print_configurations(ConfigCommand.config.configurations)
        ...
        if MULTIPLE_CONFIGURATIONS:
            ...
            while key_input != "no-apikey":
                if key_input.startswith("nvapi-"):
                    if Authentication.validate_sak_key(sak_key=key_input):
                    ...
                    if Authentication.validate_api_key(app_key=key_input):
        ...
        ConfigCommand.config.app_key = key_input
        ConfigCommand.config.starfleet_kas_email = None
        ConfigCommand.config.starfleet_kas_session_key = None
        # pylint: disable=protected-access
        ConfigCommand.config._starfleet_kas_email.global_arg_remove = True
        ConfigCommand.config._starfleet_kas_session_key.global_arg_remove = True
    ...
    def _set_api_key_multiple_configuration(self, key_input, key_name, current_key, disp_app_key, base_url):
        while key_input.isdigit() or (
            key_input != "no-apikey" and not Authentication.validate_api_key(app_key=key_input)
        ):
            try:
        ...
        ConfigCommand.config.key_name = key_name
        ConfigCommand.config.key_id = key_id_for_input
        return key_input
    ...
    def _set_starfleet_kas_session_key(self):
        ...
        cur_starfleet_kas_email = ConfigCommand.config.starfleet_kas_email
        ...
        if starfleet_kas_email != "no-email":
            session_key, expiration_time = Authentication.get_starfleet_kas_session_key(starfleet_kas_email)
        else:
            session_key, expiration_time = None, None
        ConfigCommand.config.starfleet_kas_session_key = session_key
        ConfigCommand.config.starfleet_kas_email = starfleet_kas_email
        ConfigCommand.config.starfleet_session_key_expiration = expiration_time
        ConfigCommand.config.app_key = None
        ConfigCommand.config._app_key.global_arg_remove = True  # pylint: disable=protected-access

    def _set_format_type(self, interactive):
        cur_format_type = ConfigCommand.config.format_type or "ascii"
        ...
        while not ConfigCommand.config.validate_format_type(format_type_in):
            self.printer.print_error(f"Invalid format type. Please choose from [{', '.join(FORMAT_TYPES)}]\n")
            format_type_in = get_user_tty_input(format_type_prompt).strip() or cur_format_type

        ConfigCommand.config.format_type = format_type_in

    def _set_org(self, interactive):
        cur_org = ConfigCommand.config.org_name
        if cur_org is None:
            cur_org = "no-org"
        # The keys for 'org_choices' are the full names for the orgs
        org_choices = ConfigCommand.config.get_org_names()
        if not org_choices:
        ...
        ConfigCommand.config.org_display_name = org_display_name
        ConfigCommand.config.org_name = org_name

    def _set_team(self, interactive):
        cur_team = ConfigCommand.config.team_name
        if cur_team is None:
            cur_team = "no-team"
        team_choices = ConfigCommand.config.get_team_list()
        ...
        ConfigCommand.config.team_name = team_in

    def _set_ace(self, interactive):
        cur_ace = ConfigCommand.config.ace_name
        if cur_ace is None:
            cur_ace = "no-ace"

        try:
            ace_choices = ConfigCommand.config.get_ace_list()
        ...
        ConfigCommand.config.ace_name = ace_in
