...
def confirm_remove(target, default):  # noqa: D103
    msg = "Are you sure you would like to remove {}?".format(target)
    if not question_yes_no(msg, default_yes=default):
        raise NgcException("Remove confirmation failed, remove cancelled.")


def confirm(message, default):  # noqa: D103
    if not question_yes_no(message, default_yes=default):
