...
def glob_filter_in_paths(paths, patterns):
    ...
    if patterns:
        ...
        for unmatched_pattern in set(patterns) - matched_patterns:
            get_nvPrettyPrint().print_ok("{} not found, skipping.".format(unmatched_pattern))
    ...
    try:
        for i, entry in enumerate(scantree(path, followlinks=followlinks)):
            ...
            if entry.is_file() and (not entry.is_symlink() or followlinks):
                ...
                if print_paths:
                    get_nvPrettyPrint().print_ok(entry_full_name)
                ...
                get_nvPrettyPrint().print_ok(
                    "Skipping: " + entry_full_name + " (file is a symbolic link that points to nothing.)"
                )
    except NotADirectoryError:
        ...
        if print_paths:
            get_nvPrettyPrint().print_ok(str(file_path))
