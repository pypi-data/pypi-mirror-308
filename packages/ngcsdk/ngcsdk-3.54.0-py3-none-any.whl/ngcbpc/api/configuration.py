...
from ngcbpc.singleton import ParentClassSingleton
from ngcbpc.util.base_utils import get_connection, get_nvPrettyPrint
...
class Configuration(metaclass=ParentClassSingleton):
    ...
    def set_config(self):
        ...
        """  # noqa: D401
        get_nvPrettyPrint().print_ok("Saving configuration...")
        ...
        get_nvPrettyPrint().print_ok(f"Successfully saved NGC configuration to {self._get_config_file()}")
    ...
    def clear_configs(self):
        ...
        get_nvPrettyPrint().print_ok(f"Clearing NGC configuration {cfg_file} and meta data {meta_data_file}")
        self._reset_configs()
        ...
        try:
            ...
            get_nvPrettyPrint().print_ok(f"Successfully cleared NGC configuration {cfg_file}")
        finally:
            ...
            get_nvPrettyPrint().print_ok(f"Successfully cleared NGC meta data {meta_data_file}")
        self.clear_tokens()
    ...
    def clear_tokens():
        ...
        get_nvPrettyPrint().print_ok("Successfully cleared cache values.")
    ...
    def validate_configuration(  # noqa: D102
        ...
        if remote_validation:
            get_nvPrettyPrint().print_ok("Validating configuration...")
        ...
        if remote_validation:
            get_nvPrettyPrint().print_ok("Successfully validated configuration.")
    ...
    def _check_apikey(self, _apikey, remote_validation=False):
        if remote_validation and not ngcbpc.api.authentication.Authentication.validate_api_key(app_key=_apikey):
            raise ValueError(f"Invalid apikey for NGC service location [{self.base_url}].")

    @staticmethod
    def _check_starfleet_kas_session_key():
        if not ngcbpc.api.authentication.Authentication.validate_sf_kas_session_key():
            raise ValueError(
    ...
    def get_org_names(self):  # noqa: D102
        ...
        if self.app_key or (self.starfleet_kas_session_key and self.starfleet_kas_email):
            if self.app_key and self.app_key.startswith(SCOPED_KEY_PREFIX):
                ...
                org_api = ConfigAPI(get_connection())
                try:
    ...
    def get_team_list(self):  # noqa: D102
        ...
        if self.app_key or (self.starfleet_kas_session_key and self.starfleet_kas_email):
            if self.app_key and self.app_key.startswith(SCOPED_KEY_PREFIX):
                ...
                team_api = ConfigAPI(get_connection())
                try:
    ...
    def get_ace_list(self):  # noqa: D102
        _ace_list = []
        if self.app_key or (self.starfleet_kas_session_key and self.starfleet_kas_email):
            ace_api = ConfigAPI(get_connection())
            try:
    ...
    def _set_command_map(self):
        ...
        if self.app_key or (self.starfleet_kas_session_key and self.starfleet_kas_email):
            ...
            try:
                ...
                user_response = ConfigAPI(get_connection()).user_who()
                user_roles = []
                ...
                hidden_roles_response = get_connection().make_api_request(
                ...
                public_roles_response = get_connection().make_api_request(
    ...
    def _get_unified_catalog_product_names(self):
        # Get non-hidden/public roles from API to create command map
        product_names_response = get_connection().make_api_request(
    ...
    @staticmethod
    def _get_sak_key_details(sak_key):
        ...
        sak_key_details = ngcbpc.api.authentication.Authentication.get_sak_key_details(sak_key)  # noqa: E1120
    ...
    def _get_latest_version():
        ...
        try:
            resp = get_connection().make_api_request("GET", get_url, operation_name="get latest version")
...
class ConfigAPI:  # noqa: D101
    ...
    def user_who(self):
        ...
        response = self.connection.make_api_request("GET", request_endpoint, operation_name="get user info")
