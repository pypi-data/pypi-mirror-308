...
class Connection:  # noqa: D101
    ...
    def make_api_request(
        ...
        header_override = Authentication.auth_header(
            auth_org=auth_org,
            auth_team=auth_team,
            scopes=extra_scopes,
            renew=renew_token,
    ...
    def create_full_URL(self, endpoint, safe=None):  # noqa: D102
        return urljoin(self.base_url or self.configuration.base_url, _safe_quote(endpoint, safe))
    ...
    def _request_with_retries(
        ...
        while current_retry > 0:
            try:
                ...
                if self.configuration.format_type == "ascii":
                    # Don't print messages for machine-consumable formats
                    NVPrettyPrint().print_error(
    ...
    def _request_no_retries(
        ...
        try:
            ...
            NVPrettyPrint().print_error("Connection failed; will not retry.")
            if isinstance(e, requests.exceptions.ReadTimeout):
