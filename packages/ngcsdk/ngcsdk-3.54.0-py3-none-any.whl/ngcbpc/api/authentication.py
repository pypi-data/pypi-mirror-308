...
class Authentication:  # noqa: D101
    ...
    @staticmethod
    def _construct_sak_key_details_url():
    ...
    def _get_token(
        ...
        if starfleet_authentication:
            device_login_response = Authentication.starfleet_on_kas_device_login_flow(starfleet_kas_email)
            ...
            try:
                ...
                nv_pp = get_nvPrettyPrint()
                nv_pp.print_ok(starfleet_id_prompt)
            ...
            Authentication.start_polling_session_key(session_key, starfleet_kas_email)

            return [session_key, expires_in]
        if starfleet_kas_session_key:
            starfleet_session_key_expiration = Authentication.config.starfleet_session_key_expiration
    ...
    def get_token(
        ...
        if (
            ...
            or not Authentication.config.starfleet_kas_session_key
            and not Authentication.config.starfleet_kas_email
        ) and not starfleet_authentication:
            app_key = app_key or Authentication.config.app_key
            if not app_key:
                return None

        if app_key:
            token_hash = Authentication._token_hash(app_key, org, team, scopes)
            token, expiration_time = Authentication._get_api_cache_token(
            ...
            expiration_time = Authentication.config.starfleet_session_key_expiration
            try:
                datetime.datetime.fromtimestamp(Authentication.config.starfleet_session_key_expiration).strftime(
                    "%Y-%m-%d::%H:%M:%S"
                )
            ...
            if kas_direct:
                ...
                token = Authentication._get_starfleet_token()
            else:
        ...
        renew = Authentication._check_token_expiration(
        ...
        if renew:
            if app_key:
                ...
                token, expires_in = Authentication._get_token(app_key, org, team, scopes)
            elif starfleet_authentication:
                logger.debug("Starfleet Authentication commencing...")
                token, expires_in = Authentication._get_token(
                    app_key=None,
            ...
            if not starfleet_kas_session_key and not starfleet_authentication:
                if not Authentication.config.sdk_configuration:
    ...
    @staticmethod
    def _get_api_cache_token(app_key, token_hash=None, kas_direct=False):
        if kas_direct or app_key.startswith(SCOPED_KEY_PREFIX):
            ...
            # to be loaded again.
            if not Authentication._cache_loaded and not Authentication.config.sdk_configuration:
    ...
    @staticmethod
    def _check_token_expiration(app_key, expiration_time, token_hash):
        ...
        if expiration_time > 0:
            ...
            if time_left_for_token <= 0:
                ...
                if Authentication.config.app_key and Authentication.config.app_key.startswith(SCOPED_KEY_PREFIX):
    ...
    @staticmethod
    def _get_starfleet_token():
        """Get the Starfleet Token from KAS' `token` endpoint using X-Device-Id and the Session Key."""
        sf_device_id_and_email = f"{SF_DEVICE_ID}-{Authentication.config.starfleet_kas_email}"
        sf_device_id = shortuuid.uuid(name=sf_device_id_and_email)[:19]
        starfleet_header = {
            "X-Device-Id": sf_device_id,
            "Authorization": f"Bearer {Authentication.config.starfleet_kas_session_key}",
        }
        headers = default_headers(starfleet_header)
        token_url = Authentication._construct_starfleet_token_url()
        # ignore SSL warnings
        urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
        try:
    ...
    @staticmethod
    def get_login_url_and_session_key(starfleet_kas_email):
        ...
        device_login_url = f"{Authentication.config.base_url}/{device_login_url}"
    ...
    @staticmethod
    def starfleet_on_kas_device_login_flow(starfleet_kas_email):  # noqa: D102
        return Authentication.get_login_url_and_session_key(starfleet_kas_email)

    @staticmethod
    def start_polling_session_key(session_key, starfleet_kas_email):  # noqa: D102
        sf_device_id_and_email = f"{SF_DEVICE_ID}-{starfleet_kas_email}"
        sf_device_id = shortuuid.uuid(name=sf_device_id_and_email)[:19]
        headers = {
            "Accept": "*/*",
        ...
        ngc_ping_url = f"{Authentication.config.base_url}/v2/ping"
    ...
    @staticmethod
    def auth_header(
        auth_org=None, auth_team=None, scopes=None, app_key=None, renew=False, kas_direct=False, extra_auth_headers=None
    ):
        """Return a dictionary containing the Authorization header for requests."""
        headers = {}
        if Authentication.config.app_key:
        ...
        elif Authentication.config.starfleet_kas_session_key and Authentication.config.starfleet_kas_email:
            auth_token = Authentication.get_token(
                starfleet_kas_session_key=Authentication.config.starfleet_kas_session_key,
                starfleet_kas_email=Authentication.config.starfleet_kas_email,
                kas_direct=kas_direct,
                renew=renew,
            )
            sf_device_id_and_email = f"{SF_DEVICE_ID}-{Authentication.config.starfleet_kas_email}"
    ...
    @staticmethod
    def validate_api_key(app_key):
        """Retrieves new, refreshed Token."""  # noqa: D401
        try:
            Authentication.get_token(app_key=app_key, org=None, team=None, renew=True)
            return True
        except AuthenticationException:
            return False

    @staticmethod
    def validate_sak_key(sak_key):
        """Validates the Scoped API Key (SAK)"""  # noqa: D401 D415
        try:
            Authentication.get_sak_key_details(sak_key=sak_key)
            return True
        except AuthenticationException:
            return False

    @staticmethod
    def validate_sf_kas_session_key():
    ...
    @staticmethod
    def get_sak_key_details(sak_key):
        ...
        sak_key_details_url = Authentication._construct_sak_key_details_url()
    ...
    @staticmethod
    def get_starfleet_kas_session_key(starfleet_kas_email):
