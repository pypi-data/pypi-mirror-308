...
class ModelPrinter(NVPrettyPrint):
    ...
    def print_model_list(self, model_list, columns=None):
        ...
        if self.config.format_type == "json":
            ...
            output = self.generate_model_list(model_list, self.config, columns)
    ...
    def print_model_version_list(self, model_version_list, columns=None):
        ...
        if self.config.format_type == "json":
            ...
            output = self.generate_model_list([model_version_list], self.config, columns)
        self.print_data(output, is_table=True)

    @staticmethod
    def generate_model_list(gen, config, columns):  # noqa: D102
        cols, disp = zip(*columns)
        yield list(disp)

        for page in gen or []:
            for model in page or []:
                out = ModelOutput(model, config)
...
class ModelOutput:  # noqa: D101
    ...
    def permission(self):  # noqa: D102
        return derive_permission(self.model.guestAccess, self.config.is_guest_mode)
