...
class CspPrinter(NVPrettyPrint):
    ...
    def print_csp_list(self, csp_list_gen):
        ...
        output = self.generate_csp_list(csp_list_gen, self.config, columns)
        self.print_data(output, is_table=True)

    @staticmethod
    def generate_csp_list(gen, config, columns):  # noqa: D102
        cols, disp = zip(*columns)
        yield list(disp)

        for page in gen or []:
            for csp in page.cloudServiceProviders or []:
                out = CspOutput(csp, config)
                yield [getattr(out, col, None) for col in cols]


class CspOutput:  # noqa: D101
    def __init__(self, csp, config=None):
        self.csp = csp
        self.config = config
