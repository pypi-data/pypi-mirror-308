...
class ResourcePrinter(NVPrettyPrint):
    ...
    def print_resource_list(self, resource_list, columns=None):
        ...
        if self.config.format_type == "json":
            ...
            output = self.generate_resource_list(resource_list, self.config, columns)
    ...
    def print_resource_version_list(self, version_list, columns=None):
        ...
        if self.config.format_type == "json":
            ...
            output = self.generate_resource_list([version_list], self.config, columns)
        self.print_data(output, is_table=True)

    @staticmethod
    def generate_resource_list(gen, config, columns):  # noqa: D102
        cols, disp = zip(*columns)
        yield list(disp)

        for page in gen or []:
            for resource in page or []:
                out = ResourceOutput(resource, config)
                yield [getattr(out, col, None) for col in cols]

    def create_resource_table(self, gen, config, columns):  # noqa: D102
        table = self.create_table_columns(columns)
        for page in gen or []:
            for resource in page or []:
                new_vals = []
                output = ResourceOutput(resource, config)
...
class ResourceOutput:  # noqa: D101
    ...
    def permission(self):  # noqa: D102
        return derive_permission(self.resource.guestAccess, self.config.is_guest_mode)
