...
class PublishAPI:  # noqa: D101
    ...
    def __init__(self, connection):
        ...
        self.publish_printer = PublishPrinter()
    ...
    def publish(
        ...
        if metadata_only:
            ...
            if resp.requestStatus and resp.requestStatus.statusCode == "ACCEPTED":
                workflow_id = resp.requestStatus.workflowId
                self.publish_printer.print_ok(
                    (
                    ...
                    ).format(target, COMMAND_NAME_MAP[artifact_type], workflow_id)
                )
                return
