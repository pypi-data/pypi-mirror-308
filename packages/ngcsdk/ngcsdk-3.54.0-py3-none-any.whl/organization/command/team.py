...
from ngcbpc.command.completers import team_completer
...
class TeamCommand(CLICommand):  # noqa: D101
    ...
    def __init__(self, parser):
        ...
        self.printer = OrgTeamUserPrinter()

    if bool(Configuration().user_role_choices):
        user_roles = Configuration().user_role_choices
    else:
    ...
    if get_environ_tag() <= STAGING_ENV:
        ...
        role_choices = sorted(list(user_roles_set | user_roles_forge_set))
    else:
        # Make sure Forge Roles are not included.
        user_roles = user_roles_set - user_roles_forge_set
        role_choices = sorted(list(user_roles))
    ...
    create_role_help_str = (
        f"Specify the user role. Options: [{', '.join(role_choices)}]. "
    ...
    update_add_role_help_str = (
        f"Specify the user role to assign. Options: [{', '.join(role_choices)}]. "
        "To specify more than one role, use multiple --add-role arguments."
    )
    update_remove_role_help_str = (
        f"Specify the user role to remove. Options: [{', '.join(role_choices)}]. "
    ...
    def remove_user(self, args):  # noqa: D102
        ...
        if team_name is None:
            raise Exception("Provide team name using --team option or set team name using `ngc config set`.")
        confirm_remove("user", args.default_yes)
    ...
    def delete_invitation(self, args):  # noqa: D102
        ...
        answer = question_yes_no(msg, default_yes=args.default_yes)
        if answer:
