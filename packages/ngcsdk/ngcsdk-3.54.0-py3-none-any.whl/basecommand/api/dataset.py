...
class DatasetAPI:  # noqa: D101
    def __init__(self, connection, api_client=None, dataset_service_connection=None):
        ...
        self.search_api = BaseCommandSearchAPI(connection=connection)
        self.org_api = OrgAPI(connection=connection)
        self.client = api_client
        self.printer = DatasetPrinter()
        ...
        if enable_paging:
            ...
            if dataset_service_enabled:
                ...
                if dataset_files_url:
                    async_downloader = AsyncDownload(connection, dataset_id, org_name=org_name, team_name=team_name)
    ...
    def download(
        ...
        if storage_type == STORAGE_TYPE_OBJECT:
            ...
            async_downloader = AsyncDownload(self.connection, download_id, org_name=org_name, team_name=team_name)
    ...
    def remove(
        ...
        for ds_id in ids:
            try:
                ...
                confirm_remove(f"the dataset {ds_id}", default_yes)
