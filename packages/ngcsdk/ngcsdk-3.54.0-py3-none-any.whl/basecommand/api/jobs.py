...
class JobsAPI:  # noqa: D101
    def __init__(self, connection, api_client=None):
        ...
        self.printer = NVPrettyPrint()
        self.jobs_to_kill = False
    ...
    def _get_job_create_request(self, args):  # noqa: C901
        ...
        if args.file:
            ...
            parsed = self._get_parsed_json(self.config.org_name, args.clone, self.config.team_name)
            job_create_request = JobCreateRequest(parsed)

        job_create_request.aceName = self.config.ace_name or job_create_request.aceName
    ...
    def kill(
        ...
        if self.jobs_to_kill is False and dry_run is False:
    ...
    def log(
        ...
        transfer_config = TransferConfig(url=log_url)
        controller = TransferController(job_id, transfer_config)
        try:
