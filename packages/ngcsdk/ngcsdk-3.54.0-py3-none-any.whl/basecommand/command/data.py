...
from basecommand.command.completers import dataset_id_completer, job_id_completer
...
class DataCommand(BaseCommand, CLICommand):  # noqa: D101
    ...
    def __init__(self, parser):
        ...
        self.search_api = Client(api_key=self.config.app_key, base_url=self.config.base_url).basecommand.search
    ...
    def share(self, args):  # noqa: D102
        ...
        answer = question_yes_no(share_str, default_yes=args.default_yes)
        if answer:
    ...
    @CLICommand.arguments(
        ...
        action=valid_value(1, MAX_UPLOAD_THREADS, DatasetPrinter()),
    )
    @CLICommand.arguments(
        "-y",
        "--yes",
