...
from basecommand.command.completers import job_id_completer, workspace_id_completer
from basecommand.command.data import dataset_id_completer
...
class Batch(BaseCommand, CLICommand):  # noqa: D101
    ...
    def __init__(self, parser):
        ...
        self.client = Client(api_key=self.configuration.app_key, base_url=self.configuration.base_url)
        self.batch_printer = BatchPrinter()
    ...
    def attach(self, args):  # noqa: D102
        ...
        try:
            ...
            kube_wrapper = KubeWrapper(org_name, team_name, job_target.job_id, proxy_ip, container_id, replica_id)
            kube_wrapper.attach()
            return
        except ResourceNotFoundException:
    ...
    def _exec_into_job(self, jobid, command, replica_id=None, detach=False):
        ...
        try:
            ...
            kube_wrapper = KubeWrapper(org_name, team_name, jobid, proxy_ip, container_id, replica_id)
            return kube_wrapper.exec_(command)
        except ResourceNotFoundException:
            logger.debug("Kube exec not found, trying docker.")
            with ContainerWrapper(
                org_name, team_name, jobid, docker_daemon_ip, proxy_ip, cluster_id, replica_id
            ) as container_wrapper:
