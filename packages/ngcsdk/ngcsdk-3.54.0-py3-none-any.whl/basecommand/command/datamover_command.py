...
class DataMoverCommand(BaseCommand):  # noqa: D101
    ...
    def __init__(self, parser):
        ...
        self.data_mover_api = Client(api_key=self.config.app_key, base_url=self.config.base_url).basecommand.data_mover
        self.printer = DataMoverPrinter()
    ...
    def enqueue(self, args):  # noqa: C901, D102
        ...
        if import_job:
            ...
            if not question_yes_no(
                prompt_msg + " Are you sure you want to continue?", default="no", default_yes=args.default_yes
            ):
