...
class TextCommandParser:
    ...
    def __init__(self, parent):
        ...
        self.interpreters = {
            ...
            # BACKEND options: openai, groq, mistral
            ...
            "iotv": (self.textAnswerIOTV, """
            # [KEYWORD] IOTV
            # Feature - Interpret an Old Testament Verse.
            # Usage - IOTV:::[BACKEND]:::[BIBLE]:::[BIBLE_VERSE]
            # BACKEND options: openai, groq, mistral
            # Use value of `config.llm_backend` as backend if BACKEND is not given.
            # e.g. IOTV:::Deut 6:4"""),
            "intv": (self.textAnswerINTV, """
            # [KEYWORD] INTV
            # Feature - Interpret a New Testament Verse.
            # Usage - INTV:::[BACKEND]:::[BIBLE]:::[BIBLE_VERSE]
            # BACKEND options: openai, groq, mistral
            # Use value of `config.llm_backend` as backend if BACKEND is not given.
            # e.g. INTV:::John 3:16"""),
            "map": (self.textMap, """
...
The WHERE condition is described as: {query}"""
    ...
    # IOTV
    def textAnswerIV(self, command, source, systemMessage):
        try:
            if not re.search("^(groq|mistral|openai):::", command):
                command = f"{config.llm_backend}:::{command}" if ":::" in command else f"{config.llm_backend}:::{config.mainText}:::command"
            elif command.count(":::") == 1:
                command = re.sub("^(groq|mistral|openai):::", rf"\1:::{config.mainText}:::", command)
        ...
        _, textOutput, _ = self.textAnswer(textInput, source, systemMessage)
        if config.rawOutput:
            textOutput = f"""# {verseReference}
...
{textOutput}"""
            ...
            textOutput = f"""<h2>{verseReference}</h2>
...
{textOutput}"""
    ...
    # IOTV
    def textAnswerIOTV(self, command, source):
        return self.textAnswerIV(command, source, config.answer_systemMessage_interpretot)
    # INTV
    def textAnswerINTV(self, command, source):
        return self.textAnswerIV(command, source, config.answer_systemMessage_interpretnt)
        ...
        if command.strip():
            ...
            if not config.rawOutput:
                ...
                if config.runMode == "http-server":
                    textOutput += """<hr><p><button type='button' onclick='history.back()' class='ubaButton'>{0}</button></p>""".format(config.thisTranslation["youtube_back"])
                # Disclaimer
