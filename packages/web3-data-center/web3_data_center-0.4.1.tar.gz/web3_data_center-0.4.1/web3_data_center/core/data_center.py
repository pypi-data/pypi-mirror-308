...
class DataCenter:
    ...
    async def is_token_rugged(self, token_contract: str, chain: str = 'eth') -> bool:
        try:
            # use web3 to check if the pair's reserve is rugged
            pair_address = await self.get_pairs_info(token_contract)
            # pair_address = await self.calculate_pair_address(token_contract, token_contract, pair_type)
            return await self.is_pair_rugged(pair_address, pair_type, chain)
        except Exception as e:
            logger.error(f"Error checking pair rugged: {str(e)}")
            return None
