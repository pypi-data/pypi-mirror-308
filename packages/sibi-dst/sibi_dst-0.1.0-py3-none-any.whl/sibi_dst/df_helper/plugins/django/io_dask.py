...
class ReadFrameDask:
    ...
    def read_frame(self, fillna_value=None):
        ...
        if datetime_index:
            dask_df = dask_df.map_partitions(lambda x: x.set_index(pd.to_datetime(x.index, errors="ignore")))

        return dask_df

class ReadFrameDask2:
    FieldDoesNotExist = (
        django.db.models.fields.FieldDoesNotExist
        if django.VERSION < (1, 8)
        else django.core.exceptions.FieldDoesNotExist
    ...
    def read_frame(self):
        ...
        if datetime_index:
            df = df.map_partitions(lambda x: x.set_index(pd.to_datetime(x.index, errors="ignore")))
