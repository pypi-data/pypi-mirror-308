				...
				while True:
					try:
						if lunch_string != None:
							print(lunch_string)
							try:
								if lunch_string.lower() in ['no-lunch','no_lunch','nl']:
...
{Style.bold}Bold{Style.reset} - Shifts that occur after 12PM
{Style.dim}Dim{Style.reset} - Shifts that occur before 12PM
		"""
		print(header)
			...
			for num,i in enumerate(RS):
				...
				if dpt.dptId in whichDpts:
					...
					if i.ShiftStart.hour >= 12:
						...
						dayEnd=f'{Style.dim}{Style.underline}'
