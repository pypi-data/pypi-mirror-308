...
class EthernityCloudRunner:
    ...
    def dispatch_ec_event(
        ...
        if (log_level >= self.log_level):
            if (log_level == ECLog.DEBUG):
                debug_details = 'Event: {event_type}, Message: {message}, Status: {status}'
    ...
    def approve_order(self, order: int) -> bool:
        ...
        for i in range(100):
            try:
                ...
                self.dispatch_ec_event(
                    f"{transaction_hash} was confirmed!",ECStatus.DEFAULT,ECEvent.TASK_PROGRESS,ECLog.INFO
                )
                break
        ...
        try:
            ...
            if is_node_operator_address:
                ...
                if connected:
                    ...
                    self.dispatch_ec_event(
                        "Unable to connect to MetaMask", ECStatus.ERROR,ECEvent.TASK_NEW,ECLog.ERROR
                    )
        except Exception as e:
