...
class EthernityCloudRunner:
    ...
    def create_do_request(
        ...
        try:
            ...
            for i in range(100):
                try:
                    self.dispatch_ec_event(
                        f"{transaction_hash} is pending...",ECStatus.DEFAULT,ECEvent.TASK_NEW,ECLog.INFO
                    )
    ...
    def get_result_from_order(self, order_id: int) -> Any:
        ...
        try:
            ...
            self.dispatch_ec_event(
                f"Downloading {parsed_order_result['result_ipfs_hash']} ...",ECStatus.SUCCESS,ECEvent.TASK_COMPLETED,ECLog.INFO
            )
            ipfs_result = ipfsClient.get_file_content(
                parsed_order_result["result_ipfs_hash"]
            )
            self.dispatch_ec_event(
                f"Validating proof result...",ECStatus.SUCCESS,ECEvent.TASK_COMPLETED,ECLog.INFO
            )
            ...
            self.dispatch_ec_event(
                f"Collecting result transaction hash...",ECStatus.SUCCESS,ECEvent.TASK_COMPLETED,ECLog.INFO
            )
