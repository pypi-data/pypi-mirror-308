...
import soundfile
from smyoon_sense_api import ApiClient, Configuration
from smyoon_sense_api.api.audio_session_api import AudioSessionApi, SessionResult
...
from smyoon_sense_api.model.window_hop import WindowHop as ClientWindowHop

from . import config
from .exceptions import SmyoonSenseException, TimeoutError
...
class FileClient:
    """FileClient is used to predict an audio file with Smyoon.Sense."""
    ...
    def _get_all_results(self, session_id: str, timeout: Optional[float] = None) -> FileResult:
        ...
        while True:
            ...
            if (state is None) or (has_more is None) or (type(has_more) is not bool):
                raise SmyoonSenseException(f"invalid Get Result API response: {response}")

            if state == "error":
                error_msg = response.get("error", None)
                if not error_msg:
                    error_msg = "Unexpected error occured."
                raise SmyoonSenseException(error_msg)
                ...
                raise SmyoonSenseException(f"invalid Get Result API response state: {state}")
