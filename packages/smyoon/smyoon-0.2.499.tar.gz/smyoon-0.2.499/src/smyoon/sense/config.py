...
class SensitivityScale(Enum):
    ...
    It means that Smyoon.Sense API determines a sound tag is detected even though its confidence is low.
    """
...
class ChunkSize(Enum):
    ...
    A file is split into multiple chunks before it is uploaded to Smyoon.Sense cloud.
...
class APIConfig:
    def __init__(
        ...
        """
        Args:
            window_hop: WindowHop to apply in Smyoon.Sense API Client.
            sensitivity: SensitivityConfig to apply in Smyoon.Sense API Client.
        """
    ...
    def get_host(self) -> str:
        return "https://api.smyoon.ai/sense/api/v1" if self.host is None else self.host
