...
class Cat(nn.Module):
    ...
    - Only evaluates the requantizer if there is a need to do so (removes unecessary shift-by-zeros)
    - With mixed-precision inputs, the output precision is the maximum precision
        of the inputs
    - Uses fixed-quanta observers, where the quanta is the maximum of the inputs
        (corrected for any differences in input precisions)
    """

    def __init__(self, dim):
        super().__init__()
        self.requantizer = RequantizeFromBitwidthQuanta()
        self.dim = dim
        self.cat = BareCat(dim=self.dim)

    def get_precision_quanta(self, *inputs: Tensor):
        # select the highest precision
        # and pick the quanta that covers the largest-dynamic-range input
        bw = 0
        log2_maxabs = -float("inf")
        for x in inputs:
            bw = max(bw, x.bitwidth.bitwidth)
            log2_maxabs = max(log2_maxabs, x.bitwidth.bitwidth + x.quanta - 1)
        quanta = log2_maxabs - bw + 1
        precision = {8: fqint8, 16: fqint16, 24: fqint24}[bw]

        if isinstance(quanta, Tensor):
            quanta = quanta.detach().cpu().item()

        return precision, quanta
    ...
    def forward(self, tensors: list[Tensor]) -> Tensor:
        ...
        quantized = all(xx.quantized for xx in tensors)
        if quantized:
            bitwidth, quanta = self.get_precision_quanta(*tensors)
        else:
            bitwidth, quanta = None, None

        if quantized:
            to_cat = []
            for x_i in tensors:
                if x_i.quanta != quanta or x_i.bitwidth != bitwidth:
                    to_cat.append(self.requantizer(x_i, bitwidth, quanta))
                else:
                    to_cat.append(x_i)
        else:
            to_cat = tensors

        return self.cat(*to_cat)
...
class RequantizeFromBitwidthQuanta(AtomicModule):
    ...
    def forward(self, x: Tensor, bitwidth: Bitwidth, quanta: float):
        logger.debug(f"requantize: hasattr(x, proto) = {hasattr(x, 'proto')}")
        if x.quantized:
            y = fake_quantize(
