...
def adjust_features_sparse(X_test, model):
    ...
    #print(f"Number of model features: {num_features}")
    #print(f"Current X_test features: {current_features}")

    if current_features < num_features:
        # Compute the padding required
        padding = num_features - current_features
        #print(f"Padding required: {padding}")

        # Create a sparse matrix with padding columns
        padding_array = csr_matrix((X_test.shape[0], padding), dtype=X_test.dtype)

        # Concatenate along the columns
        X_test_adjusted = hstack([X_test, padding_array])
        #print(f"New X_test shape after padding: {X_test_adjusted.shape}")

    elif current_features > num_features:
        # Truncate X_test to match the number of features
        X_test_adjusted = X_test[:, :num_features]
        #print(f"New X_test shape after truncation: {X_test_adjusted.shape}")

    else:
        # No adjustment needed
        X_test_adjusted = X_test
        #print(f"No adjustment needed. X_test shape: {X_test_adjusted.shape}")
