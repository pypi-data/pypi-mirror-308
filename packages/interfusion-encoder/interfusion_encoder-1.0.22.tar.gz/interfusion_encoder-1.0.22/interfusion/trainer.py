...
def train_model(candidates, jobs, positive_matches, candidates_eval=None, jobs_eval=None, positive_matches_eval=None, user_config=None):
    ...
    # Build a list of all candidate IDs and job IDs
    all_candidate_ids = list(candidate_id_to_text.keys())
    all_job_ids = list(job_id_to_text.keys())
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if config.get('train_on_job_to_candidate', False):
            # Sample of jobs
            sampled_job_ids = random.sample(all_job_ids, k=int(len(all_job_ids)*sample_amount))
            sampled_job_set = set(sampled_job_ids)

            # Build data_samples for sampled jobs
            data_samples_epoch = []
            for match in positive_matches:
                job_id = match['job_id']
                if job_id in sampled_job_set:
                    candidate_id = match['candidate_id']
                    data_samples_epoch.append({
                        'job_id': job_id,
                        'job_text': job_id_to_text[job_id],
                        'positive_candidate_id': candidate_id,
                        'positive_candidate_text': candidate_id_to_text[candidate_id],
                        'job_features': job_id_to_features.get(job_id, None),
                        'positive_candidate_features': candidate_id_to_features.get(candidate_id, None)
                    })

            # Build positive_matches_epoch
            positive_matches_epoch = [match for match in positive_matches if match['job_id'] in sampled_job_set]

            # Build sampled_jobs
            sampled_jobs = [job for job in jobs if job['job_id'] in sampled_job_set]

            # Precompute negatives using bi-encoder
            print("Precomputing negatives using bi-encoder...")
            negatives = precompute_bi_encoder_negatives(bi_encoder, tokenizer, sampled_jobs, candidates, positive_matches_epoch, config)

            # Generate initial hard negatives
            print("Generating initial hard negatives...")
            hard_negatives = generate_hard_negatives(model, data_samples_epoch, tokenizer, negatives, config, job_feature_size, candidate_feature_size)

            # Precompute N random negatives per job
            print("Precomputing random negatives...")
            random_negatives = precompute_random_negatives(sampled_jobs, candidates, positive_matches_epoch, config)
...
def train(model, train_dataset, optimizer, device, config, epoch, scaler, scheduler):
    ...
    optimizer.zero_grad()
    accumulation_steps = config['accumulation_steps']  # Update weights every 4 batches
    ...
    for batch_idx, batch in enumerate(tqdm(train_dataloader, desc=f"Training Epoch {epoch+1}")):
        ...
        with torch.cuda.amp.autocast():
            ...
            for candidate_id, indices in candidate_to_indices.items():
                candidate_logits = logits[indices]
