    ...
    def fit_cols(self, attributes, x, n_vals):
        ...
        def nowarn(msg, cat, *args, **kwargs):
            if cat is RuntimeWarning and msg in (
                ...
                if self.normalize:
                    raise ValueError(_tr.m[89, "some columns have no defined values"])
            else:
        ...
        if self.normalize and not stdvars.all():
            raise ValueError(_tr.m[90, "some columns are constant"])
...
class Manhattan(FittedDistance):
    ...
    def fit_cols(self, attributes, x, n_vals):
        ...
        if self.normalize and (np.isnan(mads).any() or not mads.all()):
            raise ValueError(
                (_tr.m[91, "some columns have zero absolute distance from median, "] + _tr.m[92, "or no values"]))
...
class Mahalanobis(Distance):
    ...
    def fit(self, data):
        ...
        try:
            ...
            raise MemoryError(_tr.m[93, "Covariance matrix is too large."])
        try:
            vi = np.linalg.inv(c)
        except:
            raise ValueError(_tr.m[94, "Computation of inverse covariance matrix failed."])
