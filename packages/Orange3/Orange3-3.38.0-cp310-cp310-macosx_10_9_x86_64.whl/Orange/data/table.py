...
class Table(Sequence, Storage):
    ...
    def unlocked(self, *parts):
        ...
        for part, flag, name in self._lock_parts_val():
            if not flag & self._unlocked \
                ...
                raise ValueError((f"'{name}' is a view into another table " + "and cannot be unlocked"))
        return contextmanager(self.__unlocked)(*parts)

    def __new__(cls, *args, **kwargs):
        def warn_deprecated(method):
            warnings.warn(("Direct calls to Table's constructor are deprecated " + ("and will be removed. Replace this call with " + f"Table.{method}")), OrangeDeprecationWarning,
                          stacklevel=3)
        ...
        if isinstance(args[0], str):
            ...
            warnings.warn(("Omitting domain in a call to Table(X, Y, metas), is " + ("deprecated and will be removed. " + "Call Table.from_numpy(None, X, Y, metas) instead.")),
    ...
    def from_numpy(cls, domain, X, Y=None, metas=None, W=None,
        ...
        if Y.ndim == 1:
            if not domain.class_var:
                raise ValueError(
                    ("Invalid number of class columns " + f"(1 != {len(domain.class_vars)})"))
        ...
        if isinstance(row_idx, Integral):
            ...
            if not isinstance(col_idx, str) and isinstance(col_idx, Sized):
                if isinstance(value, (Sequence, np.ndarray)):
                    ...
                    raise TypeError(("Setting multiple values requires a " + "sequence or numpy array"))
    ...
    def concatenate(cls, tables, axis=0, *, ignore_domains=None):
        ...
        names = [table.name for table in tables if table.name != _tr.m[84, "untitled"]]
        if names:
            conc.name = names[0]
        return conc
    ...
    def get_column_view(self, index: Union[Integral, Variable]) -> np.ndarray:
        ...
        if sparse:
            ...
            warnings.warn(("get_column_view is returning a dense copy column " + f"{index}"))
        ...
        if isinstance(index, DiscreteVariable) \
            ...
            warnings.warn(("get_column_view is returning a mapped copy of " + f"column {index.name}"))
        return col, sparse
    ...
    def set_column(self, index: Union[int, str, Variable], data):
        ...
        if not isinstance(index, Integral):
            if isinstance(index, DiscreteVariable) \
                ...
                raise ValueError((f"cannot set data for variable {index.name} " + "with different encoding"))
            index = self.domain.index(index)
    ...
    def _compute_contingency(self, col_vars=None, row_var=None):
        ...
        if any(not (var.is_discrete or var.is_continuous)
               for var in col_desc):
            raise ValueError(("Contingency can be computed only for categorical " + "and numeric values."))
    ...
    def transpose(cls, table, feature_names_column="",
                  meta_attr_name=_tr.m[85, "Feature name"], feature_name="Feature",
