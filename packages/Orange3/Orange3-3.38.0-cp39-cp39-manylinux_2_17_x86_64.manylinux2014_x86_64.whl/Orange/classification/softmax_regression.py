...
class SoftmaxRegressionLearner(Learner):
    ...
    def fit(self, X, Y, W=None):
        if len(Y.shape) > 1:
            raise ValueError((_tr.m[68, 'Softmax regression does not support '] + _tr.m[69, 'multi-label classification']))

        if np.isnan(np.sum(X)) or np.isnan(np.sum(Y)):
            raise ValueError((_tr.m[70, 'Softmax regression does not support '] + _tr.m[71, 'unknown values']))
