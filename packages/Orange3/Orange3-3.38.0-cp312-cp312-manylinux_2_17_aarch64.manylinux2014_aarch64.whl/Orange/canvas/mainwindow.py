...
class OUserSettingsDialog(UserSettingsDialog):
    def __init__(self, *args, **kwargs):
        ...
        cb = QCheckBox(self.tr(_tr.m[11, "Automatically check for updates"]))
        cb.setAttribute(Qt.WA_LayoutUsesWidgetRect)

        layout.addRow(_tr.m[12, "Updates"], cb)
        self.bind(cb, "checked", "startup/check-updates")

        # Reporting Tab
        tab = QWidget()
        self.addTab(tab, self.tr(_tr.m[13, "Reporting"]),
                    toolTip=_tr.m[14, "Settings related to reporting"])

        form = FormLayout()
        line_edit_mid = QLineEdit()
        self.bind(line_edit_mid, "text", "reporting/machine-id")
        form.addRow(_tr.m[15, "Machine ID:"], line_edit_mid)

        box = QWidget()
        layout = QVBoxLayout()
        layout.setContentsMargins(0, 0, 0, 0)
        cb1 = QCheckBox(
            self.tr(_tr.m[16, "Share"]),
            toolTip=self.tr(
                _tr.m[17, "Share anonymous usage statistics to improve Orange"])
        )
        self.bind(cb1, "checked", "reporting/send-statistics")
        cb1.clicked.connect(UsageStatistics.set_enabled)
        layout.addWidget(cb1)
        box.setLayout(layout)
        form.addRow(self.tr(_tr.m[18, "Anonymous Statistics"]), box)
        label = QLabel(("<a " + ("href=\"https://orange.biolab.si/statistics-more-info\">" + (_tr.m[19, "More info..."] + "</a>"))))
        ...
        # Notifications Tab
        tab = QWidget()
        self.addTab(tab, self.tr(_tr.m[20, "Notifications"]),
                    toolTip=_tr.m[21, "Settings related to notifications"])
        ...
        layout.setContentsMargins(0, 0, 0, 0)
        cb = QCheckBox(
            self.tr(_tr.m[22, "Enable notifications"]), self,
            toolTip=_tr.m[23, "Pull and display a notification feed."]
        )
        self.bind(cb, "checked", "notifications/check-notifications")

        layout.addWidget(cb)
        box.setLayout(layout)
        form.addRow(self.tr(_tr.m[24, "On startup"]), box)

        notifs = QWidget(self, objectName="notifications-group")
        notifs.setLayout(QVBoxLayout())
        notifs.layout().setContentsMargins(0, 0, 0, 0)

        cb1 = QCheckBox(self.tr(_tr.m[25, "Announcements"]), self,
                        toolTip=(_tr.m[26, "Show notifications about Biolab announcements.\n"] + (_tr.m[27, "This entails events and courses hosted by the developers of "] + _tr.m[28, "Orange."])))

        cb2 = QCheckBox(self.tr(_tr.m[29, "Blog posts"]), self,
                        toolTip=(_tr.m[30, "Show notifications about blog posts.\n"] + _tr.m[31, "We'll only send you the highlights."]))
        cb3 = QCheckBox(self.tr(_tr.m[32, "New features"]), self,
                        toolTip=(_tr.m[33, "Show notifications about new features in Orange when a new "] + (_tr.m[34, "version is downloaded and installed,\n"] + _tr.m[35, "should the new version entail notable updates."])))
        ...
        form.addRow(self.tr(_tr.m[36, "Show notifications about"]), notifs)
        tab.setLayout(form)
...
class MainWindow(OWCanvasMainWindow):
    ...
    def open_canvas_settings(self):
        # type: () -> None
        """Reimplemented."""
        dlg = OUserSettingsDialog(self, windowTitle=self.tr(_tr.m[37, "Preferences"]))
        dlg.show()
        status = dlg.exec()
        if status == 0:
