...
class CSVReader(FileFormat, DataTableMixin):
    ...
    EXTENSIONS = ('.csv',)
    DESCRIPTION = _tr.m[74, 'Comma-separated values']
    DELIMITERS = ',;:\t$ '
            ...
            with self.open(self.filename, mode='rt', newline='',
                ...
                try:
                    ...
                    if error and isinstance(error, UnicodeDecodeError):
                        ...
                        warning = ('Skipped invalid byte(s) in position ' + '{}{}').format(pos,
...
class TabReader(CSVReader):
    ...
    EXTENSIONS = ('.tab', '.tsv')
    DESCRIPTION = _tr.m[75, 'Tab-separated values']
    DELIMITERS = '\t'
    PRIORITY = 10
...
class PickleReader(FileFormat):
    ...
    DESCRIPTION = _tr.m[76, 'Pickled Orange data']
    SUPPORT_COMPRESSED = True
    ...
    def read(self):
        with self.open(self.filename, 'rb') as f:
            ...
            if not isinstance(table, Table):
                raise TypeError(_tr.m[77, "file does not contain a data table"])
            else:
...
class BasketReader(FileFormat):
    ...
    DESCRIPTION = _tr.m[78, 'Basket file']
    SUPPORT_SPARSE_DATA = True
...
class _BaseExcelReader(FileFormat, DataTableMixin):
    ...
    def read(self):
        try:
            ...
            raise IOError(_tr.m[79, "Couldn't load spreadsheet from "] + self.filename)
        return table
...
class ExcelReader(_BaseExcelReader):
    ...
    EXTENSIONS = ('.xlsx',)
    DESCRIPTION = _tr.m[80, 'Microsoft Excel spreadsheet']
...
class XlsReader(_BaseExcelReader):
    ...
    EXTENSIONS = ('.xls',)
    DESCRIPTION = _tr.m[81, 'Microsoft Excel 97-2004 spreadsheet']
...
class UrlReader(FileFormat):
    ...
    def _trim_googlesheet(url):
        match = re.match((r'(?:https?://)?(?:www\.)?' + (r'docs\.google\.com/spreadsheets/d/' + (r'(?P<workbook_id>[-\w_]+)' + r'(?:/.*?gid=(?P<sheet_id>\d+).*|.*)?'))),
                         url, re.IGNORECASE)
        try:
