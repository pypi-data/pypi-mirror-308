...
def _get_sheet(filename, sheet_name):
    ...
    if sheet_name not in workbook.sheetnames:
        raise ValueError(_tr.e(_tr.c(124, f"No such sheet: {sheet_name}")))
...
def _non_empty_cells(sheet):
    ...
    """
    def raise_empty():
        raise ValueError(_tr.m[125, "empty sheet"])
    ...
    for y, row in enumerate(cells):
        for x, value in enumerate(row):
            ...
            try:
                ...
                raise ValueError(
                    (_tr.m[126, "invalid data in cell "] + (f"{openpyxl.utils.get_column_letter(x + col_offset + 1)}" + f"{y + row_offset + 1}"))) from None
    return matrix
