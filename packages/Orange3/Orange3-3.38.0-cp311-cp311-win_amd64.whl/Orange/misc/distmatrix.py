...
class DistMatrix(np.ndarray):
    ...
    def _from_dst(cls, filename):
        ...
        with open(filename, encoding=detect_encoding(filename)) as fle:
            line = fle.readline()
            if not line:
                raise ValueError(_tr.m[127, "empty file"])
            data = line.strip().split()
            if not data[0].strip().isdigit():
                raise ValueError(_tr.m[128, "distance file must begin with dimension"])
            n = int(data.pop(0))
            symmetric = True
            axis = 1
            ...
            if col_labels is not None:
                ...
                if len(col_labels) != n:
                    raise ValueError((_tr.m[129, "mismatching number of column labels, "] + f"{len(col_labels)} != {n}"))
            ...
            for i, line in enumerate(fle):
                if i >= n:
                    raise ValueError(_tr.m[130, "too many rows"])
                line = line.strip().split("\t")
                if row_labels is not None:
                    row_labels.append(line.pop(0).strip())
                if len(line) > n:
                    raise ValueError(
                        (_tr.e(_tr.c(131, f"too many columns in matrix row ")) + f"{num_or_lab(i, row_labels)}"))
                for j, e in enumerate(line[:i + 1 if symmetric else n]):
                    try:
                        matrix[i, j] = float(e)
                    except ValueError as exc:
                        raise ValueError(
                            (_tr.m[132, "invalid element at "] + (_tr.e(_tr.c(133, f"row {num_or_lab(i, row_labels)}, ")) + _tr.e(_tr.c(134, f"column {num_or_lab(j, col_labels)}"))))) from exc
                    if symmetric:
