...
class VertexAIProvider(ProviderCore):
                                    ...
                                    tool_calls=[
                                        ChoiceDeltaToolCall(
                                            ...
                                            function=ChoiceDeltaToolCallFunction(
                                                name=functioncall["functionCall"].get('name'),
                                                arguments="",
                                                type="function",
                                            ),
                                        )
    ...
    def _create_request_payload(
        ...
        if isinstance(input_data, str):
            return self._create_vertexai_payload(user_payload=input_data,tool_payload=tool_payload)
            ...
            for message in input_data:
                ...
                if message.get("role") in ["user","assistant"]:
                    if message.get("tool_calls"):
                        ...
                        payload["contents"].append(
                        {"role": message.get("role"), "parts": [{"text": message["content"]}]}
                        )
                elif message.get("role") == "tool":
                    function_name = message["name"]
                    response = message["content"]
                    payload["system_instruction"]["parts"]["text"] += f"\nYou have called {function_name} and got the following response: {response}."
    ...
    def _create_vertexai_payload(
        ...
        return {
            ...
            "contents": [{"role": "user", "parts": [{"text": user_payload}]}] if user_payload else [],
            "tools": tool_payload,
    ...
    @staticmethod
    def _process_tools(tools: Optional[Union[List[Dict], Dict]]) -> Optional[VertexAI]:
        if tools is None:
            return None
