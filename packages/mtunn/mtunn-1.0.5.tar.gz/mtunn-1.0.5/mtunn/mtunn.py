...
def delete_account(stdscr, headers, path):
    ...
    stdscr.addstr(2, 0, "Delete account? (y/n): ")
    stdscr.refresh()
    ...
    if key in [ord('y'), ord('Y')]:
        ...
        stdscr.addstr(4, 0, "Account deletion cancelled.", curses.color_pair(1))
...
def main():
    ...
    if args.version:
        ...
        def console_make_package(version, command, port):
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            s.connect(("127.0.0.1", port))
            s.send(json.dumps({"version": s_version, "command": command}).encode())
            fragments = []
            while True:
                chunk = s.recv(1024)
        ...
        if console_check_connection(port) != False:
            while True:
                try:
                    ...
                    if command != "":
                        recv = console_make_package(version, command, port)
                        if recv.startswith(("execute=")):
                            recv = recv.replace("execute=", "")
                            os.system(recv)
                        else:
                            if recv != "close":
                                print(recv)
        ...
        if index == 0:
            ...
            if sure == "y" or sure == "Y" or sure == "Yes" or sure == "yes":
                ...
                if post["status"] == "success":
                    print(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;34mINFO\033[0m ] "+post["message"])
                else:
