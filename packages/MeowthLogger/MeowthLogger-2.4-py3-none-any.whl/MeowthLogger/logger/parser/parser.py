...
from MeowthLogger.logger.settings import LoggerSettings
...
class LogParser:
    ...
    #### Stream name was looking as "(13h 01-01-1999) (13h 01-01-2099).log"
    U can add custom headder for filename
    ```
    stream = parser.stream_logs(date_from, stream_name_header="LOGGER!")
    ```
    Filename should be looking as "LOGGER!(13h 01-01-1999) (13h 01-01-2099).log"
    ...
    @classmethod
    def prepare_dates(cls, date_from: str, date_to: str) -> tuple[datetime, datetime]:
        """Preparing dates before load logs files
        """
        date_from = set_null_minutes(
            convert_request_datestring(date_from)
        )

        date_to = set_null_minutes(
            convert_request_datestring(date_to) if date_to
            else datetime.now()
        ) + timedelta(hours=1)
    ...
    def stream_logs(
        ...
        if date_to > datetime.now():
            root_logfile = RootLogFile(DEFAULT_LOGGING_FILENAME, self.settings.path)
            dump_file.write(root_logfile.read())

        # change filename
        dump_file.name = self.prepare_filename(date_from, date_to, stream_name_header)
        # saving file
        dump_file.seek(0)
