    ...
    def finalize_response(self, request: Request | HttpRequest, response: Response | HttpResponse, *args, **kwargs):
        ...
        if not getattr(response, "exception", False):
            request_line, request_line_arguments = get_request_line(
                ...
                *(getattr(request, field, None) for field in self.logger_auth_info_fields),
            )
...
def exception_handler(exc, context):
    ...
    if isinstance(exc, exceptions.ValidationError):
        ...
        if isinstance(detail, dict):
            ...
            data = {"detail": detail}
        data.setdefault("accident_id", get_accident_id())
        data.setdefault("code", getattr(data.get("detail"), "code", data.get("error")))
    else:
        data = {
            "detail": "Internal server error",
            "code": "server",
            "accident_id": get_accident_id(),
        }
    ...
    (default_logger.warning if status_code < 500 else default_logger.error)(
        ...
        *content_lines_arguments,
        ...
        context,
        TRACEBACK_LINE,
        "".join(format_exception(exc, limit=5)).strip(),
