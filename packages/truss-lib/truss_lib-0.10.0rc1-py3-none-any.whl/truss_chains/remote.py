...
def _push_to_baseten(
    ...
    options: definitions.PushOptionsBaseten,
    chainlet_name: str,
    ...
    if isinstance(options, definitions.PushOptionsLocalDocker):
        ...
        with utils.log_level(logging.INFO):
            ...
            service = _push_to_baseten(
                truss_dir, options, chainlet_descriptor.display_name, progress_bar
            )
    else:
    ...
    def push(
        ...
        for chainlet_descriptor in _get_ordered_dependencies([entrypoint]):
            ...
            service = _push_service(
                chainlet_dir, chainlet_descriptor, self._options, self._progress_bar
            )
            chainlet_display_name_to_url[chainlet_descriptor.display_name] = (
                service.predict_url
            )
...
def push(
    ...
    return _Pusher(options, gen_root, progress_bar).push(
        entrypoint, non_entrypoint_root_dir
    )
...
class _Watcher:
    ...
    def __init__(
        ...
        if not remote:
            remote_str = remote_cli.inquire_remote_name(
