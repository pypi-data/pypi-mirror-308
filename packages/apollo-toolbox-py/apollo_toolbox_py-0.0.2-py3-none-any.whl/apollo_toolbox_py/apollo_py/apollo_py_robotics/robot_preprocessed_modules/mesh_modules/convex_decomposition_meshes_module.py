...
class ApolloConvexDecompositionMeshesModule:
    def __init__(self, stl_link_mesh_relative_paths: List[List[str]], obj_link_mesh_relative_paths: List[List[str]], glb_link_mesh_relative_paths: List[List[str]]):
        self.stl_link_mesh_relative_paths: List[List[PathBufPyWrapper]] = list(
            map(lambda x: list(map(lambda y: PathBufPyWrapper().append(y), x)), stl_link_mesh_relative_paths))
        self.obj_link_mesh_relative_paths: List[List[PathBufPyWrapper]] = list(
            map(lambda x: list(map(lambda y: PathBufPyWrapper().append(y), x)), obj_link_mesh_relative_paths))
        self.glb_link_mesh_relative_paths: List[List[PathBufPyWrapper]] = list(
            map(lambda x: list(map(lambda y: PathBufPyWrapper().append(y), x)), glb_link_mesh_relative_paths))
