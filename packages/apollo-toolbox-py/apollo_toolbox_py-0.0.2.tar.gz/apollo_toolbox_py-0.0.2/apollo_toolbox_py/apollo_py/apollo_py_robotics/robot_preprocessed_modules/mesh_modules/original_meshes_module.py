...
class ApolloOriginalMeshesModule:
    def __init__(self, link_mesh_relative_paths: List[Optional[str]]):
        self.link_mesh_relative_paths: List[Optional[PathBufPyWrapper]] = list(
            map(lambda x: None if x is None else PathBufPyWrapper().append(x), link_mesh_relative_paths))
    ...
    def recover_full_path_bufs(self, resources_root_directory) -> List[Optional[PathBufPyWrapper]]:
