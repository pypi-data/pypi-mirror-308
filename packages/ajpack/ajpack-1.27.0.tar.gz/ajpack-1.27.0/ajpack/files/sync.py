...
def sync_files(srcDir: str, tarDir: str) -> tuple[dict[str, str], int, float, float]:
    ...
    :return backupFiles, len(backupFiles.keys()), seconds (tuple[dict[str, str], int, float, float]): The synced files with their source paths as keys and their destination paths as values; The length of the dictionary; The time needed of the scan in seconds; The time needed of the backup in seconds.
    """
    global fileCounter
