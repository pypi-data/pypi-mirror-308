...
#     if p is None and q is None:
#         for p in range(recon_mat_reshape_tensor.shape[0]):
#             if isinstance(adata, AnnData):
#                 x = adata.obsm['spatial'][p][0]
#                 y = adata.obsm['spatial'][p][1]
#             else:
#                 x = adata[p][0]
#                 y = adata[p][1]
#             C = np.zeros((N**2, 2), dtype=int)
...
#         for q in range(recon_mat_reshape_tensor.shape[2]):
#             all_spot_all_variable[:, q] = recon_mat_reshape_tensor[:, :, q].flatten().cpu().detach().numpy()
