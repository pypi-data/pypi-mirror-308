...
def test_start(temp_chat_file):
    ...
    with patch('src.chat.Chat.cmdloop') as mock_cmdloop:
        chat.start()
...
def test_assemble_prompt(temp_chat_file, chat_history, expected_text_content, expected_image_data_list):
    ...
    with patch('src.prompt_handler.append_images_to_content', return_value="mocked combined content") as mock_append:
...
def test_send_message(temp_chat_file, chat_history, last_line_in_file, expected_written_content):
    ...
    with patch('src.chat.send_prompt', return_value=mock_chunks), \
...
def test_do_EXTRACT(temp_chat_file, capsys):
    ...
    with patch('src.prompt_extractor.extract_responses') as mock_extract_responses:
        chat.do_EXTRACT("")
...
def test_do_LOAD_TEMPLATE(temp_chat_file):
    ...
    directory = '/project/src/templates'
