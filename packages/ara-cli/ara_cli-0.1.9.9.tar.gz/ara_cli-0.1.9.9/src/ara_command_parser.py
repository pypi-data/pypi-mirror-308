...
from src.commandline_completer import ArtefactCompleter, ParentNameCompleter
...
def list_parser(subparsers):
    ...
    branch_group = list_parser.add_mutually_exclusive_group()
    branch_group.add_argument(
        "-b", "--branch",
        dest="branch_args",
        nargs=2,
        metavar=("classifier", "artefact_name"),
        help="Output the contents of artefacts in the parent chain with specified classifier and artefact_name"
    )
    branch_group.add_argument(
        "-c", "--children",
        dest="children_args",
        nargs=2,
        metavar=("classifier", "artefact_name"),
        help="Output the contents of child artefactsof the artefact with specified classifier and artefact_name"
    )
    ...
    def format_help(self):
        ...
        if '-h' in argv or '--help' in argv:
            ...
            examples = (
                ...
                "  > list all existing features:                                                      ara list Feature:\n"
                "  > list all child artefacts contributing value to a parent artefact:                ara list \"Contributes to {name_of_parent_artefact} {ara classifier_of_parent_artefact}\"\n"
