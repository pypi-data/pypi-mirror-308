...
import sys
import json
from src.output_suppressor import suppress_stdout
...
def create_action(args):
    from src.artefact_creator import ArtefactCreator
    from src.classifier import Classifier
    from src.filename_validator import is_valid_filename
    from src.template_manager import SpecificationBreakdownAspects
...
def delete_action(args):
    from src.artefact_deleter import ArtefactDeleter
...
def rename_action(args):
    from src.artefact_renamer import ArtefactRenamer
    from src.classifier import Classifier
    from src.filename_validator import is_valid_filename
...
def list_action(args):
    from src.artefact_lister import ArtefactLister
...
def prompt_action(args):
    ...
    def handle_init():
        from src.prompt_handler import initialize_prompt_templates
        initialize_prompt_templates(classifier, param)

    def handle_init_rag():
        from src.prompt_handler import initialize_prompt_templates
        from src.prompt_rag import search_and_add_relevant_files_to_prompt_givens
        initialize_prompt_templates(classifier, param)
        search_and_add_relevant_files_to_prompt_givens(classifier, param)

    def handle_load():
        from src.prompt_handler import load_selected_prompt_templates
        load_selected_prompt_templates(classifier, param)

    def handle_send():
        from src.prompt_handler import create_and_send_custom_prompt
        create_and_send_custom_prompt(classifier, param)

    def handle_load_and_send():
        from src.prompt_handler import load_selected_prompt_templates, create_and_send_custom_prompt
        load_selected_prompt_templates(classifier, param)
        create_and_send_custom_prompt(classifier, param)

    def handle_extract():
        from src.prompt_extractor import extract_and_save_prompt_results
        from src.update_config_prompt import update_artefact_config_prompt_files
    ...
    def handle_chat():
        from src.prompt_chat import initialize_prompt_chat_mode
...
def template_action(args):
    from src.classifier import Classifier
    from src.template_manager import TemplatePathManager
...
def fetch_templates_action(args):
    import shutil
    from src.ara_config import ConfigManager
    from src.template_manager import TemplatePathManager
...
def read_action(args):
    from src.artefact_reader import ArtefactReader
    from src.artefact import Artefact
