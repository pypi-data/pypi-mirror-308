from src.ara_config import ensure_directory_exists
...
def test_ensure_directory_exists_when_directory_does_not_exist():
    ...
    with patch("src.ara_config.exists", return_value=False) as mock_exists:
        with patch("src.ara_config.os.makedirs") as mock_makedirs:
...
def test_ensure_directory_exists_when_directory_exists():
    ...
    with patch("src.ara_config.exists", return_value=True) as mock_exists:
        with patch("src.ara_config.os.makedirs") as mock_makedirs:
            result = ensure_directory_exists(directory)
