...
from ..core import ManagedObject, ManagedObjectCollection, WeakedReference
...
class Entity(ManagedObject):
	...
	def RemoveCompoent(self, componentType: Type[T]) -> bool:
		for componentIdentifier in self.__componentIdentifiers:
			obj: ManagedObject = ManagedObjectCollection.Find(componentIdentifier)
			if not obj:
				continue
	...
	def GetComponent(self, componentType: Type[T]) -> Optional[WeakedReference[T]]:
		for componentIdentifier in self.__componentIdentifiers:
			component: WeakedReference[T] = ManagedObjectCollection.FindWeakedReference(componentIdentifier)
			if component:
				return component
		return None