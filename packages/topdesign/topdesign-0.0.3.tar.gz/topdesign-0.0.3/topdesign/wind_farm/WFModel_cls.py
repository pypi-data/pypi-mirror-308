...
class WFModel_Floris(WFModel):
    """ Wind farm model using the FLORIS model (v3.5).
    """
    def __init__(self, wf_obj, wind_rose_obj,
                 input_arg_keys=['layout_x', 'layout_y'], 
                 output_arg_keys=['AEP'], 
        ...
        super().__init__(wf_obj, input_arg_keys, output_arg_keys, 
                         parameters_dict=parameters_dict,
                         name=name, info=info)
        self.wind_rose_obj = wind_rose_obj
    ...
    def func(self, input_dict):
        ...
        self.wf_obj.reinitialize(
            layout_x = layout_x,
            layout_y = layout_y
        )

        # Compute the AEP (GWh) using the default settings
        aep = self.wf_obj.get_farm_AEP_wind_rose_class(
            wind_rose=self.wind_rose_obj) / 1.0e9
