...
##############################################################################
# Make sure you have py_wake and floris (==3.5) installed
...
if __name__ == '__main__':
    ...
    wind_rose = WindRose()
    wind_rose.read_wind_rose_csv("outputs/wind_rose_horns_rev.csv")

    # Show the wind rose
    fig_wind = wind_rose.plot_wind_rose()
    ...
    wt_floris = TurbineInterface.from_internal_library("iea_15MW.yaml")
    D = wt_floris.turbine.rotor_diameter

    power_range = wt_floris.power_curve(ws)[1]
    Ct_range = wt_floris.Ct_curve(ws)[1]
    ...
    for i_run in range(num_runs):
        ...
        save_path = f'{num_wt}wt_{num_iter}iter_run{i_run}_RS'
