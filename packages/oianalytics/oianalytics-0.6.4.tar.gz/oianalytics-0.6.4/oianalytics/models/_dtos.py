...
class InstantParameter(BaseModel):
    ...
    def parse_value(cls, value):
        if value is None:
            ...
            return pd.to_datetime(value, format="ISO8601").to_pydatetime()
...
class EventDataValue(BaseModel):
    ...
    id: StrictStr
    reference: StrictStr
    dataType: Literal["STORED_EVENT"]
    unit: DataUnit
    aggregationFunction: StrictStr


class EventDataParameter(BaseModel):
    type: Literal["STORED_EVENT_DATA"]
    sourceCodeName: StrictStr
    ...
    def get_data_output_dict(
        ...
        if data_type == "any":
            if values_type == "any":
                ...
                output_types = ["BATCH_TAG_KEY", "EVENT_TAG_KEY"]
