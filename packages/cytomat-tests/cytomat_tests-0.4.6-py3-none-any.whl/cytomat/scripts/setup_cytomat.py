...
def find_file_in_meipass(filename):
    # Überprüfen, ob das Programm als PyInstaller .exe läuft
    if not getattr(sys, 'frozen', False):
        raise RuntimeError("Dieses Skript muss als .exe (mit PyInstaller) ausgeführt werden.")
    
    # Der Pfad zum _MEIPASS Ordner (temporäres Verzeichnis von PyInstaller)
    meipass_dir = sys._MEIPASS
    
    # Rekursive Suche nach der Datei im _MEIPASS Verzeichnis
    for root, dirs, files in os.walk(meipass_dir):
        if filename in files:
            # Absoluten Pfad der Datei zurückgeben, sobald sie gefunden wurde
            return os.path.join(root, filename)
    
    # Falls die Datei nicht gefunden wurde, None zurückgeben
    return None

# Der Name der Datei, die wir suchen
