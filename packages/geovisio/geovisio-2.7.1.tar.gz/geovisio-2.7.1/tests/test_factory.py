...
@pytest.mark.parametrize(
    ("name", "desc", "logo", "color", "fails"),
    (
        (None, None, None, None, False),
        ("Panoramax", "La super plateforme", "https://panoramax.fr/logo.svg", "#ff0000", False),
        ("Panoramax", "La super plateforme", None, None, False),
        (None, None, "not an url", None, True),
        (None, None, None, "blablabla", True),
    ),
)
def test_config_app_summary(dburl, tmp_path, name, desc, logo, color, fails):
    summary = {}
    if name:
    ...
    if fails:
        ...
        with create_test_app(config) as app:
            ...
            assert app.config["API_SUMMARY"].name == {"en": name or "GeoVisio"}
            assert app.config["API_SUMMARY"].description == {"en": desc or "The open source photo mapping solution"}
            assert app.config["API_SUMMARY"].logo == logo or "https://gitlab.com/panoramax/gitlab-profile/-/raw/main/images/logo.svg"
