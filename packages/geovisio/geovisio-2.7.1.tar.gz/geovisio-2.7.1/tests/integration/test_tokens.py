...
def test_list_user_token(server, keycloak, auth_app):
...
def test_get_jwt_token(server, keycloak, auth_client):
    """An authenticated user can get the JWT token associated to a token"""
    with requests.session() as s:
        _login(session=s, server=server)
...
def test_get_unknown_token(server, keycloak, auth_app):
...
def _login(session: requests.Session, server: str):
...
def test_delete_unknown_token(server, keycloak, auth_app):
...
def test_generate_bearer_token_flow(server, keycloak, auth_app):
    ...
    with requests.session() as s:
        ...
        # we should be redirected to the claim url
...
def test_generate_bearer_token_flow_double_claim(server, keycloak, auth_app):
    ...
    with requests.session() as new_user_session:
        ...
        assert redirect_history(r) == [
            ...
            "/api/auth/redirect",
            f"/api/auth/tokens/{token_id}/claim",  # <- but geovisio is not happy
        ]
