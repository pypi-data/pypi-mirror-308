...
class PromptManager(Base):
    ...
    async def fullmemory_save_log_ai_interaction_prompt(
        ...
        try:
            ...
            if prompt_details and "messages" in prompt_details:
                logger.info(
                    f"Prompt details messages   I WILL SEE: {prompt_details['messages']}"
                )
                published_messages = prompt_details["messages"]
                logger.info(f"Found published messages: {published_messages}")
                if published_messages:
                    prompt_collection_msg.extend(published_messages)
                    logger.info(f"Added {len(published_messages)} published messages")

            # Add previous messages from history
            if interaction_history and interaction_history.get("messages"):
                logger.info(f" I WILL SHOW U: {interaction_history}")
                prompt_collection_msg.extend(interaction_history["messages"])
                logger.info(
                    f"Added {len(interaction_history['messages'])} previous messages"
                )
    ...
    async def summarymemory_save_log_ai_interaction_prompt(
        ...
        try:
            ...
            logger.info("\nCache Status BEFORE Processing:")
            logger.info("-" * 30)
            logger.info("Persistent Cache Contents:")
            CacheManager.print_cache_contents()
            logger.info("\nInteraction Cache Contents:")
            InteractionCacheManager.print_cache_contents()

            session_type, session_id = self._fetch_session_id(interaction_request)
            logger.info(f"\nSession Info - Type: {session_type}, ID: {session_id}")

            # Get existing interaction history
            interaction_history = InteractionCacheManager.get_interaction_history(
                user_prompt_id, session_id, interaction_request.get("version")
            )
            logger.info(f"\nExisting Interaction History:")
            logger.info(f"History found: {bool(interaction_history)}")
            if interaction_history:
                logger.info(
                    f"Current summarized_content: {interaction_history.get('summarized_content', 'None')}"
                )
            ...
            variables = prompt_details["variables"]
            ...
            if session_type == "new_session":
                ...
                for message in interaction_request["user_message"]:
            ...
            logger.info(f"Response1111111111111111: {response}")
            ...
            logger.info(f"Response33333333: {get_ai_response}")
            logger.info(f"Prompt collection message: {prompt_collection_msg}")

            logger.info(
                f"\n{'='*50}\nResponse from AI platform: {response}\n{'='*50}\n"
            )
            ...
            logger.info(f"Processed AI response: {processed_ai_response}")
            ...
            summary_message_collection = prompt_collection_msg

            logger.info(f"Summary message collection: {summary_message_collection}")
                    ...
                    [
                        {
                            ...
                            "content": [
                                {
                                    "type": "text",
                                    "text": f"Summary of previous AI Interaction: {summarized_content}\n\nNew user message that needs to be responded: {interaction_request['user_message']}",
                                }
                            ],
                        },
                        {
            ...
            logger.info(f"Create summary: {create_summary}")
            logger.info(f"Create ai_platform: {create_summary}")
            ...
            logger.info(f"New summarized_content: {new_summarized_content}")
            ...
            logger.info(f"Interaction data to save: {interaction_data}")
            ...
            # Verify cache update - Add delay or force cache update
            ...
            # Verify the summary was saved - with safe access
            cache_key = f"{user_prompt_id}_{session_id}_v{version}"
            updated_history = InteractionCacheManager._interaction_cache.get(
                cache_key, {}
            )
            logger.info("\nVerifying Summary Storage:")
            logger.info(f"Cache key: {cache_key}")
            logger.info(f"Updated history found: {bool(updated_history)}")
            logger.info(
                f"Updated summarized_content: {updated_history.get('summarized_content', 'No summary found')}"
            )
    ...
    async def chat_with_prompt(
        ...
        logger.info(f"self.bypass: {self.bypass}")
...
def gemini_interaction_chat_with_prompt(
    ...
    logger.info(f"Messages  gemini_interaction_chat_with_prompt: {messages}")
    ...
    logger.info(f"Generated history11111111111: {generated_history}")
