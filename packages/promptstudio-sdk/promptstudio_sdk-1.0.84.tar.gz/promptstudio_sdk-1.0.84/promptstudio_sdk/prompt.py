...
async def summarymemory_save_log_ai_interaction_prompt(
    ...
    try:
        ...
        session_type, session_id = fetch_session_id(interaction_request)
        ...
        processed_ai_response = process_ai_response_by_format(
            response["response"], prompt_details["response_format"]
        )
        ...
        if session_type == "new_session":
            summary_message_collection.extend(
                [
                    {"role": "user", "content": interaction_request["user_message"]},
                    {
                        "role": "assistant",
        ...
        if prompt_details["ai_platform"] == "gemini":
            ...
            ai_response = {"type": "text", "text": ai_response}

        else:
            new_summarized_content = create_summary["content"][0]["text"]
            ai_response = response["response"]["content"][0]

        # save_messages_and_summary(user_prompt_id, session_id, interaction_request, ai_response, new_summarized_content)
        ...
        if prompt_details["ai_platform"] == "gemini":
            ...
            final_response = response["response"]["content"][0]["text"]
...
def fetch_session_id(interaction_request):
    if interaction_request.session_id == "":
        new_session = str(ObjectId())
        return "new_session", new_session
    else:
        existing_session = interaction_request.session_id
        return "existing_session", existing_session
...
def merge_default_and_provided_variables(interaction_request, default_variables):
