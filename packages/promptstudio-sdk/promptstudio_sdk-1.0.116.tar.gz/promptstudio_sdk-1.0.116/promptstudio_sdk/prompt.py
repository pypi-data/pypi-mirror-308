...
class PromptManager(Base):
    ...
    async def fullmemory_save_log_ai_interaction_prompt(
        ...
        try:
            ...
            logger.info(f"User prompt user_prompt: {user_prompt}")
            ...
            logger.info(f"Retrieved interaction history: {interaction_history}")
            logger.info(
                f"User prompt prompt_details['messages'][0]['content']: {prompt_details['messages'][0]['content']}"
            )

            if user_prompt["platform"] == "claude":
                user_prompt["messages"][0]["content"] = self.modify_messages_for_claude(
                    prompt_details["messages"][0]["content"]
                )
            def_variables = user_prompt.get("variables") if user_prompt else []
            default_variables = self.convert_data(def_variables)
            # platform_key = user_prompt["platformConfig"]["platform_key"]

            # Determine which variables to use based on the conditions
            if not interaction_request.variables:
                # Condition 1: No variables given in interaction_request
                variables = default_variables
            else:
                # Condition 2 and 3: Check how many variables are provided
                provided_variables = interaction_request.variables
    ...
    async def _fetch_and_cache_prompt_details(
        ...
        try:
            ...
            logger.info(f"Using cache key for prompt details: {cache_key}")
            CacheManager.set_prompt_details(cache_key, prompt_details)
            logger.info(f"Saved prompt details to cache with key: {cache_key}")
...
def openai_interaction(
    ...
    try:
        ...
        logger.info(f"OpenAI response vaishu: {response}")

        # Get the response content using the new API format
        assistant_reply = response.choices[0].message.content.strip()
        logger.info(f"OpenAI response assistant_reply shru: {assistant_reply}")
