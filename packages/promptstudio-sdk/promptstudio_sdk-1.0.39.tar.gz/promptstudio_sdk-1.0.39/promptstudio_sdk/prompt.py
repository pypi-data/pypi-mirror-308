...
class PromptManager(Base):
    ...
    async def fullmemory_save_log_ai_interaction_prompt(
        ...
        try:
            ...
            # Print cache contents before processing
            logger.info("Cache contents before processing:")
            ...
            if not prompt_details:
                ...
                logger.info(f"Cached prompt details for {cache_key}: {prompt_details}")

                # Print cache after storing prompt details
                logger.info("Cache contents after storing prompt details:")
                CacheManager.print_cache_contents()

            logger.info(f"Prompt details qqqqqqqq: {prompt_details}")

            # Use system message from cached prompt details
            system_message = prompt_details.get("system_prompt", "")
            ...
            # Before saving new interaction
            logger.info("Cache contents before saving interaction:")
            InteractionCacheManager.print_cache_contents()
            ...
            logger.info("Final cache contents:")
            CacheManager.print_cache_contents()
            InteractionCacheManager.print_cache_contents()
