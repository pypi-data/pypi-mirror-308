...
class InteractionCacheManager:
    _interaction_cache: Dict[str, Dict] = (
        {}
    )  # Structure: {prompt_id: {session_id: {version: {messages: [], lastResponseAt: None}}}}
    ...
    def save_interaction(
        ...
        """
        Save interaction with hierarchical tracking
        ...
        existing_messages = cls._interaction_cache[prompt_id][session_id][version][
            "messages"
        ]

        # Add new messages while preserving the existing ones
        cls._interaction_cache[prompt_id][session_id][version]["messages"] = (
            existing_messages + interaction_data["messages"]
        )
        cls._interaction_cache[prompt_id][session_id][version]["lastResponseAt"] = (
            interaction_data["lastResponseAt"]
        )

        logger.info(
            f"Saved interaction. Total messages in cache: "
            f"{len(cls._interaction_cache[prompt_id][session_id][version]['messages'])}"
        )
        logger.debug(
    ...
    def get_interaction_history(
        ...
        """Get complete interaction history for a specific prompt, session and version"""
        try:
            history = (
                cls._interaction_cache.get(prompt_id, {})
                .get(session_id, {})
                .get(version, {})
            )
            logger.info(
                f"Retrieved interaction history - Prompt ID: {prompt_id}, "
