...
#   timestamp: 2024-11-13T15:42:18+00:00
...
class Scope2(TypedDict):
    EverythingThatDependsOn: List[Id]


type Scope = Union[Scope1, Scope2]
...
class SubModelLoader1(TypedDict):
    SourceMdl: str
...
class Metadata13(TypedDict):
    internal: NotRequired[VpopMetadata]
...
class ConflictStrategy1(TypedDict):
    ScopeTheseConflictingIds: List[Id]


class ConflictStrategy2(TypedDict):
    ScopeAllConflictingIdsBut: List[Id]


type ConflictStrategy = Union[
    Literal['ScopeAllConflictingIds'],
    Literal['RemoveAllConflictingIds'],
    ConflictStrategy1,
    ConflictStrategy2,
]
...
class ConflictOptions(TypedDict):
    bioreacConflict: ConflictStrategy
    compartmentConflict: ConflictStrategy
    eventConflict: ConflictStrategy
    odeConflict: ConflictStrategy
    paramConflict: ConflictStrategy
    scoringConflict: ConflictStrategy
    speciesConflict: ConflictStrategy
...
class ReplaceBy(TypedDict):
    inModel: SubModelLoader
    rename: Dict[str, Id]


class SubModelLoader2(TypedDict):
    ReplaceBy: ReplaceBy


class RenameModel(TypedDict):
    inModel: SubModelLoader
    newName: Id


class SubModelLoader3(TypedDict):
    RenameModel: RenameModel


class Prefix(TypedDict):
    inModel: SubModelLoader
    prefix: NotRequired[Union[Id, Dict[str, Any]]]
    scope: Scope


class SubModelLoader4(TypedDict):
    Prefix: Prefix


class ReduceTo(TypedDict):
    inModel: SubModelLoader
    scope: Scope


class SubModelLoader5(TypedDict):
    ReduceTo: ReduceTo
...
class SubModelLoader6(TypedDict):
    Delete: Delete


class Combine(TypedDict):
    onConflicts: ConflictOptions
    subModels: List[SubModelLoader]


class SubModelLoader7(TypedDict):
    Combine: Combine


class LoadMdlTemplate(TypedDict):
    inModel: SubModelLoader
    placeholders: Dict[str, str]


class SubModelLoader8(TypedDict):
    LoadMdlTemplate: LoadMdlTemplate


type SubModelLoader = Union[
    SubModelLoader1,
    SubModelLoader2,
    SubModelLoader3,
    SubModelLoader4,
