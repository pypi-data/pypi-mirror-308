...
"""
    ...
    pip3 cache purge
    pip3 install -i https://pypi.tuna.tsinghua.edu.cn/simple Fr1997v011==2.3.3
...
class DouyinJike:
    ...
    def douyin_search_data_20231025(keyword, data_data, res_page):
        ...
        if status_code == 0 and data:
            ...
            for index, v in enumerate(data):
                ...
                try:
                    aweme_type = aweme_info['aweme_type']
                except:
                    aweme_type = 0
    ...
    # 数据解析 抖音作品详情 html版本
    def res_html_data(self, response):
        base_ret_data = {}
        ...
        return {'code': 200, 'date': base_ret_data}
...
# DESC = 'DESC'
#
# sql = (f"SELECT id,hightitle FROM `cd_shool_list` WHERE id > 0 ORDER BY ID DESC LIMIT 10")
# # all_data = MysqlJike(conn_tp=0).select(sql)
# # for i in all_data:
# #     print(i)
#
# mode_mysql.select(field="*", table='cd_shool_list', sort='', order=DESC)

# sql = """
# SELECT t1.id,t1.hightitle,t2.l_name
# FROM cd_shool_list as t1
# LEFT JOIN cd_shool_l as t2
# ON t1.`level`= t2.l_id
# WHERE t1.id > 0 limit 10
# """
# all_data = MysqlJike(conn_tp=3).select(sql)
# for i in all_data:
#     print(i)
...
# """
#
# all_data = mode_mysql.select(sql, conn_tp=3)
# for i in all_data:
#     print(i)


# # 如何判断 作品是笔记还是视频
# video_id = '7421789420619124003'
# headers = {
#     'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
#     'accept-language': 'zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6',
#     'cache-control': 'max-age=0',
#     'cookie': '__ac_nonce=065d28d51003491caf715; __ac_signature=_02B4Z6wo00001n4veuQAAIDA.7jUUwxIuY5-L35AAPpo6f; ttwid=1%7C2cALTIDMmCKk_dcAfQIkn08tnBLcwurRzQq-ox_RqdA%7C1708297553%7C2af605878f9409784522c820956a8fbb43fce740be75ec42983711596e0afe3f; __ac_referer=__ac_blank',
#     'priority': 'u=0, i',
#     'referer': 'https://www.douyin.com/video/7421789420619124003',
#     '^sec-ch-ua': '^\\^Microsoft',
#     'sec-ch-ua-mobile': '?0',
#     '^sec-ch-ua-platform': '^\\^Windows^\\^^',
...
# }
# response = requests.get(f'https://www.douyin.com/note/{video_id}', headers=headers)
# print(response.url)
# res = mode_douyin.res_html_data(response)
# print(res)

# print(mode_douyin.api_douyin_user(sec_uid='MS4wLjABAAAAlvoD8D_vGYwfoJhsn0DVu0-kjrnvPwMM28JGzAh_25skbBVK-h8uQ0eRvr5IUe4-'))
