...
class RPCHandler(tornado.web.RequestHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        try:
            ...
            self.write(str(e))
            return
        logging.debug(" ", method_name, "(", args if args is not None else '', ')')
        args = list(voxe.loads(self.request.body)) if len(self.request.body) > 0 else []
        try:
            method = self.scope.__getattribute__(method_name)
        except Exception as e:
            err = f'No such method {method_name}!'
            logging.error(err)
            traceback.print_exception(e)
            self.write(voxe.dumps(Exception(err)))
            return
        try:
...
if __name__ == '__main__':
    ...
    logging.basicConfig(format='%(asctime)s - %(levelname)s: %(message)s', level=[e[1] for e in options if re.match(e[0], args.log_level)][0])
    try:
