...
class ModelAdminBase(ModelAdminAbstract):
    ...
    def get_search_results(self, request, queryset, search_term):
        ...
        """
        from django.contrib.admin.options import LOOKUP_SEP, FieldDoesNotExist, smart_split, unescape_string_literal, \
            reduce, operator, lookup_needs_distinct
        ...
        if search_fields and search_term:
            orm_lookups = [x for x in [construct_search(str(search_field))
                                       for search_field in search_fields] if x is not None]
