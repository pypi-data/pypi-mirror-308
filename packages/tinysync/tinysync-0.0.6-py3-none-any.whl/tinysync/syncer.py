...
class Sync:
    ...
    def __init__(self,workdir,syncConfig, backendA:Backend, backendB:Backend, break_lock=None):
        """
        ...
        #self.currA: DictTable
        #self.currB: DictTable
        ...
        if self.syncConfig['set_lock']:
            self.api.check_lock()
            # no lock for dry-run since we won't change anything
            self.api.lock()
    ...
    def stats(self):
        ...
        dt_rclone = utils.time_format(self.api.rclonetime)
...
def syncronization(backendA:Backend, backendB:Backend,Config:dict=None, break_lock=None):
    with tempfile.TemporaryDirectory() as temp_dir:
        print('Temporary directory created:', temp_dir)
        if Config is None:
            Config = configuration
        Sync(workdir=temp_dir,syncConfig=Config, backendA=backendA, backendB=backendB, break_lock=break_lock)
