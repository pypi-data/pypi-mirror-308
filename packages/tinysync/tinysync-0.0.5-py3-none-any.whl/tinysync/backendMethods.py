...
class BackendHandler():
    ...
    def _RecursivelylistFilesInPath1(self,rpath:str,mtime:bool=True,hash=False,linkMode=2) -> list[ dict ]:
        ...
        for t in currentListDir:
            ...
            if t['type'] == 'd':
                ...
                res |= self._RecursivelylistFilesInPath1(rpath=rp,hash=hash,linkMode=linkMode) 
            elif t['type'] == 'f': 
                _path = prefix + t['name']
                t['Path'] = _path
                del t['type']
    ...
    def _RecursivelylistFilesInPath(self,rpath:str,mtime:bool=True,hash=False,linkMode=2) -> list[ dict ]:
        return self._RecursivelylistFilesInPath1(rpath,mtime,hash,linkMode).values() 
    ...
    # def _listRootAll(self,rpath:str) -> list[ dict ]:
    #     """recursively list all files in root path.
    #        Consider:  --filter + /.syncrclone/LOCK/* --filter - /.syncrclone/**
    ...
    #         list[ dict ]: dict = { "Path":str, "Size":int, "mtime":float }
    #     """ 
    #     res1 = self._RecursivelylistFilesInPath(rpath='')
    #     res1 = [ r for r in res1 if not ( r['Path'].startwith(".syncrclone") and ( not r['Path'].startswith(".syncrclone/LOCK") ) ) ] 
    #     return res1 


    def _RecursivelylistFilesInPathWithLocalFilter(self,rpath:str,filter:Callable[[str],bool],mtime:bool=True,hash:bool=False,linkMode=2) -> list[ dict ]:
        """recursively list files (not dir) in a path 

        Args:
            rpath (str): relative path , example: rpath = "this/path", rpath = "" 
            filter (Callable[[str],bool]): for a given path (rpath inside input <rpath>), check if this item must be included
            mtime (bool, optional): contains mtime or not.
            hash (bool, optional): contains hash value or not.
        ...
        """  
        items = self._RecursivelylistFilesInPath(rpath=rpath,mtime=mtime,hash=hash,linkMode=linkMode)
        res = [ item for item in items if filter(item['Path']) ] 
        return res 
...
# def copyFileWithoutCheckFatherDir(filePath,srcBackend,dstBackend):
#     with tempfile.NamedTemporaryFile(delete=True) as tmpfile:
#         localPath = tmpfile.name
#         # 从 A 服务器下载文件到本地临时文件
#         srcBackend.backend.getFile(rPathRemote=filePath, localPath=localPath)
#         # 将临时文件上传到 B 服务器对应的路径
#         dstBackend.backend.putFile(localPath=localPath, rPathRemote=filePath)


def copyFileWithoutCheckFatherDir(filePath,srcBackend,dstBackend,localWorkDir):
...
class Rclone:
    ...
    def getWorkDirFromBackend(self,backend):
        wd = backend.backend.workdir 
        if wd is None:
             wd = ".tinysync"
        return wd 
    ...
    def file_list(self, *, prev_list=None, remote=None)->tuple[DictTable,DictTable]:
        ...
        res1 = backend._RecursivelylistFilesInPath(rpath='',hash=isNeed_Hash,mtime=isNeed_modtime,linkMode=self.syncConfig['linkMode'])
        files = [ r for r in res1 if not ( r['Path'].startswith(workdirName) and ( not r['Path'].startswith(lockdirName) ) ) ] 
        ...
        normalized_A = [item.rstrip('/') for item in not_hashed]
        def is_rclone_path_in_list(path: str) -> bool:
            """判断path是否在预处理后的A中，或是其子目录"""
            # 规范化给定路径（去掉末尾的斜杠）
            normalized_path = path.rstrip('/')

            for normalized_item in normalized_A:
                # 如果路径直接匹配，或者是目录并且path是其子路径
                if normalized_path == normalized_item or normalized_path.startswith(f"{normalized_item}/"):
                    return True
            return False
        

        updated = backend._RecursivelylistFilesInPathWithLocalFilter(rpath=".",hash=True,filter=is_rclone_path_in_list,linkMode=self.syncConfig['linkMode'])
    ...
    def transfer(self, mode, matched_size, diff_size):
        ...
        if diff_size: # diff_size 是一个 list，内容是文件路径 
            ...
            for p in diff_size:
                # print(f"[{mode}]copy-sigle-file:{p}, srcMeta={srcMeta},dstMeta={dstMeta}")
    ...
    def lock(self, breaklock=False, remote="both"):
        ...
        backend = self.backend[remote]
        workDir = self.getWorkDirFromBackend(backend)#getattr(config, f"workdir{AB}")
        ...
        if not breaklock:
            ...
            try:
                backend.backend.deleteFile(lockDest)
    ...
    def check_lock(self, remote="both"):
        ...
        if touch is None:
            ...
            raise LockedRemoteError(f"Locked on {remote}, {lockDest}")
