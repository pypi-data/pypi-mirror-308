...
class TestGreeksFlow:
    parametrize = pytest.mark.parametrize("client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    def test_method_retrieve(self, client: Unusualwhales) -> None:
        greeks_flow = client.options_flows.greeks_flow.retrieve()
        assert_matches_type(GreeksFlowRetrieveResponse, greeks_flow, path=["response"])
    ...
    def test_raw_response_retrieve(self, client: Unusualwhales) -> None:
        response = client.options_flows.greeks_flow.with_raw_response.retrieve()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        greeks_flow = response.parse()
        assert_matches_type(GreeksFlowRetrieveResponse, greeks_flow, path=["response"])

    @parametrize
    def test_streaming_response_retrieve(self, client: Unusualwhales) -> None:
        with client.options_flows.greeks_flow.with_streaming_response.retrieve() as response:
...
class TestAsyncGreeksFlow:
    parametrize = pytest.mark.parametrize("async_client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    async def test_method_retrieve(self, async_client: AsyncUnusualwhales) -> None:
        greeks_flow = await async_client.options_flows.greeks_flow.retrieve()
        assert_matches_type(GreeksFlowRetrieveResponse, greeks_flow, path=["response"])
    ...
    async def test_raw_response_retrieve(self, async_client: AsyncUnusualwhales) -> None:
        response = await async_client.options_flows.greeks_flow.with_raw_response.retrieve()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        greeks_flow = await response.parse()
        assert_matches_type(GreeksFlowRetrieveResponse, greeks_flow, path=["response"])

    @parametrize
    async def test_streaming_response_retrieve(self, async_client: AsyncUnusualwhales) -> None:
        async with async_client.options_flows.greeks_flow.with_streaming_response.retrieve() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
