...
class RandomPropagate(nn.Module):
    ...
    def __init__(self, habitat, terrain, num_spreads=100, spread_size=1, device=None):
        ...
        tf = 1 - 0.5 ** (1 / num_spreads)
        self.min_transmission = 1 - tf / 2
...
def analyze_geotiffs(habitat_fn=None, 
    ...
    :param single_tile: if true, reads the entire geotiff as a single tile (no iteration).
    ...
    def do_analysis(conn_file, flow_file):
        ...
        for i, (hab_tile_iter, per_tile_iter) in enumerate(joint_reader):
            ...
            if skip_tile:
                ...
                continue
            # We process the tile.
            connectivity_tile, flow_tile = analysis_fn(np.ones_like(permeability) if raw_habitat is None else habitat, permeability)
