...
def generate_single_markdown(all_reports: List[Dict], ci_commit_tag: Optional[str]) -> None:
    template = get_local_template("combined_test_report.j2")

    file_name = "sw_ut_result.md"
    title = "Lupin Software Unit Tests Result"
    if ci_commit_tag:
        file_name = f"sw_ut_result_{ci_commit_tag}.md"
        title = f"{title} {ci_commit_tag}"

    markdown_content = template.render(reports=all_reports, title=title)
