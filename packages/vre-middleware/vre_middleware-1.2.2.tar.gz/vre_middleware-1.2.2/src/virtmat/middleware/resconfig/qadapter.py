...
def get_pre_rocket(wcfg, **kwargs):
    ...
    """
    default_mods = []
    for mod in wcfg.default_modules:
        default_mods.append(mod.get_command(mod.name))
    default_venv = []
    ...
    if kwargs.get('modules') or kwargs.get('envs'):
        pre_rocket_lst = []
        if kwargs.get('modules'):
            pre_rocket_lst.append('module purge')
            pre_rocket_lst.extend(default_mods)
    ...
    if default_mods or default_venv:
        purge = ['module purge'] if default_mods else []
        return '; '.join(purge+default_mods+default_venv)
    return None
