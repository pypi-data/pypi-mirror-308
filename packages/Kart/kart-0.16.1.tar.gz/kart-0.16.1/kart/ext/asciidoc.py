...
from kart.utils import str_to_bool
...
def asciidoc_to_html(context: Context, asciidoc: str) -> str:
    ...
    with Popen(["asciidoctor", "-e", "-"], stdin=PIPE, stdout=PIPE) as p:
...
def asciidoc_to_toc(asciidoc: str) -> str:
    ...
    asciidoc = ":toc:\n:toclevels: 4\n" + asciidoc
    with Popen(["asciidoctor", "-e", "-a", "toc", "-"], stdin=PIPE, stdout=PIPE) as p:
...
class AsciidocMiner(DefaultMarkupMiner):
