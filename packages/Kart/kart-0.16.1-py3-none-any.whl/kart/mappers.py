...
class DefaultCollectionMapper(Mapper):
    ...
    def map(self, config: dict, site: KartDict) -> KartMap:
        ...
        for obj in collection:
            slug = f"{self.collection}.{obj['slug']}"
            url = f"/{slugify(self.collection)}/{slugify(obj['slug'])}/"
            if "template" in obj:
...
class DefaultRobotsTxtMapper(Mapper):
    """Mapper that adds an entry for DefaultRobotsTxtRenderer."""

    def __init__(self, renderer: str = "default_robotstxt_renderer"):
        self.renderer = renderer
