...
def find_artifacts(env, project, type, token):
    ...
    if not validation_criteria_artifact_id:
        raise Exception("Could not find a Validation Criteria artifact on the project")

    artifact_v_get = requests.get(
        f"https://frontoffice.{env}.escrow.beekeeperai.com/artifact-version/?artifact_id={data_attestation_artifact_id}",
        headers={
            "Content-type": "application/json",
            "Authorization": "Bearer " + token,
            "User-Agent": "curl/7.71.1",
    ...
    if type == "validation":
        artifact_v_get = requests.get(
            f"https://frontoffice.{env}.escrow.beekeeperai.com/artifact-version/?artifact_id={validation_criteria_artifact_id}",
            headers={
                "Content-type": "application/json",
                "Authorization": "Bearer " + token,
                "User-Agent": "curl/7.71.1",
