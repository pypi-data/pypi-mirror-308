...
def takagitaupin(scantype, scan, constant, polarization, crystal_str, hkl,
                 asymmetry, thickness, displacement_jacobian=None,
                 debyeWaller=1.0, min_int_step=1e-10):
    '''
    ...
    scan =  relative to the Bragg's energy in meV (energy scan) OR relative to
    the Bragg's angle in arcsec (angle scan). scan is an numpy array containing
    these values OR an integer N generating automatic scan with N points.

    constant = incidence angle respect to the diffracting planes in degrees
    (energy scan) OR photon energy in keV (angle scan)
    ...
    displacement_jacobian = a function giving the jacobian of the displacement
    field as function of position (x, y). Note: y points upwards the crystal
    ...
    hc = physical_constants['Planck constant in eV s'][0] * \
        physical_constants['speed of light in vacuum'][0]*1e3  # in meV*m
    ...
    if is_escan:
        ...
        for ii in range(escan.size):
            ...
            # crystal, (E0+escan[ii])*1e-6, 0, 0, 0, 1.0, 1.0)
            F0[ii] = 0
            # xraylib.Crystal_F_H_StructureFactor(
            # crystal, (E0+escan[ii])*1e-6, hkl[0], hkl[1], hkl[2],
            # debyeWaller, 1.0)
            Fh[ii] = 0
            # xraylib.Crystal_F_H_StructureFactor(
            # crystal, (E0+escan[ii])*1e-6, -hkl[0], -hkl[1], -hkl[2],
        ...
        # xraylib.Crystal_F_H_StructureFactor(
        # crystal, E0*1e-6, 0, 0, 0, 1.0, 1.0)
        F0 = 0
        # xraylib.Crystal_F_H_StructureFactor(
        # crystal, E0*1e-6, hkl[0], hkl[1], hkl[2], debyeWaller, 1.0)
        Fh = 0
        # xraylib.Crystal_F_H_StructureFactor(
        # crystal, E0*1e-6, -hkl[0], -hkl[1], -hkl[2], debyeWaller, 1.0)
        Fb = 0
    ...
    if displacement_jacobian is None:
        def strain_term(z):
            return 0
    ...
    for step in range(len(scan)):
        ...
        if geometry == 'bragg':
            ...
            def TTE(z, Y):
                return [1j*(cb_step*Y[0]*Y[0] +
                            (c0_step+beta_step+strain_term(z))*Y[0] + ch_step),
                        -1j*(g0_step+gb_step*Y[0])*Y[1]]

            def TTE_jac(z, Y):
                return [[2j*cb_step*Y[0] +
                         1j*(c0_step+beta_step+strain_term(z)), 0],
                        [-1j*gb_step*Y[1], -1j*(g0_step+gb_step*Y[0])]]

            r = ode(TTE, TTE_jac)

        r.set_integrator('zvode', method='bdf', with_jacobian=True,
                         min_step=min_int_step, max_step=1e-4, nsteps=50000)

        if geometry == 'bragg':
            r.set_initial_value(0, -thickness)
            res = r.integrate(0)
            # gamma-part takes into account beam footprints
            reflectivity[step] = np.abs(res[0])**2*gamma0[step]/gammah[step]
        else:
