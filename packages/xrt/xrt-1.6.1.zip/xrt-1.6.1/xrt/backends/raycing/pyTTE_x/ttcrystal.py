...
from .deformation import isotropic_plate, anisotropic_plate_fixed_shape, \
    anisotropic_plate_fixed_torques
...
class TTcrystal:
    ...
    *OR*
    ...
    def __init__(self, filepath=None, **kwargs):
            ...
            for line in lines:
                ...
                if len(line) > 0 and not line[0] == '#':
                    ...
                    if ls[0] == 'crystal' and len(ls) == 2:
                        ...
                        if len(ls) == 4:
                            kwargs['in_plane_rotation'] = [
                                float(ls[1]), float(ls[2]), float(ls[3])]
                        elif len(ls) == 3:
                            kwargs['in_plane_rotation'] = Quantity(
                                float(ls[1]), ls[2])
                        else:
                            print('Skipped an invalid line in the file: ' +
                                  line)
                    elif ls[0] == 'debye_waller' and len(ls) == 2:
                        kwargs['debye_waller'] = float(ls[1])
                    elif ls[0] in ['thickness', 'asymmetry', 'E'] and \
                            len(ls) == 3:
                        ...
                        j = int(ls[0][2])-1
                        if i > j:
                            print('Omitted the lower triangle element ' +
                                  ls[0] + '.')
                        else:
        ...
        self.set_in_plane_rotation(
            params['in_plane_rotation'], skip_update=True)
    ...
    def set_crystal(self, crystal_str, skip_update=False):
        ...
        Input:
            crystal_str = string representation of the crystal in compliance
            with xraylib
        '''
    ...
    def set_reflection(self, hkl, skip_update=False):
        '''
        Set a new reflection and calculate the new crystallographic data and
        deformation for rotated crystal.
        ...
        if len(hkl_list) == 3:
            for i in range(3):
                if not type(hkl_list[i]) in [type(1), type(1.0)]:
                    raise ValueError(
                        'Elements of hkl have to be of type int or float!')
            self.hkl = hkl_list
        else:
    ...
    def set_thickness(self, thickness, skip_update=False):
        ...
        Input:
            thickness = the thickness of the crystal wrapped in a Quantity
            instance e.g. Quantity(300,'um')
        '''
        ...
        if isinstance(thickness, Quantity) and thickness.type() == 'length':
            ...
            raise ValueError(
                'Thickness has to be a Quantity instance of type length!')
    ...
    def set_asymmetry(self, asymmetry, skip_update=False):
        ...
        Input:
            asymmetry = clockwise-positive asymmetry angle wrapped in a
            Quantity instance 0 for symmetric Bragg case (default), 90 deg for
            symmetric Laue
        '''
        ...
        if isinstance(asymmetry, Quantity) and asymmetry.type() == 'angle':
            ...
            raise ValueError(
                'Asymmetry angle has to be a Quantity instance of type angle!')
    ...
    def set_in_plane_rotation(self, in_plane_rotation, skip_update=False):
        ...
        Input:
            in_plane_rotation = counterclockwise-positive rotation of the
            crystal directions about hkl-vector wrapped in a Quantity instance
            of type angle
            OR
            a crystal direction [q,r,s] corresponding to a direct space vector
            R = q*a1 + r*a2 + s*a3 which will be rotated about the hkl vector
            so that its component perpendicular to hkl (and the crystal as a
            whole with it) will be aligned with the y-axis. Will raise an error
            if R || hkl.
        '''

        if isinstance(in_plane_rotation, Quantity) and \
                in_plane_rotation.type() == 'angle':
            self.in_plane_rotation = in_plane_rotation.copy()
        elif type(in_plane_rotation) in \
            [type([]), type((1,)), type(np.array([]))] and \
                len(in_plane_rotation) == 3:

            # Check the list entry types
            for i in in_plane_rotation:
                if not np.isreal(i):
                    raise ValueError(
                        'In-plane rotation angle has to be a Quantity '
                        'instance of type angle OR a list of floats size 3!')
            ...
            if abs(np.dot(r, h) - np.sqrt(np.dot(r, r)*np.dot(h, h))) < \
                    np.finfo(type(1.0)).eps:
                raise ValueError(
                    'in_plane_rotation can not be parallel to the reciprocal '
                    'diffraction vector!')
            ...
            print('Deviation of the given in_plane_rotation direction from '
                  'the rotation plane: ' + str(np.degrees(incl)) + ' deg.')
            ...
            self.in_plane_rotation = Quantity(
                np.degrees(rotation_angle), 'deg')
        else:
            raise ValueError(
                'In-plane rotation angle has to be a Quantity instance of '
                'type angle OR a list of floats size 3!')
    ...
    def set_debye_waller(self, debye_waller, skip_update=False):
        ...
        if not isinstance(debye_waller, Quantity) and \
            np.array(debye_waller).size == 1 \
            and np.real(debye_waller) and debye_waller >= 0 and \
                debye_waller <= 1:
            self.debye_waller = debye_waller
        else:
            raise ValueError(
                'Debye-Waller factor has to be a float or integer in '
                'range [0,1]!')
    ...
    def set_elastic_constants(
            self, S=None, E=None, nu=None, skip_update=False):
        '''
        Set either the compliance matrix (fully anisotropic) or Young's modulus
        and Poisson ratio (isotropic).
        ...
        Input:
            ...
            S = 6x6 compliance matrix wrapped in a instance of Quantity of type
            pressure^-1
        ...
        if (E is not None) and (nu is not None):
            if isinstance(E, Quantity) and E.type() == 'pressure':
                ...
                raise ValueError(
                    'E has to be an instance of Quantity of type pressure!')
        elif S is not None:
            if isinstance(S, Quantity) and S.type() == 'pressure^-1':
                ...
                raise ValueError(
                    'S has to be an instance of Quantity of type pressure^-1!')
        else:
            self.isotropy = 'anisotropic'
            self.S0 = Quantity(
                0.01*elastic_matrices(self.crystal_data['name'])[1], 'GPa^-1')
    ...
    def set_fix_to_axes(self, fix_to_axes, skip_update=False):
        ...
        if fix_to_axes in ['shape', 'torques']:
            ...
            raise ValueError("The allowed values for fix_to_axes are "
                             "'torques' and 'shape'!")
        ...
        if not skip_update:
            if self.deformation_model[0] == 'custom':
                self.set_deformation(
                    jacobian=self.deformation_model[1], skip_update=True)
            else:
                self.set_deformation(jacobian=None, skip_update=True)
    ...
    def set_bending_radii(self, Rx, Ry, skip_update=False):
        ...
        if isinstance(Rx, Quantity) and Rx.type() == 'length':
            ...
            raise ValueError('Rx has to be an instance of Quantity of type '
                             'length, inf, or None!')
        if isinstance(Ry, Quantity) and Ry.type() == 'length':
            ...
            raise ValueError('Ry has to be an instance of Quantity of type '
                             'length, inf, or None!')
        ...
        if jacobian is not None:
            ...
            if len(ujac) == 2:
                if len(ujac[0]) == 2 and len(ujac[1]) == 2:
                    ...
                    raise ValueError(
                        'Output of jacobian has to be 2x2 list or array!')
            else:
                raise ValueError(
                    'Output of jacobian has to be 2x2 list or array!')
        else:
    ...
    def bragg_energy(self, bragg_angle):
        '''
        Returns the energy of the photons corresponding to the given Bragg
        angle.
        ...
        if not (isinstance(bragg_angle, Quantity) and
                bragg_angle.type() == 'angle'):
            raise TypeError(
                'bragg_angle has to be an instance of Quantity of type angle!')

        if np.any(bragg_angle.in_units('deg') <= 0) or \
                np.any(bragg_angle.in_units('deg') >= 180):
    ...
    def bragg_angle(self, bragg_energy):
        '''
        Returns the Bragg angle corresponding to the energy of incident
        photons.
        ...
        if not (isinstance(bragg_energy, Quantity) and
                bragg_energy.type() == 'energy'):
            raise TypeError('bragg_energy has to be an instance of Quantity '
                            'of type energy!')
    ...
    def update_rotations_and_deformation(self):
        '''
        Applies the in-plane and asymmetry rotations to the elastic matrix
        (for anisotropic crystal) and calculates the Jacobian of the
        deformation field based on the elastic parameters and the bending
        radii.
        '''
        ...
        if self.deformation_model[0] == 'anisotropic':
            self.S = Quantity(
                rotate_elastic_matrix(self.S0.value, 'S', Rmatrix),
                self.S0.units())
    ...
    def __str__(self):
        ...
        if self.isotropy == 'anisotropic':
            elastic_str = 'Compliance matrix S (with rotations applied):\n' + \
                np.array2string(self.S.in_units('GPa^-1'), precision=4,
                                suppress_small=True) + ' GPa^-1'
        else:
            elastic_str = "Young's modulus E: " + \
                str(self.E) + "\nPoisson's ratio nu: " + str(self.nu)

        if self.deformation_model[0] == 'custom':
            deformation_str = 'custom Jacobian (bending radii and elastic '\
            ...
            if self.deformation_model[1] == 'fixed_shape':
                deformation_str = \
                    'anisotropic toroidal, fixed shape (built-in)'
            else:
                deformation_str = \
                    'anisotropic toroidal, fixed torques (built-in)'
