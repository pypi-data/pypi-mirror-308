...
def axis_angle(u, theta):
    '''
    Computes a matrix which performs a rotation of theta degrees
    counterclockwise about axis u.

    Input:
        u = 3-element vector giving the axis of rotation (does not need to be
        normalized)
...
def align_vector_with_z_axis(h):
    ...
    # rotation matrix
    R = np.array([[np.cos(th) + u[0]**2*(1-np.cos(th)),
                   u[0]*u[1]*(1-np.cos(th)),  u[1]*np.sin(th)],
                  [u[0]*u[1]*(1-np.cos(th)), np.cos(th)+u[1]**2 *
                   (1-np.cos(th)), -u[0]*np.sin(th)],
...
def inplane_rotation(alpha):
    '''
    Rotates the given tensor around the z-axis by alpha degrees
    counterclockwise.
