...
codePath = r"c:\github\xrt" if newCode else os.path.join('..', '..', '..')
...
#    print(what)
    ...
    beamLine.limitsFSM0X = 'symmetric'
    beamLine.limitsFSM0Z = 'symmetric'
    beamLine.suffix = '_new' if newCode else '_old'
#    E0 = 6900  # eV
    R0 = 25000.  # Distance to the screen [mm]

    uniformRayDensity=True if what=='wave' else False
    filamentBeam=True if what=='wave' else False
    eEpsilonX=0.0 if what=='wave' else 0.263
    eEpsilonZ=0.0 if what=='wave' else 0.008
    eEspread=0.0 if what=='wave' else 0.001
    eSigmaX=0.0 if what=='wave' else 48.65
    eSigmaZ=0.0 if what=='wave' else 6.197
    
    #for sourceType in ['u', 'w', 'bm', 'eu', 'wu']:
    ...
    if sourceType == 'u':
        ...
        kwargs = dict(
            ...
            # R0=R0,   # Near Field.
            eEpsilonX=eEpsilonX, eEpsilonZ=eEpsilonZ)  # Emittance [nmrad]
        beamLine.xlimits = [-2, 2]  # Horizontal limits of the plot [mm]
        beamLine.zlimits = [-2, 2]  # Vertical limits of the plot [mm]
        beamLine.xlimitsZoom = [-2, 2]
        beamLine.zlimitsZoom = [-2, 2]
        beamLine.xPrimelimits = [-0.3, 0.3]  # Angular limits of the plot [mrad]
        ...
        beamLine.xlimits = [-40, 40]
        beamLine.zlimits = [-20, 20]
        beamLine.xlimitsZoom = [-8, 8]
        beamLine.zlimitsZoom = [-4, 4]
        beamLine.xPrimelimits = [-2.3, 2.3]
        beamLine.limitsFSM0X = [-1000, 1000]
        beamLine.limitsFSM0Z = [-100, 100]
    elif sourceType == 'bm':
        whose = '_xrt' if isInternalSource else '_ws'
    ...
    #    kwargs['uniformRayDensity'] = True
        ...
        beamLine.xlimitsZoom = [-8, 8]
        beamLine.zlimitsZoom = [-4, 4]
        beamLine.xPrimelimits = 'symmetric'
        beamLine.limitsFSM0X = [-500, 500]
        beamLine.limitsFSM0Z = [-20, 20]
    elif sourceType == 'eu':
        ...
        if isInternalSource:
            ...
            kwargs['xPrimeMaxAutoReduce'] = False
            kwargs['zPrimeMaxAutoReduce'] = False
        beamLine.xlimits = [-12, 12]
        beamLine.zlimits = [-12, 12]
        beamLine.xlimitsZoom = [-5, 5]
        beamLine.zlimitsZoom = [-5, 5]
        beamLine.xPrimelimits = [-0.55, 0.55]
        ...
        kwargs = dict(period=80., K=13., n=10, eE=3., eI=0.5)
        kwargs['targetOpenCL'] = targetOpenCL
        beamLine.xlimits = [-40, 40]
        beamLine.zlimits = [-20, 20]
        beamLine.xlimitsZoom = [-5, 5]
        beamLine.zlimitsZoom = [-5, 5]
    ...
    #    kwargs['gIntervals'] = 99
        ...
        beamLine.limitsFSM0X = [-1000, 1000]
        beamLine.limitsFSM0Z = [-100, 100]
    elif sourceType == 'tu':  # tapered undulator
        whose = '_xrt'
        pprefix = '7'+sourceType+whose
        Source = rs.Undulator
        kwargs = dict(
            eE=3., eI=0.5,  # Parameters of the synchrotron ring [GeV], [Ampere]
            eEspread=eEspread,  # Energy spread of the electrons in the ring
        ...
        beamLine.xlimits = [-2, 2]  # Horizontal limits of the plot [mm]
        beamLine.zlimits = [-2, 2]  # Vertical limits of the plot [mm]
        beamLine.xlimitsZoom = [-0.2, 0.2]
        beamLine.zlimitsZoom = [-0.2, 0.2]
        beamLine.xPrimelimits = [-0.3, 0.3]  # Angular limits of the plot [mrad]
        kwargs['xPrimeMaxAutoReduce'] = False
        kwargs['zPrimeMaxAutoReduce'] = False
    elif sourceType == 'custom':  # tapered undulator from custom field
        whose = '_xrt'
        Np = 40
        L0 = 30
        Kmax = 1.45
        ...
        plt.savefig("B vs Z.png")
        #plt.show()
        fieldArray = np.vstack((fieldZ, fieldB)).T

        pprefix = '8'+sourceType+whose
...
#            K=1.45,  # Deflection parameter (ignored if targetE is not None)
    ...
    kwargs['xPrimeMax'] = beamLine.xlimits[-1] / R0 * 1e3
    kwargs['zPrimeMax'] = beamLine.zlimits[-1] / R0 * 1e3
    
    beamLine.prefix = pprefix+'-{0}-{1}E-'.format(eEpsilonC, energyRange)
    if energyRange == 'mono':
        beamLine.eMinRays, beamLine.eMaxRays = E0-1, E0+1
        beamLine.eUnit = 'eV'
    elif energyRange == '1harmonic':
        beamLine.eMinRays, beamLine.eMaxRays = E0-300, E0+300
        beamLine.eUnit = 'eV'
    elif energyRange == 'smaller':
        beamLine.eMinRays, beamLine.eMaxRays = 1500, 7500
        beamLine.eUnit = 'keV'
    elif energyRange == 'wide':
        beamLine.eMinRays, beamLine.eMaxRays = 1500, 37500
        beamLine.eUnit = 'keV'
    
    kwargs['eMin'] = beamLine.eMinRays
    kwargs['eMax'] = beamLine.eMaxRays
...
def run_process(beamLine):
    startTime = time.time()
    kw = {}
    if 'mono' in beamLine.prefix and isInternalSource:
        kw['fixedEnergy'] = beamLine.alignE
#    print(beamLine.what, isinstance(beamLine.source, rs.Undulator))

    if 'wave' in beamLine.what: # and isinstance(beamLine.source, rs.Undulator):
...
def define_plots(beamLine):
    ...
    caxis = xrtp.XYCAxis('energy', beamLine.eUnit, fwhmFormatStr=None)
    plot = xrtp.XYCPlot(
        'beamSource', (1,), xaxis=xaxis, yaxis=yaxis, caxis=caxis,
        aspect='auto', title='YX source cross-section')
    plot.saveName = beamLine.prefix + 'crossectionYX' + beamLine.suffix + '.png'
    ...
    plot.saveName = beamLine.prefix + 'crossectionYZ' + beamLine.suffix + '.png'
    plot.caxis.fwhmFormatStr = None
    plots.append(plot)
    plotsE.append(plot)

    xaxis = xrtp.XYCAxis(r'$x$', '$\mu$m', limits=beamLine.limitsFSM0X,
                         bins=bins, ppb=ppb)
    yaxis = xrtp.XYCAxis(r'$z$', '$\mu$m', limits=beamLine.limitsFSM0Z,
                         bins=bins, ppb=ppb)
    caxis = xrtp.XYCAxis('energy', beamLine.eUnit, fwhmFormatStr=None,
                         bins=bins, ppb=ppb)
    plot = xrtp.XYCPlot(
    ...
    plot.saveName = beamLine.prefix + 'fsm0' + beamLine.suffix + '.png'
    plot.caxis.fwhmFormatStr = None
    plots.append(plot)
    plotsE.append(plot)
    ...
    beam = 'beamFSM0'
    xaxis = xrtp.XYCAxis(r'$x$', '$\mu$m', limits=beamLine.limitsFSM0X)
    yaxis = xrtp.XYCAxis(r"$x'$", 'mrad', limits=beamLine.xPrimelimits)
    caxis = xrtp.XYCAxis('energy', beamLine.eUnit, fwhmFormatStr=None)
    plot = xrtp.XYCPlot(
        beam, (1,), xaxis=xaxis, yaxis=yaxis, caxis=caxis,
        aspect='auto', title='horizontal phase space')
    plot.saveName = beamLine.prefix + 'horPhaseSpace' + beamLine.suffix + '.png'
    plot.caxis.fwhmFormatStr = None
    plots.append(plot)
    plotsE.append(plot)
    ...
    plot.saveName = beamLine.prefix + 'horPhaseSpaceZoom' + beamLine.suffix + '.png'
    plot.caxis.fwhmFormatStr = None
    plots.append(plot)
    plotsE.append(plot)

    xaxis = xrtp.XYCAxis(r'$x$', 'mm', limits=beamLine.xlimits,
                         bins=bins, ppb=ppb)
    yaxis = xrtp.XYCAxis(r'$z$', 'mm', limits=beamLine.zlimits,
                         bins=bins, ppb=ppb)
    caxis = xrtp.XYCAxis('energy', beamLine.eUnit, fwhmFormatStr=None,
                         bins=bins, ppb=ppb)
    plot = xrtp.XYCPlot(
    ...
    plot.caxis.fwhmFormatStr = None
    plot.saveName = beamLine.prefix + '1TotalFlux' + beamLine.suffix + '.png'
    ...
    xaxis = xrtp.XYCAxis(r'$x$', 'mm', limits=beamLine.xlimitsZoom, bins=bins, ppb=ppb)
    yaxis = xrtp.XYCAxis(r'$z$', 'mm', limits=beamLine.zlimitsZoom, bins=bins, ppb=ppb)
    caxis = xrtp.XYCAxis('energy', beamLine.eUnit, fwhmFormatStr=None,
                         bins=bins, ppb=ppb)
    plot = xrtp.XYCPlot(
    ...
    plot.caxis.fwhmFormatStr = None
    plot.saveName = beamLine.prefix + '1TotalFluxZoom' + beamLine.suffix + '.png'
    plots.append(plot)
    plotsE.append(plot)
    ax = plot.xaxis
    edges = np.linspace(ax.limits[0], ax.limits[1], ax.bins+1)
    ...
    xaxis = xrtp.XYCAxis(r'$x$', 'mm', limits=beamLine.xlimits, bins=bins, ppb=ppb)
    yaxis = xrtp.XYCAxis(r'$z$', 'mm', limits=beamLine.zlimits, bins=bins, ppb=ppb)
    caxis = xrtp.XYCAxis('energy', beamLine.eUnit, fwhmFormatStr=None,
                         bins=bins, ppb=ppb)
    plot = xrtp.XYCPlot(
        'beamFSM1', (1,), xaxis=xaxis, yaxis=yaxis, caxis=caxis,
    ...
    plot.caxis.fwhmFormatStr = None
    plot.saveName = beamLine.prefix + '2horizFlux' + beamLine.suffix + '.png'
    plots.append(plot)
    plotsE.append(plot)

    xaxis = xrtp.XYCAxis(r'$x$', 'mm', limits=beamLine.xlimitsZoom,
                         bins=bins, ppb=ppb)
    yaxis = xrtp.XYCAxis(r'$z$', 'mm', limits=beamLine.zlimitsZoom,
                         bins=bins, ppb=ppb)
    caxis = xrtp.XYCAxis('energy', beamLine.eUnit, fwhmFormatStr=None,
                         bins=bins, ppb=ppb)
    plot = xrtp.XYCPlot(
    ...
    plot.caxis.fwhmFormatStr = None
    plot.saveName = beamLine.prefix + '2horizFluxZoom' + beamLine.suffix + '.png'
    plots.append(plot)
    plotsE.append(plot)
    ...
    plot.caxis.fwhmFormatStr = None
    plot.saveName = beamLine.prefix + '3vertFlux' + beamLine.suffix + '.png'
    plots.append(plot)
    plotsE.append(plot)
    ...
    plot.caxis.fwhmFormatStr = None
    plot.saveName = beamLine.prefix + '3vertFluxZoom' + beamLine.suffix + '.png'
    plots.append(plot)
    plotsE.append(plot)
    ...
    plot.saveName = beamLine.prefix + '4DegPol' + beamLine.suffix + '.png'
    plot.caxis.fwhmFormatStr = None
    plots.append(plot)
    ...
    plot.saveName = beamLine.prefix + '4DegPolZoom' + beamLine.suffix + '.png'
    plot.caxis.fwhmFormatStr = None
    plots.append(plot)
    ...
    plot.saveName = beamLine.prefix + '5CircPolRate' + beamLine.suffix + '.png'
    plot.caxis.fwhmFormatStr = None
    plots.append(plot)
    ...
    plot.saveName = beamLine.prefix + '5CircPolRateZoom' + beamLine.suffix + '.png'
    plot.caxis.fwhmFormatStr = None
    plots.append(plot)

    for plot in plotsE:
        f = plot.caxis.factor
        plot.caxis.limits = beamLine.eMinRays*f, beamLine.eMaxRays*f
    for plot in plots:
...
def afterScript(plots, beamLine):
    ...
    cwd = os.getcwd()
    pickleName = os.path.join(cwd, beamLine.prefix+'.pickle')
...
def main(**mainKWargs):
    beamLine = build_beamline(**mainKWargs)
    if showIn3D:
        beamLine.glow()
    else:
...
if __name__ == '__main__':
    # ['custom','u', 'bm', 'eu', 'wu', 'tu']
    kwargs = {}
    for sourceType in ['eu', 'wu']:
        kwargs['sourceType'] = sourceType
        kwargs['what'] = 'wave'
        kwargs['energyRange'] = 'mono'
        kwargs['nrays'] = 1e6
        kwargs['targetOpenCL'] = None
        main(**kwargs)
        print(sourceType, "done")
