...
class DictOpenMapper(BaseTypeMapper):
    """Dict open character Mapper."""
    ...
    def perform_mapping_on_list(self, json_schema: str) -> str:
        dict_open_index = json_schema.find('{')
        new_json_schema = json_schema[dict_open_index + 1:]

        attr_name = None

        if dict_open_index:
            attr_name = json_schema[:dict_open_index].split(':')[0]

        new_mapped_type = MappedType(
            attr_name=attr_name,
            attr_type=Dict[str, Any]
        )
