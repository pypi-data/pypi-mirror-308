...
def get_time_lap_interval(
    laps_info: pd.DataFrame,
    lap_interval: List[int]
    ...
    if lap_interval[0] == laps_info.index[0] \
            or lap_interval[1] == laps_info.index[-1]:
        first_or_last = True
    ...
    arrival_time = min(prev_lap_stop, lap_start) +\
        (lap_start - min(prev_lap_stop, lap_start)) / 2

    # estimed departure time
    departure_time = lap_stop +\
        (max(next_lap_start, lap_stop) - lap_stop) / 2

    return park_time, arrival_time, departure_time, first_or_last


def get_lap_time_info(
    ...
    return data.groupby([constants.SEGMENT,
                         constants.SIDE_OF_STREET,
                         'day', 'lap'])['datetime'].agg(['first', 'last'])
...
def compile_parking_time(
    ...
    for cur_seg, group in grouped:
        ...
        plaque_consecutive_lap_index = (
            plaqued[['lap', constants.POINT_ON_STREET]].apply(
                lambda x: get_breaks(
                    x.lap.to_list(),
                    x[constants.POINT_ON_STREET].to_list(),
                    dist_m=5.50
                )
            ).to_frame('intervals')
        )
        ...
        for plaque, intervals in plaque_consecutive_lap_index.iterrows():
            for interval in intervals.values[0]:
                p = group[
                    np.logical_and(group.plaque == plaque,
                                   group.lap == interval[0])
                ].copy()
                p['lap_start'] = p['lap']
                ...
                (parktime,
                 ar_time,
                 dep_time,
                 first_or_last) = get_time_lap_interval(laps_info, interval)
...
def park_time_street(park_time, res_handler, seg_gis, handle_restriction):
    ...
    pt_street = park_time.groupby(
        [constants.SEGMENT, constants.SIDE_OF_STREET]
    )['park_time'].agg(
        lambda x: restriction_aggfunc(x, stats_type='median')
    ).reset_index()

    return pt_street


def park_time_distribution(park_time: pd.DataFrame,
                           level: str = 'secteur') -> pd.DataFrame:
    ...
    park_time['category'] = pd.cut(
        park_time.h_pt, [0] + list(numeric_cuts.values()),
        labels=labels,
        include_lowest=True
    ).astype(str)
    ...
    pt_cat = park_time[[
        not (
            pd.api.types.is_numeric_dtype(type(x)) or
            pd.isna(x)
    ...
    if level == 'secteur':
        ...
        # normalize
        park_time_distrib = park_time_distrib.div(park_time_distrib.sum())\
                                             .multiply(100)
        names = 'category'
        ...
        park_time_distrib.set_index(
            ['segement', constants.SIDE_OF_STREET, 'category'],
            inplace=True
        )
        park_time_distrib = park_time_distrib.div(park_time_distrib.sum())\
                                             .multiply(100)
        ...
        raise NotImplementedError(
            f'There is no aggregation function for {level}'
        )
...
def categorize_parking_usage(park_time, occ_ts, numeric_cuts=None):
    ...
    numeric_cuts_ind = {'Indéterminé ' + label: value
                        for label, value in numeric_cuts.items()}
    ...
    cat_park_time = cat_park_time.div(cat_park_time.sum(axis=1), axis=0)\
                                 .fillna(0)
    ...
    for col in cols_cat:
        cat_park_time[col] = \
            [cat * occ
             if not isinstance(occ, str) else 0
             for (cat, occ) in zip(cat_park_time[col],
                                   cat_park_time['mean_occ'])]
    ...
    cat_park_time = cat_park_time[
        cat_park_time[
            list(cols_cat) + [constants.REMPL_CAT_NO_PARK]
        ].sum(axis=1) != 0
    ]
...
def parking_time_by_provenance(park_time, trips,
    ...
    classe_str = \
        [f'<{i} km' for i in classe_dist[1:-1]] + [f'{classe_dist[-2]}+ km']
    trips['dist_class'] = pd.cut(
        trips.dist_ori_m/1000,
        bins=classe_dist,
        labels=classe_str
    )
    ...
    # filter string
    pt_cat = park_time[[
        not (pd.api.types.is_numeric_dtype(type(x)) or
             pd.isna(x)) for x in park_time.park_time
    ]].copy()
    ...
    park_time = park_time.merge(
        ...
        left_on=[constants.SEGMENT, constants.SIDE_OF_STREET,
                 constants.PLATE, 'lap_start'],
        right_on=[constants.SEGMENT, constants.SIDE_OF_STREET,
                  constants.PLATE, constants.LAP],
        how='left'
    )

    # 6 - Compute dist_class by category distribution
    park_time = park_time.groupby(['dist_class', 'category'])\
                         .size()\
