...
from lapin.io.sqlalchemy_utils import getEngine
...
from lapin.tools.curbsnapp import get_project_geobases


def data_from_conf(data_config: dict) -> pd.DataFrame:
    ...
    if data_config['type'] == 'sql':
        ...
        if data.crs is None:
            raise ValueError(f'Missing crs for {data_config["filename"]},' +
        ...
        data = get_project_geobases(**data_config['config'])
...
def saaq_data(
    ...
    for r, p, w in itertools.product(regions_list, *periods.values()):
        data_tmp = pd.read_excel(os.path.join(folder_path, basefilename.format(r, p, w)))
        data_tmp.columns = ['rtaudl', 'champ2', 'code_muni',
                            'muni', 'code_region', 'nb_plaque']
        data_tmp = data_tmp[['rtaudl', 'nb_plaque']]
        data_tmp['region'] = r
        data_tmp['period'] = p
        data_tmp['week_day'] = w
        centroid = regions.loc[regions[regions_bounds_names] == r]\
