...
def crs_reproject(
    ...
    projection = pyproj.Transformer.from_crs(
        orig_crs, dest_crs, always_xy=True).transform
...
def compute_lr_double_over_simple(
    ...
    pt_start = crs_reproject(
        pt_start, from_crs='epsg:4326',
        to_crs='epsg:32188'
    )
    pt_end = crs_reproject(
        pt_end,
        from_crs='epsg:4326',
        to_crs='epsg:32188'
    )

    if m_start <= 0 + total_length * 0.05:
        return 0, geom.project(pt_end), geom.length
    if m_end >= total_length - total_length * 0.05:
...
def _is_on_cleaning_period(
    date: datetime.date,
    cleaning_from: datetime.date,
    cleaning_to: datetime.date
) -> bool:
...
class Period():
    ...
    def __post_init__(self):
        if (
            self.start_time and
            self.start_time.hhmm == 0 and
            self.end_time and
            self.end_time.hhmm == 2359
        ):
            self.start_time = None
            self.end_time = None

        if self.days and (
            self.days == 'lun+mar+mer+jeu+ven+sam+dim' or
            self.days == 'lun-ven'
        ):
            self.days = None
        ...
        if (
            self.start_date and
            self.start_date.month == 1 and
            self.start_date.day == 1 and
            self.end_date and
            self.end_date.month == 12 and
            self.end_date.day == 31
        ):
    ...
    def from_dataframe(
        ...
        return cls(start_time, end_time, days,
    ...
    def from_json(cls, array: dict = None):
        ...
        if start_time and end_time:
            time_interval = (
                Ctime.from_string(start_time),
                Ctime.from_string(end_time)
            )
    ...
    def is_empty(self) -> bool:
        ...
        if isinstance(self._period, PeriodNull):
            return True
        return not (self.has_date_regulation() or
                    self.has_time_regulation() or
                    self.has_day_regulation())

    def is_active_on(
        self,
        time: np.datetime64 | datetime.date | datetime.time = None
    ) -> bool:
        ...
        if isinstance(time, datetime.time):
            ...
            time = datetime.datetime.combine(datetime.date(1970, 1, 1),
                                             time)
            ...
            isinstance(time, datetime.date) and
    ...
    def __key(self):
        ...
        return (
            ...
            self.end_date if self.start_date
            else datetime.datetime(1970, 1, 1))
...
class Regulation:
    ...
    def __data__(self):
        return np.array(
            (self.name, self.period.as_numpy()),
            dtype=regulationdt
        )
...
divisiondt = np.dtype([
    ('location', locationdt),
    ('regulation', regulationdt),
    ('spot_number', np.int64)
])
...
class SegmentDivision:
    ...
    def __eq__(self, other):
        if isinstance(other, SegmentDivision):
            return self.location == other.location and \
                   self.spot_number == other.spot_number
    ...
    def __data__(self):
        ...
        regls = np.array(
            [reg.as_numpy() for reg in self.regulations],
            dtype=regulationdt
        )
    ...
    def get_active_reg(
        ...
        self._sort_reg()
        regs_active = [reg.is_active(time)
                       if reg.name not in ignore_reg
                       else False
    ...
    def finalize(
        ...
        if regulation_ignored:
            regs = [reg for reg in self.regulations
                    if reg.name not in regulation_ignored]
    ...
    def from_json(cls, regulation: dict):
        ...
        if (
                (  # there is a <<simple>> regulation
                    definition['interdiction_complete'] or
                    not definition['restrictions']
                )
                and (  # this regulation has been entered by a human
                    definition.get('type_interdiction', None) or
                    definition['restrictions'] or
                    definition['completed'] or
                    definition['user_modified']
        ...
        # if end_loc - start_loc == geom_l:
        #     location = Location()
        # else:
        location = Location(start_loc, end_loc)

        # this is a blank restriction
        if (
            not definition['restrictions'] and
            definition['extended_attributes']['number_of_places'] == ''
        ):
            return

        parse_regulations = np.vectorize(Regulation.from_json)
        regulations = list(parse_regulations(definition['restrictions']))
        # check if it's a complete interdiction
        spot_number = definition['extended_attributes'].get(
            'number_of_places',
            -1
        )
...
class Segment:
    ...
    def _assert_marked_div_do_not_overlap(self) -> None:
        ...
        for id_div1, id_div2, dist in overlaps:
            ...
            msg += f"{divisions_id[id_div2]} overlaps by {dist}" +\
    ...
    def _complete_div(self):
        ...
        divisions_pos = (
            [div.location.start, div.location.end]
            for div in sorted(self.divisions)
        )
        ...
        try:
            ...
            while last_point < self.end:
                if last_point != curr_div[0]:
                    empty_div.append(SegmentDivision(
                        location=Location(last_point, curr_div[0])
                    ))
        ...
        if last_point < self.end:
            empty_div.append(SegmentDivision(
                location=Location(last_point, self.end)
            ))
    ...
    def finalize(
        ...
        for div in self.divisions:
            div.finalize(veh_size=veh_size,
        ...
        self.divisions = reshape_divisions(
            current_divisions_pos,
            self.divisions
        )
        self._complete_div()
    ...
    def from_json(cls, segment: dict):
        ...
        side_of_street = (SideOfStreet.RIGHT.value
...
class Curbs(object):
    ...
    def __post_init__(self):
        for _, seg in self.curbs.items():
            seg.finalize(
                ...
                regulation_ignored=self.parameters.get(
                    'regulation_ignored',
                    None
                )
            )
    ...
    def from_json(
        ...
        **kwargs : dict
        ...
        try:
            ...
            id_trc = np.intersect1d(id_trc, roads.index.values)
            roads_geom = roads.loc[id_trc, 'geometry'].values
        ...
        return cls(
            dict(zip([seg.id for seg in segments], segments)),
            parameters=kwargs
        )
    ...
    def from_dataframe(cls, data: pd.DataFrame, **kwargs):
        ...
        **kwargs : dict
        ...
        for (segment_id, _, _), div_data in data.groupby(['_id',
                                                          'loc_start',
                                                          'loc_end']):
            curbs[segment_id].divisions.append(
                SegmentDivision.from_dataframe(div_data)
            )
    ...
    def _parse_dates_params(
        ...
        if day and not hour:
            ...
            date += datetime.timedelta(days=(parse_days(day)
                                       [0] - date.weekday()) % 7)
            return date.date()
    ...
    def get_capacity(
        ...
        if as_dataframe:
            cap = pd.DataFrame.from_dict(
                cap,
                orient='index',
                columns=[constants.CAP_N_VEH]
            )
            cap.index.name = 'cote_rue_id'
            cap = cap.reset_index()
            cap['segment'] = cap.cote_rue_id // 10
            cap['side_of_street'] = [-1 if c % 10 == 2
                                     else c % 10
                                     for c in cap.cote_rue_id]
    ...
    def get_regulation_name(
        ...
        time = self._parse_dates_params(time, hour, day)
        regs = {k: seg.get_regulation_name(time)
                for k, seg in self.curbs.items()}
        return regs
    ...
    def apply_restriction_on_lprdataframe(
        ...
        lpr_df['seg_id'] += [sos if sos >= 0 else 2
                             for sos in lpr_df[constants.SIDE_OF_STREET]]
        ...
        restrict = lpr_df.loc[comon_idx].apply(
            lambda x:
                self.curbs[x.name].is_restriction(
    ...
    def to_dataframe(self):
        ...
        flatten_dtype = np.dtype(
            list(rfn.flatten_descr(arr_without_geom.dtype))
        )
