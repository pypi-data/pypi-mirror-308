...
class VisitedSegment:
    ...
    def from_dataframe(cls, veh_data: TrajDataFrame):
        ...
        veh_data_first = (veh_data.groupby(['lap_time'])
                                  .first()
                                  .reset_index()
                                  .set_index('datetime'))
        veh_data_last = (veh_data.groupby(['lap_time'])
                                 .last()
                                 .reset_index()
                                 .set_index('datetime'))
        ...
        if 'regulations' in veh_data.columns:
            ...
            regulations = list(itertools.repeat(
                None,
                veh_data_first.shape[0]
            ))
    ...
    def _chose_lap_when_arrival_and_departure_conflicts(self, lpr_readings):
        ...
        # 3.1.
        mask_conf3_end = ((conflicts.start_time > conflicts.datetime) &
                          (conflicts.end_time < conflicts.datetime) &
                          ((conflicts.datetime -
                            conflicts.end_time) < (conflicts.start_time -
                                                   conflicts.datetime)))
        conflicts.loc[mask_conf3_end, 'veh_lap'] = conflicts.end
        # 3.2.
        mask_conf3_start = ((conflicts.start_time > conflicts.datetime) &
                            (conflicts.end_time < conflicts.datetime) &
                            ((conflicts.datetime -
                              conflicts.end_time) >= (conflicts.start_time -
                                                      conflicts.datetime)))
        ...
        # resolve conflicts
        lpr_readings.loc[(lpr_readings.start !=
                          lpr_readings.end), 'start'] = conflicts.veh_lap
        lpr_readings.loc[(lpr_readings.start !=
                          lpr_readings.end), 'end'] = conflicts.veh_lap
    ...
    def filter(self, vehicules_conf: VehiculeConf):
        ...
        for uuid, conf_sides in vehicules_conf.items():
            ...
            for side, dates in conf_sides.items():
                if side != (self.segment.traffic_dir *
                            self.segment.side_of_street):
                    continue
                for date in dates:
                    mask = ((self.time_beg > date['from']) &
                    ...
                    self.regulations = list(itertools.compress(
                        self.regulations,
                        mask
                    ))
    ...
    def merge_lpr_readings(self, lpr_reading: LprDataFrame) -> LprDataFrame:
        ...
        lap_based_arrival = self.time_beg.get_indexer(
            target=lpr_reading[constants.DATETIME],
            method='nearest'
        )
        ...
        lpr_reading = self._chose_lap_when_arrival_and_departure_conflicts(
            lpr_reading
        )
        ...
        plates_list = lpr_reading.groupby('veh_lap')['plaque'].agg(list)
    ...
    def get_laps(self) -> pd.DataFrame:
        ...
        side_of_street = list(itertools.repeat(
            self.segment.side_of_street,
            self.time_beg.shape[0])
        )

        return pd.DataFrame(
            data=list(zip(uuid, ids, side_of_street, laps,
                          self.time_beg, self.time_end)),
            columns=['uuid', 'segment', 'side_of_street',
                     'lap', 'start_time', 'end_time']
        )
...
class CollectedArea:
    ...
    _sides_of_street_aggregation_performed: bool = field(
        default=False,
        init=False
    )
    ...
    def __post_init__(self):
        ...
        for i, vsegment in enumerate(self.segments):
            ...
            sos = 2 if vsegment.segment.side_of_street == -1 else \
                vsegment.segment.side_of_street

            segment_idx_list = segment_idx.get(
                int(vsegment.segment.id * 10 + sos),
                []
            )
    ...
    def populate_road_geometry(
        ...
        for vsegment in self.segments:
            ...
            roads_covered.append(
                vsegment.segment.unique_id
            )
        if force_expand:
            roads = roads.reset_index()
            roads['unique_id'] = (
                roads[constants.SEGMENT] * 10 +
                roads[constants.SIDE_OF_STREET].map({1: 1, -1: 2})
            )
            roads_to_expand = roads[
                ~roads.unique_id.isin(roads_covered)
            ].copy()
    ...
    def from_dataframe(cls, veh_data: TrajDataFrame, res_h: Curbs):
        ...
        segments = []
        grouped = veh_data.groupby(['segment', 'side_of_street',
                                    constants.UUID, 'traffic_dir'])
        for (_, side_of_street, _, _), data in grouped:
            if side_of_street == 0:
                segments.append(
                    VisitedSegment.from_dataframe(
                        veh_data=data
                    )
                )
                segments.append(
    ...
    def merge_lpr_readings(self, lpr_readings: LprDataFrame) -> None:
        ...
        grouped = lpr_readings.groupby([constants.UUID,
                                        constants.SEGMENT,
                                        constants.SIDE_OF_STREET])
    ...
    def filter_vehicules_side_of_street(
        self,
        vehicules_conf: VehiculeConf
    ) -> None:
    ...
    def aggregate_side_of_street_unidirectionnal(
        ...
        """
        grouped = network_config.groupby(['segment', 'uuid'])
        for (segment, uuid), sides in grouped:
            ...
            same_passages = (
                ...
                and
                (segment_left.time_beg == segment_right.time_beg).all()
            )
    ...
    def update_with_config(self, network_config: pd.DataFrame,
                           vehicules_conf: VehiculeConf) -> None:
        """ Aggregate side of street when traffic direction is unidirectional
    ...
    def get_by_segment(self, segment: int,
                       side_of_street: int = None,
                       index_only: bool = False) -> list[VisitedSegment]:
        ...
        try:
            if side_of_street:
                ...
                if index_only:
                    ...
                    results = [self.segments[i]
                               for i in self._index_by_segment[id_seg]]
            ...
            # aggregated
                ...
                if index_only:
                    ...
                    results = [self.segments[i]
                               for i in
                               self._index_by_segment[segment * 10 + 0]]
                ...
                if id_r in self._index_by_segment:
                    if index_only:
                        ...
                        right_part = [self.segments[i]
                                      for i in self._index_by_segment[id_r]]
                if id_l in self._index_by_segment:
                    if index_only:
                        left_part = self._index_by_segment[id_l]
                    else:
                        left_part = [self.segments[i]
                                     for i in self._index_by_segment[id_l]]
    ...
    def remove_segment(self, segment: int, uuid: str,
                       side_of_street: int) -> None:
        ...
        id_seg = segment * 10 + side_of_street
        indx = np.intersect1d(self._index_by_segment[id_seg],
                              self._index_by_uuid[uuid])
        if indx.shape[0] != 1:
            raise ValueError
    ...
    def get_laps(self) -> pd.DataFrame:
        ...
        lap_df = lap_df.sort_values(['segment', 'side_of_street',
                                     'start_time'])
        lap_df['lap'] = lap_df.groupby(['segment',
                                        'side_of_street',
                                        'day']).cumcount()
    ...
    def to_dataframe(self) -> pd.DataFrame | gpd.GeoDataFrame:
        ...
        occ_df = occ_df.explode(
            ['veh_sighted', constants.CAP_N_VEH,
             constants.LAP_TIME, constants.LAP,
             'occ', constants.PLATE,
        ...
        occ_df = occ_df.join(
            other=laps.reset_index().set_index(
                [constants.SEGMENT, constants.SIDE_OF_STREET,
                 constants.UUID, 'start_time']
            )[['lap']],
            on=[constants.SEGMENT, constants.SIDE_OF_STREET,
                constants.UUID, constants.LAP_TIME],
            how='left'
        )
    ...
    def occupancy_time_aggregation(
        ...
        if export_human_readable:
            if not isinstance(roads, RoadNetwork):
                raise ValueError("You must pass a instance of RoadNetwork " +
    ...
    def occupancy_worst_hour(
        ...
        if worst_hour is None:
            idx = np.argmax(self.occupancy_whole_study(freq='1h'))
            worst_hour = self.occupancy_whole_study(freq='1h').iloc[idx].name
        ...
        occ_worst_hour = data.xs(
            worst_hour,
            level='hour',
            drop_level=False
        )
        occ_worst_hour = prepare_data_before_export(
            occ_worst_hour.reset_index(),
    ...
    def occupancy_optimal(
        ...
        return occup.get_optimal_occupancy(
            occ,
            cap,
            veh,
            res,
    ...
    def parking_time(
        ...
        lap_info.columns = ['uuid', 'first', 'last']
    ...
    def reassignement(
        ...
        if spot_r == -1:
            raise ValueError('Street name not in roads dataframe',
                             street_name_to_report)

        if not hours:
            hours = [col
                     for col in cap.columns
                     if col not in [constants.SEGMENT,
                                    constants.SIDE_OF_STREET]]

        report, dist, dist_max, _, _ = spot_r.compute_hr_reassignement(
            cap,
            veh,
            hours
        )
    ...
    def number_of_vehicules_scanned(
        ...
        return basic.compute_observation_stats(
            occ_df,
            days,
            timeslot_beg,
            timeslot_end
    ...
    def street_geometry(self):
        ...
        for visited_segment in self.segments:
            ...
            street_geom.append(
                pd.DataFrame.from_dict(
                    visited_segment.segment.to_json(),
                    orient='index'
                ).T
            )
            unique_regulations = set(
                visited_segment.regulations
            )
            if len(unique_regulations) == 1:
                perm_regulations.append(
                    unique_regulations.pop()
                )
            else:
        ...
        if (
            isinstance(street_geom, gpd.GeoDataFrame) and
            not_visited.crs != street_geom.crs
        ):
        ...
        street_geom = street_geom.set_index(['segment',
                                             'side_of_street'])
