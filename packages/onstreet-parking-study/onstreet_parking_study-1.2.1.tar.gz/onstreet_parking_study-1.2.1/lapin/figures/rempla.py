...
def remplacement_map(park_time_street, cols, delims, savepath='cache/',
    ...
    for zone, delim in delims.items():
        for col in cols:
            path = os.path.join(
                savepath,
                f"{fig_name_base}_taux_park_time_{zone}_colonne_{col}.png"
            )
...
def park_cat_weighted_average(df, data_col, weight_col, by_col, res_col='mean', confidence=0.95):
...
def do_a_barrel_roll(cat_park_time):
    ...
    cat_cols = [col for col in cat_park_time.columns if col not in [constants.SEGMENT, constants.SIDE_OF_STREET, 'weight']]
...
def prepare_data_buterfly_plot(
    ...
    cat_rempla = categorize_parking_usage(
        park_time=park_time,
        occ_ts=occ_ts,
        numeric_cuts=numeric_cuts_rempla
    )

    # 2-compute KPI on determined and undetermined duration separetly
    # recover columns for each dataframe
    cols_det = [c for c in cat_rempla.columns
                if not c.startswith('Indéterminé')]
    cols_indet = [
        c
        for c in cat_rempla.columns
        if c.startswith('Indéterminé') or
        c in ['segment', 'side_of_street',
              'weight', constants.REMPL_CAT_NO_PARK]
    ]
    cols = [constants.SEGMENT, constants.SIDE_OF_STREET,
            'weight'] + list(numeric_cuts_rempla.keys())
    ...
    cat_rempla_indet[
        list(
            set(cols).difference(set(cat_rempla_indet.columns))
        )
    ] = 0
    ...
    for label, value in numeric_cuts_rempla.items():
        ...
        mask = (
            ...
            (park_time.park_time >
             numeric_cuts_rempla.get(previous_label, 0) * 3600)
        )
    ...
    perc_veh_tot.append(
        park_time[
            park_time.first_or_last &
            (park_time.park_time < max_hour_label * 3600)
        ].shape[0] / park_time.shape[0]
    )
...
def buterfly_parking_time_plot(
    ...
    axs['bar1'].barh(
        ...
        edgecolor='black',
        linewidth=0.5,
        color=['#221F1F', '#c2c9c9'] + list(
            itertools.repeat('#2070b4', len(determine) - 2)
        )
    )
    axs['bar1'].barh(
        label,
    ...
    axs['bar'].barh(
        ...
        edgecolor='black',
        linewidth=0.5,
        color=['#c2c9c9', '#c2c9c9'] + list(
            itertools.repeat('#2070b4', len(veh_per_label) - 2)
        )
    )
    ...
    # labels
    fig.text(0.5 + middle_label_offset,
             0.875,
             'Durée du \nstationnement',
             fontsize=14,
             ha='center')
...
def plot_cat_park_time(
    ...
    def weighted_average(df, data_col, weight_col, by_col, res_col='mean', confidence=0.95) -> pd.DataFrame:
        g = df.groupby(by_col)
        result = pd.DataFrame(index=df[by_col].unique())
        result.index.names = [by_col]
        for idx, data in g:
...
def park_time_hist_plot(data, savepath, fig_base_name, fig_dpi=config.FIG_DPI):
    ...
    data['Période'] = ['Fin de semaine' if x in [5,6] else 'Semaine' for x in data.datetime.dt.weekday]

    bins = [0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5, 5.5, 6, 6.5, max(data.park_time.max(), 24)]
    with sns.axes_style('whitegrid'):
        ...
        f = plt.gcf()
        f.savefig(os.path.join(savepath, f'{fig_base_name}parking_time_distribution.png'), dpi=fig_dpi)
