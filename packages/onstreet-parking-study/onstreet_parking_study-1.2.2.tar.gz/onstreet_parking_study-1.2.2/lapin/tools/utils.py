...
def filter_hour_bounds(data, hour_bounds):
    ...
    for days, hour_intervals in hour_bounds.items():
        for interval in hour_intervals:
            ...
            data.loc[mask, constants.HOUR_BOUNDS] = np.repeat(f"{days} ; {ts_beg} - {ts_end}", data.loc[mask].shape[0])
...
def nan_weighted_agg(arr: np.array, weights: np.array,
                     agg_func: str | list = 'mean') -> float | list[float]:
...
def restriction_aggfunc(list_occ: list[float], stats_type: str = 'mean',
                        weight: list = None) -> list[float]:
    ...
    nb_str_valued_occ = np.count_nonzero(
        [isinstance(occ, str) for occ in list_occ]
    )

    # is occ part-string part-float ? then remove strings
    if nb_str_valued_occ < len(list_occ):
        if weight:
            weight = [w for i, w in enumerate(weight) if not isinstance(list_occ[i], str)]
...
def expand_network(data, geodbl):
    ...
    # remove duplicates
    geodbl = geodbl.groupby([constants.SEGMENT, constants.SIDE_OF_STREET])\
                   .nth(0)\
                   .reset_index()
    ...
    missing = (
        geodbl[~geodbl.COTE_RUE_ID.isin(presents)]
    )[
        [
            constants.SEGMENT,
...
def compute_viz_side_of_street(df:pd.DataFrame, seg_geom:pd.DataFrame, cols_to_crunch=constants.DEFAULT_COLS_CRUNCH):
    """ TODO
    """
    ...
    seg_plex = seg_geom.set_index(constants.SIDE_OF_STREET_VIZ)[constants.SEGMENT].get(0, pd.Series([np.nan]))
    ...
    if cols_to_crunch:
        for col, f in cols_to_crunch.items():
            df[col] = (
                df
                    .groupby([constants.SEGMENT, constants.SIDE_OF_STREET_VIZ])[col]
                    .transform(f)
            )

        df = df.groupby([constants.SEGMENT, constants.SIDE_OF_STREET_VIZ]).nth(0).reset_index()

    # add geom if exist
    if cols:
        df = df.join(
            other=seg_geom.set_index([
                      constants.SEGMENT,
...
def enforce_restrictions(occ_h, resh, zero_observ=True, time_period='hour'):
    ...
    occ_h = occ_h.set_index([constants.SEGMENT,
                             constants.SIDE_OF_STREET]).sort_index()

    # find relevant columns to parse
    cols = [c for c in occ_h.columns if c not in [constants.SEGMENT,
                                                  constants.SIDE_OF_STREET,
...
def compare_two_lapi_results(res1, res2, seg_gis, metric_name='mean_occ'):
    ...
    res2 = (
        res2.join(
            other=seg_gis.set_index([constants.SEG_DB_ID, constants.SEG_DB_SIDE])[['geometry', constants.SEG_DB_STREET]].drop_duplicates(),
...
def read_hr_saved_data(path):
    ...
    cols = [col for col in df.columns if col not in ['segment', 'side_of_street_viz', 'NomSecteur']]
    ...
    df = df.groupby(
        ...
        lambda x: restriction_aggfunc(x.to_list())
    )
...
def generate_random_plaque_from_hach(df:pd.DataFrame, mask:List[bool]):
    ...
    plate_map = pd.DataFrame(
        data=[f'{i:03}{"".join(j)}' for j in
                itertools.combinations_with_replacement('ABCDEFJFHIJKLMNOPQRSTUVWXYZ',
                                                        3)
                                    for i in range(1000)
                                    if j[0] == j[1] == j[2]], columns=['plaque']
    )
    ...
    mapping = {
        df[mask].plaque.unique()[i]:
            plate_map.loc[i, 'plaque'] for i in range(nb_unique_plate)
    }

    df['plaque'] = df['plaque'].replace(mapping)

    return df

def xform_list_entre_to_dict(listValues: Iterable,
                             keyList: Iterable,
                             default: Any = None,
    ...
    if not listValues:
        ...
        raise TypeError(f"`{valueVarName}` must be None or one of {{tuple, list, np.ndarray}}, received {listValues.__class__}")
    return listValues
