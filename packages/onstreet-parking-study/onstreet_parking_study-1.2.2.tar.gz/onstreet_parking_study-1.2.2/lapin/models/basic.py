...
def compute_observation_stats(
    ...
    lapi_enhance = slice_data_by_hour(lapi_enhance, days,
                                      timeslot_beg, timeslot_end, 'datetime')
    ...
    base_stats = pd.concat([
        ...
        lapi_enhance.pivot_table(
            ...
            aggfunc='count'
        ).rename(columns={'plaque': 'nb_plaques'}),

        lapi_enhance.pivot_table(
            values='plaque',
            index=lapi_enhance.datetime.dt.date,
            aggfunc=lambda x: len(x.unique())
        ).rename(columns={'plaque': 'nb_plaques_unq'}),

        lapi_enhance.pivot_table(
            values='datetime',
            index=lapi_enhance.datetime.dt.date,
            aggfunc=lambda x: np.unique((x.dt.hour)).shape[0]
        ).rename(columns={'datetime': 'nb_hours'})
    ...
    ttal = [
        ...
        lapi_enhance.plaque.unique().shape[0],
    ...
    base_stats = pd.concat([
        ...
        pd.DataFrame(
            data=[ttal],
            columns=['datetime', 'nb_plaques', 'nb_plaques_unq', 'nb_hours']
        ).set_index('datetime')
    ], axis=0)
