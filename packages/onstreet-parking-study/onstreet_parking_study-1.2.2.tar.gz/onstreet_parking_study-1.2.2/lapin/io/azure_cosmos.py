...
def query_between_dates(
    ...
    where_cond = ' ('
    for date in dates:
        where_cond += f"c.{date_col} >= \"{date['from']}\"" + \
...
def query_container(
    ...
    res_query = container.query_items(
        query=query,
        enable_cross_partition_query=cross_partitions
    )

    return list(res_query)


def cosmos_db_to_geodataframe(query: str,
                              cosmos_config: dict,
                              dates: list = None,
    ...
    enable_cross_partition_query = kwargs.get(
    ...
    if dates:
        ...
        geo_res[dt_col] = pd.to_datetime(geo_res[dt_col], format='ISO8601')
        mask = (False)
        for date in dates:
            mask = mask | ((geo_res[dt_col] >= date['from']) &
                           (geo_res[dt_col] <= date['to']))
