...
def _lap_sequence_by_time(
    ...
    for _, group in grouped:
        ...
        group['primary_lap_time'] = group.groupby(
            'primary_lap'
...
def _lap_sequence_by_seg(
    ...
    tdf['seq'] = tdf.groupby(constants.UUID)[constants.SEGMENT]\
                    .transform(lambda x: (x.diff() != 0).cumsum())

    # label lap from 1 to n for each car and segment
    tdf['primary_lap'] = tdf.groupby([constants.UUID,
                                      constants.SEGMENT])['seq']\
                            .transform(lambda x: (x.diff() != 0).cumsum())
...
def compute_veh_lap_on_segment(
    ...
    data = data.assign(veh_id=data.uuid.map(
        {data.uuid.unique()[i]: i+1
...
def _create_lap_offset(
    ...
    idx_change = np.where(
        data.groupby('uuid').transform('size').diff(-1) != 0
    )[0][:-1] + 1
    offset[idx_change] = data.groupby('uuid').size().values[:-1]
...
def compute_lpr_lap_sequences(
    ...
    data['lap'] = data.groupby(
        group_column
    )['primary_lap'].transform(lambda x: (x.diff() != 0).cumsum())

    data['lap_id'] = (
        'veh' +
        data['veh_id'].astype('str') +
        '_' + data['lap'].astype('str')
    )
...
def lap_smoothing(data: LprDataFrame, max_lap_creation: int = 3600):
    ...
    laps_grouped = data.groupby(
        ['segment', 'lap']
    )[['datetime', 'lap_id', 'dir_veh']]
    laps = laps_grouped.first().rename(
        columns={'datetime': 'first'}
    )

    data_to_insert = []
    data['modification'] = np.nan

    grouped = data.groupby(['plaque', 'segment',
                            'side_of_street', 'cluster_id'])
    ...
    for (_, segment, dir_veh, _), plaque_data in grouped:
        ...
        for i in range(1, plaque_data.shape[0]):
            ...
            if (
                (lap_time_after - lap_time_before).total_seconds() >=
                max_lap_creation
            ):
                continue
            ...
            if lap_before != lap_after - 1 and day_before == day_after:
                for lap_missing in range(lap_before+1, lap_after):
                    ...
                    try:
                        ...
                        row_to_insert['lap_id'] = \
                            laps.loc[(row_to_insert['segment'], lap_missing),
                                     'lap_id']
                        row_to_insert['lap_time'] = \
                            laps.loc[(row_to_insert['segment'], lap_missing),
                                     'first']
