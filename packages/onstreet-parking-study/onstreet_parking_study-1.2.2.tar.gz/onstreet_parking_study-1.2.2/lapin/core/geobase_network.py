...
class BaseRoadNetwork(object):
    ...
    def graph(self) -> nx.MultiGraph:
        ...
        if isinstance(self, RoadNetworkDouble):
            raise NotImplementedError(
                'Cannot construct graph for RoadNetworkDouble.'
            )
    ...
    def human_readable(self):
        ...
        columns = [constants.HR_ROAD_NAME, constants.HR_FROM_ROAD,
                   constants.HR_TO_ROAD, constants.SEGMENT,
                   constants.GEOMETRY]
        return data[columns]
...
class RoadNetwork(BaseRoadNetwork, gpd.GeoDataFrame):
    ...
    def _has_rn_columns(self):
        ...
        if (
            (constants.SEGMENT in self) and
            (constants.ROAD_NAME in self) and
            (constants.TRAFFIC_DIR in self)
        ):
            return True
    ...
    def _is_roadnetwork(self):
        if (
            self._has_rn_columns() and
            (
                pd.core.dtypes.common.is_integer_dtype(
                    self[constants.SEGMENT]
                )
            ) and (
                pd.core.dtypes.common.is_string_dtype(
                    self[constants.ROAD_NAME]
                )
            ) and (
                pd.core.dtypes.common.is_integer_dtype(
                    self[constants.TRAFFIC_DIR]
                )
            )
        ):
            return True
...
class RoadNetworkDouble(BaseRoadNetwork, gpd.GeoDataFrame):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # # Rename geometry
        if (
            isinstance(self, gpd.GeoDataFrame) and
            constants.SEG_DB_GIS not in self and
    ...
    def _has_rn_columns(self):
        ...
        if (
            (constants.SEGMENT in self) and
            (constants.ROAD_NAME in self) and
            (constants.TRAFFIC_DIR in self) and
            (constants.SIDE_OF_STREET in self)
        ):
            return True
    ...
    def __getitem__(self, key):
        ...
        if (
            not result._is_roadnetwork() or
            constants.SEG_DB_GIS not in result
        ):
            result.__class__ = old_class
...
def _dataframe_set_roadnetwork(
    self,
    col,
    drop=False,
    inplace=False,
