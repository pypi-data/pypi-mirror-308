...
class TrajDataFrame(pd.DataFrame):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.parameters['date_kwargs'] = date_kwargs
    ...
    def _has_traj_columns(self):
        ...
        if (
            (constants.DATETIME in self) and
            (constants.LATITUDE in self) and
    ...
    def _is_trajdataframe(self):
        if (
            (
                constants.DATETIME in self and
                pd.core.dtypes.common.is_datetime64_any_dtype(
                    self[constants.DATETIME]
                )
            ) and (
                constants.LONGITUDE in self and
                pd.core.dtypes.common.is_float_dtype(
                    self[constants.LONGITUDE]
                )
            ) and (
                constants.LATITUDE in self and
                pd.core.dtypes.common.is_float_dtype(
                    self[constants.LATITUDE]
                )
            )
        ):
            return True
    ...
    def _set_traj(self, timestamp=False, inplace=False):
        ...
        if timestamp:
            frame[constants.DATETIME] = pd.to_datetime(
                frame[constants.DATETIME],
                unit="s"
            )
        ...
        if not pd.core.dtypes.common.is_datetime64_any_dtype(
            ...
            frame[constants.DATETIME] = pd.to_datetime(
                frame[constants.DATETIME],
                **self._parameters['date_kwargs']
            )

        if not pd.core.dtypes.common.is_float_dtype(
            frame[constants.LONGITUDE].dtype
        ):
            frame[constants.LONGITUDE] = frame[constants.LONGITUDE].astype(
                "float"
            )

        if not pd.core.dtypes.common.is_float_dtype(
            frame[constants.LATITUDE].dtype
        ):
            frame[constants.LATITUDE] = frame[constants.LATITUDE].astype(
                "float"
            )
        ...
        # concat operation: using metadata of the first object
            ...
            for name in self._metadata:
                object.__setattr__(self, name,
                                   getattr(other.objs[0], name, None))
        else:
            for name in self._metadata:
    ...
    def sort_by_uid_and_datetime(self):
        ...
        if constants.UUID in self.columns:
            return self.sort_values(by=[constants.UUID, constants.DATETIME],
                                    ascending=[True, True])
        else:
    ...
    def timezone_conversion(self, from_timezone, to_timezone):
        ...
        self["datetime"] = (
            self["original_datetime"].dt.tz_localize(from_timezone)
