...
def enhance(
    ...
    if matcher.engine == 'valhalla':
        ...
        lpr_data = replace_zero_position(lpr_data, veh_data)
        veh_data = match_custom_roadnetwork(
            veh_data,
            roads,
            mapmatching=True
        )
    ...
    outside_study = lpr_data[~lpr_data[constants.SEGMENT].isin(
        roads[constants.SEGMENT]
    )].copy()
    outside_study['reason'] = 'segment_outside_study'
    lpr_data = lpr_data[lpr_data[constants.SEGMENT].isin(
        roads[constants.SEGMENT]
    )]
    ...
    lpr_data, far_from_road = remove_points_far_from_road(
        data=lpr_data,
        roads=roads,
        roads_id_col='segment',
        distance=10
    )
    far_from_road['reason'] = 'distance_sup_10m_from_road'

    logger.debug('%s records removed from data after mapmatching.',
                 data_removed.shape[0] + outside_study.shape[0] +
                 far_from_road.shape[0])
    ...
    lpr_data, plate_removed = filter_plates(
        lpr_data,
        personalized_plates=False
    )
    ...
    data_removed = pd.concat([data_removed, outside_study,
                              plate_removed, plate_other_side,
                              far_from_road])
    ...
    if save_path:
        lpr_data.to_csv(
            os.path.join(save_path, 'lpr_data_enhanced.csv'),
            index=False
        )
        veh_data.to_csv(
            os.path.join(save_path, 'veh_data_enhanced.csv'),
            index=False
        )
        data_removed.to_csv(
            os.path.join(save_path, 'data_removed_by_enhancer.csv'),
            index=False
        )
...
def replace_zero_position(
    ...
    timestamps = lpr_data.loc[(lpr_data['lat'] == 0) |
                              (lpr_data['lng'] == 0),
                              'datetime'].to_list()
    lat_idx = time_index.get_indexer(timestamps, method='nearest')
    lpr_data.loc[(lpr_data['lat'] == 0) |
                 (lpr_data['lng'] == 0),
                 ['lat', 'lng']] = veh_data.loc[lat_idx, ['lat', 'lng']].values
...
def find_vehicule_wayid_for_detection(
    ...
    if 'segment' not in veh_data.columns:
        raise IncompleteDataError(
            veh_data,
            'segment',
            'find_vehicule_wayid_for_detection'
        )
