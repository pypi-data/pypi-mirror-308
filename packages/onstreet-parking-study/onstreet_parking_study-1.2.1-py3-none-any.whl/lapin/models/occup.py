...
def lapin_date_range(
    ...
    **kwarg:
    ...
    return pd.DatetimeIndex(create_ununiform_bins(start, end, freq), freq=None)
...
def _occup_pivot_handle(
    ...
    occ_h = occ_df.pivot_table(
        pivot_col,
        index=index,
        aggfunc=lambda x: DescrStatsW(
            data=x.to_list(),
            weights=occ_df.loc[x.index, weight].to_list()
        ).mean if occ_df.loc[x.index, weight].sum() > 0 else 0,
        columns='time',
        dropna=False
    )
...
def associate_time_freq(
    ...
    if not freq:
        if (timeslot_end - timeslot_beg).hour >= 23 and \
           (timeslot_end - timeslot_beg).minute >= 59:
            #
            freq = "1D"
    ...
    bins_dt = lapin_date_range(start=start, end=end,
                               freq=freq, tz=data['time'].dt.tz)
    bins_str = bins_dt.time.astype(str)

    labels = [f'{bins_str[i][:-3]}' for i in range(1, len(bins_str))]

    data['time'] = pd.cut(data['time'], bins=bins_dt,
...
def occupancy_time_aggregation(
    ...
    occ_df = associate_time_freq(occ_df, days, timeslot_beg,
                                 timeslot_end, freq)
    ...
    restrictions = restrictions.loc[
        ~restrictions.isna().all(axis=1)
    ].reset_index()
...
def agg_regions(
    ...
    data = data.join(
        other=regions.set_index([constants.SEGMENT,
...
def get_street_optimum_and_maximum(
    ...
    occ_hour.drop(
        columns=constants.SEG_DB_GIS,
        inplace=True,
        errors='ignore'
    )

    cols = [
        col for col in occ_hour.columns if col not in [
            constants.SEGMENT,
    ...
    occ_hour_tmp = occ_hour.set_index([constants.SEGMENT,
                                       constants.SIDE_OF_STREET_VIZ])[cols]
    ...
    optimum = occ_hour_tmp[(occ_hour_tmp.occ >= 0.8) &
                           (occ_hour_tmp.occ < 0.9)]
...
def street_hourly_formater(
    ...
    data = (
        ...
        .round(2)
        .join(
            other=segments[[constants.HR_ROAD_NAME,
...
def _melt_hr_data(
    ...
    data['hour'] = data['hour'].str.split(':')\
                               .apply(lambda x: x[0])\
                               .astype(int)
    data = data.set_index([
        constants.SEGMENT, constants.SIDE_OF_STREET, 'hour'
    ])
...
def get_optimal_occupancy(
    ...
    nb_hours_sup_opti = data[
        (data.occ > optimal_occ_value) &
        data.restrictions.isna()
    ...
    nb_hours_sup_opti.columns = ['hour_count', 'veh_sighted',
                                 'del', 'nb_places_total',
                                 'del', 'occ',
                                 'del', 'restrictions']
    ...
    nb_hours_sup_opti = prepare_data_before_export(
        nb_hours_sup_opti,
        seg_gis
    )
...
@deprecated(
    reason='Use lapin.core.study.CollectedAera.occupancy_time_aggregation' +
           'instead.'
)
def occupancy(
    ...
    timeslot_beg = Ctime.from_string(timeslot_beg)
    timeslot_end = Ctime.from_string(timeslot_end)
    occ_df = occ_df[(occ_df['ctime'] >= timeslot_beg) &
                    (occ_df['ctime'] <= timeslot_end)
                    ]
    ...
    bins_dt = lapin_date_range(
        start=start,
        end=end,
        freq=freq,
        tz=occ_df['time'].dt.tz
    )
    bins_str = bins_dt.time.astype(str)
...
def _occup_h_pivot_handle(occ_df, geodbl, resh, pivot_col='occ', weight=None,
                          time_period='hour'):
    """ Aggregate the occupation at the segment and side of street level. The
    ...
    # pivot this to have a column per hour
    occ_h = occ_df.pivot_table(
        pivot_col, index=[constants.SEGMENT, constants.SIDE_OF_STREET_VIZ],
...
@deprecated(
    reason='Use lapin.core.study.CollectedArea.to_dataframe instead'
)
def compile_occupancy(enh_lapi):
    ...
    grouped = enh_lapi.groupby([constants.SEGMENT,
                                constants.SIDE_OF_STREET_VIZ,
                                'lap_id'])
    res = grouped.agg({
        constants.PLATE: 'count',
        'lap_time': 'first',
        constants.CAP_N_VEH: 'first'
    }).rename(columns={
        constants.PLATE: 'veh_sighted',
        'lap_time': 'time'
    })

    res['time'] = pd.to_datetime(
        res['time'],
        **enh_lapi.parameters.get(
            'date_kwargs',
            {}
        )
    )
    ...
    res['occ'] = res.apply(
        lambda x: x['occ'] if x[constants.CAP_N_VEH] != 0 else 0,
        axis=1
    )
