...
from lapin.constants import MONTREAL_CRS, DEFAULT_CRS
...
def derive_delim_from_data(
    data: pd.DataFrame,
    destination_crs: str = DEFAULT_CRS
):
    ...
    data = gpd.GeoDataFrame(
        ...
        geometry=gpd.points_from_xy(
            x=data[constants.DESTINATION_LNG],
            y=data[constants.DESTINATION_LAT]
        ),
        crs=DEFAULT_CRS
    )
...
class SchedulerDataStore():
    ...
    trips: gpd.GeoDataFrame = None
    grid_trips_origin: gpd.GeoDataFrame = None
    delim: gpd.GeoDataFrame = None
    vis_delim: gpd.GeoDataFrame = None
    _regions: pd.DataFrame = None
    ...
    def regions(self):
        if not isinstance(self._regions, pd.DataFrame) and not self._regions:
            logger.warning("Regions havn't been set. Please set the region " +
    ...
    def regions(self, regions: pd.DataFrame):
        if not (
            constants.SEGMENT in regions.columns and
            constants.SIDE_OF_STREET in regions.columns and
            constants.SECTEUR_NAME in regions.columns
        ):
    ...
    def compute_regions(self, street):
        ...
        regions = regions[[constants.SEGMENT,
                           constants.SIDE_OF_STREET,
                           constants.SECTEUR_NAME]].drop_duplicates()
        return regions
...
class Scheduler():
    ...
    def from_datas(
        ...
        network_config = analysis_data.roads.create_network_config(
            uuid_list=list(analysis_data.veh_data['uuid'].unique()),
        ...
        study.populate_road_geometry(
            analysis_data.roadsdb,
            force_expand=True
        )
        _ = study.merge_lpr_readings(
            analysis_data.lpr_data[['segment', 'side_of_street',
                                    'uuid', 'datetime', 'plaque']]
        )
        study.update_with_config(
            network_config,
            json_keys_2_int(conf.vehicule_conf)
        )

        work_folder = os.path.join(conf.work_folder, 'resultat/')

        analysis_data.regions = analysis_data.compute_regions(
            study.street_geometry
        )
    ...
    def load_kpi_in_memory(
        ...
        if self.config.act_occup:
            ...
            occ_h, occ_ts, veh_s, cap, res = self._occup(
                days,
                hour_from,
                hour_to
            )
            ...
            self.res['export_human_readable'] = \
        ...
        if self.config.act_rempla:
            ...
            if not self.config.act_occup:
                raise ValueError(
                    'Occupancy needs to be computed if parking' +
                    ' time is set'
                )

            pk_time = self._rempla(days, hour_from, hour_to)

            pk_time = prepare_data_before_export(
                pk_time,
    ...
    def _vis_delim_iterable(self) -> dict:
        ...
        if isinstance(self.datastore.vis_delim, pd.core.series.Series):
            if (
                self.config.gis_vis_bounds_names in
                self.datastore.vis_delim.index
            ):
                vis_delim = {
                    self.datastore.vis_delim[
                        self.config.gis_ana_vis_same_bounds
                    ]: self.datastore.vis_delim['geometry']}
            else:
            ...
            if (
                self.config.gis_vis_bounds_names in
                self.datastore.vis_delim.columns
            ):
                if self.datastore.vis_delim.shape[0] > 1:
                    vis_delim = self.datastore.vis_delim.set_index(
                        self.config.gis_vis_bounds_names
                    )['geometry'].to_dict()
                else:
                    vis_delim = {
                        self.datastore.vis_delim[
                            self.config.gis_vis_bounds_names
                        ].iloc[0]: self.datastore.vis_delim['geometry'].iloc[0]
                    }

            else:
                if self.datastore.vis_delim.shape[0] > 1:
                    vis_delim = {
                        f'Secteur {i+1}': geom for i, geom in enumerate(
                            self.datastore.vis_delim['geometry']
                        )
                    }
                else:
                    vis_delim = {
                        'Secteur 1': self.datastore
                                         .vis_delim['geometry']
                                         .iloc[0]
                    }
    ...
    def _plot_capacity(
        ...
        capacity = self.datastore.restriction_handler.get_capacity(
            as_dataframe=True
        )
    ...
    def plot(self, **kwargs):
        ...
        rotation = (
            self.config.vis_rotation[0]
            if self.config.vis_rotation
            else 0
        )
        f_base._generic_plot_map(
            ...
            f'/{self.config.num_proj}_data_row_plot.png',
            rotation=rotation,
            compass_rose=compass_rose,
            dpi=300,
        ...
        if self.config.act_occup:
            ...
            cols = [col for col in self.res['occ_timeslot'].columns
            ...
            cols = [col for col in self.res['occ_hour'].columns if col not in [
                constants.SEGMENT,
            ...
            # occupancy by hour
            f_occup.occupancy_map(
                occ_df=self.res['occ_hour'], cols=cols,
                delims=vis_delim,
                savepath=path,
                basename=self.config.num_proj,
                add_cat_prc=add_cat_prc,
        ...
        if self.config.act_rempla:
            ...
            principal = self.datastore.roadsdb[
                self.datastore.roadsdb[constants.ROAD_NAME].isin(
                    street
                )
            ].segment.values
    ...
    def save(self):
        ...
        for name, data in self.res.items():
            if isinstance(data, (dict, list)):
                with open(
                    os.path.join(self.save_path, name + '.txt'),
                    'w+',
                    encoding='utf-8'
                ) as f:
                    f.write(str(data))
                ...
                sv_index = False
                if (
                    (data.index.name and data.index.name != 'index') or
                ...
                with open(
                    os.path.join(self.save_path, 'var_help.txt'),
                    'w',
                    encoding='utf-8'
                ) as file:
                    file.write(
        ...
        for days, hours in self.config.hour_bounds.items():
            for hour in hours:
                ...
                self.save_path = os.path.join(
                    self.save_path,
                    f"{days}_{hour['from']}_a_{hour['to']}"
                )
                logger.info(
                    'launching computation for %s: %s to %s',
                    days, hour['from'], hour['to']
                )
    ...
    def _occup(
        ...
        (occ_h, veh_sighted,
         capacities, restrictions) = self.study.occupancy_time_aggregation(
            days=days,
            timeslot_beg=hour_from,
            timeslot_end=hour_to,
    ...
    def _prov(self):
        ...
        """
        raise NotImplementedError(
            'This has not been refactored since SAAQ stopped' +
            'giving us data'
        )
