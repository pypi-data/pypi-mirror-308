...
VALHALLA_MATCH_COLUMNS = ['lng_match', 'lat_match', 'type', 'edge_index',
                          'end_route_discontinuity', 'point_on_segment',
                          'segment', 'nxt_segment', 'prv_segment']
...
def build_request_valhalla(tdf: TrajDataFrame, costing='auto',
                           shape_match='map_snap', **kwargs):
    """_summary_
    ...
    payload['shape'] = tdf[['lat', 'lng', 'time']].rename(columns={'lng':'lon'}).to_dict(orient='records')
...
def get_closest_roads_id(point, road1, road2, id1, id2):
    ...
    if distance_line_from_point(road1, point) > 10 and distance_line_from_point(road2, point) > 10:
        return np.nan
...
def parse_valhalla_trace_attribues(res):
    ...
    match_points = gpd.GeoDataFrame(
        match_points,
        geometry=gpd.points_from_xy(match_points.lon,
                                    match_points.lat),
        crs='epsg:4326'
    )
...
class MapMatcher():
    ...
    def match(self, coords, **kwargs):
        ...
        if self.engine == 'osrm':
            ...
            raise NotImplementedError(
                'There is no implementation for this matcher'
            )
    ...
    def _valhalla_match(self, coords, costing, shape_match, **kwargs):
        ...
        kwargs.update({"trace_options": {
            "search_radius": 20
        }})
        payload = create_valhalla_payload(coords, costing,
                                          shape_match, **kwargs)
    ...
    def post_process_matching(self, match_points, roads):
        ...
        if self.engine == 'valhalla':
            match_points = match_points.sort_values([constants.UUID, constants.DATETIME])
