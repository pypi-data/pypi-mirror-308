...
def get_transformed_extents(
    ...
    for x in ax.collections:
        for path in x.get_paths():
            [p1x, p1y], [p2x, p2y] = path.transformed(
                transform=transform
            ).get_extents().get_points()
...
@deprecated("Les méthodes de pandas sont plus simples et plus rapides " +
            "à utiliser")
...
def create_bounds_gdf(
    ...
    bounds = geopandas.GeoDataFrame(geopandas.GeoSeries(bb_polygon),
                                    columns=['geometry'], crs=STUDY_CRS)

    bbox = bounds.total_bounds

    return bounds, bbox

def basemap_from_shapefile(bounds: Bounds,
    ...
    # hydro
    geopandas.clip(geopandas.read_file(hydro_path).to_crs(STUDY_CRS), df2)\
       .plot(
    ...
    # road
    geopandas.clip(geopandas.read_file(road_path).to_crs(STUDY_CRS), df2)\
       .plot(
...
################################
def _numerical_cuts(y: ArrayLike, k: int = 5, base: float = 1.0, decimals: int = 0) -> Dict[str, float]:
...
def set_color(
    ...
    data_num = data[[
        pandas.api.types.is_number(x) and
        not pandas.isna(x)
        for x in data[col]
    ]].copy()
    ...
    if not pandas.api.types.is_numeric_dtype(data[col].dtype):
        data_cat = data[[not (
            pandas.api.types.is_number(x) or
            pandas.isna(x)
        ) for x in data[col]]].copy()
...
def _generic_plot_map(
    ...
    if rotation.__class__ not in Numeric.__args__:
        raise ValueError(
            '`rotate` must be of type int or float,' +
            f'received {rotation.__class__}'
        )
    ...
    if anotate:
        ...
        x, y = (delim_gdf.unary_union.centroid.x,
                delim_gdf.unary_union.centroid.y)

        data_num = data[[pandas.api.types.is_number(x) and not pandas.isna(x)
                         for x in data[col]]].copy()
        if pandas.api.types.is_numeric_dtype(data[col].dtype):
            data_num = data[[not pandas.isna(x) for x in data[col]]].copy()
    ...
    left, right, bottom, top = _reproj_bb(ominx, omaxx, ominy, omaxy,
                                          data.crs.to_string(), {"init": "epsg:4326"})
    zoom = ctx.tile._calculate_zoom(left, bottom, right, top)

    # query the tile
    # Contextily already cache tile (see: https://bit.ly/contextily-cache)
    ctx.add_basemap(data_ax, crs=data.crs.to_string(), source=config.CURRENT_TILE,
                    zoom=min(config.CURRENT_TILE['max_zoom'],zoom))
    ctx.add_basemap(bg_ax, crs=data.crs.to_string(), source=config.CURRENT_TILE,
                    zoom=min(config.CURRENT_TILE['max_zoom'],zoom))
    ...
    save_path_parts = [save_path_parts[0]] + [parts.replace(':', '-') for parts in save_path_parts[1:]]
...
def _plot_leg(numeric_cuts: Dict[str, Numeric],
    ...
    # assign colors
    if base_cat_colors is None: base_cat_colors = config.BASIC_CAT_COLORS
...
def _handle_patch(patch_kwargs, patch_type='point', label_order=None, memory=None):
    ...
    if label_order is None: label_order = []
    if patch_type == 'point':
...
def _build_legend(ax: plt.Axes,
    ...
    """
    if lignes_kwords is None: lignes_kwords = []
    if points_kwords is None: points_kwords = []
    if kdes_kwords is None: kdes_kwords = []
    if label_order is None: label_order = []

    memory={}
    #handle kde like patches
    memory, sorted_boxes, unsorted_boxes = _handle_patch(kdes_kwords,
                                                         patch_type='boxes',
                                                         label_order=label_order,
                                                         memory=memory)
    #handle lines
    memory, sorted_lines, unsorted_lines = _handle_patch(lignes_kwords,
                                                         patch_type='lines',
                                                         label_order=label_order,
                                                         memory=memory)
    #handle points
    memory, sorted_point, unsorted_point = _handle_patch(points_kwords,
                                                         patch_type='point',
                                                         label_order=label_order,
                                                         memory=memory)

    sorted_handles={**sorted_boxes, **sorted_lines, **sorted_point}
    unsorted_handles = list(unsorted_boxes.values()) + \
                       list(unsorted_lines.values()) + \
                       list(unsorted_point.values())
    unsorted_handles_labels = list(unsorted_boxes.keys()) + \
    ...
    #generate the legend
    leg = ax.legend([sorted_handles[key] for key in label_order] + unsorted_handles,
                    #labels
...
### Generic figs
##############
def plot_base_dist_graphs(counts: CountList,
                          groupnames: List[str],
    ...
    with sns.axes_style("darkgrid"):
        ...
        if x is not None:
            ...
            axe.stackplot(range(len(counts[0])), *counts, labels=groupnames,
                          colors=colormap)
