...
class Iconnector_using_points(Iconnector):
    ...
    def _exchange_points(self):
        ...
        # buffer surdimmensionnee
        recv = np.empty(max_size*n_proc, dtype=pts.dtype)
        self.univers.Allgather(pts, recv)
        recv.shape = (n_proc, max_size)
        new_recv = []
        for i, size in enumerate(sizes):
            data = recv[i][:size]
            data.shape = (-1, 3)
            new_recv.append(data)
        self.all_points = new_recv
