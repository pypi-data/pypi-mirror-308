...
class FileUploaded:
    ...
    def valid_content(self, **kwargs):
        ...
        try:
            ...
            self._encoding = result_charset['encoding']
            if confidence < 0.6:
                logging.warning(
            ...
            if self._encoding is None:
                self._encoding = "ISO-8859–1"
        logging.debug(f'Detected Encoding is: {self._encoding}')
        # if self._encoding == 'ISO-8859–1':
        #     # try to decoding and encoding again:
        #     s = str(self._content.decode('ISO-8859–1').encode('ISO-8859–1'), self._encoding)
        # else:
        s = str(self._content, self._encoding)
        try:
            data = StringIO(s)
