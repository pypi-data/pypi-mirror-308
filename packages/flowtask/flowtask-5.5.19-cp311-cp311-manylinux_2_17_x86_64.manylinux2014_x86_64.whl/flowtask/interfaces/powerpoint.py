...
class PowerPointClient(ABC):
    ...
    def _add_content_to_slide(
        ...
        :param content: The content dictionary for the slide.
        :param image_info: Dictionary with image path and optionally size details.
        """
        # Create a placeholder map by name
        placeholder_map = {shape.name: shape for shape in slide.placeholders}
        # Map text to specific placeholders by ID
            ...
            for placeholder_id, text_details in text_entry.items():
                ...
                if placeholder_id in placeholder_map:
                    try:
                        ...
                        # Make sure the placeholder is indeed a text frame
                        if placeholder.has_text_frame:
                            text_frame = placeholder.text_frame
                            new_paragraph = text_details.get("new_paragraph", False)

                            if new_paragraph:
                                # Add text as a new paragraph if it's not the first entry
                                if len(text_frame.paragraphs) > 0:
                                    p = text_frame.add_paragraph()
                                else:
                                    p = text_frame.paragraphs[0]
                            else:
                                # Use the first paragraph and add runs to avoid new lines
                                p = text_frame.paragraphs[0]

                            # Set text and styling for this entry
                            run = p.add_run()
                            run.text = text_content
                            font = run.font
                            font.name = text_details.get("font_name", "Calibri")
                            font.size = Pt(text_details.get("font_size", 18))
                        ...
                        # try to add the content manually
                        # Always add a textbox
                        left = Inches(1)
                        top = Inches(1)
                        width = Inches(8)
                        height = Inches(2)
        ...
        if image_info:
            ...
            img_id = image_info.get("placeholder_id")
            # print('HERE > ', len(slide.placeholders))
            # for shape in slide.placeholders:
            #     print('%d %s' % (shape.placeholder_format.idx, shape.name))
            if img_id in placeholder_map:
                slide_holder = placeholder_map[img_id]
                try:
                    picture = slide_holder.insert_picture(image_path)
                    ...
                    # Define the position with optional offsets
                    left = image_info.get("left", Inches(1))
                    top = image_info.get("top", Inches(2))
                    ...
                    # Calculate scaling factor considering the offset
                    if top == "center":
                    ...
                    scale_factor = image_info.get("scale_factor", None)
                    if not scale_factor:
                        scale_factor = min(
                    ...
                    width = image_info.get("width", width)
                    height = image_info.get("height", height)
                    slide.shapes.add_picture(
                        image_path, left, top, width=width, height=height
                    )
            else:
                # Adding Manually the image at center of slide
