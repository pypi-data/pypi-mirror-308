...
def to_time(
    ...
    def convert_to_time(value):
        if pandas.isnull(value):
            return None
        if isinstance(value, (datetime.time, datetime.datetime, pandas.Timestamp)):
    ...
    try:
        df[field] = df[field].apply(lambda x: convert_to_time(x) if pandas.notnull(x) else None)
    except Exception as err:
    ...
    try:
        if as_timedelta:
            df[field] = df[field].apply(
                ...
                ) if t is not None else pandas.NaT
            )
        else:
    ...
    if replace_nulls:
        ...
        df[field] = df[field].apply(lambda x: value if pandas.isnull(x) else x)
...
def convert_to_array(
    ...
    """
    Converts the values in a specified column of a pandas DataFrame to arrays, optionally removing empty strings and duplicates.
...
def string_to_date(df: pandas.DataFrame, field: str, column="", format="%Y-%m-%d"):
    ...
    """
    df[field] = pandas.to_datetime(df[column], format=format, errors="coerce")
    df[field] = df[field].replace({pandas.NaT: None})
...
def from_currency(df, field: str, symbol="$", remove_nan=True):
    """
    Converts a currency field to numeric values, handling symbols and NaNs.

    :param df: The DataFrame containing the field to be converted.
    :param field: The name of the field to be converted.
    :param symbol: The currency symbol to be removed.
    :param remove_nan: Whether to remove NaN values.
    :return: The DataFrame with the converted field.
    """
    df[field] = (
        df[field]
...
def split_cols(df, field: str, separator=",", columns=[], numcols=2):
    ...
    if isinstance(columns, list):
        ...
        try:
            ...
            df[columns] = pandas.DataFrame(split_data.tolist(), index=df.index)
        except Exception as err:
            print(
...
def to_excel_date(df: pandas.DataFrame, field: str, column: str = None):
    """to_excel_date.
...
def fill_map(
    ...
    try:
        ...
        df[field] = df.apply(
            lambda row: mapa.get(row[column], row[field]) if pandas.isnull(row[field]) else row[field],
            axis=1
        )
...
def create_daterange_from_columns(df: pandas.DataFrame, field: str, start_column: str, end_column: str):
    """
    ...
    df[field] = df.apply(
        ...
        if pandas.isnull(row[end_column]) or row[end_column].strip() == ""
