...
from snowflake.core._common import CreateMode, DeleteMode, ObjectCollection, ObjectReferenceMixin
...
class DatabaseRoleCollection(ObjectCollection["DatabaseRoleResource"]):
    ...
    def __init__(self, database: "DatabaseResource", root: "Root") -> None:
        super().__init__(DatabaseRoleResource)
    ...
    def create(
        ...
        self._api.create_database_role(self.database.name, database_role, StrictStr(real_mode))
...
class DatabaseRoleResource(ObjectReferenceMixin[DatabaseRoleCollection]):
