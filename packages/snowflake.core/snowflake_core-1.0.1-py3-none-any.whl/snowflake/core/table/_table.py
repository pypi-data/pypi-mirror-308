    ...
    def create(
        ...
        *,
        ...
        if as_select:
            ...
            source_table_path = real_clone.source.split(".")
            source_database = self.database.name
            source_schema = self.schema.name
            source_table = source_table_path[-1]
            if len(source_table_path) == 3:
                source_database = source_table_path[0]
                source_schema = source_table_path[1]
            elif len(source_table_path) == 2:
                source_schema = source_table_path[0]
            elif len(source_table_path) != 1:
