    ...
    def create(
        ...
        **Creating a stream from scratch**
        ...
        if clone_stream:
            ...
            source_stream_path = real_clone.source.split(".")
            source_database = self.database.name
            source_schema = self.schema.name
            source_stream = source_stream_path[-1]
            if len(source_stream_path) == 3:
                source_database = source_stream_path[0]
                source_schema = source_stream_path[1]
            elif len(source_stream_path) == 2:
                source_schema = source_stream_path[0]
            elif len(source_stream_path) != 1:
                raise NotFoundError(reason=f"{real_clone.source} does not exists", root=self.root)

            self._api.clone_stream(
                source_database, source_schema, source_stream,
