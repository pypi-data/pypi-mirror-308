...
SETTINGS = {
    "Indentation size (spaces)": INDENT_LVL
}
...
config_filename = "SB_NATIVE_CONFIG.yaml"
filepath = d.joinpath(config_filename)
if config_filename not in os.listdir(d):
    with open(d.joinpath(filepath), "w") as stream:
        yaml.dump(SETTINGS, stream)

with open(d.joinpath(filepath), "r") as stream:
