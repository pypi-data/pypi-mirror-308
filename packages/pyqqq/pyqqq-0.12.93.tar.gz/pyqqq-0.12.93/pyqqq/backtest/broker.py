    ...
    def get_account(self):
        ...
        Returns:
            dict: 다음 정보를 포함하는 계좌 정보:
                - total_balance (int|float): 총 평가 금액
                - purchase_amount (int|float): 매입 금액
                - evaluated_amount (int|float): 평가 금액
                - pnl_amount (int|float): 손익 금액
                - pnl_rate (float): 손익률
...
class MockBroker(BaseBroker):
    ...
    Note:
        - 모든 가격과 수량은 정수형으로 처리됩니다.
        - 거래 시간은 분 단위 또는 일 단위로 설정할 수 있습니다.
        - 거래 이력은 CSV 파일로 저장할 수 있습니다.
    ...
    .. code-block:: python
        ...
        broker = MockBroker(clock, position_provider, time_unit="minutes")
    ...
    def __init__(self, clock: WallClock, position_provider, time_unit="minutes"):
    ...
    def _get_minute_price_with_cache(self, date, code) -> pd.DataFrame:
        """
        리턴값에서 해당 날짜의 첫 데이터가 9시 정각이 아닐 수 있다.
        """
        dfs = get_all_day_data(date, [code], dtm.timedelta(minutes=1), source="kis")
        df = dfs[code]
        if df.empty:
            return df
    ...
    def _sell_position(self, order_no: str, asset_code: str, quantity: int, price: int):
        for pos in self.positions:
            if pos.asset_code == asset_code:
                ...
                tax = sell_value * Decimal(0.0018)
                fee = sell_value * Decimal(0.000036396)
