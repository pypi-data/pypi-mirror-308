use cpython::{
    ObjectProtocol, PyBytes, PyClone, PyErr, PyIterator, PyObject, PyResult, PyString, PyTuple,
    Python, PythonObject, TypeError, ValueError,
};
use log::{debug, error};
use pyo3_ffi::PyObject_GetIter;
...
impl<C: Connection> WSGIResponse<C> {
    ...
    fn check_content_length_exceeds_data(
        ...
        if let Some(cl) = start_response.content_length(py) {
            if start_response.content_bytes_written(py) < cl {
                ...
                match self.iterable.as_mut() {
                    ...
                    Some(obj) => match PyIterator::from_object(py, obj.clone_ref(py)) {
                        Ok(mut iter) => match iter.next() {
                            None => {
                                if start_response.headers_not_sent(py) {
