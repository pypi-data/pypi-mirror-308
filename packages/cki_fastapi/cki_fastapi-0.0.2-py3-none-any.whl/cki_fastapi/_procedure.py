...
import requests
from ._metadata import FilePathInput, FilePathsInput, FilePathOutput, FilePathsOutput, ProcedureInfo, ProcedurePageInfo
...
class UpdateInputCellAction(BaseModel):
    type: Literal['update_input_cell']
    input_name: str
    row: int
    column: str


ProcedureAction = Annotated[Union[RunAction, InitInputsAction,
                                  UpdateInputAction, UpdateInputCellAction], Field(discriminator="type")]

_UpdateAction = Annotated[Union[InitInputsAction, UpdateInputAction,
                                UpdateInputCellAction], Field(discriminator="type")]
...
class ProcedureContext:
    ...
    _run_context: _RunContext | None = None

    _validations: list[Validation]
    _input_validations: dict[str, Validation]

    _report: Callable[[Validation], None] | None = None
...
class _ProcInfoFull:
    def __init__(self, proc_func, *, module_name: str, unique_id: str, name: str, group: str, version: int, immediate: bool, inputs_require_initialization: bool, page_info: ProcedurePageInfo | None) -> None:
        self.module_name = module_name
        self.func = proc_func
        sig = signature(proc_func)
        self.sig = sig
        proc = ProcedureInfo(sig, unique_id=unique_id, name=name, group=group, version=version,
                             immediate=immediate, inputs_require_initialization=inputs_require_initialization, page_info=page_info)
        self.basic = proc
...
def _create_run_operation(info: _ProcInfoFull, dependencies: Sequence[Depends] | None):
    ...
    router = _routers[info.module_name]
    
    deps = [Depends(_initialize)]
    if dependencies:
        deps.append(dependencies)
    @router.post("/run/" + info.basic.unique_id, dependencies=deps)
    ...
    async def run(**kwargs):
        ...
        entry_id = -1
        def report_validation(validation: Validation):
            nonlocal entry_id
            entry_id = entry_id + 1
            assert _ck_client is not None
            _ck_client.store_log_entry(run_id, entry_id, validation.model_dump())

        context._report = report_validation
...
def _create_update_operation(info: _ProcInfoFull, dependencies: Sequence[Depends] | None):
    ...
    if dependencies:
        deps.append(dependencies)
    @router.post("/updateui/" + info.basic.unique_id, dependencies=deps)
...
class _CodeKraftClient:
    ...
    def set_procedure_ended(self, run_id: int, proc_id: int, status: str, run_context: dict[str, Any]):
        ...
        asyncio.run_coroutine_threadsafe(self.request_token_async(), self.loop)
    ...
    def upload_files(self, paths: list[str]):
        ...
        for path in paths:
            ...
            resp = requests.post(self.ck_url + 'api/extapp/uploadfile',
                                 headers={'authorization': self.auth_header}, files={'file': (os.path.basename(path), open(path, 'rb'))}, timeout=timeout)
            if resp.ok:
...
class ModuleInfo:
    def __init__(self, *, name: str, display_name: str):
        ...
        self.procedures = []
        _routers[self.name] = APIRouter(prefix="/procedures")
    ...
    def set_static_path(path):
        global _static_path
        _static_path = os.path.realpath(path)

    def configure(self, app: FastAPI):
        _modules[self.name] = self

        app.include_router(_routers[self.name])

        global _ck_client
        if _ck_client is None:
            _ck_client = _CodeKraftClient(app)

        all_errors = [proc.name + ": " + e for proc in self.procedures for e in proc.errors]
        if all_errors:
    ...
    def procedure(self, *, unique_id: str, name: str, group="", version: int, immediate=False, inputs_require_initialization=False, page_info: ProcedurePageInfo | None = None, dependencies: Optional[Sequence[Depends]] = None):

        def decorator(proc_func):

            info = _ProcInfoFull(proc_func, module_name=self.name, unique_id=unique_id, name=name, group=group, version=version, immediate=immediate, inputs_require_initialization=inputs_require_initialization, page_info=page_info)
