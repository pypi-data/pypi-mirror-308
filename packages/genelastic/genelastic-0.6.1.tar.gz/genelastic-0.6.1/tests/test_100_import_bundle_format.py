...
from genelastic.import_data import (ImportBundle, make_import_bundle_from_files,
...
def test_v1() -> None:
    files = glob.glob(os.path.join(RES_DIR, 'import_bundles', 'v1_*.yml'))
...
def test_v1_single_vcf() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'v1_single_vcf.yml')
...
def test_v1_two_vcf_files() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'v1_two_vcf_files.yml')
    bdl_dicts = load_import_bundle_file(f)
    assert len(bdl_dicts) == 1
    assert bdl_dicts[0]['version'] == 1
    bdl = make_import_bundle_from_files([f])
...
def test_v1_two_duplicated_vcf_files() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'v1_two_duplicated_vcf_files.yml')
    bdl_dicts = load_import_bundle_file(f)
    assert len(bdl_dicts) == 1
    assert bdl_dicts[0]['version'] == 1
    bdl = make_import_bundle_from_files([f])
...
def test_v2_file_pattern() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'v2_file_pattern.yml')
    bdl = make_import_bundle_from_files([f])
    assert bdl.get_nb_files('vcf') == 1
    assert bdl.get_nb_files('cov') == 0
    ...
    assert bdl.get_files('vcf')[0].metadata['sequencer_indices'] == \
...
def test_v3_file_pattern() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'v3_analyses.yml')
    bdl = make_import_bundle_from_files([f])
    assert bdl.get_nb_files('vcf') == 1
    assert bdl.get_nb_files('cov') == 0
    ...
    assert len(bdl.analyses) == 1
    assert bdl.analyses[0].metadata == {'barcode': 'C0034BP',
                                        'sample_name': 'HG0003',
                                        'source': 'CNRGH',
...
def test_v3_files_list() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'v3_files_list.yml')
    bdl = make_import_bundle_from_files([f])
    assert bdl.get_nb_files('vcf') == 2
...
def test_v3_files_list_with_gz() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'v3_files_list_with_gz.yml')
    bdl = make_import_bundle_from_files([f])
...
def test_v3_empty_data_folder() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'v3_analyses_empty_folder.yml')
    bdl = make_import_bundle_from_files([f])
...
def test_v3_non_matching_files() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'v3_non_matching_files.yml')
    bdl = make_import_bundle_from_files([f])
    assert bdl.get_nb_files('vcf') == 0
    assert bdl.get_nb_matched_files() == 0
...
def test_v3_one_matching_one_non_matching() -> None:
    f = os.path.join(RES_DIR, 'import_bundles',
                     'v3_one_matching_one_non_matching.yml')
    bdl = make_import_bundle_from_files([f])
    assert bdl.get_nb_files('vcf') == 1
    assert bdl.get_nb_matched_files() == 1
...
def test_v3_two_analyses_in_two_documents() -> None:
    f = os.path.join(RES_DIR, 'import_bundles',
                     'v3_two_analyses_in_two_documents.yml')
    bdl = make_import_bundle_from_files([f])
    assert bdl.get_nb_files('vcf') == 2
    assert bdl.get_nb_files('cov') == 0
    assert len(bdl.analyses) == 2
...
def test_v3_varying_coverage() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'v3_varying_coverage.yml')
    bdl = make_import_bundle_from_files([f])
...
def test_v3_several_dummy_files() -> None:
    pattern = os.path.join(RES_DIR, 'import_bundles', 'v3_dummy*.yml')
...
def test_v3_file_pattern_with_rep1() -> None:
    f_rep1 = os.path.join(RES_DIR, 'import_bundles', 'v3_rep1.yml')
...
def test_v3_file_pattern_with_custom_regex() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'v3_custom_regex.yml')
    bdl = make_import_bundle_from_files([f])
    assert bdl.get_nb_files('vcf') == 1
    assert bdl.get_nb_files('cov') == 0
    assert bdl.get_nb_matched_files() == 1
...
def test_v3_file_pattern_with_custom_end_of_file() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'v3_custom_regex_end_of_file.yml')
    bdl = make_import_bundle_from_files([f])
    assert bdl.get_nb_files('vcf') == 4
    assert bdl.get_nb_files('cov') == 0
...
def test_v3_random_string_regex() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'v3_random_string_regex.yml')
    bdl = make_import_bundle_from_files([f])
    assert bdl.get_nb_files('vcf') == 2
    assert bdl.get_nb_files('cov') == 0
...
def test_v3_no_analyses() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'v3_no_analyses.yml')
    bdl = make_import_bundle_from_files([f])
    assert bdl.get_nb_files('vcf') == 0
    assert bdl.get_nb_files('cov') == 0
    assert bdl.get_nb_matched_files() == 0
...
def test_v3_wet_processes_only_mandatory_keys() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'wet_processes', 'only_mandatory_keys.yml')
    bdl = make_import_bundle_from_files([f])
...
def test_v3_wet_processes_both_mandatory_and_optional_keys() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'wet_processes',
...
def test_v3_wet_processes_two_in_one_file() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'wet_processes', 'two_in_one_file.yml')
    bdl = make_import_bundle_from_files([f])
    wet_processes = bdl.wet_processes
    assert len(wet_processes) == 2
    assert wet_processes.get_process_ids() == {"wgs-novaseqxplus-25b", "wgs-novaseqxplus-10b"}
...
def test_v3_wet_processes_two_in_one_file_with_the_same_id() -> None:
    ...
    assert (exc_info.value.args[0] ==
            "A wet process with the id 'wgs-novaseqxplus-25b' is already present.")


def test_v3_wet_processes_mandatory_key_omitted() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'wet_processes', 'mandatory_key_omitted.yml')
...
def test_v3_wet_processes_empty_processes() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'wet_processes', 'empty_processes.yml')
    bdl = make_import_bundle_from_files([f])
    wet_processes = bdl.wet_processes
...
def test_v3_wet_processes_no_processes() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'wet_processes', 'no_processes.yml')
    bdl = make_import_bundle_from_files([f])
    wet_processes = bdl.wet_processes
    assert len(wet_processes) == 0
    assert not wet_processes.get_process_ids()
...
def test_v3_bi_processes_only_mandatory_keys() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'bi_processes', 'only_mandatory_keys.yml')
    bdl = make_import_bundle_from_files([f])
...
def test_v3_bi_processes_both_mandatory_and_optional_keys() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'bi_processes',
                     'both_mandatory_and_optional_keys.yml')
    bdl = make_import_bundle_from_files([f])
    bi_processes = bdl.bi_processes
    assert len(bi_processes) == 1
...
def test_v3_bi_processes_two_in_one_file() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'bi_processes', 'two_in_one_file.yml')
    bdl = make_import_bundle_from_files([f])
    bi_processes = bdl.bi_processes
...
def test_v3_bi_processes_two_in_one_file_with_the_same_id() -> None:
    ...
    with pytest.raises(ValueError) as exc_info:
        make_import_bundle_from_files([f])
    assert exc_info.value.args[0] == "A bi process with the id 'varscope-3' is already present."


def test_v3_bi_processes_mandatory_key_omitted() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'bi_processes', 'mandatory_key_omitted.yml')
    with pytest.raises(SchemaError) as exc_info:
        make_import_bundle_from_files([f])
    assert "Missing key: 'proc_id'" in str(exc_info.value)
...
def test_v3_bi_processes_empty_processes() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'bi_processes', 'empty_processes.yml')
    bdl = make_import_bundle_from_files([f])
    bi_processes = bdl.bi_processes
...
def test_v3_bi_processes_no_processes() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'bi_processes', 'no_processes.yml')
    bdl = make_import_bundle_from_files([f])
    bi_processes = bdl.bi_processes
    assert len(bi_processes) == 0
    assert not bi_processes.get_process_ids()
...
def test_v3_bi_processes_no_steps() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'bi_processes', 'no_steps.yml')
    with pytest.raises(SchemaError) as exc_info:
        make_import_bundle_from_files([f])
    assert "Key 'steps' error" in str(exc_info.value)


def test_v3_bi_processes_one_step() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'bi_processes', 'one_step.yml')
    bdl = make_import_bundle_from_files([f])
    bi_processes = bdl.bi_processes
    assert len(bi_processes) == 1
    assert bi_processes.get_process_ids() == {"varscope-3"}
...
def test_v3_load_analysis_with_defined_bi_and_wet_processes() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'multi',
...
def test_v3_load_analysis_from_multiple_files() -> None:
    filepaths = [
        os.path.join(RES_DIR, "import_bundles", "v3_analyses.yml"),
        os.path.join(RES_DIR, "import_bundles", "v3_two_analyses_in_two_documents.yml")
    ]
...
def test_v3_load_analysis_and_processes_from_multiple_files() -> None:
    filepaths = [
        os.path.join(RES_DIR, 'import_bundles', "v3_analyses.yml"),
        os.path.join(RES_DIR, 'import_bundles', "multi",
                     "analysis_with_defined_bi_and_wet_processes.yml"),
        os.path.join(RES_DIR, 'import_bundles', "wet_processes", "only_mandatory_keys.yml"),
        os.path.join(RES_DIR, 'import_bundles', "bi_processes", "only_mandatory_keys.yml"),
    ]

    bdl = make_import_bundle_from_files(filepaths)
    assert bdl.get_nb_files("vcf") == 2
    assert bdl.analyses.get_nb_files() == 2
    assert bdl.wet_processes.get_process_ids() == {"wgs-novaseqxplus-25b", "novaseqxplus-10b"}
    assert bdl.bi_processes.get_process_ids() == {"dragen-4123", "varscope-3"}
    assert bdl.analyses.get_all_categories() == {"vcf"}


def test_v3_custom_prefix() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'tags', 'v3_custom_prefix.yml')
    bdl = make_import_bundle_from_files([f])
    assert bdl.analyses.get_nb_files() == 1


def test_v3_custom_suffix() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'tags', 'v3_custom_suffix.yml')
    bdl = make_import_bundle_from_files([f])
    assert bdl.analyses.get_nb_files() == 1


def test_v3_custom_prefix_and_suffix() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'tags', 'v3_custom_prefix_and_suffix.yml')
    bdl = make_import_bundle_from_files([f])
    assert bdl.analyses.get_nb_files() == 1


def test_v3_default_prefix_and_suffix() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'tags', 'v3_default_prefix_and_suffix.yml')
    bdl = make_import_bundle_from_files([f])
    assert bdl.analyses.get_nb_files() == 1
...
def test_v3_tags_must_be_defined_only_once() -> None:
    files = [
        os.path.join(RES_DIR, 'import_bundles', 'tags', 'v3_default_prefix_and_suffix.yml'),
        os.path.join(RES_DIR, 'import_bundles', 'tags', 'v3_tags.yml')
    ]
    with pytest.raises(RuntimeError) as exc_info:
        make_import_bundle_from_files(files)
    assert exc_info.value.args[0] == (f"Only one 'tags' key should be defined "
                                      f"across all documents, "
                                      f"but multiple were found : {', '.join(files)}")


def test_v3_tags_are_applied_to_all_analyses() -> None:
    files = [
        os.path.join(RES_DIR, 'import_bundles', 'tags', 'v3_tags.yml'),
        os.path.join(RES_DIR, 'import_bundles', 'tags', 'v3_no_tags.yml')
    ]
...
def test_v3_word_char_in_prefix() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'tags', 'v3_word_char_in_prefix.yml')
    with pytest.raises(SchemaError) as exc_info:
        make_import_bundle_from_files([f])
    assert (exc_info.value.args[0] ==
...
def test_v3_excluded_char_in_prefix() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'tags', 'v3_excluded_char_in_prefix.yml')
    with pytest.raises(SchemaError) as exc_info:
        make_import_bundle_from_files([f])
    assert (exc_info.value.args[0] ==
            "Key 'prefix' should only contain one special character, "
...
def test_v3_more_than_one_char_in_prefix() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'tags', 'v3_more_than_one_char_in_prefix.yml')
    with pytest.raises(SchemaError) as exc_info:
        make_import_bundle_from_files([f])
    assert (exc_info.value.args[0] ==
            "Key 'prefix' should only contain one special character, "
...
def test_v3_non_word_chars_in_tags() -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'tags', 'v3_non_word_chars_in_tags.yml')
    with pytest.raises(SchemaError) as exc_info:
        make_import_bundle_from_files([f])
...
def test_v3_undefined_tags_used_in_file_prefix_default_prefix(caplog: LogCaptureFixture) -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'tags',
    ...
    assert ("String '%a_' in key 'file_prefix' looks like an undefined tag. "
            "If this string is not a tag, you can ignore this warning.") in caplog.text
    assert ("String '%b' in key 'file_prefix' looks like an undefined tag. "
            "If this string is not a tag, you can ignore this warning.") in caplog.text


def test_v3_undefined_tags_used_in_file_prefix_custom_prefix_and_suffix(caplog: LogCaptureFixture) \
        -> None:
    f = os.path.join(RES_DIR, 'import_bundles', 'tags',
    ...
    # 'file_prefix' is wrong, so we don't expect any matched files.
    assert bdl.analyses.get_nb_files() == 0
    assert ("String '$a$' in key 'file_prefix' looks like an undefined tag. "
            "If this string is not a tag, you can ignore this warning.") in caplog.text
    assert ("String '$b$' in key 'file_prefix' looks like an undefined tag. "
            "If this string is not a tag, you can ignore this warning.") in caplog.text
