...
class Solver_Ignore_Multiplicity(Parent_Solver):

    def solve(self) -> None:
        """
        Implements the Ignoring Incremental Multiplicity Column Generation method (Algorithm 5).
        """
        ...
        while True:
            ...
            new_routes = self.sp_solver.solve_ignoring_shortest_path(duals)

            if not new_routes:
                print("\t=== Ignoring algorithm fails to find any new routes, adding resource(s) with biggest multiplicity ===")
                # Attempt to find new routes using the incremental multiplicity shortest path solver
                new_routes = self.sp_solver.solve_ignoring_incremental_multiplicity_shortest_path(duals)
                if not new_routes:
                    print("Solved SP. Found no column with <0 reduced cost.")
                    break

            print(f"Solved SP. Found {len(new_routes)} columns with <0 reduced cost.")
            print(f"Here are some new routes: ")
            i = 0
        ...
        print("\n The value in the brackets behind the routes indicates the  time cost")
        print("\n=== Final active routes ===")
        for var in result_variables:
            result_var_value = var.X  # Get the value of the variable
            if result_var_value > 1e-6:
                print(f"\t{var.VarName}, with the variable value {result_var_value}")  # Assumes Route class has __str__ or __repr__
