...
class InvalidTokenError(WError):
    status_code = 422
    operational_code = WBaseOpCode.INVALID_TOKEN
    message = "Invalid token, impossible to check"
...
class JwtService(AbstractService):
    ...
    def create_jwt_tokens(self, payload: dict, ts=None) -> JwtTokens:
        if ts is None:
    ...
    def _decode_token(self, token, secret_key) -> DecodedJwtToken:
        try:
        ...
        return DecodedJwtToken(decoded_token, has_expired)
