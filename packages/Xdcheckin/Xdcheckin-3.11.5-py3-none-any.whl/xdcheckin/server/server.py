...
async def _ids_login_prepare(req):
	try:
		...
		ids = server_ses.pop("ids", None)
		if ids:
			_create_task(ids.__aexit__(None, None, None))
		server_ses["ids"] = ids = await _IDSSession(
		...
		if server_ses.get("ids", None):
			_create_task(server_ses["ids"].__aexit__(
				None, None, None
			))
		data = {"err": f"{e}"}
	finally:
...
async def _ids_login_finish(req):
	try:
		...
		_data = {
			...
			"chaoxing_config": data[
				"chaoxing_config"
			] if "chaoxing_config" in data else "{}"
		}
...
async def _chaoxing_login(req, data = None):
	try:
		...
		courses = dict(_islice(cx.courses.items(), config[
			"chaoxing_course_get_activities_courses_limit"
		]))
		data = {
			"fid": cx.fid, "courses": courses, "cookies":
			_dumps({k: v.value for k, v in cx.cookies.items()})
		}
	except Exception as e:
		if server_ses.get("cx", None):
			_create_task(server_ses["cx"].__aexit__(
				None, None, None
			))
		if server_ses.get("nx", None):
