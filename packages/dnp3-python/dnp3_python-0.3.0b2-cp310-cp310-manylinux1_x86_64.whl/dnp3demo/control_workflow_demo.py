...
def main():
    ...
    while count < 10:
        ...
        if count % 3 == 1:
            ...
            for i, pts in enumerate([point_values_0, point_values_1, point_values_2]):
                ...
                master_application.send_direct_operate_command(
                    opendnp3.AnalogOutputDouble64(float(p_val)), i, command_callback
                )
        ...
        p_val = random.choice(
            [opendnp3.ControlCode.LATCH_ON, opendnp3.ControlCode.LATCH_OFF]
        )
        master_application.send_direct_operate_command(
            opendnp3.ControlRelayOutputBlock(p_val), 1, command_callback
        )
        ...
        command_set.Add(
            [
                opendnp3.WithIndex(
                    opendnp3.ControlRelayOutputBlock(opendnp3.ControlCode.LATCH_ON), 3
                ),
                opendnp3.WithIndex(
                    opendnp3.ControlRelayOutputBlock(opendnp3.ControlCode.LATCH_OFF), 4
                ),
                opendnp3.WithIndex(
                    opendnp3.ControlRelayOutputBlock(opendnp3.ControlCode.LATCH_ON), 5
                ),
            ]
        )
        master_application.send_direct_operate_command_set(
            command_set, command_callback
        )
        ...
        if count % 3 == 1:
            ...
            for i, pts in enumerate([point_values_0, point_values_1, point_values_2]):
                ...
                master_application.send_direct_point_command(
                    group=40, variation=4, index=i + 4, val_to_set=p_val + 0.0025
                )  # operate on 4,5,6
                # for group40var2 AnalogOutputInt32
                master_application.send_direct_point_command(
                    group=40, variation=2, index=i + 4 + 4, val_to_set=int(p_val)
                )  # operate on 8,9,10
                # for group10var2 BinaryOutput
                master_application.send_direct_point_command(
                    group=10, variation=2, index=i + 4 + 4, val_to_set=True
                )  # operate on 8,9,10
        ...
        print(
            f"===important log: case6 get_db_by_group_variation ==== {count}",
            "\n",
            datetime.datetime.now(),
            result,
        )
        ...
        print(
            f"===important log: case6b get_db_by_group_variation ==== {count}",
            "\n",
            datetime.datetime.now(),
            result,
        )
        ...
        print(
            f"===important log: case6c get_db_by_group_variation ==== {count}",
            "\n",
            datetime.datetime.now(),
            result,
        )
