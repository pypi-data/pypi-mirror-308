...
DbStorage = Dict[
    opendnp3.GroupVariation, Dict[int, DbPointVal]
]  # e.g., {GroupVariation.Group30Var6: {0: 4.8, 1: 14.1, 2: 27.2, 3: 0.0, 4: 0.0}
...
class MyMaster:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not self.stack_config:
            ...
            self.stack_config.link.RemoteAddr = (
                outstation_id  # meant for outstation, use 1 as default
            )
            self.stack_config.link.LocalAddr = (
                master_id  # meant for master station, use 2 as default
            )
        ...
        )  # TODO: check why this seems not working
        ...
        self.slow_scan = self.master.AddClassScan(
            opendnp3.ClassField().AllClasses(),  # TODO: add interface entrypoint
    ...
    def send_direct_operate_command_set(
        self,
        command_set,
    ...
    def send_select_and_operate_command(
        self,
        command,
        index,
        callback=asiodnp3.PrintingCommandCallback.Get(),
    ...
    def _retrieve_all_obj_by_gvids_w_ts(
        self,
        ...
        for gv_id in gv_ids:
            ...
            filtered_db_w_ts.update(
                {
                    gv_cls: (
                        self.soe_handler.gv_last_poll_dict.get(gv_cls),
    ...
    def _get_updated_val_storage(self, gv_id: opendnp3.GroupVariationID) -> DbStorage:
        ...
        while gv_db_val is None and n_retry < retry_max:
            ...
            if gv_db_val is None:
                _log.debug(
                    f"No value returned when polling {gv_cls}. "
            ...
            if n_retry >= retry_max:
                _log.warning(
                    "==Retry numbers hit retry limit {}, when polling {}==".format(
                        retry_max, gv_cls
                    )
                )
                ...
                self.soe_handler.gv_index_value_nested_dict[gv_cls] = (
                    None  # Note: redundant, but explicitly define again.
                )
    ...
    def get_db_by_group_variation_index(
        self, group: int, variation: int, index: int
    ) -> DbStorage:
        ...
        stale_if_longer_than = self.stale_if_longer_than
        if (
            ts and (datetime.datetime.now() - ts).total_seconds() < stale_if_longer_than
        ):  # Use aggressive caching
            vals: Dict[int, DbPointVal] = (
                self.soe_handler.gv_index_value_nested_dict.get(gv_cls)
            )
        else:  # Use normal routine
            vals: Dict[int, DbPointVal] = self.get_db_by_group_variation(
                group, variation
            ).get(gv_cls)
    ...
    def get_val_by_group_variation_index(
        self, group: int, variation: int, index: int
    ) -> DbPointVal:
    ...
    def send_direct_point_command(
        ...
        master_cmd: opendnp3.AnalogOutputDouble64 = parsing_gv_to_mastercmdtype(
            group=group, variation=variation, val_to_set=val_to_set
        )
        self.send_direct_operate_command(
            command=master_cmd, index=index, callback=call_back, config=config
        )

    def send_select_and_operate_point_command(
        self,
        group: int,
        variation: int,
        index: int,
    ...
    def shutdown(self):
        ...
        _log.info(
            f"Master station shutting down in {sleep_before_master_shutdown} seconds..."
        )
        time.sleep(
            sleep_before_master_shutdown
        )  # Note: hard-coded sleep to avoid hanging process
        # del self.master
    ...
    def send_scan_all_request(
        ...
        if gv_ids is None:
            gv_ids = [
                GroupVariationID(group=30, variation=6),
