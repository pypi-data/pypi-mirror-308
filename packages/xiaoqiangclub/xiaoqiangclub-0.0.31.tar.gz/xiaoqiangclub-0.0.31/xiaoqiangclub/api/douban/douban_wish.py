...
class DoubanWish:
    ...
    async def get_all_wish_data(self, user_ids: Union[str, List[str]] = None,
                                save_file_path: str = None, get_all_data: bool = False,
                                skip_existing: bool = True, print_results: bool = False) -> List[dict]:
        """
        获取所有豆瓣用户想看影视的详细数据。
        ...
        if skip_existing and os.path.exists(save_file_path):
            ...
            if history_data:
                ...
                douban_all_wish.update(history_data)
        ...
        if need_parse_urls:
            ...
            for link, details in zip(need_parse_urls, results):
                if details:
                    new_data_titles.append(details.get('title', "").split(' ')[0].strip())
                    new_data[link] = details
                    douban_all_wish[link] = details
        ...
        # 打印结果
        if print_results:
            for link, details in douban_all_wish.items():
                print(f"详情链接: {link}")
        ...
        log.info(
            f'任务完成，共收录"想看的影视" {len(douban_all_wish)} 部。新收录 {len(new_data_titles)} 部 >>> {" | ".join(new_data_titles)}')
