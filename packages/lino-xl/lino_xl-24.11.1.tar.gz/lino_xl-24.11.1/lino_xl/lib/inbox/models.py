...
def process_message(ar, key, msg):
    ...
    for part in msg.walk():
        ...
        if filename:
            if upload_area is None or upload_volume is None:
                ar.logger.info("[%s] Ignore attached %s because upload area is none.", key, filename)
                continue
            counter += 1
            ...
            if ct == "text/html":
                if html_body is None:
                    ...
                    ar.logger.warning("Multiple %s parts without filename!", ct)
            elif ct == "text/plain":
    ...
    if body:
        ...
        ar.logger.debug("[%s] No comment because body is empty", key)
