...
class SecondsDollarBars:

    def __init__(self, symbol: str, start: dt.datetime, end: dt.datetime, freq: int,
    ...
    def _get_seconds_resample_data(self):
        ticks = TimeBars(symbol=self.symbol, start=self.start, end=self.end, source=MarketSource.IQFEED_SECOND, freq='5s',
                         kwargs=self.kwargs, )
        return ticks.data
