...
def from_message_request(message_request : MessageDraft, sent_by: str) -> ChattyMessage:
...
def from_chatty_fast_answer(chatty_fast_answer: ChattyFastAnswer, sent_by: str) -> List[ChattyMessage]:
    ...
    return [MessagefromMessageRequestFactory.from_request(message=message, sent_by=sent_by) for message in chatty_fast_answer.messages]