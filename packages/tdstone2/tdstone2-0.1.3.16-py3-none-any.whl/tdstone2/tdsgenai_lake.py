...
from tdstone2.utils import generate_create_table_teradata
...
def install_zip_in_vantage(zip_file_path: str, oaf_env_name: str, replace=False):
    ...
    oaf_env = tdml.get_env(oaf_env_name)
...
def setup_and_execute_script(model_name: str, dataset, text_column, hash_columns: list, accumulate_columns=[],
    ...
    try:
        ...
        command = f"""python3 tds_vector_embedding_lake.py {model_name} {batch_size} {text_column_position} {accumulate_positions} {delimiter} {device} {half}"""
...
def execute_and_create_pivot_view(oaf, schema_name: str, table_name: str, hash_columns: list = None, if_exists: str ='replace'):
    ...
    try:
        ...
        tac = time.time()
        logger.info(f"Script execution and storage in destination table. Computation time: {tac - tic:.2f} seconds")
        if query:
            logger.info(query)
        try:
...
def compute_vector_embedding(model_name, dataset, schema_name, table_name, text_column, hash_columns = None, accumulate_columns=[], oaf_env = None, batch_size = 32, half_precision = False, device = 'cuda'):
    """
    ...
    # Get the environment
    if oaf_env in tdml.list_user_envs()['env_name'].tolist():
        ...
        logger.error(tdml.list_user_envs()['env_name'].tolist())
        raise
    ...
    installed_models_df = list_installed_models(oaf_env)
    ...
    res = execute_and_create_pivot_view(oaf, schema_name, table_name, hash_columns = hash_columns)
    ...
    try:
        ...
        if output:  # Ensure there is output to process
            ...
            df_output = pd.DataFrame(rows,
                                     columns=accumulate_columns + ['jobid','process_time','elapsed_time'] + ['model', 'device', 'half_precision', 'batch_size', 'Vector_Dimension', 'V'])
