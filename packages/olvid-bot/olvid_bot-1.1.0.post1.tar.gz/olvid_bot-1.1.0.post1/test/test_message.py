...
async def send_reply_and_edit_messages(client_1: ClientWrapper, client_2: ClientWrapper):
	...
	original_message_1, _ = await send_message_wait_and_check_content(client_1, client_2, "A")
	# original_message_2, _ = await send_message_wait_and_check_content(client_2, client_1, "B")
	logging.info("Message send finished")

	# send reply message
	sent_message_count += 1
	reply_message_1, _ = await send_message_wait_and_check_content(client_1, client_2, f"reply {client_1.identity.id}", replied_message_id=original_message_1.id)
	# reply_message_2, _ = await send_message_wait_and_check_content(client_2, client_1, f"reply {client_2.identity.id}", replied_message_id=original_message_2.id)
	logging.info("Message reply finished")

	# update message body
	ideal_updated_message_1 = datatypes.Message(discussion_id=discussion_1.id, sender_id=0, body=f"Updated body from {client_1.identity.id}", replied_message_id=original_message_1.id)
	ideal_updated_message_2 = datatypes.Message(discussion_id=discussion_2.id, sender_id=contact_2.id, body=f"Updated body from {client_1.identity.id}", replied_message_id=datatypes.MessageId(type=datatypes.MessageId.Type.TYPE_INBOUND))
	bots = [
