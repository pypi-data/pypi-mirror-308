...
log_file = log_dir / f"commandrex_{datetime.now().strftime('%Y%m%d')}.log"
...
class CommandExecutor:
    ...
    DIRECT_EXECUTE_COMMANDS = {
        'ps', 'ls', 'dir', 'ping', 'netstat', 'top', 'htop',
    ...
    def __init__(self):
        ...
        logger.info(f"CommandExecutor initialized - Platform: {self.platform_info.os_type}, Shell: {self.platform_info.shell_path}")
    ...
    def _prepare_command(self, command: Command) -> List[str]:
        ...
        # For other commands, wrap in shell
        cmd_list = [
                            ...
                            try:
                                ...
                                for line in lines_text:
                                    if line:
                                        ...
                                        if is_error:
                                            console.print(f"[red]{line}[/red]")
                                            logger.error(f"Process error output (fallback encoding): {line}")
                                        else:
                                            console.print(line)
                                            logger.debug(f"Process output (fallback encoding): {line}")
                            except:
                                logger.error("Failed to decode process output", exc_info=True)
                                continue
            except (OSError, IOError) as e:
                logger.error(f"Stream reading error: {str(e)}", exc_info=True)
    ...
    def _handle_cd_command(self, command: Command) -> CommandResult:
        ...
        try:
            ...
            logger.error(f"Directory change failed: {str(e)}", exc_info=True)
            return CommandResult(
                success=False,
                output="",
                error=str(e),
    ...
    def execute(self, command: Command) -> CommandResult:
        ...
        try:
            ...
            if self.is_dangerous(command):
                logger.warning(f"Dangerous command detected: {command.executable}")
                ...
                if confirm != 'y':
                    logger.info("User cancelled dangerous command execution")
                    return CommandResult(
                        success=False,
            ...
            logger.debug(f"Prepared command list: {cmd_list}")
            ...
            try:
                ...
                if self.platform_info.os_type == 'windows':
                    ...
                    # For Unix-like systems, run the command directly without shell=True
                    # This should help prevent the stalling issue
                    process = subprocess.Popen(
                        cmd_list,
                        stdout=subprocess.PIPE,
                        stderr=subprocess.PIPE,
            ...
            try:
                ...
                if command.executable.lower() in self.CONTINUOUS_COMMANDS:
                    ...
                    try:
                        ...
                        logger.debug(f"Continuous command completed with exit code: {exit_code}")
                    except subprocess.TimeoutExpired:
                        logger.info("Continuous command timeout expired, stopping process")
            ...
            if error:
                logger.error(f"Command produced error output: {error}")
