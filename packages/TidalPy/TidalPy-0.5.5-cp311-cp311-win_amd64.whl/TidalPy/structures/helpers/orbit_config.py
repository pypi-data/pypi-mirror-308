...
def pull_out_orbit_from_config(world_config: dict) -> Tuple[float, float, float]:
    ...
    if orbital_freq is not None:
        if np.any(orbital_freq > 0.001):
            log.warning(
                f'Unusual Value Encountered in {name}: User-provided orbital frequency set in [rad s-1] but '
    ...
    if orbital_period is not None:
        if orbital_freq is not None:
            ...
            if np.any(orbital_period > 500.):
                log.warning(
                    f'Unusual Value Encountered in {name}: User-provided orbital period set in [days] but '
    ...
    if semi_major_axis is not None:
        ...
        if semi_major_axis_inau:
            if np.any(semi_major_axis > 1000.):
                log.warning(
                    f'Unusual Value Encountered in {name}: User-provided semi-major axis set in [Au] but value '
            ...
            if np.any(semi_major_axis < 1.e6):
                log.warning(
                    f'Unusual Value Encountered in {name}: User-provided semi-major axis set in [m] but value '
