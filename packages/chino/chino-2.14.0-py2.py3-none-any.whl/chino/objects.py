...
class BulkGetResult(ChinoBaseObject):
    ...
    def __init__(self, class_obj, result, **kwargs):
        ...
        # Mapping of class object IDs and the class object
        class_objs = {}
        for result_id, result_dict in result[class_obj.__str_names__].items():
            class_objs[result_id] = class_obj(**result_dict)

        self.__setattr__(class_obj.__str_names__, class_objs)
        self.class_obj = class_obj.__str_names__
    ...
    def to_dict(self):
        ...
        res['total_count'] = self.total_count
        res['errors'] = self.errors
        res[self.class_obj] = [
            o.to_dict() for o
            in self.__getattribute__(self.class_obj).values()
        ]
    ...
    def __iter__(self):
        # We return values here since the key is already contained in the
        # value
        return iter(self.__getattribute__(self.class_obj).values())
