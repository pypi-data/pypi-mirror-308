...
class ResourcesBase(ABC):
    ...
    @abstractmethod
    def create_engine(self, name:str, size:str|None=None, pool:str|None=None) -> dict | None:
        pass
...
class ProviderBase(ABC):
    ...
    def create_engine(self, name:str, size:str|None=None):
...
class Client():
    def __init__(
        ...
        self.use_graph_index = config.get("use_graph_index", False)
    ...
    def _manage_packages(self):
        ...
        try:
            ...
            enable_full_package_manager = self._config.get("use_package_manager", False)
            # query the current state of the registry and packages in the database
            if enable_full_package_manager:
                res = self._timed_query("query_package_manager", dependencies.generate_query_package_manager())
                update_registry, update_project = dependencies.check_package_manager(res, platform, app_name, engine_name, database_name)

                # it may be necessary to update (refresh) the registry
                if update_registry:
                    self._timed_query("update_registry", dependencies.generate_update_registry(), readonly=False)

                # it may be necessary to update the packages
                if update_project:
                    self._timed_query("update_packages", dependencies.generate_update_packages(), readonly=False)
            else:
                res = self._timed_query("query_package_manager", dependencies.generate_query_version_check())
                dependencies.check_package_manager(res, platform, app_name, engine_name, database_name, warn_on_packages=True)

        finally:
            self._database = database_name

    def _timed_query(self, span_name:str, code: str, readonly=True):
        with debugging.span(span_name, model=self._database) as end_span:
            start = time.perf_counter()
            res = self._query(code, None, end_span, readonly=readonly, raw_results=True, abort_on_error=True)
    ...
    def query(self, task:m.Task, rentrant=False, readonly=True, raw_results=False, inputs={}, format="pandas", tag=None, nowait_durable=None, headers={}, internal=False, abort_on_error=True) -> DataFrame|Any:
        ...
        readonly = False if task.has_persist() else readonly
        with debugging.span("query", model=self._database, task=task, tag=tag, internal=internal, task_id=task.id, readonly=readonly) as end_span:
            ...
            if format == "pandas":
                ...
                debugging.time("query", time.perf_counter() - start, DataFrame(), source_map=source_map)
            return results
