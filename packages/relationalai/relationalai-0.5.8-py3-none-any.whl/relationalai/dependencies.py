...
PACKAGE_DEPENDENCIES = [
    ('std', '24c1872c-de8a-5b12-7648-3d72f007a7a9', version_range('0.1.6', '0.2.0', '0.1.6 - 0.1.*'))
]
...
# the database, as well as whether the registry is up to date.
#
...
def generate_query_version_check():
    return _generate_query_version_check(PACKAGE_DEPENDENCIES, STATIC_DEPENDENCIES)
...
def _compare_dependencies(package_deps, static_deps, lock, std, platform: str, app_name: str, engine_name: str, model_name:str, warn_on_packages = False):
    def warn(kind: InvalidVersionKind, errors: list[tuple[InvalidVersionKind, str, str]] = []):
        RAIInvalidVersionWarning(
            kind=kind,
            expected=static_deps,
            lock=lock,
            platform=platform,
            app_name=app_name,
    ...
    if not lock and not std:
        return warn(InvalidVersionKind.SchemaOutOfDate)

    # database is pre-package manager (between 0.1.0 and 0.1.5)
    if std:
        return warn(InvalidVersionKind.LibraryOutOfDate)

    # database has a package manager, first, check the static dependencies and potentially
    # raise warnings
    # If `warn_on_packages` is set, also check and raise on package depdendencies.
...
def _generate_query_version_check(package_deps, static_deps):
    static_lock_bindings = " ;\n            ".join(
        ["(\"%s\", \"%s\")" % (name, uuid) for name, uuid, _ in package_deps + static_deps]
    )
