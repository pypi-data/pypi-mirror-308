...
from relationalai.clients.config import Config # to get the users root file path
import __main__

from relationalai.clients.profile_polling import TransactionEventsFeedbackHandler # to get the users root file path
...
def start_debugger_session(config: Config, host: str|None = None, port: int|None = None):
    ...
    debugging.logger.addHandler(TransactionEventsFeedbackHandler(resources))
