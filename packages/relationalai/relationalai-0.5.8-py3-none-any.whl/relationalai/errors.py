...
class EngineNotFoundException(RAIException):
    ...
    def format_message(self):
        return textwrap.dedent(f"""
        We were unable to detect an existing engine or provision a new engine for you. You can manually create an engine with either of the following commands:

        [green]rai engines:create --name {self.engine_name}[/green]

        [green]rai.Provider().create_engine('{self.engine_name}')[/green]
        """)
...
class EngineAutoCreateFailed(RAIException):
    ...
    def format_message(self):
        return textwrap.dedent(f"""
        We tried to auto-create the engine [yellow]{self.engine_name}[/yellow] but it failed.

        You can manually create the engine with either of the following commands:
...
class EnginePending(RAIException):
    ...
    def format_message(self):
        return textwrap.dedent(f"""
        The engine [yellow]{self.engine_name}[/yellow] is in a pending state. You can see the status of engines with either of the following commands:

        [green]rai engines:list[/green]

        [green]rai.Provider().list_engines()[/green]
        """)
...
class SnowflakeImportMissingException(RAIException):
    ...
    def format_message(self):
        ...
        You can create an import for it using either of the following commands:
...
class SnowflakeChangeTrackingNotEnabledException(RAIException):
    ...
    def format_message(self):
        if isinstance(self.obj, str):
            ...
            obj_str = ', '.join(fqn for fqn, _ in self.obj)
