        ...
        for resource in resources:
            try:
                ...
                if match_string in map_approach:
                    logger.debug(f'When building match map of resources with approach "{strategy["name"]}": resource id {resource["id"]} duplicates already used string "{match_string}" from resource id {map_approach[match_string]} so it will be excluded from the map.')
...
def find_matching_resource(source_strings:list, resource_map:list, strategies:list, ridx:int):
    ...
    for idx,match_string in enumerate(source_strings):
        if match_string in resource_map[idx]:
            ...
            logger.info(f'Source record #{ridx+1}: found matching resource id {matching_resource_id} for match string "{match_string}" using strategy #{idx+1}: {strategies[idx]["name"]}')
            break
...
def update_existing_resource_native_attrs(ops:OpsRampEnv, resourceId, resource:pd.Series, spec:dict, existing_resource:dict):
    ...
    if has_updated_values:
        ...
        logger.info(f'No updates to resource main attributes name:{payload["resourceName"]} with id:{resourceId}')
...
async def process_resource_data(ops:OpsRampEnv, args, job:dict, df:pd.DataFrame, idx, resource:pd.Series, resource_map, match_strategies, customattr_names_in_file, existing_resources_by_id, attrinfo, matched_resource_ids:set):
        ...
        if resourceId: # Existing resource
            ...
            if 'update_existing_resources' in job and job['update_existing_resources']:
                update_existing_resource_native_attrs(ops, resourceId, resource, job['update_existing_resources'], existing_resources_by_id[resourceId])
...
#        json.dump(resources, f4, indent=2)
    ...
    for idx,resource in df.iterrows():
        ...
        await process_resource_data(ops, args, job, df, idx, resource, resource_map, match_strategies, customattr_names_in_file, existing_resources_by_id, attrinfo, matched_resource_ids)

    # Delete dups if configured and they exist
    dups_to_delete = set()
    for sidx, strategy in enumerate(match_strategies):
        if 'delete_duplicates' in strategy and strategy['delete_duplicates'] and '__duplicates__' in resource_map[sidx] and resource_map[sidx]['__duplicates__']:
