...
class Client(ExClient):
    ...
    async def pms(self) -> dict[int, dict]:
        pms = (await self._coin_curs_pms_country())["payMethod"]
        pmsd = {
            pm["payMethodId"]: {
                "name": pm["name"],
                "type_": pm["template"],
    ...
    async def curs(self) -> dict[int, str]:
        res = await self._coin_curs_pms_country()
        return {
    ...
    async def coins(self) -> dict[int, str]:
        coins: list[dict] = (await self._coin_curs_pms_country())["coin"]
    ...
    async def _coin_curs_pms_country(self) -> (dict, dict, dict, dict):
