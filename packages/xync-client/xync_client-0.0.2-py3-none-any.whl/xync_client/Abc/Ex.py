...
class ExClient(Client):
    ...
    def _pmnorm(self, s: str, idtf: str = None) -> str:
        ...
        def remove(rms: str | list[str], st: str, regexp: bool = False) -> str:
            for rm in rms:
                st = re.sub(rm, "", s) if regexp else st.replace(rm, "")
            return st
    ...
    async def set_pmcurexs(self):
        ...
        for k, pm in pmsd.items():
            ...
            if prev[1:] == (norm, pm["name"]):
                ...
                await Pmex.update_or_create({"pm": pms.get(prev[0], await Pm.get(name=prev[2]))}, ex=self.ex, exid=k)
            elif prev[1] == norm:
                logging.error(f"Pm: '{pm['name']}' overnormed as '{norm}' with '{prev[2]}'")
            else:
