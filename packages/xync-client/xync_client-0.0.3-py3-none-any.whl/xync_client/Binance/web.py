...
class Public(ExClient):
    ...
    async def pms(self) -> list[dict]:
        pass

    # 22: Cur -> [Pm] rels
    async def cur_pms_map(self) -> dict[int, set[int]]:  # {cur.exid: [pm.exid], [pm.exid]}
        res = await self.curs()
        mp = {c: await self._get_pms_and_country_for_cur(c) for c in res.keys()}
        return mp
    ...
    async def _get_pms_and_country_for_cur(self, cur: str) -> ([str], [str]):
