...
def find_notebook_file(step: int, directory: str):
...
def download_repo(repo_url: str, repo_name: str, refresh_data: bool = False):
    ...
    if repo_name is None:
        raise ValueError("Invalid repository URL, please contant snowflake support")
    ...
    if response.status_code == 200:
        ...
        raise Exception(
            "Failed to download the repository. Please check your internet connection and reload the demo using load_demo('<name>', refresh_demo=True)."  # noqa: E501
        )


# In-memory caching
def read_demo_mapping_with_cache():
    ...
    if not hasattr(read_demo_mapping_with_cache, "cached_data"):
        with open(demo_file_path) as f:
            ...
            read_demo_mapping_with_cache.cached_data = demo_map
    return read_demo_mapping_with_cache.cached_data


def get_url_from_demo_name(demo_name: str):
...
def get_notebook_name_from_file_name(file_name: str):
...
def print_demo_list() -> None:
    ...
    try:
        ...
        logger.warning("[WARNIGN] Failed to print demo list using rich. Falling back to simple print. Reason: %s", e)
        format_string = " ".join([f"{{:<{width}}}" for width in DEMO_MAPPING_COLUMN_WIDTHS])
        print("-" * sum(DEMO_MAPPING_COLUMN_WIDTHS))
        print(format_string.format(*DEMO_MAPPING_COLUMNS))
        print("-" * sum(DEMO_MAPPING_COLUMN_WIDTHS))
...
def create_demo_notebooks(demo_name: str, repo_name: str, num_steps: int, root: Root):
    ...
    for i in range(0, num_steps):
        ...
        if notebook_file is None:
            raise ValueError(
                f"Notebook for step {i} not found in the demo repository. Please try running load_demo again with refresh_demo=True."  # noqa: E501
            )
        ...
        notebook_name = get_notebook_name_from_file_name(notebook_file)
        notebook = Notebook(
...
def get_notebook_file_directory(demo_name: str):
...
def get_demo_directory(demo_name: str):
...
def create_notebook_url_from_demo_name(demo_name: str, demo_connection: DemoConnection, step: int = 0):
    ...
    if notebook_file is None:
        raise ValueError(
            f"Notebook for step {step} not found in the demo repository. The demo is not setup correctly. Please try running load_demo again with refresh_demo=True."  # noqa: E501
        )
    notebook_name = get_notebook_name_from_file_name(notebook_file)
    return f"https://app.snowflake.com/{demo_connection.get_organization()}/{demo_connection.get_account()}/notebooks/{DEMO_DATABASE_NAME}.{DEMO_SCHEMA_NAME}.{notebook_name}"


def cleanup_demos_download():
