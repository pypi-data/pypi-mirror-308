...
from .utils import get_or_create_tag, save_cache_csv, col_is_null_or_empty, upsert_disseminated_table
...
class SWSGoldIcebergSparkHelper:
    def __init__(
        ...
        self.cols_to_keep_sws = self.dim_columns_w_time + ["value"] + self.flag_columns
    ...
    def rearrange_sdmx_columns(self, df: DataFrame) -> DataFrame:
        ...
        logging.info(f"Primary Measure: {time_dimensions}")
    ...
    def gen_gold_sws_disseminated_data(self) -> DataFrame:
        return (
            self.spark.read.option("tag", self.tag_name).table(self.iceberg_tables.SILVER.iceberg_id)
    ...
    def gen_gold_sws_validated_data(self) -> DataFrame:
        return self.spark.read.option("tag", self.tag_name).table(self.iceberg_tables.BRONZE.iceberg_id).transform(
            self.keep_dim_val_attr_columns
        )

    def gen_gold_sdmx_data(self) -> DataFrame:
        return (
            self.spark.read.option("tag", self.tag_name).table(self.iceberg_tables.SILVER.iceberg_id).option("")
            .transform(self.apply_diss_flag_filter)
