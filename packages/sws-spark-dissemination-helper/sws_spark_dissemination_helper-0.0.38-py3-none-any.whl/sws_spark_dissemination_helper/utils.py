...
def upsert_disseminated_table(tag:DisseminatedTag, dataset_id, tag_name, table:BaseDisseminatedTagTable):
    if table_exists(tag=tag, table_name=table.id):
        tag.update_dissemination_table(dataset=dataset_id, tag_id=tag_name, table=table)
    else:
        tag.add_dissemination_table(dataset=dataset_id, tag_id=tag_name, table=table)