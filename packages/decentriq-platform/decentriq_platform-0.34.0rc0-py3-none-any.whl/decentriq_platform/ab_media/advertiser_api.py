...
class AdvertiserApi:
    ...
    def __init__(self, dcr: AudienceBuilderDcr) -> None:
        self.dcr_id = dcr.id
    ...
    def create_audience(
        self, audience: Union[LookalikeAudienceDefinition | RuleBasedAudienceDefinition]
    ):
        """
        Create an audience.
        ...
        **Parameters**:
        ...
        if isinstance(audience, LookalikeAudienceDefinition):
            ...
            if audience.reach < 0 or audience.reach > 30:
                raise Exception("Reach value must be an integer in the range 0 - 30")
        ...
        audience._set_source_ref_id(audiences_dict)

        if isinstance(audience, LookalikeAudienceDefinition):
            parsed_audience = Audience2.parse_raw(json.dumps(audience.as_dict()))
        elif isinstance(audience, RuleBasedAudienceDefinition):
            parsed_audience = Audience3.parse_raw(json.dumps(audience.as_dict()))
        else:
            raise Exception(f"Unknown audience type {type(audience)}")

        # Add the audience to the list of audiences.
    ...
    def get_lookalike_audience_statistics(self, audience_name: str) -> Dict[str, Any]:
        """
