...
class IdeaDevelopment:
    ...
    async def get_idea_plan(self, user_prompt, original_prompt_language):
        ...
        prompt = (
            ...
            f"For tree structures and file organization, use plaintext markdown in a code block, with path separators matching the user's OS. Provide this ONCE for your entire response, ONLY when new files are added or moved. Example for the detected OS:\n"
            f"```plaintext\n"
            f"project/\n"
            f"├── src/\n"
            f"│   └── main.py\n"
