...
class ShortIdeaDevelopment:
    ...
    async def get_idea_plan(self, user_prompt, original_prompt_language):
        ...
        prompt = (
            ...
            f"IMPORTANT: For any new files being added or existing files being moved, provide a complete tree structure under '#### Directory Structure' showing the full path from root to the affected files. Example:\n"
            f"```plaintext\n"
            f"project_root/              # Start from root directory\n"
            f"├── src/                   # Show all parent directories\n"
            f"│   └── new_feature/       # Leading to new/moved files\n"
            f"│       └── new_file.py    # [NEW] New file to be created\n"
            f"└── old_dir/\n"
            f"    └── old_file.py        # [MOVE] → src/new_feature/new_name.py\n"
            f"```\n"
            f"Show complete paths for all affected files with clear labels ([NEW], [MOVE], etc).\n\n"
