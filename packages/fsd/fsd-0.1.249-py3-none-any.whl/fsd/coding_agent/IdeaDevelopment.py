...
class IdeaDevelopment:
    ...
    async def get_idea_plan(self, user_prompt, original_prompt_language):
        ...
        prompt = (
            ...
            f"IMPORTANT: Only if new files are being added or existing files are being moved, provide a minimal tree structure under '#### Directory Structure' showing ONLY the specific directories and files being added/moved, using plaintext markdown in a code block. Example:\n"
            f"```plaintext\n"
            f"src/\n"
            f"└── new_feature/     # Only show paths relevant to new/moved files\n"
            f"    └── new_file.py\n"
            f"```\n"
            f"DO NOT show the full project tree or unaffected directories.\n\n"
