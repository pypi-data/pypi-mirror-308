...
class ProjectManager:
    ...
    def _validate_inputs(self, full_path: str) -> bool:
        if self.is_windows:
            ...
            invalid_chars = '<>:"|?*'
            if any(char in full_path for char in invalid_chars):
                logger.debug(f"Invalid characters found in path: {full_path}")
                return False
            # Check path length limit for Windows
            if len(full_path) > 260:
                logger.debug(f"Path exceeds Windows MAX_PATH limit: {full_path}")
                return False
        return True
    ...
    def _extract_app_name(relative_path: str) -> str:
        ...
        return path_parts[0] if path_parts else 'UnknownApp'
    ...
    def _add_file_to_xcode_project(self, xcodeproj_path: str, full_path: str, relative_path: str) -> None:
        ...
        # Convert Windows paths to Unix-style for Xcode
        if self.is_windows:
    ...
    def _move_file(self, existing_file_path: str, new_full_path: str) -> None:
        ...
        shutil.move(existing_file_path, new_full_path)
    ...
    def _update_xcode_project(self, xcodeproj_path: str, existing_file_path: str, new_full_path: str, new_relative_path: str) -> None:
        ...
        # Convert Windows paths to Unix-style for Xcode
        if self.is_windows:
            existing_file_path = existing_file_path.replace('\\', '/')
