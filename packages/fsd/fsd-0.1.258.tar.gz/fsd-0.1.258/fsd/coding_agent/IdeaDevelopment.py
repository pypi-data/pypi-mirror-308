...
class IdeaDevelopment:
    ...
    async def get_idea_plan(self, user_prompt, original_prompt_language):
        ...
        prompt = (
            ...
            f"IMPORTANT: For any new files being added or existing files being moved, you MUST provide a tree structure under '#### Directory Structure' showing the COMPLETE PATH from root to those specific files. The tree should ONLY include directories and files relevant to additions/moves. Example:\n"
            f"```plaintext\n"
            f"project_root/           # Always start from root\n"
            f"└── src/               # Show full path to affected files\n"
            f"    └── new_feature/   # Only include dirs with new/moved files\n"
            f"        └── file.py    # [NEW] or [MOVE] label required\n"
            f"```\n"
            f"CRITICAL TREE REQUIREMENTS:\n"
            f"- Start from root directory\n" 
            f"- Show complete path to each new/moved file\n"
            f"- Include ONLY directories containing new or moved files\n"
            f"- Label files as [NEW] or [MOVE →destination]\n"
            f"- Exclude any files/dirs not directly involved\n"
            f"- Keep tree minimal but complete for file paths\n\n"
