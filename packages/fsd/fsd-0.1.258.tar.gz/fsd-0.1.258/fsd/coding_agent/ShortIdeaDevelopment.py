...
class ShortIdeaDevelopment:
    ...
    async def get_idea_plan(self, user_prompt, original_prompt_language):
        ...
        prompt = (
            ...
            f"CRITICAL: Under '#### Directory Structure', show ONLY the minimal tree path for files directly involved in this task:\n"
            f"- Show complete path from root to any new or moved files\n"
            f"- Include ONLY folders containing affected files\n"
            f"- Exclude any files/folders not being modified\n"
            f"Example:\n"
            f"```plaintext\n"
            f"project_root/           # Only show relevant paths\n"
            f"└── src/               # Include parent folders\n"
            f"    └── feature/       # Leading to affected files\n"
            f"        └── main.py    # [NEW] Clear file action\n"
            f"```\n"
            f"Label each file clearly with [NEW], [MOVE], [MODIFY] etc.\n\n"
