...
class FileManagerAgent:
    ...
    async def get_file_planning(self, idea):
        ...
        prompt = (
                ...
                "Existing_files must include only the full paths for file names explicitly mentioned in the instruction that will be updated or modified. Exclude all image files except .svg files. "
                "Context_files must include only the full path context file names explicitly mentioned in the instruction that provide context but won't be modified. Exclude all image files except .svg files. Existing_files and Context_files must not overlap. Exclude all dependency files, configuration files, and non-code files such as .gitignore, package.json, Podfile, etc. Focus on including only relevant code files. "
