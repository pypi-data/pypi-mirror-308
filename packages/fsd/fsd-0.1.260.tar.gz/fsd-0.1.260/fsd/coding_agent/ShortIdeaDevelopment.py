...
class ShortIdeaDevelopment:
    ...
    async def get_idea_plan(self, user_prompt, original_prompt_language):
        ...
        prompt = (
            ...
            f"   - For each file, specify exact purpose, functionality, and technical requirements\n"
            f"   - Detail all UI components, layouts, styles, animations, and interactions\n"
            f"   - List specific data structures, algorithms, and processing logic needed\n"
            f"   - Define exact API endpoints, request/response formats, and data flows\n"
            f"   - Outline database schema changes, queries, and data relationships\n\n"
            f"2. For each file that needs modification or creation:\n"
            f"   - Specify complete technical stack and frameworks to be used\n" 
            f"   - Detail exact functions/methods to implement or modify\n"
            f"   - Define data models, validation rules, and error handling\n"
            f"   - List specific UI/UX requirements including colors, fonts, spacing\n"
            f"   - Outline performance requirements and optimization strategies\n\n"
            f"3. Map out precise technical implementation details:\n"
            f"   - Component architecture and inheritance relationships\n"
            f"   - State management approach and data flow patterns\n"
            f"   - Service communication protocols and interfaces\n"
            f"   - Security measures and access control rules\n"
            f"   - Testing requirements and validation criteria\n\n"
