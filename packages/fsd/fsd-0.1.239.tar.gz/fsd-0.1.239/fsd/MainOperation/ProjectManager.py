...
class ProjectManager:
    ...
    async def execute_files_creation(self, instructions: List[Dict]) -> None:
        ...
        if self.is_windows:
            ...
            # Expand all folders and set view to details in Windows Explorer
            powershell_script = '''
                $folderPath = "%s"
                $shell = New-Object -ComObject shell.application
                $folder = $shell.Namespace($folderPath)
                $folderItem = $folder.Self
                $folderItem.InvokeVerb("explore")
                Start-Sleep -Seconds 1
                $wshell = New-Object -ComObject wscript.shell
                $wshell.SendKeys("^{+}")  # Expand all folders
                $wshell.SendKeys("^+2")  # Set view to details
            ''' % repo_path
            subprocess.run(['powershell', '-Command', powershell_script])
        else:  # macOS and Linux
            # Open Finder and expand project view
            applescript = '''
                tell application "Finder"
                    activate
                    open folder (POSIX file "%s")
                    tell application "System Events"
                        keystroke "0" using {command down} -- Expand all folders
                        keystroke "1" using {command down} -- List view
                        keystroke "2" using {command down} -- Sort by name
                    end tell
                end tell
            ''' % repo_path
