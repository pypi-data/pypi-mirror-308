...
class IdeaDevelopment:
    ...
    async def get_idea_plan(self, user_prompt, original_prompt_language):
        ...
        prompt = (
            ...
            f"IMPORTANT: For new files being added or existing files being moved, provide a tree structure under '#### Directory Structure' showing the FULL PATH from root to the specific files being added/moved, using plaintext markdown in a code block. Example:\n"
            f"```plaintext\n"
            f"project_root/\n"
            f"├── src/                          # Show full path to new/moved files\n"
            f"│   └── features/\n"
            f"│       └── auth/\n"
            f"│           └── new_file.py       # [NEW] New authentication handler\n"
            f"└── old/\n"
            f"    └── auth.py                   # [MOVE] Moving to src/features/auth/auth.py\n"
            f"```\n"
            f"Label each file as [NEW] or [MOVE] and include source/destination for moves.\n\n"
