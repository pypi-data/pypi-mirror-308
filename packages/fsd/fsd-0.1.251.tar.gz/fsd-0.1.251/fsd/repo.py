...
class GitRepo:
    ...
    def deploy_to_server(self, path, domain=DEFAULT_DOMAIN, subdomain=DEFAULT_DOMAIN, project_type=""):
        try:
            logger.info(f"Starting deployment for path: {path}")
            logger.info(f"Domain: {domain}, Subdomain: {subdomain}, Project Type: {project_type}")
            
            # Zip all files in repo and upload to S3
            logger.info("Zipping files and uploading to S3...")
            s3_key = deploy_zip_to_s3(path)
            logger.info(f"Files uploaded to S3 with key: {s3_key}")
            ...
            # Send message to SQS
            logger.info("Preparing to send deployment message to SQS...")
            action = "deploy"
            ...
            logger.info(f"SQS message parameters - Action: {action}, Type: {type}")

            if response.get('MessageId'):
                logger.info(f"Deployment message successfully sent to SQS. MessageId: {response['MessageId']}")
                logger.debug(f"Deployment message sent to SQS. MessageId: {response['MessageId']}")
            else:
                logger.info("Message sent to SQS but no MessageId received")
            ...
            logger.info(f"Deployment failed with exception: {str(e)}")
            return None
