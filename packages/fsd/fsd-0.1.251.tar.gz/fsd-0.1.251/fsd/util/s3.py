...
def upload_file_to_s3(file_path, bucket_name, s3_key):
    ...
    logger.info(f"Successfully uploaded file to S3: s3://{bucket_name}/{s3_key}")
...
def deploy_zip_to_s3(local_directory):
    ...
    logger.info(f"Starting deployment process for directory: {local_directory}")
    logger.info(f"Using bucket: {bucket_name}, prefix: {s3_prefix}")
    logger.info(f"Ignoring files: {ignore_files}")
    ...
    with tempfile.NamedTemporaryFile(suffix='_web.zip', delete=False) as temp_zip:
        ...
        logger.info(f"Creating zip archive at: {zip_filename}")
        with zipfile.ZipFile(zip_filename, 'w', zipfile.ZIP_DEFLATED) as zipf:
            ...
            for root, dirs, files in os.walk(local_directory, topdown=True):
                ...
                if not root.startswith(local_directory):
                    logger.info(f"Skipping directory outside of local_directory: {root}")
                    continue
    ...
    logger.info(f"Generated zip file size: {zip_size} bytes")
    if zip_size == 0:
    ...
    logger.info(f"Cleaning up temporary zip file: {zip_filename}")
    os.unlink(zip_filename)
