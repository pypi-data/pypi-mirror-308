...
class ProjectManager:
    ...
    def _open_project_folder(self, paths: List[str]) -> None:
        """Open project folder and expand subfolders containing modified files"""
        if not paths:
            return

        # Get unique parent directories that need to be expanded
        parent_dirs = set()
        for path in paths:
        ...
        # Sort paths to ensure parent directories are opened first
        ...
        if self.is_macos:
            ...
            try:
                ...
                subprocess.run(['open', sorted_dirs[0]], check=True)
                
                # Expand subfolders
                apple_script = ['osascript']
                script_commands = []
                for dir_path in sorted_dirs[1:]:
                    script_commands.extend([
                        '-e', f'tell application "Finder"',
                        '-e', f'    set folderPath to (POSIX file "{dir_path}") as alias',
                        '-e', f'    reveal folderPath',
                        '-e', f'    activate',
                        '-e', 'end tell'
                    ])
                if script_commands:
                    subprocess.run(apple_script + script_commands, check=True)
            except subprocess.CalledProcessError as e:
                logger.debug(f"Error opening folders on macOS: {e}")
        else:
            # On Windows, use Explorer
            try:
                # Open main project folder
                subprocess.run(['explorer', sorted_dirs[0].replace('/', '\\')])
                
                # Expand subfolders
                for dir_path in sorted_dirs[1:]:
                    subprocess.run(['explorer', dir_path.replace('/', '\\')])
            except subprocess.CalledProcessError as e:
                logger.debug(f"Error opening folders on Windows: {e}")
    ...
    async def execute_files_creation(self, instructions: List[Dict]) -> None:
        ...
        for instruction in instructions:
            ...
            if "full_path" in parameters:
                paths_to_open.append(parameters["full_path"])
            if "old_full_path" in parameters and "new_full_path" in parameters:
                paths_to_open.extend([parameters["old_full_path"], parameters["new_full_path"]])

        # Open project folder and expand relevant subfolders once before processing
        self._open_project_folder(paths_to_open)
