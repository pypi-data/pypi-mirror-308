...
def ppcp_api(url, creditCard, proxy):
    ...
    try:
        ...
        if not id_match:
            return logging.error(f"{cc}|{mm}|{yy}|{cvv} Product ID not found in the response.")
        id = id_match.group(1)

    except Exception as e:
        return logging.error(f"requests 1 An error occurred during processing: {e}")
    ...
    try:
        ...
        if response.status_code not in (200, 302):
            return logging.error(f'{cc}|{mm}|{yy}|{cvv} Failed to add item to cart.')

    except Exception as e:
        return logging.error(f"requests 2 An error occurred during processing: {e}")
    ...
    try:
        ...
        if not nonce_match:
            return logging.error(f"{cc}|{mm}|{yy}|{cvv} Checkout nonce not found in the response.")
        ...
        if not PayPalCommerceGateway_match:
            return logging.error(f"{cc}|{mm}|{yy}|{cvv} PayPalCommerceGateway variable not found in the response.")
        ...
        return logging.error(f"requests 3 An error occurred during processing: {e}")
    ...
    try:
        ...
        return logging.error(f"requests 4 An error occurred during processing: {e}")
    ...
    try:
        ...
        if 'errors' in response.text:
            return logging.error(f"{cc}|{mm}|{yy}|{cvv} No shipping method has been selected. Please double check your address, or contact us if you need any help.")
        ...
        return logging.error(f"requests 5 An error occurred during processing: {e}")
    ...
    try:
        ...
        if response.status_code == 422:
            return logging.error(f'{cc}|{mm}|{yy}|{cvv} The requested action could not be performed, semantically incorrect, or failed business validation.')

    except Exception as e:
        return logging.error(f"requests 6 An error occurred during processing: {e}")
    ...
    try:
        ...
        return logging.error(f"requests 7 An error occurred during processing: {e}")
    ...
    try:
        ...
        if 'success' in status:
            ...
            return logging.info(f"{cc}|{mm}|{yy}|{cvv} {receipt}")
        elif 'failure' in status:
            message = stripTags(json.loads(response.text)["messages"])
            return logging.info(f"{cc}|{mm}|{yy}|{cvv} {message}")
        else:
            return logging.info(f"{cc}|{mm}|{yy}|{cvv} {response.text}")

    except Exception as e:
        return logging.error(f"requests 8 An error occurred during processing: {e}")