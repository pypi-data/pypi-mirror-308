    ...
    try:
        if choice == '1':
            ...
            if api_choice == "1":
                ...
                print(f"[{Y}!{R}] Invalid API choice. Please enter 1 or 2.")
                return
            ...
            with ThreadPoolExecutor(max_workers=threads) as executor:
                for creditCard in creditCards:
                    if api_type == 'ppcp':
                        executor.submit(ppcp_api, domain, creditCard.strip(), proxy)
                    elif api_type == 'nmi':
                        executor.submit(nmi_api, domain, creditCard.strip(), proxy)
                    elif api_type == 'paypalpro':
                        executor.submit(paypalpro_api, domain, creditCard.strip(), proxy)
                    elif api_type == 'paypal_pro_payflow':
                        executor.submit(paypal_pro_payflow_api, domain, creditCard.strip(), proxy)
                    elif api_type == 'stripe_cc':
                        executor.submit(stripe_cc_api, domain, creditCard.strip(), proxy)
        
        else:
            print(f"[{Y}!{R}] Invalid choice. Please select either 1 or 2.")
    
    except Exception as e:
        print(f"[{Y}!{R}] An error occurred during processing: {e}")
