...
def ppcp_api(url, creditCard, proxy):
    ...
    try:
        ...
        if not id_match:
            return print(f"[{R2}!{R}] {cc}|{mm}|{yy}|{cvv} Product ID not found in the response.")
        id = id_match.group(1)

    except Exception as e:
        return print(f"[{R2}!{R}] requests 1 An error occurred during processing: {e}")
    ...
    try:
        ...
        if response.status_code not in (200, 302):
            return print(f'[{R2}!{R}] {cc}|{mm}|{yy}|{cvv} Failed to add item to cart.')

    except Exception as e:
        return print(f"[{R2}!{R}] requests 2 An error occurred during processing: {e}")
    ...
    try:
        ...
        if not nonce_match:
            return print(f"[{R2}!{R}] {cc}|{mm}|{yy}|{cvv} Checkout nonce not found in the response.")
        ...
        if not PayPalCommerceGateway_match:
            return print(f"[{R2}!{R}] {cc}|{mm}|{yy}|{cvv} PayPalCommerceGateway variable not found in the response.")
        ...
        return print(f"[{R2}!{R}] requests 3 An error occurred during processing: {e}")
    ...
    try:
        ...
        return print(f"[{R2}!{R}] requests 4 An error occurred during processing: {e}")
    ...
    try:
        ...
        if 'errors' in response.text:
            return print(f"[{Y}-{R}] {cc}|{mm}|{yy}|{cvv} No shipping method has been selected. Please double check your address, or contact us if you need any help.")
        ...
        return print(f"[{R2}!{R}] requests 5 An error occurred during processing: {e}")
    ...
    try:
        ...
        headers = {
            ...
            "Braintree-Sdk-R2ersion": "3.32.0-payments-sdk-dev",
        ...
        if response.status_code == 422:
            return print(f'[{Y}-{R}] {cc}|{mm}|{yy}|{cvv} The requested action could not be performed, semantically incorrect, or failed business validation.')

    except Exception as e:
        return print(f"[{R2}!{R}] requests 6 An error occurred during processing: {e}")
    ...
    try:
        ...
        return print(f"[{R2}!{R}] requests 7 An error occurred during processing: {e}")
    ...
    try:
        ...
        if 'success' in status:
            ...
            return print(f"[{G}+{R}] {cc}|{mm}|{yy}|{cvv} {receipt}")
        elif 'failure' in status:
            message = stripTags(json.loads(response.text)["messages"])
            return print(f"[{Y}-{R}] {cc}|{mm}|{yy}|{cvv} {message}")
        else:
            return print(f"[{R2}!{R}] {cc}|{mm}|{yy}|{cvv} {response.text}")

    except Exception as e:
        return print(f"[{R2}!{R}] requests 8 An error occurred during processing: {e}")