...
class IterFilterAgent(SimpleSingularExecutor):
    ...
    def filter_positive(self, items: List[str]) -> List[str]:
        resp = super().execute(items=str(items))
        answer: ItemSequence = resp.data
        return answer.data

    def filter_negative(self, items: List[str]) -> List[str]:
...
class IterMappingAgent(SimpleSingularExecutor):
    ...
    def execute(self, requirement: str, items: List[str]) -> Dict[str, str]:
        ...
        List[str]
            The new values.
        """
        resp = super().execute(requirements=requirement, items=str(items))
