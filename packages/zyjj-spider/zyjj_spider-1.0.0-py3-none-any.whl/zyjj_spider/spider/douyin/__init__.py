import logging
import re

import httpx

from zyjj_spider.base import Base, SDKException
from zyjj_spider.spider.base import BaseSpider, extract_valid_urls
from zyjj_spider.spider.douyin.utils import get_base_params
from zyjj_spider.spider.douyin.xbogus import XBogus
from zyjj_spider.spider.douyin.abogus import ABogus, BrowserFingerprintGenerator
from datetime import datetime

aweme_id_pattern = r"video/(\d*)"

class DouyinSpider:

    def __init__(self, base: Base, cookie: str):
        self.__base = base
        self.__spider = BaseSpider(
            base,
            'https://www.douyin.com/',
            extra_header={
                "cookie": cookie,
                "refer": "https://www.douyin.com",
                "origin": "https://www.douyin.com"
            },
            agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36 Edg/126.0.0.0",
        )
        self.__xb = XBogus(self.__spider.agent)

    # 从链接中提取aweme_id
    async def tool_aweme_id_get(self, url: str) -> str:
        url = extract_valid_urls(url)
        if url is None:
            raise SDKException('url不合法')
        # 访问一下url
        url = await self.__spider.request_get_redirect_url(url)
        match = re.search(aweme_id_pattern, str(url))
        if match:
            return match.group(1)
        raise SDKException('无法提取aweme_id')

    # 获取msToken
    async def get_ms_token(self):
        res = await self.__spider.request_post_raw('https://mssdk.bytedance.com/web/report', {
            "magic": 538969122,
            "version": 1,
            "dataType": 8,
            "strData": '',
            "tspFromClient": int(datetime.now().timestamp() * 1000),
        })
        print(res.cookies.values())
        return str(httpx.Cookies(res.cookies).get("msToken"))

    # 获取加密参数
    async def __get_query(self, params: dict) -> str:
        params.update(get_base_params(await self.get_ms_token()))
        param_str = "&".join([f"{k}={v}" for k, v in params.items()])
        try:
            logging.info(param_str)
            browser_fp = BrowserFingerprintGenerator.generate_fingerprint("Edge")
            ab_value = ABogus(fp=browser_fp, user_agent=self.__spider.agent).generate_abogus(param_str, "")
            # xb_value = self.__xb.getXBogus(param_str)
        except Exception as e:
            raise SDKException(f"生成a-Bogus失败: {e})")

        return f"{param_str}&a_bogus={ab_value[1]}"

    # 获取视频播放链接
    async def video_detail_get(self, aweme_id: str) -> dict:
        query = await self.__get_query({"aweme_id": aweme_id})
        return await self.__spider.request_get_json(f'aweme/v1/web/aweme/detail/?{query}')



