...
def Air_xH2O(T_db=None,T_wb=None):
      Mdry=28.965
      Mv=18.01528
      w=18.05
...
# 		Enthalpie=None
# 	return Enthalpie
def Air_T_wb(T_db=None, RH=None, P=101325.0):
    """
...
def Air_h(T_db=None, w=None, T_wb=None):
    ...
    # Si T_db et T_wb sont fournis
        ...
        w = Air_w(T_db=T_db, T_wb=T_wb)  # Calculer w à partir de T_db et T_wb
        Enthalpie = 1.006 * T_db + (w / 1000) * (2501 + 1.0805 * T_db)
...
def Air_v_hum(T_db=None, RH=None, P=101325,T_wb=None):
    rho_hum=Air_rho_hum(T_db=T_db, RH=RH, P=P,T_wb=T_wb)
    v_hum=1/rho_hum
    return v_hum
...
def Air_rho_hum(T_db=None, RH=None, P=101325, T_wb=None):
    ...
    if T_db is None or RH is None:
        raise ValueError("T_db et RH (ou T_wb) doivent être fournis pour calculer la masse volumique de l'air humide.")
