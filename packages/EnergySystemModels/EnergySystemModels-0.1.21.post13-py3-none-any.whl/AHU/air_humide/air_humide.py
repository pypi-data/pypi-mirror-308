...
# def func_RH(w=None, P=101325.0, Pv_sat=None, T_db=None,T_wb=None):
#     if Pv_sat is None:
#         if T_db is None:
...
# 		else:
# 			w_w=func_w(RH=100,T_db=T_wb)
# 			h=func_h(T_db=T_wb,w=w_w)
# 			w=func_w(h=h,T_db=T_db)
...
def func_RH(w=None, P=101325.0, Pv_sat=None, T_db=None, T_wb=None):
    ...
    if w is None:
        if T_wb is None:
            ...
            w_w = func_w(RH=100, T_db=T_wb)  # Calculer l'humidité absolue à saturation (RH = 100)
            h = func_h(T_db=T_wb, w=w_w)  # Calculer l'enthalpie de l'air humide à T_wb et w_w
            w = func_w(h=h, T_db=T_db)  # Calculer w pour la température de bulbe sec T_db avec l'enthalpie h
...
# def func_w( RH=None, P=101325.0,Pv_sat=None,T_db=None,h=None,T_wb=None):
...
def T_sat(w_target):
	...
	T = -100
	Erreur = func_w(func_Pv_sat(T), 100) - w_target
