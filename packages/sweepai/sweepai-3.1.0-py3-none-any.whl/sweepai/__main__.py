...
def compare_changes(changes1: list[CodeSuggestion], changes2: list[CodeSuggestion]):
    return all(change1.file_path == change2.file_path and change1.original_code == change2.original_code and change1.new_code == change2.new_code for change1, change2 in zip(changes1, changes2))
...
class SyncManager(BaseModel):
    ...
    def from_url(cls, url: str):
        ...
        instance = cls(message_id=message_id, github_token=os.environ["GITHUB_PAT"])
    ...
    def _request(self, method: Literal["GET", "POST"], path: str, **kwargs):
        response = requests.request(method, f"{self.base_url}/backend/{path}", headers={"Authorization": f"Bearer {self.github_token}"}, **kwargs)
    ...
    def pull_changes(self):
        ...
        if self.branch != remote_changes.branch:
            logger.error(f"Branch mismatch, skipping sync. Expected {self.branch}, got {remote_changes.branch}")
            return []
        if self.updated_at is None or remote_changes.updated_at > self.updated_at:
            logger.info(f"Pulling changes from server, {len(remote_changes.applied_changes)} changes applied.")
            self._apply_changes(remote_changes.applied_changes)
            self.updated_at = remote_changes.updated_at
        return remote_changes.applied_changes
    ...
    def write_changes(self, last_changes: list[CodeSuggestion] | None = None):
        changes = self._get_local_changes()
        if last_changes is not None and compare_changes(last_changes, changes):
