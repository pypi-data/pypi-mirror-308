...
from sweepai.app import handle_request
...
app = typer.Typer(name="sweepai", context_settings={"help_option_names": ["-h", "--help"]})
...
def watch(
    ...
    if not os.path.exists(config_path):
        ...
        raise ValueError("Configuration not found, please run 'sweep init' to initialize the CLI.")
    posthog_capture(
        "sweep_watch_started",
        {
            "repo": repo_name,
    ...
    g = Github(os.environ["GITHUB_PAT"], base_url=GITHUB_API_BASE_URL)
        ...
        while True:
            ...
            for i, event in enumerate(events_iterator):
                if event.id not in processed_event_ids:
                    local_time = event.created_at.replace(tzinfo=datetime.timezone.utc).astimezone(local_tz)
    ...
    def handle_event(event: Event | IssueEvent, do_async: bool = True):
        ...
        if record_events:
            ...
            pickle.dump(
                event,
                open(
                    "tests/events/" + f"{_type}_{payload.get('action')}_{str(event.id)}.pkl",
                    "wb",
                ),
            )
        if do_async:
            thread = threading.Thread(target=handle_request, args=(payload, get_event_type(event)))
            thread.start()
            return thread
        else:
