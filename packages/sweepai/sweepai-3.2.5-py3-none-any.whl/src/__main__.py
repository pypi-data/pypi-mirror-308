...
from src.code_suggestions import CodeSuggestion
...
class SyncManager(BaseModel):
    ...
    @classmethod
    def from_url(cls, url: str, base_url: str | None = None):
        parsed_url = urlparse(url)
        _c, message_id = parsed_url.path.strip('/').split('/')
        instance = cls(message_id=message_id, github_token=os.environ["GITHUB_PAT"], base_url=get_backend_url(f"{parsed_url.scheme}://{parsed_url.netloc}") if base_url is None else base_url)
    ...
    def _request(self, method: Literal["GET", "POST"], path: str, **kwargs):
        ...
        try:
            response = requests.request(method, f"{self.base_url}/backend/{path}", headers={"Authorization": f"Bearer {self.github_token}"}, proxies={"no_proxy": parsed_url.hostname or ""}, **kwargs)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.SSLError as e:
            raise Exception(f"Failed to connect to the server. Make sure to install your CA certificates.") from e
        except requests.exceptions.ConnectionError as e:
            raise Exception(f"Failed to connect to the server. Make sure {parsed_url.hostname} is reachable and check for any proxies in your environment.") from e
    ...
    def pull_changes(self):
        ...
        if self.branch != remote_changes.branch:
            raise Exception(f"You're currently on `{self.branch}`, but the changes were made on `{remote_changes.branch}`. Please run `git checkout {remote_changes.branch}` and try again.")
...
@app.command()
def sync(url: str, base_url: str | None = None):
    sync_manager = SyncManager.from_url(url, base_url)
