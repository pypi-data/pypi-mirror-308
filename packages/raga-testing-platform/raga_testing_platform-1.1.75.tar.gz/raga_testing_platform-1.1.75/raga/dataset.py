...
class ODDataset(Dataset):
    ...
    def __init__(self, test_session: TestSession, name: str, type: Optional[str],
        ...
        self.json_file_path = json_file_path
        self.image_folder_path = image_folder_path
        self.coco_obj = CocoDataset(test_session, name, type, json_file_path, image_folder_path, s3_bucket_name, s3_key, aws_raga_access_key, aws_raga_secret_key, aws_session_token, "od")
            ...
            if imageId in grouped_annotations:
                for detection in grouped_annotations[imageId]:
                    for key, value in detection.items():
                        annotations_object.add(key, value)
                annotations_df_column.append(annotations_object)
            else:
                annotations_df_column.append(annotations_object)
        df[column_name] = annotations_df_column
...
class KDataset(Dataset):
    ...
    def __init__(self, test_session: TestSession, name: str, type: Optional[str],
                 ...
                 aws_session_token: Optional[str] = None, parent_dataset_name:Optional[str]=None, image_folder_path:Optional[str]=None, bucket_region : Optional[str] = None):
        self.json_file_path = json_file_path
        self.image_folder_path = image_folder_path
        self.label_map = label_map
    ...
    def __create_kd_schema(self):
        ...
        # schema.add("annotations", InferenceSchemaElement(model="GT", label_mapping=self.label_map, skeleton_mapping=self.skeleton_map))
        # schema.add("model_inferences", InferenceSchemaElement(model = "ask model name in parameters"))
        schema.add("timeStamp", TimeOfCaptureSchemaElement())
        for attribute_name in self.attribute_list:
            schema.add(attribute_name, AttributeSchemaElement())
        return schema
    ...
    def upload(self):
        if self.coco_obj.s3_bucket_name is not None:
            print("Uploading images to s3")
            spinner.start()
            self.upload_images_to_s3(self.coco_obj.s3, self.raga_extracted_dataset, "imageUri", self.coco_obj.s3_bucket_name, self.coco_obj.s3_key)
            spinner.stop()
